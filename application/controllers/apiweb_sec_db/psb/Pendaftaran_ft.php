    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        $sekolah_asal = "";
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select * from calon_siswa_sma where email = '".$this->post("email")."' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".$this->post("tgl_lahir")."'", "row");
          if(!empty($cek_email)){
            ...
            $id_siswa = $this->second_db->insertid("calon_siswa_sma",$data);
            if (!empty($id_siswa)) {
              ...
              $id_raport = $this->second_db->insertid("nilai_raport_ft",$data_raport);
              //insert transaksi
              $no_transaksi = "LI-FT-".date("Ymd")."-".$id_siswa;
              //get biaya pendaftaran
              $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","FRANCE TRACK","row");
              ...
              if (!empty($data_transaksi)) {
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_sma as id_siswa from calon_siswa_sma where va_number like '".$prefix.$client_id.date('y', strtotime('+1 years'))."09%' and va_number != '' and is_ft = '1' order by id_siswa_sma DESC","row");
                ...
                $this->second_db->update("siswa_ft", array("no_transaksi" => $no_transaksi, "va_number" => $va_number), "id_siswa_sma", $id_siswa);
              ...
              $msg = array('status' => 0, 'message'=>'Form tidak diisi sesuai permintaan (Input siswa data gagal)' ,'data'=>array(), 'data_raport' => array(), 'transaksi' => array());
              $status="200";
            }
          }
          else{
