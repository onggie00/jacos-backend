...
class Riwayat_presensi extends REST_Controller {
    ...
    public function index_get()
    {
        ...
        if (empty($token)) {
            $this->response(array('status' => 0, 'message' => 'Data tidak ditemukan', 'data' => array(), 'header'=> $headers), 200);
            return;
        }
        ...
        //pegawai
        $get_data = $this->mymodel->withquery("select u.id_pegawai as id_user, concat('pegawai') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from pegawai u where u.token = '".$token."'","row");
        //pimpinan
        if (empty($get_data)) {
            $get_data = $this->mymodel->withquery("select u.id_pimpinan as id_user, concat('pimpinan_sd') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from pimpinan_sd u where u.token = '".$token."'","row");
        }
        if(empty($get_data)){
            $get_data = $this->mymodel->withquery("select u.id_pimpinan as id_user, concat('pimpinan_smp') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from pimpinan_smp u where u.token = '".$token."'","row");
        }
        if (empty($get_data)) {
            $get_data = $this->mymodel->withquery("select u.id_pimpinan as id_user, concat('pimpinan_sma') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from pimpinan_sma u where u.token = '".$token."'","row");
        }
        //guru
        if (empty($get_data)) {
            $get_data = $this->mymodel->withquery("select u.id_guru as id_user, concat('guru_ft') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from guru_ft u where u.token = '".$token."'","row");
        }
        if (empty($get_data)) {
            $get_data = $this->mymodel->withquery("select u.id_guru as id_user, concat('guru_sma') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from guru_sma u where u.token = '".$token."'","row");
        }
        if (empty($get_data)){
            $get_data = $this->mymodel->withquery("select u.id_guru as id_user, concat('guru_smp') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from guru_smp u where u.token = '".$token."'","row");
        }
        if (empty($get_data)){
            $get_data = $this->mymodel->withquery("select u.id_guru as id_user, concat('guru_sd') as role, u.npp, u.emp_code, u.presensi_role, u.nama_lengkap from guru_sd u where u.token = '".$token."'","row");
        }
        ...
        if(!empty($get_data)){
            ...
            $riwayat = $this->mymodel->withquery("select p.id_presensi, p.npp, p.nama_lengkap, p.presensi_date, p.check_in, p.status_presensi, p.check_out, p.status_presensi_selesai, p.presensi_device, p.file_report, p.report_status, p.file_report_end, p.report_status_end from presensi_office p 
            $where order by presensi_date DESC, check_in DESC, check_out DESC, p.id_presensi DESC","result");
            // echo $this->db->last_query();
