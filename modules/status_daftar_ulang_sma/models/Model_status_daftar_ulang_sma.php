...
class Model_status_daftar_ulang_sma extends MY_Model {

    private $primary_key    = 'id_daftar_ulang';
    private $table_name     = 'status_daftar_ulang_sma';
    private $field_search   = ['id_siswa_sma', 'status', 'slip_pembayaran', 'kwitansi', 'kartu_sementara', 'tanggal_lulus', 'tgl_daftar_ulang'];
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "status_daftar_ulang_sma.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "status_daftar_ulang_sma.".$field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
    {
        ...
        $this->db->limit($limit, $offset);
                $this->db->order_by('status_daftar_ulang_sma.'.$this->primary_key, "DESC");
                $query = $this->db->get($this->table_name);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_sma', 'siswa_sma.id_siswa_sma = status_daftar_ulang_sma.id_siswa_sma', 'LEFT');

        $this->db->select('status_daftar_ulang_sma.*,siswa_sma.nama_lengkap,siswa_sma.email');
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->select('siswa_sma.nama_lengkap,
                            siswa_sma.no_peserta,
                            siswa_sma.nama_ayah,
                            siswa_sma.notelp_ayah,
                            siswa_sma.nama_ibu,
                            siswa_sma.notelp_ibu,
                           status_daftar_ulang_sma.*,
                           (CASE
                                WHEN status_daftar_ulang_sma.status = 0 THEN "Menunggu Aktivasi"
                                WHEN status_daftar_ulang_sma.status = 1 THEN "Menunggu Pembayaran"
                                WHEN status_daftar_ulang_sma.status = 2 THEN "Lunas"
                                ELSE "-"
                            END) as status
                          ');
...
/* End of file Model_status_daftar_ulang_sma.php */
/* Location: ./application/models/Model_status_daftar_ulang_sma.php */