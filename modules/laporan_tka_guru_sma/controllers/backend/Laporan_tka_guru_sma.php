...
/**
*| --------------------------------------------------------------------------
*| Laporan Tka Guru Sma Controller
*| --------------------------------------------------------------------------
*| Laporan Tka Guru Sma site
*|
*/
class Laporan_tka_guru_sma extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_laporan_tka_guru_sma');
		$this->lang->load('web_lang', $this->current_lang);
	}

	public function index($offset = 0)
	{
		$this->is_allowed('laporan_tka_guru_sma_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');
		$tahun 	= $this->input->get('tahun');

		$this->data['laporan_tka_guru_smas'] = $this->model_laporan_tka_guru_sma->get($filter, $field, $this->limit_page, $offset, [], $tahun);
		$this->data['laporan_tka_guru_sma_counts'] = $this->model_laporan_tka_guru_sma->count_all($filter, $field, $tahun);
		$this->data['list_tahun'] = $this->model_laporan_tka_guru_sma->get_distinct_tahun();

		$baseUrl = 'administrator/laporan_tka_guru_sma/index/';
		if (!empty($tahun)) {
			$baseUrl .= '?tahun='.$tahun;
		}
		...
		$config = [
			'base_url'     => $baseUrl,
			'total_rows'   => $this->model_laporan_tka_guru_sma->count_all($filter, $field, $tahun),
		...
		$this->template->title('TKA Guru SMA List');
		$this->render('backend/standart/administrator/laporan_tka_guru_sma/laporan_tka_guru_sma_list', $this->data);
	}
	
	public function add()
	{
		$this->is_allowed('laporan_tka_guru_sma_add');
		$this->template->title('TKA Guru SMA New');
		$this->render('backend/standart/administrator/laporan_tka_guru_sma/laporan_tka_guru_sma_add', $this->data);
	}

	public function add_save()
	{
		if (!$this->is_allowed('laporan_tka_guru_sma_add', false)) {
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_tka_guru_sma = $this->model_laporan_tka_guru_sma->store($save_data);

			if ($save_laporan_tka_guru_sma) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] = $save_laporan_tka_guru_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/laporan_tka_guru_sma/edit/' . $save_laporan_tka_guru_sma, 'Edit'),
						anchor('administrator/laporan_tka_guru_sma', ' Go back to list')
					]);
				} else {
					set_message(cclang('success_save_data_redirect', [anchor('administrator/laporan_tka_guru_sma/edit/' . $save_laporan_tka_guru_sma, 'Edit')]), 'success');
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/laporan_tka_guru_sma');
				}
			} else {
	...
	{
		$this->is_allowed('laporan_tka_guru_sma_update');
		$this->data['laporan_tka_guru_sma'] = $this->model_laporan_tka_guru_sma->find($id);
		$this->template->title('TKA Guru SMA Update');
		$this->render('backend/standart/administrator/laporan_tka_guru_sma/laporan_tka_guru_sma_update', $this->data);
	}

	public function edit_save($id)
	{
		if (!$this->is_allowed('laporan_tka_guru_sma_update', false)) {
			echo json_encode(['success' => false, 'message' => cclang('sorry_you_do_not_have_permission_to_access')]);
			exit;
		}
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_tka_guru_sma = $this->model_laporan_tka_guru_sma->change($id, $save_data);

			if ($save_laporan_tka_guru_sma) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] = $id;
					$this->data['message'] = cclang('success_update_data_stay', [anchor('administrator/laporan_tka_guru_sma', ' Go back to list')]);
				} else {
	...
	{
		$this->is_allowed('laporan_tka_guru_sma_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'laporan_tka_guru_sma'), 'success');
		} else {
			set_message(cclang('error_delete', 'laporan_tka_guru_sma'), 'error');
		}
	...
	public function delete_by_tahun()
	{
		...
		$deleted = $this->db->delete('laporan_tka_guru_sma');
		...
		redirect('administrator/laporan_tka_guru_sma');
	}

	public function view($id)
	{
		$this->is_allowed('laporan_tka_guru_sma_view');
		$this->data['laporan_tka_guru_sma'] = $this->model_laporan_tka_guru_sma->join_avaiable()->filter_avaiable()->find($id);
		$this->template->title('TKA Guru SMA Detail');
		$this->render('backend/standart/administrator/laporan_tka_guru_sma/laporan_tka_guru_sma_view', $this->data);
	}
	
	private function _remove($id)
	{
		$laporan_tka_guru_sma = $this->model_laporan_tka_guru_sma->find($id);
		return $this->model_laporan_tka_guru_sma->remove($id);
	}
	
	public function export()
	{
		$this->is_allowed('laporan_tka_guru_sma_export');
		...
		$join_sql = "from laporan_tka_guru_sma k 
		...
		$judul = "REKAP HASIL TES KOMPETENSI AKADEMIK GURU SMA LABSCHOOL CIBUBUR ".(!empty($ahun) ? $tahun : "");
		...
		header('Content-Disposition: attachment;filename="Laporan TKA Guru SMA - '.date("Y-m-d").'.xls"');
	...
	{
		$this->is_allowed('laporan_tka_guru_sma_export');
		$this->model_laporan_tka_guru_sma->pdf('laporan_tka_guru_sma', 'laporan_tka_guru_sma');
	}

	public function single_pdf($id = null)
	{
		$this->is_allowed('laporan_tka_guru_sma_export');
		$table = $title = 'laporan_tka_guru_sma';
		$this->load->library('HtmlPdf');
        $config = array('orientation' => 'p', 'format' => 'a4', 'marges' => array(5, 5, 5, 5));
        $this->pdf = new HtmlPdf($config);
        $this->pdf->setDefaultFont('stsongstdlight'); 
        $result = $this->db->get($table);
        $data = $this->model_laporan_tka_guru_sma->find($id);
		...
		if (isset($_FILES["file_upload"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 5; $row <= $highestRow; $row++) {
					...
					if (!$guru_not_found) {
						$check_nilai = $this->mymodel->withquery("select * from laporan_tka_guru_sma where id_guru = '" . $check->id_guru . "' and jenjang = '" . $jenjang . "' and tahun = '" . $tahun . "'", 'row');
					} else {
						$check_nilai = null;
					}
					...
					if (!empty($check_nilai)) {
						...
						$this->mymodel->update("laporan_tka_guru_sma", $data_laporan, 'id_laporan', $check_nilai->id_laporan);
						$insertId = $check_nilai->id_laporan;
					} else {
						$insertId = $this->mymodel->insertid("laporan_tka_guru_sma", $data_laporan);
					}
	...
	public function import_detail()
	{
		...
		try {
			...
			for ($row = 2; $row <= $highestRow; $row++) {
				...
				if (!empty($guru)) {
					$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'sd' AND tahun = '".$tahun."'", 'row');
					if (!empty($lap)) $id_laporan = $lap->id_laporan;
				}
				if (empty($id_laporan)) {
					$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'sd' AND tahun = '".$tahun."'", 'row');
					if (!empty($lap)) $id_laporan = $lap->id_laporan;
				}
				...
				if (empty($id_laporan)) {
					...
					if (!empty($guru)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'smp' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
					if (empty($id_laporan)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'smp' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
				}
				...
				if (empty($id_laporan)) {
					...
					if (!empty($guru)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'sma' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
					if (empty($id_laporan)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sma WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'sma' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
				}
				...
				if (!in_array($id_laporan, $processed_laporan)) {
					$this->db->where('id_laporan', $id_laporan)->delete('laporan_tka_detail_sma');
				...
				foreach ($no_columns as $no_soal => $col_idx) {
					...
					$save = $this->mymodel->insertid('laporan_tka_detail_sma', [
	...
	*/
	public function detail_nilai($id_laporan)
	{
		...
		$laporan = $this->model_laporan_tka_guru_sma->find($id_laporan);
		if (empty($laporan)) {
