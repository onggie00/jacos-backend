...
class Model_status_daftar_ulang_kb extends MY_Model {

    private $primary_key    = 'id_daftar_ulang';
    private $table_name     = 'status_daftar_ulang_kb';
    private $field_search   = ['id_siswa_kb', 'status', 'slip_pembayaran', 'kwitansi', 'kartu_sementara', 'tanggal_lulus', 'tgl_daftar_ulang', 'tgl_aktivasi', 'tgl_bayar', 'va_bri'];
    private $field_search_join   = ['siswa_kb.nama_lengkap', 'siswa_kb.email', 'siswa_kb.no_peserta', 'siswa_kb.nisn', 'siswa_kb.tahun_ajaran', 'siswa_kb.gelombang'];

    /**
     * Whitelist alias -> DB column for multi-filter (anti SQL injection via GET)
     */
    private $field_map = array(
        'nama_lengkap'        => 'siswa_kb.nama_lengkap',
        'email'               => 'siswa_kb.email',
        'no_peserta'          => 'siswa_kb.no_peserta',
        'nisn'                => 'siswa_kb.nisn',
        'tahun_ajaran'        => 'siswa_kb.tahun_ajaran',
        'gelombang'           => 'siswa_kb.gelombang',
        'status'              => 'status_daftar_ulang_kb.status',
        'va_bri'              => 'status_daftar_ulang_kb.va_bri',
        'id_daftar_ulang'     => 'status_daftar_ulang_kb.id_daftar_ulang',
        'id_siswa_kb'         => 'status_daftar_ulang_kb.id_siswa_kb',
        'tanggal_lulus'       => 'DATE(status_daftar_ulang_kb.tanggal_lulus)',
        'tgl_daftar_ulang'    => 'status_daftar_ulang_kb.tgl_daftar_ulang',
        'tgl_aktivasi'        => 'DATE(status_daftar_ulang_kb.tgl_aktivasi)',
        'tgl_bayar'           => 'DATE(status_daftar_ulang_kb.tgl_bayar)',
        'expired_datetime'    => 'DATE(transaksi.expired_datetime)',
        'custom_payment'      => 'status_daftar_ulang_kb.custom_payment',
    );
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "status_daftar_ulang_kb.".$field . " LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_kb.nama_lengkap LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_kb.email LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_kb.no_peserta LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "status_daftar_ulang_kb.".$field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
            ...
            $where .= "OR (" . "siswa_kb.nama_lengkap LIKE '%" . $q . "%' )";
            $where .= "OR (" . "siswa_kb.email LIKE '%" . $q . "%' )";
            $where .= "OR (" . "siswa_kb.va_number LIKE '%" . $q . "%' )";
        }
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $this->db->order_by('status_daftar_ulang_kb.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_kb', 'siswa_kb.id_siswa_kb = status_daftar_ulang_kb.id_siswa_kb', 'LEFT');
        $this->db->join('transaksi', 'transaksi.va_number = status_daftar_ulang_kb.va_bri and transaksi.no_transaksi like "%LDUI%"', 'LEFT');
        $this->db->group_by('status_daftar_ulang_kb.id_daftar_ulang');
        $this->db->select('status_daftar_ulang_kb.*,
            siswa_kb.nama_lengkap,
            siswa_kb.email,
            siswa_kb.no_peserta,
            siswa_kb.nisn,
            siswa_kb.tahun_ajaran,
            siswa_kb.gelombang,
            siswa_kb.va_number,
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->select('siswa_kb.nama_lengkap,
                            siswa_kb.no_peserta,
                            siswa_kb.nama_ayah,
                            siswa_kb.notelp_ayah,
                            siswa_kb.nama_ibu,
                            siswa_kb.notelp_ibu,
                           status_daftar_ulang_kb.*,
                           (CASE
                                WHEN status_daftar_ulang_kb.status = 0 THEN "Menunggu Aktivasi"
                                WHEN status_daftar_ulang_kb.status = 1 THEN "Menunggu Pembayaran"
                                WHEN status_daftar_ulang_kb.status = 2 THEN "Lunas"
                                ELSE "-"
                            END) as status
                          ');
...
/* End of file Model_status_daftar_ulang_kb.php */
/* Location: ./application/models/Model_status_daftar_ulang_kb.php */