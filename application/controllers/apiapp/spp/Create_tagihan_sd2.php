...
class Create_tagihan_sd2 extends REST_Controller
{
  ...
  public function index_post()
  {
    ...
    foreach ($id_trans as $key => $item) {
      ...
      foreach($get_tahun_ajaran_lalu as $key => $item){
        $cek_exist_tagihan=$this->mymodel->withquery("select * from transaksi_spp where status_transaksi in(0,1) and id_siswa_aktif = $id_siswa and id_tahun_ajaran = $item->id_tahun_ajaran", "result");
    ...
    $code='80';
      $get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,s.nama_lengkap as nama_lengkap from siswa_sd_aktif sa join siswa_sd s on s.id_siswa_sd = sa.id_siswa_sd join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran join kelas_sd k on k.id_kelas_sd = sa.id_kelas where id_siswa_sd_aktif = ".$id_siswa."","row");
    ...
    $get_transaksi = $this->mymodel->withquery("select va_number from transaksi_spp where user_email = '".$email."'" , "row");
    ...
    $this->mymodel->update("transaksi_spp", array("va_number" => $va_number), "user_email", $email);

    $msg = array('status' => 1, 'message' => 'Tagihan Berhasil Dibuat', 'data' => $payment_response);
    $status = "200";
  ...
  function update_va_bri_status($datas)
  {
    ...
    return BriApi::updateBayar($clientID, $clientSecret, $endpoint, $institutionCode, $datas['brivaNo'], $datas['custCode'], '', $url);
  }

  function delete_va_bri($datas)
  {
    $this->load->library('BriApi');
    ...
    return BriApi::delete($clientID, $clientSecret, $endpoint, $institutionCode, $datas, $url);
  }
