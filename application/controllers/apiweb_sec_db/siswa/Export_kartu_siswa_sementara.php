    ...
    public function index()
    {
      ...
      if(isset($headers['user_role']))
        ...
        if (!empty($data)) {
          ...
          $data->kota = $this->second_db->withquery("select name from regencies where id = '".$data->kota."'","row")->name;
          $tahun_ajaran = $this->second_db->withquery("select * from tahun_ajaran_psb","row");

        $datas['siswa'] = $data;
        $datas['tipe_siswa'] = $tipe_siswa;
        $datas['tahun_ajar'] = $tahun_ajaran->label;
        $datas['catatan_kartu_siswa'] = $this->second_db->withquery("select catatan from catatan_kartu_siswa_sementara where jenjang = '".$tipe_siswa."'","row")->catatan;
        $datas['kepsek'] = $this->second_db->withquery("select * from data_kepala_sekolah where jenjang = '".$tipe_siswa."'","row");
