...
class Get_device_by_id extends MY_Controller {
    ...
    public function index()
    {
      ...
      if(isset($headers['x-token']))
        ...
        if (!empty($token)) {
            ...
            $status="200";
            //Get device Detail
            if(!empty($this->input->get('device_id'))){
                $device_id = $this->input->get('device_id');
                $detail_device = $this->get_detail_device($host, $port, $token, $this->input->get('device_id'));
                $data->detail_device = $detail_device;
                $msg = array('status' => 1, 'message'=>'Success Get Detail Device' ,'token'=>$token, 'data'=>$data);
                $status="200";
            }
            else{
                $msg = array('status' => 0, 'message'=>'Detail device not found' ,'token'=>$token, 'data'=>$data);
                $status="200";
            }
        }
        else{
    ...
    private function get_detail_device($host, $port, $token, $id){ //Work
