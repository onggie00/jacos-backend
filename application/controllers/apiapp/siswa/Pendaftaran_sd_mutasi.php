          ...
          if (!empty($data)) {
            ...
            if (!empty($id_siswa)) {
              ...
              if (!empty($data_transaksi)) {
                //create billing
                //flag bank aktif alur Mutasi SD (rollback cepat: ganti value 'bank_mutasi_sd' di pengaturan_akun ke 'BRI')
                $bank_aktif = 'BRI';
                $client_id = '';
                $prefix = '';
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number_bri, id_siswa_sd as id_siswa from siswa_sd where va_number_bri like '".$brivaNo.date('y', strtotime('+1 years'))."01%' and va_number_bri != '' and is_mutasi = '1' order by id_siswa_sd DESC","row");
