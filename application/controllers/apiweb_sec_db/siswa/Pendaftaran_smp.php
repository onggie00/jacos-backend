...
class Pendaftaran_smp extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_smp from siswa_smp where email like '%".$this->post("email")."%' and is_show = 1","row");
        ...
        if (empty($cek_email)) {
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select * from siswa_smp where email = '".$this->post("email")."'", "row");
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              if ($ppsbb == "1") {
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_smp as id_siswa from siswa_smp where va_number like '".$prefix.$client_id.date('y', strtotime('+1 years'))."03%' and va_number != '' order by id_siswa_smp DESC","row");
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_smp as id_siswa from siswa_smp where va_number like '".$prefix.$client_id.date('y', strtotime('+1 years'))."06%' and va_number != '' order by id_siswa_smp DESC","row");
                if (empty($get_no_urut)) {
                  $no_urut = "0001";
                }
                else{
