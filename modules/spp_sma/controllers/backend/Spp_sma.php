...
class Spp_sma extends Admin
{
	...
	{
		$this->is_allowed('spp_sma_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['spp_smas'] = $this->model_spp_sma->get($filter, $field, $this->limit_page, $offset);
		$this->data['spp_sma_counts'] = $this->model_spp_sma->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/spp_sma/index/',
			'total_rows'   => $this->model_spp_sma->count_all($filter, $field),
		...
		$this->template->title('Spp Sma List');
		$this->render('backend/standart/administrator/spp_sma/spp_sma_list', $this->data);
	}


	/**
	 * Update view Spp Smas
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('spp_sma_update');

		$this->data['spp_sma'] = $this->model_spp_sma->find($id);

		$this->template->title('Spp Sma Update');
		$this->render('backend/standart/administrator/spp_sma/spp_sma_update', $this->data);
	}

	public function add_save()
	{
		$this->load->library("session");
		if (!$this->is_allowed('spp_sma_list', false)) {
			echo json_encode([
				'success' => false,
		...
		if (empty($id_siswa)) {
			...
			$list_siswa = $this->mymodel->getbywhere("siswa_sma_aktif", "id_kelas", $this->input->post("id_kelas"), "result");
			$id_siswa = array();
			foreach ($list_siswa as $key => $value) {
				array_push($id_siswa, $value->id_siswa_sma_aktif);
			}
		...
		foreach ($id_siswa as $key_siswa => $value_siswa) {
			...
			$get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.* from siswa_sma_aktif sa join siswa_sma s on s.id_siswa_sma = sa.id_siswa_sma join kelas_sma k on k.id_kelas_sma = sa.id_kelas where sa.id_siswa_sma_aktif = " . (int)$value_siswa, 'row');
			if (!$get_siswa) {
			...
			if (!empty($custom_nominal_spp) && $custom_nominal_spp > 0) {
				...
				$this->mymodel->update('siswa_sma_aktif', ['spp_custom' => $biaya], 'id_siswa_sma_aktif', $value_siswa);
				$spp_custom = $biaya;
			...
			$cek_detail = $this->mymodel->getbywhere("spp_sma", array("id_siswa_aktif" => $value_siswa, "id_tahun_ajaran" => $get_tahun_ajaran_aktif->id_tahun_ajaran), "row");
			...
			if (!$cek_detail) {
				...
				if (!empty($update_spp)) {
					$this->mymodel->update("spp_sma", $update_spp, "id", $id_detail_spp);
					$is_update = true;
				}
			}

			// Update kolom bulan di spp_sma
			foreach ($arrMonth as $in) {
				...
				if ($bln && in_array((int)$bulan[1], $bln)) {
					$this->mymodel->update("spp_sma", array($kolom_bulan => null), "id", $id_detail_spp);
				} else if (!$bln) {
					$this->mymodel->update("spp_sma", array($kolom_bulan => null), "id", $id_detail_spp);
				} else {
					$this->mymodel->update("spp_sma", array($kolom_bulan => '-'), "id", $id_detail_spp);
				}
			}
			...
			foreach ($arrMonth as $i) {
				...
				// Cek transaksi dengan filter jenjang SMA
				$cek_transaksi_bln = $this->mymodel->withquery("
					...
					AND (no_transaksi LIKE '%sma%' OR no_transaksi LIKE '%SMA%')
				", "row");
	...
	{
		if (!$this->is_allowed('spp_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		$this->form_validation->set_rules('september', 'September', 'trim|required');
		...
		if ($this->form_validation->run()) {
			...
			$save_spp_sma = $this->model_spp_sma->change($id, $save_data);
			...
			if ($save_spp_sma) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/spp_sma', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/spp_sma');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	{
		$this->is_allowed('spp_sma_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'spp_sma'), 'success');
		} else {
			set_message(cclang('error_delete', 'spp_sma'), 'error');
		}
	...
	{
		$this->is_allowed('spp_sma_view');

		$this->data['spp_sma'] = $this->model_spp_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Spp Sma Detail');
		$this->render('backend/standart/administrator/spp_sma/spp_sma_view', $this->data);
	}
	...
	{
		$spp_sma = $this->model_spp_sma->find($id);
		...
		return $this->model_spp_sma->remove($id);
	}
	...
	{
		$this->is_allowed('spp_sma_export');

		//$this->model_spp_sma->export('spp_sma', 'spp_sma');
		$field = $this->input->get("f");
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_sma spp join siswa_sma_aktif s on spp.id_siswa_aktif = s.id_siswa_sma_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
			...
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_sma spp join siswa_sma_aktif s on spp.id_siswa_aktif = s.id_siswa_sma_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran where ".$where." order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
		else{
		...
		header('Content-Disposition: attachment;filename="Rekap SPP sma_'.date("Y-m-d Hi").'.xls"'); 
	...
	public function export_pdf()
	{
		...
		$this->model_spp_sma->pdf('spp_sma', 'spp_sma');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_spp_sma->find($id);
	...
	public function sync_spp_data(){
		...
		redirect('administrator/spp_sma');
	}
...
/* End of file spp_sma.php */
/* Location: ./application/controllers/administrator/Spp Sma.php */