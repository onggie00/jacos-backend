<style>
/* Action Buttons - copied from acara_list.php */
.btn-action {
  border: 1px solid;
...
<section class="content-header">
   <h1>
      <?= cclang('prestasi_siswa_sd'); ?> <small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('prestasi_siswa_sd'); ?></li>
   </ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               <h3 class="box-title">
                  <i class="fa fa-trophy"></i> <?= cclang('prestasi_siswa_sd'); ?>
                  <span class="label bg-yellow" style="margin-left:10px"><?= $prestasi_siswa_sd_counts; ?> Data</span>
               </h3>
               <div class="box-tools pull-right">
                  <?php is_allowed('prestasi_siswa_sd_add', function(){?>
                  <a class="btn btn-sm btn-success btn-top" id="btn_add_new" title="<?= cclang('add_new_button', array(cclang('prestasi_siswa_sd'))); ?> (Ctrl+a)" href="<?= site_url('administrator/prestasi_siswa_sd/add'); ?>">
                     <i class="fa fa-plus"></i> <?= cclang('add_new_button', array(cclang('prestasi_siswa_sd'))); ?>
                  </a>
                  <?php }) ?>
                  <?php is_allowed('prestasi_siswa_sd_export', function(){?>
                  <a class="btn btn-sm btn-success btn-top" title="<?= cclang('export'); ?> <?= cclang('prestasi_siswa_sd'); ?>" href="<?= site_url('administrator/prestasi_siswa_sd/export') . '?' . http_build_query($this->input->get()); ?>">
            ...
            <div class="box-body">
               ...
               <?php
               // Detect if any additional filter is active (selain q+f)
               ...
               <form name="form_prestasi_siswa_sd" id="form_prestasi_siswa_sd" action="<?= base_url('administrator/prestasi_siswa_sd/index'); ?>">
                  ...
                  <div class="col-md-5">
                     <div class="input-group">
                        ...
                        <span class="input-group-btn">
                           ...
                           <a class="btn btn-flat btn-default" href="<?= base_url('administrator/prestasi_siswa_sd'); ?>"><i class="fa fa-undo"></i></a>
                           <?php endif; ?>
                        </span>
                     </div>
                  </div>
                  ...
                  <div class="box-body" style="display: <?= $has_active_filter ? 'block' : 'none'; ?>">
                     ...
                     <div class="row filter-row">
                        ...
                        <div class="col-md-3 text-right" style="padding-top:22px">
                           ...
                           <a class="btn btn-default btn-flat" href="<?= base_url('administrator/prestasi_siswa_sd'); ?>"><i class="fa fa-undo"></i> <?= cclang('reset_semua'); ?></a>
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div class="table-responsive">
                  <table class="table table-bordered table-striped table-hover dataTable">
                     ...
                     <tbody id="tbody_prestasi_siswa_sd">
                     <?php if ($prestasi_siswa_sd_counts > 0): ?>
                     <?php foreach($prestasi_siswa_sds as $row): ?>
                        <tr>
                           <td>
                           ...
                           <td>
                              ...
                              <?= anchor('administrator/siswa_sd_aktif/view/'.$row->id_siswa.'?popup=show', $row->siswa_sd_aktif_nama_lengkap, array('class' => 'popup-view')); ?>
                              <?php else: ?>
                           ...
                           <td style="text-align:center"><?= !empty($row->kelas_sd_label) ? _ent($row->kelas_sd_label) : '<span class="text-muted">-</span>'; ?></td>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($row->foto_prestasi)): ?>
                                ...
                                <a class="fancybox" rel="group" href="<?= BASE_URL . 'uploads/prestasi_siswa_sd/' . $row->foto_prestasi; ?>">
                                  <img src="<?= BASE_URL . 'uploads/prestasi_siswa_sd/' . $row->foto_prestasi; ?>" alt="foto" width="40px">
                                </a>
                                <?php else: ?>
                                <a href="<?= BASE_URL . 'uploads/prestasi_siswa_sd/' . $row->foto_prestasi; ?>">
                           ...
                           <td style="white-space:nowrap;text-align:center">
                              <?php is_allowed('prestasi_siswa_sd_view', function() use ($row){?>
                              <a href="<?= site_url('administrator/prestasi_siswa_sd/view/' . $row->id_prestasi); ?>" class="btn btn-action btn-action-view btn-sm">
                                 <i class="fa fa-newspaper-o"></i> Detail
                              </a>
                              <?php }) ?>
                              <?php is_allowed('prestasi_siswa_sd_update', function() use ($row){?>
                              <a href="<?= site_url('administrator/prestasi_siswa_sd/edit/' . $row->id_prestasi); ?>" class="btn btn-action btn-action-edit btn-sm">
                                 <i class="fa fa-edit"></i> Edit
                              </a>
                              <?php }) ?>
                              <?php is_allowed('prestasi_siswa_sd_delete', function() use ($row){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/prestasi_siswa_sd/delete/' . $row->id_prestasi); ?>" class="btn btn-action btn-action-delete btn-sm remove-data">
                     ...
                     <?php else: ?>
                        <tr>
                           <td colspan="9" class="text-center" style="padding:30px">
                              ...
                              <span style="color:#999">Data prestasi siswa SD tidak tersedia</span>
                           </td>
                        </tr>
                     <?php endif; ?>
                     </tbody>
...
$(document).ready(function(){
   ...
   // Init datepicker (Bootstrap datepicker / fallback HTML5)
   if ($.fn.datepicker) {
   ...
   // Convert datepicker dd/mm/yyyy -> Y-m-d sebelum submit (agar controller valid)
   $('#form_prestasi_siswa_sd').on('submit', function(e) {
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var ids = $('#form_prestasi_siswa_sd input[name="id[]"]:checked');
      if (!ids.length) {
         swal({
            title: "Upss",
      ...
      if (bulk.val() == 'delete') {
         swal({
            ...
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/prestasi_siswa_sd/delete?' + serialize_bulk;
            }
         });
         return false;
      }
         ...
         swal({
            ...
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/prestasi_siswa_sd/update_status?' + serialize_bulk;
            }
         });
         return false;
      }
