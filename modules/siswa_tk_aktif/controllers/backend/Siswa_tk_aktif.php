...
/**
 *| --------------------------------------------------------------------------
 *| Siswa TK Aktif Controller
 *| --------------------------------------------------------------------------
 *| Siswa TK Aktif site
 *|
 */
class Siswa_tk_aktif extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_siswa_tk_aktif');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Siswa TK Aktifs
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('siswa_tk_aktif_list');
		...
		if ($has_multi_filter) {
			$this->data['siswa_tk_aktifs'] = $this->model_siswa_tk_aktif->get(null, null, $this->limit_page, $offset, [], $sort, $sort_type, $multi_filters);
			$this->data['siswa_tk_aktif_counts'] = $this->model_siswa_tk_aktif->count_all(null, null, $multi_filters);
		} else {
			$this->data['siswa_tk_aktifs'] = $this->model_siswa_tk_aktif->get($filter, $field, $this->limit_page, $offset, [], $sort, $sort_type);
			$this->data['siswa_tk_aktif_counts'] = $this->model_siswa_tk_aktif->count_all($filter, $field);
		}

		$config = [
			'base_url'     => 'administrator/siswa_tk_aktif/index/',
			'total_rows'   => $has_multi_filter ? $this->model_siswa_tk_aktif->count_all(null, null, $multi_filters) : $this->model_siswa_tk_aktif->count_all($filter, $field),
		...
		$this->data['list_kelas'] = $this->mymodel->withquery("SELECT id_kelas_tk, label FROM kelas_tk ORDER BY label", "result");
		...
		$this->data['export_lengkap_cols'] = $this->model_siswa_tk_aktif->export_lengkap_columns();

		$this->template->title('Siswa TK Aktif List');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_list', $this->data);
	}
	...
	/**
	 * Add new siswa_tk_aktifs
	 *
	 */
	public function add()
	{
		$this->is_allowed('siswa_tk_aktif_add');

		$this->template->title('Siswa TK Aktif New');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_add', $this->data);
	}
	...
	public function add_raport()
	{
		...
		$this->template->title('Siswa TK Tambah Raport');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_add_raport', $this->data);
	}

	public function add_raport_save()
	{
		if (!$this->is_allowed('siswa_tk_aktif_add', false)) {
			echo json_encode([
				'success' => false,
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/siswa_tk_aktif/')) {
				mkdir(FCPATH . '/uploads/siswa_tk_aktif/');
			}
			...
			if (!empty($file_raport_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/siswa_tk_aktif/' . $file_raport_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_tk_aktif/' . $file_raport_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_siswa_tk_aktif = $this->model_siswa_tk_aktif->change($item, $save_data);
		}


		if ($save_siswa_tk_aktif) {
			if ($this->input->post('save_type') == 'stay') {
				$this->data['success'] = true;
				$this->data['id'] 	   = $save_siswa_tk_aktif;
				$this->data['message'] = cclang('success_save_data_stay', [

					anchor('administrator/siswa_tk_aktif', ' Go back to list')
				]);
			} else {
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/siswa_tk_aktif/edit/' . $save_siswa_tk_aktif, 'Edit Siswa Ft')
					]),
					'success'
				);

				$this->data['success'] = true;
				$this->data['redirect'] = base_url('administrator/siswa_tk_aktif');
			}
		} else {
			if ($this->input->post('save_type') == 'stay') {
	...
	public function delete_file_raport_file($uuid)
	{
		...
		echo $this->delete_file([
			...
			'table_name'        => 'siswa_tk_aktif',
			'primary_key'       => 'id_siswa_tk_aktif',
			'upload_path'       => 'uploads/siswa_tk_aktif/'
		]);
	}

	/**
	 * Add New Siswa TK Aktifs
	 *
	 * @return JSON
	 */
	public function add_save()
	{
		...
		$this->form_validation->set_rules('id_siswa_tk', 'Id Siswa TK', 'trim|required|max_length[11]');
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'id_siswa_tk' => $this->input->post('id_siswa_tk'),
			...
			$save_siswa_tk_aktif = $this->model_siswa_tk_aktif->store($save_data);
			...
			if ($save_siswa_tk_aktif) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_siswa_tk_aktif;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/siswa_tk_aktif/edit/' . $save_siswa_tk_aktif, 'Edit Siswa TK Aktif'),
						anchor('administrator/siswa_tk_aktif', ' Go back to list')
					]);
				} else {
					set_message(
	...
	/**
	 * Update view Siswa TK Aktifs
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('siswa_tk_aktif_update');
		$this->model_siswa_tk_aktif->join_avaiable();
		$this->data['siswa_tk_aktif'] = $this->model_siswa_tk_aktif->find($id);

		$this->template->title('Siswa TK Aktif Update');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_update', $this->data);
	}

	/**
	 * Update Siswa TK Aktifs
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('siswa_tk_aktif_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$this->model_siswa_tk_aktif->change($id, $save_data);
			...
			if (isset($save_data['spp_custom'])) {
				$this->db->where('id_siswa_tk_aktif', $id)->update('siswa_tk_aktif', array('spp_custom' => $save_data['spp_custom']));
			}
			if (isset($save_data['spp_type'])) {
				$this->db->where('id_siswa_tk_aktif', $id)->update('siswa_tk_aktif', array('spp_type' => $save_data['spp_type']));
			}

			// --- Restore akun ---
			$restore_siswa = $this->input->post('restore_siswa');
			if ($restore_siswa === 'restore') {
				$this->mymodel->update('siswa_tk', array('deleted_at' => NULL), 'id_siswa_tk', $this->input->post('id_siswa_tk'));
				$this->mymodel->update('siswa_tk_aktif', array('deleted_at' => NULL), 'id_siswa_tk_aktif', $id);
			}
			$restore_ortu = $this->input->post('restore_ortu');
			if ($restore_ortu === 'restore') {
				$this->mymodel->update('siswa_tk', array('deleted_at_ortu' => NULL), 'id_siswa_tk', $this->input->post('id_siswa_tk'));
				$this->mymodel->update('siswa_tk_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_tk_aktif', $id);
			}
			...
			$this->mymodel->update('siswa_tk', $save_data_siswa, 'id_siswa_tk', $this->input->post('id_siswa_tk'));
			...
			$nama_kelas = $this->mymodel->withquery(
				"SELECT label FROM kelas_tk WHERE id_kelas_tk = '".$this->input->post('id_kelas')."'",
				"row"
			)->label;
	...
	/**
	 * delete Siswa TK Aktifs
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('siswa_tk_aktif_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'siswa_tk_aktif'), 'success');
		} else {
			set_message(cclang('error_delete', 'siswa_tk_aktif'), 'error');
		}
	...
	/**
	 * View view Siswa TK Aktifs
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('siswa_tk_aktif_view');

		$this->data['siswa_tk_aktif'] = $this->model_siswa_tk_aktif->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Siswa TK Aktif Detail');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_view', $this->data);
	}
	...
	{
		if (!$this->is_allowed('siswa_tk_aktif_view', false)) {
			echo json_encode(['error' => 'Unauthorized']);
			exit;
		}

		$siswa = $this->model_siswa_tk_aktif->join_avaiable()->filter_avaiable()->find($id);
	...
	{
		$siswa_tk_aktif = $this->model_siswa_tk_aktif->find($id);



		return $this->model_siswa_tk_aktif->remove($id);
	}
	...
	public function restore($id = null)
	{
		...
		$siswa = $this->model_siswa_tk_aktif->find($id);
		if (!$siswa) {
			set_message('Data siswa tidak ditemukan', 'error');
			redirect_back();
		}

		// Set deleted_at = NULL di tabel siswa_tk
		$this->mymodel->update('siswa_tk', array('deleted_at' => NULL), 'id_siswa_tk', $siswa->id_siswa_tk);
		// Set deleted_at = NULL di tabel siswa_tk_aktif
		$this->mymodel->update('siswa_tk_aktif', array('deleted_at' => NULL), 'id_siswa_tk_aktif', $id);
	...
	public function restore_ortu($id = null)
	{
		...
		// Set deleted_at_ortu = NULL di tabel siswa_tk
		$this->mymodel->update('siswa_tk', array('deleted_at_ortu' => NULL), 'id_siswa_tk', $siswa->id_siswa_tk);
		// Set deleted_at_ortu = NULL di tabel siswa_tk_aktif
		$this->mymodel->update('siswa_tk_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_tk_aktif', $id);
	...
	public function alumni($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			$this->mymodel->update('siswa_tk_aktif', array('is_active' => '0'), 'id_siswa_tk_aktif', $id);
	...
	{
		$this->is_allowed('siswa_tk_aktif_export');

		//$this->model_siswa_tk_aktif->export_siswa('siswa_tk_aktif', 'siswa_tk_aktif');
		$field = $this->input->get("f");
		$inputan = $this->input->get("q");
		$where = null;
		// Export columns: fields 1..N untuk import data, kolom terakhir = id_siswa_tk_aktif (primary key untuk import).
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select sa.*, s.*, k.label as kelas, t.label as tahun_ajaran, (".$case_spp.") as spp_siswa from siswa_tk_aktif sa join kelas_tk k on sa.id_kelas = k.id_kelas_tk join tahun_ajaran t on sa.id_tahun_ajaran = t.id_tahun_ajaran join siswa_tk s on sa.id_siswa_tk = s.id_siswa_tk","result");
		}
			...
			$get_data = $this->mymodel->withquery("select sa.*, s.*, k.label as kelas, t.label as tahun_ajaran, (".$case_spp.") as spp_siswa from siswa_tk_aktif sa join kelas_tk k on sa.id_kelas = k.id_kelas_tk join tahun_ajaran t on sa.id_tahun_ajaran = t.id_tahun_ajaran join siswa_tk s on sa.id_siswa_tk = s.id_siswa_tk where ".$where,"result");
		}
		else{
		...
		{
			// Kolom terakhir (id_siswa_tk_aktif) → label "ID SISWA TK AKTIF"
			if ($i == count($field_search) - 1) {
				$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, 'ID SISWA TK AKTIF');
			}
		...
		header('Content-Disposition: attachment;filename="Data Siswa TK Aktif - '.date("Y-m-d Hi").'.xls"'); 
	...
	{
		if (!$this->is_allowed('siswa_tk_aktif_export', false)) {
		...
		$rows = $this->mymodel->withquery(
			"SELECT sa.id_siswa_tk_aktif AS id, CONCAT(sa.nama_lengkap, ' (', COALESCE(k.label, '-'), ')') AS text
			 FROM siswa_tk_aktif sa
			 JOIN kelas_tk k ON sa.id_kelas = k.id_kelas_tk
			 JOIN siswa_tk s ON sa.id_siswa_tk = s.id_siswa_tk
			 WHERE " . $where . "
	...
	public function export_lengkap()
	{
		...
		$all_cols = $this->model_siswa_tk_aktif->export_lengkap_columns();
		...
		if (is_array($siswa) && !in_array('all', $siswa)) {
			...
			if (!empty($ids)) {
				$where .= ' AND sa.id_siswa_tk_aktif IN (' . implode(',', $ids) . ')';
			}
		}
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Data Lengkap Siswa TK Aktif - ' . date('Y-m-d Hi') . '.xls"');
		header('Cache-Control: max-age=0');
	...
	public function export_pdf()
	{
		...
		$this->model_siswa_tk_aktif->pdf('siswa_tk_aktif', 'siswa_tk_aktif');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_siswa_tk_aktif->find($id);
	...
	public function generate_nis()
	{
		...
		$get_data = $this->mymodel->withquery("select * from siswa_tk_aktif where id_tahun_ajaran = '".$id_tahun_ajaran."' order by nama_lengkap ASC","result");
		...
		foreach ($get_data as $key => $value) {
			...
			$cek_master_siswa = $this->mymodel->withquery("select id_siswa_tk, email, email_ms_office, email_ms_office_ortu, is_mutasi, kelas_mutasi from siswa_tk where id_siswa_tk = '".$value->id_siswa_tk."'", "row");
			if ($value->nis == "-" || empty($value->nis)) {
				...
				if(!empty($cek_master_siswa) && !empty($cek_master_siswa->email_ms_office)){
					...
					if ((int)$nis_temp[0] > 0) {
						...
						for ($i=0; $i < count($get_data); $i++) { 
							$cek = $this->mymodel->withquery("select id_siswa_tk_aktif, nis from siswa_tk_aktif where nis = '".$nis."'","result");
							if (empty($cek)) {
								break;
							}
							else{
				...
				//update siswa aktif
				$up = $this->mymodel->update("siswa_tk_aktif",$data_update, "id_siswa_tk_aktif", $value->id_siswa_tk_aktif);
				//update data siswa
				$data_update2 = array(
					"email_ms_office" => $nis."@labschoolcibubur.sch.id",
					"email_ms_office_ortu" => $nis."p@labschoolcibubur.sch.id"
				);
				$up2 = $this->mymodel->update("siswa_tk",$data_update2, "id_siswa_tk", $value->id_siswa_tk);
			}
		}
		redirect_back();		
	}
		...
		if (isset($_FILES["file_import"]["name"]) && !empty($_FILES["file_import"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					// 29=id_siswa_tk_aktif (primary key untuk import)
					...
					// Cari siswa berdasarkan id_siswa_tk_aktif (kolom 29) jika ada
					if (!empty($id_siswa_aktif_import)) {
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_tk_aktif, s.id_siswa_tk from siswa_tk_aktif sa join siswa_tk s on sa.id_siswa_tk = s.id_siswa_tk where sa.id_siswa_tk_aktif = " . $this->db->escape_str($id_siswa_aktif_import),
							"row"
						);
					}
					
					// Fallback: cari berdasarkan NIS
					if (!$get_siswa || empty($get_siswa->id_siswa_tk)) {
						$nis = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_tk_aktif, s.id_siswa_tk from siswa_tk_aktif sa join siswa_tk s on sa.id_siswa_tk = s.id_siswa_tk where sa.nis = " . $this->db->escape_str($nis),
							"row"
						);
					}
					...
					if (!$get_siswa || empty($get_siswa->id_siswa_tk)) {
						...
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_tk_aktif, s.id_siswa_tk from siswa_tk_aktif sa join siswa_tk s on sa.id_siswa_tk = s.id_siswa_tk where lower(sa.nama_lengkap) = lower(" . $this->db->escape_str($nama_lower) . ")",
							"row"
						);
					}
					...
					$id_siswa_aktif = (int)$get_siswa->id_siswa_tk_aktif;
					$id_siswa_tk    = (int)$get_siswa->id_siswa_tk;
					...
					$kelas_obj   = $this->mymodel->withquery(
						"select id_kelas_tk from kelas_tk where label like '%" . $this->db->escape_str($kelas_text) . "%'",
						"row"
					);
					...
					$kelas  = (!empty($kelas_obj))   ? $kelas_obj->id_kelas_tk   : null;
					...
					// Update siswa_tk_aktif berdasarkan id_siswa_tk_aktif
					$update_siswa_aktif = $this->mymodel->update("siswa_tk_aktif", $data_siswa_aktif, "id_siswa_tk_aktif", $id_siswa_aktif);
					...
					// Update siswa_tk
					$this->mymodel->update("siswa_tk", $data_siswa, "id_siswa_tk", $id_siswa_tk);
	...
	public function import_raport_siswa(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_raport']['name'][$i])) {
                  $trans_folder = './uploads/siswa_tk_aktif/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_raport']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("siswa_tk_aktif",$data, "nis", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/siswa_tk_aktif/").$filename_no_extension.".pdf",
	...
	/**
	 * IMPORT MASSAL CREATE (data historis, TIDAK terhubung PSB; id_siswa_tk = 0).
	...
	{
		if (!$this->is_allowed('siswa_tk_aktif_add')) {
			redirect('/', 'refresh');
		}
		$this->data['list_tahun_ajaran'] = $this->mymodel->withquery("SELECT id_tahun_ajaran, label FROM tahun_ajaran ORDER BY label DESC", "result");
		$this->data['preview'] = $this->session->userdata('import_create_tk');
		$this->template->title('Siswa Tk Aktif - Import Buat Baru');
		$this->render('backend/standart/administrator/siswa_tk_aktif/siswa_tk_aktif_import_create', $this->data);
	}
	...
	public function template_import_create()
	{
		...
		$this->load->library('excel');
		$objPHPExcel = new PHPExcel();
		$objPHPExcel->getProperties()->setTitle("Template Import Siswa TK Aktif (Buat Baru)");
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="template_import_siswa_tk_aktif.xls"');
		header('Cache-Control: max-age=0');
		PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5')->save('php://output');
		exit;
	}
	...
	public function import_create_parse()
	{
		...
		if (empty($id_tahun_ajaran)) {
			...
			redirect('administrator/siswa_tk_aktif/import_create');
		}
		...
		for ($row = 2; $row <= $highestRow; $row++) {
			...
			$cek_nis = $this->mymodel->getbywhere('siswa_tk_aktif', 'nis', $nis, 'row');
			if (!empty($cek_nis)) {
				$skipped[] = array('row' => $row, 'nama' => $nama, 'alasan' => 'NIS ' . $nis . ' sudah terdaftar (id aktif ' . $cek_nis->id_siswa_tk_aktif . ')');
				continue;
			}
		...
		$this->session->set_userdata('import_create_tk', array(
			'rows' => $rows,
			'skipped' => $skipped,
	...
	public function import_create_commit()
	{
		...
		$payload = $this->session->userdata('import_create_tk');
		...
		foreach ($payload['rows'] as $r) {
			$this->mymodel->insertid('siswa_tk_aktif', $r);
			$berhasil++;
		}
		...
		$this->session->unset_userdata('import_create_tk');
		...
		redirect('administrator/siswa_tk_aktif');
	}
...
/* End of file siswa_tk_aktif.php */
/* Location: ./application/controllers/administrator/Siswa TK Aktif.php */
