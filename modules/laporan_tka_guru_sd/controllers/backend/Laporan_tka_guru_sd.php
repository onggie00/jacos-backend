...
/**
*| --------------------------------------------------------------------------
*| Laporan Tka Guru Sd Controller
*| --------------------------------------------------------------------------
*| Laporan Tka Guru Sd site
*|
*/
class Laporan_tka_guru_sd extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_laporan_tka_guru_sd');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Laporan Tka Guru Sds
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('laporan_tka_guru_sd_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');
		$tahun 	= $this->input->get('tahun');

		$this->data['laporan_tka_guru_sds'] = $this->model_laporan_tka_guru_sd->get($filter, $field, $this->limit_page, $offset, [], $tahun);
		$this->data['laporan_tka_guru_sd_counts'] = $this->model_laporan_tka_guru_sd->count_all($filter, $field, $tahun);
		$this->data['list_tahun'] = $this->model_laporan_tka_guru_sd->get_distinct_tahun();

		$baseUrl = 'administrator/laporan_tka_guru_sd/index/';
		if (!empty($tahun)) {
			$baseUrl .= '?tahun='.$tahun;
		}
		...
		$config = [
			'base_url'     => $baseUrl,
			'total_rows'   => $this->model_laporan_tka_guru_sd->count_all($filter, $field, $tahun),
		...
		$this->template->title('TKA Guru SD List');
		$this->render('backend/standart/administrator/laporan_tka_guru_sd/laporan_tka_guru_sd_list', $this->data);
	}
	
	/**
	* Add new laporan_tka_guru_sds
	*
	*/
	public function add()
	{
		$this->is_allowed('laporan_tka_guru_sd_add');

		$this->template->title('TKA Guru SD New');
		$this->render('backend/standart/administrator/laporan_tka_guru_sd/laporan_tka_guru_sd_add', $this->data);
	}

	/**
	* Add New Laporan Tka Guru Sds
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('laporan_tka_guru_sd_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_tka_guru_sd = $this->model_laporan_tka_guru_sd->store($save_data);
            

			if ($save_laporan_tka_guru_sd) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_laporan_tka_guru_sd;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/laporan_tka_guru_sd/edit/' . $save_laporan_tka_guru_sd, 'Edit Laporan Tka Guru Sd'),
						anchor('administrator/laporan_tka_guru_sd', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/laporan_tka_guru_sd');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Laporan Tka Guru Sds
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('laporan_tka_guru_sd_update');

		$this->data['laporan_tka_guru_sd'] = $this->model_laporan_tka_guru_sd->find($id);

		$this->template->title('TKA Guru SD Update');
		$this->render('backend/standart/administrator/laporan_tka_guru_sd/laporan_tka_guru_sd_update', $this->data);
	}

	/**
	* Update Laporan Tka Guru Sds
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('laporan_tka_guru_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_tka_guru_sd = $this->model_laporan_tka_guru_sd->change($id, $save_data);
	...
	/**
	* delete Laporan Tka Guru Sds
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('laporan_tka_guru_sd_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'laporan_tka_guru_sd'), 'success');
        } else {
            set_message(cclang('error_delete', 'laporan_tka_guru_sd'), 'error');
        }
	...
	public function delete_by_tahun()
	{
		...
		$deleted = $this->db->delete('laporan_tka_guru_sd');
		...
		redirect('administrator/laporan_tka_guru_sd');
	}

		/**
	* View view Laporan Tka Guru Sds
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('laporan_tka_guru_sd_view');

		$this->data['laporan_tka_guru_sd'] = $this->model_laporan_tka_guru_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('TKA Guru SD Detail');
		$this->render('backend/standart/administrator/laporan_tka_guru_sd/laporan_tka_guru_sd_view', $this->data);
	}
	...
	*/
	...
	{
		$laporan_tka_guru_sd = $this->model_laporan_tka_guru_sd->find($id);

		
		
		return $this->model_laporan_tka_guru_sd->remove($id);
	}
	...
	*/
	...
	{
		$this->is_allowed('laporan_tka_guru_sd_export');
		...
		$join_sql = "from laporan_tka_guru_sd k 
		...
		$judul = "REKAP HASIL TES KOMPETENSI AKADEMIK GURU SD LABSCHOOL CIBUBUR ".(!empty($tahun) ? $tahun : "");
		...
		header('Content-Disposition: attachment;filename="Laporan TKA Guru SD - '.date("Y-m-d").'.xls"');
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_laporan_tka_guru_sd->pdf('laporan_tka_guru_sd', 'laporan_tka_guru_sd');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'laporan_tka_guru_sd';
        ...
        $data = $this->model_laporan_tka_guru_sd->find($id);
	...
	public function import()
	{
		...
		if (isset($_FILES["file_upload"]["name"])) {
			...
			$column_names = array(
				0 => 'NO (A)', 1 => 'KODE (B)', 2 => 'NAMA (C)', 3 => 'NPP (D)',
				4 => 'UNIT KERJA (E)', 5 => 'MATA PELAJARAN (F)', 6 => 'UMUR (G)',
				7 => 'MASA KERJA (H)', 8 => 'GOLONGAN (I)', 9 => 'B.INDO (J)',
				10 => 'B.ING (K)', 11 => 'NUMERASI (L)', 12 => 'SKOR TOTAL (M)',
				13 => 'RERATA UNIT (N)', 14 => 'RERATA SEKOLAH (O)',
				15 => 'RANK UNIT (P)', 16 => 'RANK (Q)'
			);
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 5; $row <= $highestRow; $row++) {
					...
					if (!$guru_not_found) {
						$check_nilai = $this->mymodel->withquery("select * from laporan_tka_guru_sd where id_guru = '" . $check->id_guru . "' and jenjang = '" . $jenjang . "' and tahun = '" . $tahun . "'", 'row');
					} else {
						$check_nilai = null;
					}
					...
					if (!empty($check_nilai)) {
						...
						$this->mymodel->update("laporan_tka_guru_sd", $data_laporan, 'id_laporan', $check_nilai->id_laporan);
						$insertId = $check_nilai->id_laporan;
					} else {
						$insertId = $this->mymodel->insertid("laporan_tka_guru_sd", $data_laporan);
					}
	...
	/**
	* Import Detail Nilai from excel
	* AJAX response (JSON)
	*/
	public function import_detail()
	{
		...
		try {
			...
			// Baca header Row 1 → identifikasi kolom No-X
			$no_columns = []; // ['no' => col_index]
			...
			for ($col = 0; $col < $highestColIndex; $col++) {
				...
				if (preg_match('/^No-(\d+)$/', $header, $m)) {
					$no_columns[$m[1]] = $col; // nomor_soal => col_index
				}
			}
			...
			if (empty($no_columns)) {
				echo json_encode([
					'success' => false,
					'message' => 'Tidak ditemukan kolom soal (No-1, No-2, dll) di header.'
				]);
				exit;
			}

			// Ambil semua indikator untuk tahun ini → key by no_urut
			$indikators = $this->mymodel->withquery(
				"SELECT id_indikator, no_urut FROM laporan_tka_indikator WHERE tahun = '".$tahun."'",
				"result"
			);
			$indikator_map = []; // no_urut => id_indikator
			...
			$success_count = 0;
			$detail_count = 0;
			$processed_laporan = []; // track id_laporan yang sudah dihapus detailnya

			$this->db->trans_begin();

			for ($row = 2; $row <= $highestRow; $row++) {
				$nama = trim($worksheet->getCellByColumnAndRow(1, $row)->getValue()); // col B = Nama Guru
				if (empty($nama)) continue;

				// Bersihkan nama: hilangkan tanda petik & ambil nama tanpa gelar
				$nama_bersih = trim(explode(',', str_replace("'", "", $nama))[0]);
				$nama_like = $this->db->escape_like_str($nama_bersih);

				// Cari guru berurutan: sd → smp → sma
				$id_laporan = null;

				// 1. Cek guru_sd
				$guru = $this->mymodel->withquery("SELECT id_guru FROM guru_sd WHERE nama_lengkap LIKE '%".$nama_like."%'", 'row');
				if (!empty($guru)) {
					$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'sd' AND tahun = '".$tahun."'", 'row');
					if (!empty($lap)) $id_laporan = $lap->id_laporan;
				}
				// fallback: cari by nama_lengkap di laporan (id_guru=0)
				if (empty($id_laporan)) {
					$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'sd' AND tahun = '".$tahun."'", 'row');
					if (!empty($lap)) $id_laporan = $lap->id_laporan;
				}
				...
				if (empty($id_laporan)) {
					...
					if (!empty($guru)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'smp' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
					if (empty($id_laporan)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'smp' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
				}
				...
				if (empty($id_laporan)) {
					...
					if (!empty($guru)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE id_guru = '".$guru->id_guru."' AND jenjang = 'sma' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
					if (empty($id_laporan)) {
						$lap = $this->mymodel->withquery("SELECT id_laporan FROM laporan_tka_guru_sd WHERE nama_lengkap LIKE '%".$nama_like."%' AND jenjang = 'sma' AND tahun = '".$tahun."'", 'row');
						if (!empty($lap)) $id_laporan = $lap->id_laporan;
					}
				}
				...
				// Hapus detail lama (re-import safe) - sekali per id_laporan
				if (!in_array($id_laporan, $processed_laporan)) {
					$this->db->where('id_laporan', $id_laporan)->delete('laporan_tka_detail_sd');
				...
				foreach ($no_columns as $no_soal => $col_idx) {
					...
					if (empty($keterangan) || ($keterangan !== 'Benar' && $keterangan !== 'Salah')) {
						continue; // skip selain Benar/Salah
					}
					...
					$save = $this->mymodel->insertid('laporan_tka_detail_sd', [
			...
			if (!empty($errors)) {
				...
				$error_html .= '<div style="max-height:300px;overflow-y:auto;text-align:left">';
				foreach ($errors as $i => $err) {
					$error_html .= '<b>'.($i + 1).'.</b> '.htmlspecialchars($err).'<br>';
				}
				$error_html .= '</div>';
				echo json_encode([
					'success' => false,
				...
				echo json_encode([
					'success' => true,
					'message' => 'Import berhasil! '.$success_count.' guru diproses, '.$detail_count.' detail nilai diimport.'
				]);
			}
	...
	*/
	public function detail_nilai($id_laporan)
	{
		...
		$laporan = $this->model_laporan_tka_guru_sd->find($id_laporan);
		if (empty($laporan)) {
...
/* End of file laporan_tka_guru_sd.php */
/* Location: ./application/controllers/administrator/Laporan Tka Guru Sd.php */
