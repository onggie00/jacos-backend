...
class Daftar_ekskul extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        $nominal_biaya = $this->post('nominal_biaya');

        $get_ekskul = $this->mymodel->withquery("select * from ekskul where id_ekskul = '".$id_ekskul."'", "row");
        $data = array(
            "id_ekskul" => $id_ekskul,
            "id_siswa" => $id_siswa,
            "status_member" => 0,
            ...
            "tahun_ajaran" => (!empty($tahun_ajaran)) ? $tahun_ajaran : $get_ekskul->tahun_ajaran,
            "semester" => (!empty($semester)) ? $semester : $get_ekskul->semester,
            "biaya" => (!empty($nominal_biaya)) ? $nominal_biaya : $get_ekskul->nominal_biaya,
            "jenjang" => $jenjang
        );
        ...
        if (!empty($data) && empty($cek)) {
            ...
            $insert = $this->mymodel->insertid("ekskul_member_".$jenjang, $data);
            ...
            if ($insert) {
                ...
                $msg = array('status' => 200, 'message'=>'Gagal Registrasi, Silahkan hubungi admin labscib' ,'data'=>$data);
            }
        }
        else if (!empty($cek)) {
