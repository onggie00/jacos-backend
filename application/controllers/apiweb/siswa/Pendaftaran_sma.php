...
class Pendaftaran_sma extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->mymodel->withquery("select id_siswa_sma from siswa_sma where email like '%".$this->post("email")."%' and is_show = 1","row");
        // dd($cek_email);
        if (empty($cek_email)) {
          $sekolah_asal = "";
          ...
          $cek_email = $this->mymodel->withquery("select * from siswa_sma where email like '%".$this->db->escape_like_str($this->post("email"))."%'", "row");
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              if ($ppsbb == "1") {
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_sma as id_siswa from siswa_sma where va_number like '".$prefix.$client_id.date('y', strtotime('+1 years')).$va_class."%' and va_number != '' order by id_siswa_sma DESC","row");
