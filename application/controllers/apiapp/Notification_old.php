  ...
  public function index()
  {
    ...
    if (!$data_json) {
      ...
      if (!empty($data_json['brivaNo'])) {
        ...
        //$get_transaksi = $this->mymodel->getbywhere("transaksi", "va_number = '".$data_json['brivaNo']."' and status_transaksi = ",0, "row");
        $get_transaksi = $this->mymodel->withquery("select * from transaksi where va_number = '".$data_json['brivaNo']."' and status_transaksi = '0' order by id_transaksi DESC", "row");
        if ($get_transaksi) {
          $no_peserta = "";
          ...
          $transaksi = $this->mymodel->update("transaksi", $data_transaksi, "no_transaksi", $get_transaksi->no_transaksi);
          ...
          $get_transaksi_new = $this->mymodel->withquery("select id_transaksi, no_transaksi, va_number, nama_bank, total_biaya, expired_datetime, status_transaksi from transaksi where no_transaksi = '" . $no_transaksi . "'", "row");
          ...
          $data_email = array(
            ...
            "kwitansi" => $get_transaksi->no_transaksi . '-' . $cek_no_peserta->nama_lengkap . '.pdf',
          );
        ...
        $get_transaksi = $this->mymodel->withquery("select * from transaksi where va_number = '".$data_json['brivaNo']."' and status_transaksi = '1' order by id_transaksi DESC", "row");
        if ($get_transaksi) {
          ...
          if($paymentAmount>=$get_transaksi->total_biaya){
            $this->mymodel->update("transaksi", array("status_transaksi" => 1, "updated_at" => date("Y-m-d H:i:s"), "payment_amount" => $paymentAmount), "id_transaksi", $get_transaksi->id_transaksi);
            ...
            //$this->mymodel->update("transaksi", array("payment_amount" => $paymentAmount), "va_number", "id_transaksi", $get_transaksi->id_transaksi);
