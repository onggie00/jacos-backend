...
class Model_program_anggaran_smp extends MY_Model {

    private $primary_key    = 'id';
    private $table_name     = 'program_anggaran_smp';
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "program_anggaran_smp.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "program_anggaran_smp.".$field . " LIKE '%" . $q . "%' ";
        ...
        $this->db->where($where);
        $this->db->group_by('program_anggaran_smp.id');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = array())
    {
        ...
        $this->db->limit($limit, $offset);
                $this->db->order_by('program_anggaran_smp.'.$this->primary_key, "DESC");
                $query = $this->db->get($this->table_name);
    ...
    {
        if ($field === 'status_pengajuan') {
            return 'program_anggaran_smp.status_pengajuan';
        }
        if ($field === 'status_laporan') {
            return 'program_anggaran_smp.status_laporan';
        }
        return 'program_anggaran_smp.' . $field;
    }
        ...
        if (!empty($search)) {
            ...
            foreach ($this->field_search as $field) {
                if ($field === 'status_pengajuan') {
                    ...
                    $column = 'program_anggaran_smp.' . $field;
                }
    ...
    public function join_avaiable() {
        $this->db->join('program_anggaran', 'program_anggaran.id = program_anggaran_smp.nomor_program', 'LEFT');
        $this->db->join('program_anggaran_status_pengajuan', 'program_anggaran_status_pengajuan.id_program_anggaran_status_pengajuan = program_anggaran_smp.status_pengajuan', 'LEFT');
        $this->db->join('program_anggaran_status_laporan', 'program_anggaran_status_laporan.id_program_anggaran_status_laporan = program_anggaran_smp.status_laporan', 'LEFT');

        $this->db->select('program_anggaran_smp.*,program_anggaran.nomor_program as program_anggaran_nomor_program,program_anggaran_status_pengajuan.status as program_anggaran_status_pengajuan_status,program_anggaran_status_laporan.status as program_anggaran_status_laporan_status');
...
/* End of file Model_program_anggaran_smp.php */
/* Location: ./application/models/Model_program_anggaran_smp.php */