...
class MoodleRest
{
    ...
    private function setUrl($url)
    {
        ...
        $this->url_decoded = str_replace(' ', '', urldecode($url));
    }
    ...
    public function getUrl($decoded = true)
    {
        ...
        $url = $decoded ? $this->url_decoded : $this->url;
    ...
    public function request($function, $parameters = null, $method = self::METHOD_GET)
    {
        ...
        if ($this->getMethod() != self::METHOD_POST) {
            ...
            /*$ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $this->getUrl(false));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_HEADER, 1);
            curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/37.0.2062.120 Safari/537.36");
            $output = curl_exec($ch);
            curl_close($ch);
            $moodle_request = $output;*/
            //print_r($this->getUrl(false));
            //var_dump($moodle_request);
