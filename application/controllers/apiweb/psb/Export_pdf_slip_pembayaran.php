...
class Export_pdf_slip_pembayaran extends MY_Controller {
    ...
    public function index()
    {
      ...
      if(isset($headers['user_role']))
        ...
        $datas['siswa'] = $data;
        $get_transaksi = $this->mymodel->withquery("select * from transaksi where no_transaksi = '".$data->no_transaksi."'","row");
        ...
        $datas['mohon_dibaca'] = $this->mymodel->withquery("select * from catatan_invoice","row")->catatan_mohon_dibaca;

        $pdf = new HTML2PDF('P', 'A4', 'en');
        ob_start();
        
        $this->load->view('template_slip_pembayaran', $datas);
        ...
        //var_dump($data);
        $pdf->Output(/*FCPATH.*/'https://admin.labschoolcibubur.sch.id'.'/uploads/slip_pembayaran/'.$nama_file, 'F');
        //$pdf->Output(/*FCPATH.*/'https://admin.labschoolcibubur.sch.id'.'/uploads/kartu_peserta/'.$data->no_transaksi.'.pdf', 'F');
