...
/**
*| --------------------------------------------------------------------------
*| Program Anggaran sma Controller
*| --------------------------------------------------------------------------
*| Program Anggaran sma site
*|
*/
class Program_anggaran_sma extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_program_anggaran_sma');
	...
	function get_program($id)
    {
        ...
        $get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_sma", "nomor_program = '".$data->nomor_program."' and jenis_kegiatan=", $data->jenis_kegiatan,"result");
        $okr_terpakai = 0;
	...
	/**
	* Get detail Program Anggaran SMA untuk modal
	*
	* @var $id String
	* @return JSON
	*/
	...
	{
		if (!$this->is_allowed('program_anggaran_sma_view', false)) {
		...
		$program_anggaran_sma = $this->model_program_anggaran_sma->join_avaiable()->filter_avaiable()->find($id);

		if (!$program_anggaran_sma) {
			echo json_encode(array(
				'success' => false,
		...
		// Format tanggal
		$tanggal_program       = !empty($program_anggaran_sma->tanggal_program) ? date("d-m-Y", strtotime($program_anggaran_sma->tanggal_program)) : '-';
		$tanggal_pencairan     = !empty($program_anggaran_sma->tanggal_pencairan) ? date("d-m-Y", strtotime($program_anggaran_sma->tanggal_pencairan)) : '-';
		$tanggal_pengajuan     = !empty($program_anggaran_sma->created_at) ? date("d-m-Y H:i", strtotime($program_anggaran_sma->created_at)) : '-';
		...
		$data = array(
			'success' => true,
			'nomor_program' => $program_anggaran_sma->nomor_program,
			'nama_program' => $program_anggaran_sma->nama_program,
			'jenis_kegiatan' => $program_anggaran_sma->jenis_kegiatan,
			'sub_jenis_kegiatan' => $program_anggaran_sma->sub_jenis_kegiatan,
			'tahun_ajaran' => $program_anggaran_sma->tahun_ajaran,
			'tanggal_program' => $tanggal_program,
			'tanggal_pengajuan' => $tanggal_pengajuan,
			'tanggal_pencairan' => $tanggal_pencairan,
			'nominal_okr' => number_format($program_anggaran_sma->nominal_okr, 0, ',', '.'),
			'nominal_pengajuan' => number_format($program_anggaran_sma->nominal_pengajuan, 0, ',', '.'),
			'sisa_okr' => number_format($sisa_okr, 0, ',', '.'),
			'dasar_pelaksanaan_kegiatan' => $program_anggaran_sma->dasar_pelaksanaan_kegiatan,
			'tema_kegiatan' => $program_anggaran_sma->tema_kegiatan,
			'tujuan_kegiatan' => $program_anggaran_sma->tujuan_kegiatan,
			'gambaran_acara_kegiatan' => $program_anggaran_sma->gambaran_acara_kegiatan,
			'hasil_yang_diharapkan' => $program_anggaran_sma->hasil_yang_diharapkan,
			'tempat_dan_waktu_pelaksanaan' => $program_anggaran_sma->tempat_dan_waktu_pelaksanaan,
			'mitra_kegiatan' => $program_anggaran_sma->mitra_kegiatan,
			'kepanitiaan' => $program_anggaran_sma->kepanitiaan,
			'catatan_pengajuan' => $program_anggaran_sma->catatan_pengajuan,
			'catatan_laporan' => $program_anggaran_sma->catatan_laporan,
			'status_pengajuan_text' => $program_anggaran_sma->program_anggaran_status_pengajuan_status,
			'status_pengajuan_color' => $sp_color,
			'status_laporan_text' => $program_anggaran_sma->program_anggaran_status_laporan_status,
			'status_laporan_color' => $sl_color,
			'file_proposal_pengajuan' => $program_anggaran_sma->file_proposal_pengajuan,
			'file_proposal_keuangan' => $program_anggaran_sma->file_proposal_keuangan,
			'file_laporan_kegiatan' => $program_anggaran_sma->file_laporan_kegiatan,
			'file_laporan_keuangan' => $program_anggaran_sma->file_laporan_keuangan,
			'file_kwitansi' => $program_anggaran_sma->file_kwitansi,
		);
	...
	/**
	* show all Program Anggaran smas
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('program_anggaran_sma_list');
		...
		$this->data['program_anggaran_smas'] = $this->model_program_anggaran_sma->get_filtered($filters, $search, $this->limit_page, $offset);
		$this->data['program_anggaran_sma_counts'] = $this->model_program_anggaran_sma->count_filtered($filters, $search);
		//$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_sma where (status_pengajuan != '4' or status_pengajuan != '6') and status_laporan != '4'","row")->total_proposal;
		$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_sma where (status_pengajuan = '1' or status_pengajuan = '2' or status_pengajuan = '3' or status_pengajuan = '5') or (status_laporan = '1' or status_laporan = '2' or status_laporan = '3' or status_laporan = '5' or status_laporan IS NULL)","row")->total_proposal;
		//$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_sma where (status_laporan != '4')","row")->total_proposal;

		$config = [
			'base_url'     => 'administrator/program_anggaran_sma/index/',
			'total_rows'   => $this->model_program_anggaran_sma->count_filtered($filters, $search),
		...
		$this->template->title('Program Anggaran sma List');
		$this->render('backend/standart/administrator/program_anggaran_sma/program_anggaran_sma_list', $this->data);
	}
	
	/**
	* Add new program_anggaran_smas
	*
	*/
	public function add()
	{
		$this->is_allowed('program_anggaran_sma_add');

		$this->template->title('Program Anggaran sma New');
		$this->render('backend/standart/administrator/program_anggaran_sma/program_anggaran_sma_add', $this->data);
	}

	/**
	* Add New Program Anggaran smas
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('program_anggaran_sma_add', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			$program_anggaran_sma_file_proposal_pengajuan_uuid = $this->input->post('program_anggaran_sma_file_proposal_pengajuan_uuid');
			$program_anggaran_sma_file_proposal_pengajuan_name = $this->input->post('program_anggaran_sma_file_proposal_pengajuan_name');
			$program_anggaran_sma_file_proposal_keuangan_uuid = $this->input->post('program_anggaran_sma_file_proposal_keuangan_uuid');
			$program_anggaran_sma_file_proposal_keuangan_name = $this->input->post('program_anggaran_sma_file_proposal_keuangan_name');
        	...
        	$get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_sma", "nomor_program = '".$get_program->nomor_program."' and jenis_kegiatan=", $get_program->jenis_kegiatan,"result");
        	$okr_terpakai = 0;
        	foreach ($get_program_by_jenis_kegiatan as $key => $value) {
                        $okr_terpakai = $okr_terpakai + $value->nominal_pengajuan;
        	}
			...
			if (!is_dir(FCPATH . '/uploads/program_anggaran_sma/')) {
				mkdir(FCPATH . '/uploads/program_anggaran_sma/');
			}

			if (!empty($program_anggaran_sma_file_proposal_pengajuan_name)) {
				$program_anggaran_sma_file_proposal_pengajuan_name_copy = date('YmdHis') . '-' . $program_anggaran_sma_file_proposal_pengajuan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_sma_file_proposal_pengajuan_uuid . '/' . $program_anggaran_sma_file_proposal_pengajuan_name, 
						FCPATH . 'uploads/program_anggaran_sma/' . $program_anggaran_sma_file_proposal_pengajuan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma_file_proposal_pengajuan_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['file_proposal_pengajuan'] = $program_anggaran_sma_file_proposal_pengajuan_name_copy;
			}
		
			if (!empty($program_anggaran_sma_file_proposal_keuangan_name)) {
				$program_anggaran_sma_file_proposal_keuangan_name_copy = date('YmdHis') . '-' . $program_anggaran_sma_file_proposal_keuangan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_sma_file_proposal_keuangan_uuid . '/' . $program_anggaran_sma_file_proposal_keuangan_name, 
						FCPATH . 'uploads/program_anggaran_sma/' . $program_anggaran_sma_file_proposal_keuangan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma_file_proposal_keuangan_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['file_proposal_keuangan'] = $program_anggaran_sma_file_proposal_keuangan_name_copy;
			}

			if (!empty($save_data['file_proposal_pengajuan']) && !empty($save_data['file_proposal_keuangan'])) {
				$save_data['status_pengajuan'] = "2";
				$data_email["file_proposal_keuangan"] = $program_anggaran_sma_file_proposal_keuangan_name_copy;
				$data_email["file_proposal_pengajuan"] = $program_anggaran_sma_file_proposal_pengajuan_name_copy;
				$data["pengajuan_laporan"] = "pengajuan";
			}		
			
			$save_program_anggaran_sma = $this->model_program_anggaran_sma->store($save_data);
            

			if ($save_program_anggaran_sma) {
				if ($save_data['status_pengajuan'] == 2) {
					//kirim email ke Bagian Anggaran
					$save_data["nominal_okr"] = $get_program->nominal_okr;
					$save_data["id_program"] = $save_program_anggaran_sma;
					...
					$data_email["id_program"] = $save_program_anggaran_sma;
					$data_email["jenis_anggaran"] = "SMA";
					$data_email["data_anggaran"] = $save_data;
	                $this->send_email_file("",$data_email['email'],$data_email);	
				}
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_program_anggaran_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/program_anggaran_sma/edit/' . $save_program_anggaran_sma, 'Edit Program Anggaran sma'),
						anchor('administrator/program_anggaran_sma', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/program_anggaran_sma');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = false;
		...
		/**
	* Update view Program Anggaran smas
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('program_anggaran_sma_update');

		$this->data['program_anggaran_sma'] = $this->model_program_anggaran_sma->find($id);

		$this->template->title('Program Anggaran sma Update');
		$this->render('backend/standart/administrator/program_anggaran_sma/program_anggaran_sma_update', $this->data);
	}

	/**
	* Update Program Anggaran smas
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('program_anggaran_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$program_anggaran_sma_file_proposal_keuangan_uuid = $this->input->post('program_anggaran_sma_file_proposal_keuangan_uuid');
			$program_anggaran_sma_file_proposal_keuangan_name = $this->input->post('program_anggaran_sma_file_proposal_keuangan_name');
			$program_anggaran_sma_file_laporan_kegiatan_uuid = $this->input->post('program_anggaran_sma_file_laporan_kegiatan_uuid');
			$program_anggaran_sma_file_laporan_kegiatan_name = $this->input->post('program_anggaran_sma_file_laporan_kegiatan_name');
		
			//cek OKR
			//hitung max okr yang tersisa
			$get_program = $this->mymodel->getbywhere("program_anggaran_sma","id",$id,"row");
            $okr = $get_program->nominal_okr;
            $this->db->where("nomor_program", $this->input->post('nomor_program'));
            $this->db->where("jenis_kegiatan", $this->input->post('jenis_kegiatan'));
            $this->db->where("id !=", $id);
            $get_program_by_jenis_kegiatan = $this->db->get("program_anggaran_sma")->result();
            $okr_terpakai = 0;
            foreach ($get_program_by_jenis_kegiatan as $key => $value) {
                $okr_terpakai = $okr_terpakai + $value->nominal_pengajuan;
            }
			...
			if (!empty($program_anggaran_sma_file_proposal_pengajuan_uuid)) {
				$program_anggaran_sma_file_proposal_pengajuan_name_copy = date('YmdHis') . '-' . $program_anggaran_sma_file_proposal_pengajuan_name;
			...
			if (!empty($program_anggaran_sma_file_proposal_keuangan_uuid)) {
				$program_anggaran_sma_file_proposal_keuangan_name_copy = date('YmdHis') . '-' . $program_anggaran_sma_file_proposal_keuangan_name;
			...
			if (!empty($program_anggaran_sma_file_laporan_kegiatan_uuid)) {
				$program_anggaran_sma_file_laporan_kegiatan_name_copy = date('YmdHis') . '-' . $program_anggaran_sma_file_laporan_kegiatan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_sma_file_laporan_kegiatan_uuid . '/' . $program_anggaran_sma_file_laporan_kegiatan_name, 
						FCPATH . 'uploads/program_anggaran_sma/' . $program_anggaran_sma_file_laporan_kegiatan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma_file_laporan_kegiatan_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['file_laporan_kegiatan'] = $program_anggaran_sma_file_laporan_kegiatan_name_copy;
				$save_data['status_laporan'] = 2;
				$save_data['status'] = 2;
				$save_data['catatan_laporan'] = "";
				$data_email["file_laporan_kegiatan"] = $program_anggaran_sma_file_laporan_kegiatan_name_copy;
				$data["pengajuan_laporan"] = "laporan";
			}
		
			$listed_image = [];
			//if (count((array) $this->input->post('program_anggaran_sma_file_laporan_keuangan_name'))) {
			if (!empty($this->input->post('program_anggaran_sma_file_laporan_keuangan_name')[0])) {
				foreach ((array) $_POST['program_anggaran_sma_file_laporan_keuangan_name'] as $idx => $file_name) {
					if (isset($_POST['program_anggaran_sma_file_laporan_keuangan_uuid'][$idx]) AND !empty($_POST['program_anggaran_sma_file_laporan_keuangan_uuid'][$idx])) {
						$program_anggaran_sma_file_laporan_keuangan_name_copy = date('YmdHis') . '-' . $idx . '-' . $file_name;

						rename(FCPATH . 'uploads/tmp/' . $_POST['program_anggaran_sma_file_laporan_keuangan_uuid'][$idx] . '/' .  $file_name, 
								FCPATH . 'uploads/program_anggaran_sma/' . $program_anggaran_sma_file_laporan_keuangan_name_copy);

						$listed_image[] = $program_anggaran_sma_file_laporan_keuangan_name_copy;

						if (!is_file(FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma_file_laporan_keuangan_name_copy)) {
							echo json_encode([
								'success' => false,
								'message' => 'Error uploading file'
								]);
			...
			$save_program_anggaran_sma = $this->model_program_anggaran_sma->change($id, $save_data);

			if ($save_program_anggaran_sma) {
				if (!empty($this->input->post('tanggal_pencairan')) && $this->session->userdata('user_keuangan') == true) {
					$this->cetak_kwitansi_program($id, "SMA");
				}
	...
	/**
	* delete Program Anggaran smas
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('program_anggaran_sma_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'program_anggaran_sma'), 'success');
        } else {
            set_message(cclang('error_delete', 'program_anggaran_sma'), 'error');
        }
		...
		/**
	* View view Program Anggaran smas
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('program_anggaran_sma_view');

		$this->data['program_anggaran_sma'] = $this->model_program_anggaran_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Program Anggaran sma Detail');
		$this->render('backend/standart/administrator/program_anggaran_sma/program_anggaran_sma_view', $this->data);
	}
	...
	*/
	...
	{
		$program_anggaran_sma = $this->model_program_anggaran_sma->find($id);

		if (!empty($program_anggaran_sma->file_proposal_pengajuan)) {
			$path = FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma->file_proposal_pengajuan;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($program_anggaran_sma->file_proposal_keuangan)) {
			$path = FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma->file_proposal_keuangan;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($program_anggaran_sma->file_laporan_kegiatan)) {
			$path = FCPATH . '/uploads/program_anggaran_sma/' . $program_anggaran_sma->file_laporan_kegiatan;
		...
		if (!empty($program_anggaran_sma->file_laporan_keuangan)) {
			foreach ((array) explode(',', $program_anggaran_sma->file_laporan_keuangan) as $filename) {
				$path = FCPATH . '/uploads/program_anggaran_sma/' . $filename;
		...
		return $this->model_program_anggaran_sma->remove($id);
	}
	
	/**
	* Upload Image Program Anggaran sma	* 
	* @return JSON
	*/
	public function upload_file_proposal_pengajuan_file()
	{
		if (!$this->is_allowed('program_anggaran_sma_add', false) && !$this->is_allowed('program_anggaran_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		echo $this->upload_file([
			'uuid' 		 	=> $uuid,
			'table_name' 	=> 'program_anggaran_sma',
		]);
	}

	/**
	* Delete Image Program Anggaran sma	* 
	* @return JSON
	*/
	public function delete_file_proposal_pengajuan_file($uuid)
	{
		if (!$this->is_allowed('program_anggaran_sma_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'table_name'        => 'program_anggaran_sma',
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_sma/'
        ]);
	}

	/**
	* Get Image Program Anggaran sma	* 
	* @return JSON
	*/
	public function get_file_proposal_pengajuan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_sma/',
            'delete_endpoint'   => 'administrator/program_anggaran_sma/delete_file_proposal_pengajuan_file'
        ]);
	}
	...
	*/
	public function get_file_proposal_keuangan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_sma/',
            'delete_endpoint'   => 'administrator/program_anggaran_sma/delete_file_proposal_keuangan_file'
        ]);
	}
	...
	*/
	public function get_file_laporan_kegiatan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_sma/',
            'delete_endpoint'   => 'administrator/program_anggaran_sma/delete_file_laporan_kegiatan_file'
        ]);
	}
	...
	*/
	public function get_file_laporan_keuangan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_sma/',
            'delete_endpoint'   => 'administrator/program_anggaran_sma/delete_file_laporan_keuangan_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('program_anggaran_sma_export');

		//$this->model_program_anggaran_sma->export('program_anggaran_sma', 'program_anggaran_sma');
		...
		foreach ($get_data as $key => $value) {
            ...
            $get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_sma", "nomor_program = '".$value->nomor_program."' and jenis_kegiatan=", $value->jenis_kegiatan,"result");
            $okr_terpakai = 0;
			...
			for ($j=0; $j < count($field_search); $j++) {
		        ...
		        if (strpos($kolom, "file") !== false && !empty($value_data)) {
		        	$value_data = base_url("uploads/program_anggaran_sma/").$value_data;
		        }
		...
		header('Content-Disposition: attachment;filename="Program Anggaran sma_'.date("Y-m-d Hi").'.xls"'); 
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_program_anggaran_sma->pdf('program_anggaran_sma', 'program_anggaran_sma');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'program_anggaran_sma';
        ...
        $data = $this->model_program_anggaran_sma->find($id);
	...
	public function cetak_kwitansi_program($id_program,$jenjang_program){
        ...
        foreach ($array_program as $key => $value) {
		  ...
		  $pdf->WriteHTML($html);
		  $nama_file = date("dmYHi")."-".str_replace('/', '-', $get_program->nama_program)."-SMA-".$get_program->id.'.pdf';
		  $this->mymodel->update("program_anggaran_sma", array("file_kwitansi" => $nama_file), "id", $id_program);
		  //var_dump($data);
		  $pdf->Output(FCPATH.'/uploads/program_anggaran_sma/'.$nama_file, 'F');
	  }

	  public function resend_email($id){
	  	$get_program = $this->mymodel->withquery("select * from program_anggaran_sma where id='".$id."'","row");
	  	$save_data = [
			'nomor_program' => $get_program->nomor_program,
			'jenis_kegiatan' => $get_program->jenis_kegiatan,
			'sub_jenis_kegiatan' => $get_program->sub_jenis_kegiatan,
		...
		$save_data["id_program"] = $id;
		$save_data["jenis_anggaran"] = "SMA";
		$save_data["data_anggaran"] = $save_data;
		$this->send_email_file("",$save_data['email'],$save_data);
		redirect('administrator/program_anggaran_sma','refresh');
	 }
	
}


/* End of file program_anggaran_sma.php */
/* Location: ./application/controllers/administrator/Program Anggaran sma.php */