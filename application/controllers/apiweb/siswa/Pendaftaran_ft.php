    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        $gelombang = $this->mymodel->withquery("select gelombang from web_periode_daftar where periode_pendaftaran_mulai <= '".date("Y-m-d H:i:s")."' and periode_pendaftaran_selesai >= '".date("Y-m-d H:i:s")."' and kelas = 'France Track' ","row")->gelombang;
          ...
          //validasi jenis_ppsbb
          if ($this->post("ppsbb") == "1") {
            if (empty($this->post("jenis_ppsbb"))) {
              $msg = array('success'=>0,'message'=>'Jenis PPSBB wajib diisi','data'=>[]);
              $this->response($msg,'200');
              return;
            }
            $cek_tipe_ppsbb = $this->mymodel->withquery("select id from tipe_ppsbb where id = '".$this->db->escape($this->post("jenis_ppsbb"))."'", "row");
