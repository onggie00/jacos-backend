...
class Update_setting_notifikasi extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        $get_data_setting = $this->second_db->getbywhere("setting_notifikasi_siswa","id_siswa_aktif = '".$id_akun."' and jenjang=",$jenjang,"row");
        if (!empty($data_update) && !empty($get_data_setting)) {
            $this->second_db->update("setting_notifikasi_siswa", $data_update, "id_siswa_aktif = '".$id_akun."' and jenjang=", $jenjang);
            ...
            $this->second_db->insert("setting_notifikasi_siswa", $data_update);
            $msg = array('status' => 200, 'message'=>'Data Setting Notifikasi diubah' ,'data'=>$data_update);
        }
        else{
