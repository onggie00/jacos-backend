...
class Pendaftaran_sma extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->mymodel->withquery("select id_siswa_sma from siswa_sma where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");
        // dd($cek_email);
        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->mymodel->getbywhere("list_sekolah_smp", "id_list_sekolah_smp",$this->db->escape($this->post("sekolah_asal")),"row")->nama_sekolah;
            ...
            // ponytail: fix bug — typo fatal $this->$this->post() bikin fatal error
            $sekolah_asal = $this->post("sekolah_asal");
          }
          $pekerjaan_ibu = "";
          $pekerjaan_ayah = "";
          if ($this->post("pekerjaan_ayah") > 0) {
            $pekerjaan_ayah = $this->mymodel->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->db->escape($this->post("pekerjaan_ayah"))."'","row")->nama_pekerjaan;
          ...
          if ($this->post("pekerjaan_ibu") > 0) {
            $pekerjaan_ibu = $this->mymodel->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->db->escape($this->post("pekerjaan_ibu"))."'","row")->nama_pekerjaan;
          ...
          //cek email duplicate
          // ponytail: fix bug — tambah escape_like_str di email dan is_show=1 agar konsisten dgn SMP
          $cek_email = $this->mymodel->withquery("select * from siswa_sma where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".$this->post("tgl_lahir")."' and is_show = 1", "row");
          if(!empty($cek_email)){
            ...
            $id_siswa = $this->mymodel->insertid("siswa_sma",$data);
            ...
            if (!empty($id_siswa)) {
              ...
              $id_raport = $this->mymodel->insertid("nilai_raport_sma",$data_raport);
              //insert transaksi
              ...
              if ($ppsbb == "1") {
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_sma as id_siswa from siswa_sma where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')).$va_class)."%' and va_number != '' order by id_siswa_sma DESC","row");
              ...
              if (!empty($data_transaksi)) {
                ...
                $this->mymodel->update("siswa_sma", array("no_transaksi" => $no_transaksi, "va_number" => $va_number), "id_siswa_sma", $id_siswa);
