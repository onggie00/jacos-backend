...
class Guru_smp extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_guru_smp');
	...
	*/
	public function index($offset = 0)
	{
		...
		$this->data['guru_smps'] = $this->model_guru_smp->get($filter, $field, $this->limit_page, $offset,[],$sort,$sort_type);
		$this->data['guru_smp_counts'] = $this->model_guru_smp->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/guru_smp/index/',
			'total_rows'   => $this->model_guru_smp->count_all($filter, $field),
		...
		$this->template->title('Guru Smp List');
		$this->render('backend/standart/administrator/guru_smp/guru_smp_list', $this->data);
	}
	
	public function add_slip_gaji()
	{
		$this->is_allowed('guru_smp_add');

		$this->template->title('Guru SMP Slip Gaji');
		$this->render('backend/standart/administrator/guru_smp/guru_smp_add_slip_gaji', $this->data);
	}

	public function add_slip_gaji_save()
	{
		if (!$this->is_allowed('guru_smp_add', false)) {
			echo json_encode([
				'success' => false,
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/guru_smp/')) {
				mkdir(FCPATH . '/uploads/guru_smp/');
			}
			...
			if (!empty($slip_gaji_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/guru_smp/' . $slip_gaji_name_copy
				);

				if (!is_file(FCPATH . '/uploads/guru_smp/' . $slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_guru_smp = $this->model_guru_smp->change($item, $save_data);
		}
		...
		if ($save_guru_smp) {
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->data['id'] 	   = $save_guru_smp;
				$this->data['message'] = cclang('success_save_data_stay', [

					anchor('administrator/guru_smp', ' Go back to list')
				]);
			} else {
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/guru_smp/edit/' . $save_guru_smp, 'Edit Siswa Ft')
					]),
					'success'
				);

				$this->data['success'] = true;
				$this->data['redirect'] = base_url('administrator/guru_smp');
			}
		} else {
			if ($this->input->post('save_type') == 'stay') {
	...
	*/
	public function add()
	{
		...
		$this->template->title('Guru Smp New');
		$this->render('backend/standart/administrator/guru_smp/guru_smp_add', $this->data);
	}
	...
	*/
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			$guru_smp_foto_profil_uuid = $this->input->post('guru_smp_foto_profil_uuid');
			$guru_smp_foto_profil_name = $this->input->post('guru_smp_foto_profil_name');
			$guru_smp_slip_gaji_uuid = $this->input->post('guru_smp_slip_gaji_uuid');
			$guru_smp_slip_gaji_name = $this->input->post('guru_smp_slip_gaji_name');
		
			$check=$this->mymodel->withquery("select * from guru_smp where emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
			...
			if (!empty($guru_smp_foto_profil_name)) {
				$guru_smp_foto_profil_name_copy = date('YmdHis') . '-' . $guru_smp_foto_profil_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_smp_foto_profil_uuid . '/' . $guru_smp_foto_profil_name, 
						FCPATH . 'uploads/guru_smp/' . $guru_smp_foto_profil_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_smp/' . $guru_smp_foto_profil_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['foto_profil'] = $guru_smp_foto_profil_name_copy;
			}
		
			if (!empty($guru_smp_slip_gaji_name)) {
				$guru_smp_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_smp_slip_gaji_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_smp_slip_gaji_uuid . '/' . $guru_smp_slip_gaji_name, 
						FCPATH . 'uploads/guru_smp/' . $guru_smp_slip_gaji_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_smp/' . $guru_smp_slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['slip_gaji'] = $guru_smp_slip_gaji_name_copy;
			}
		
			
			$save_guru_smp = $this->model_guru_smp->store($save_data);
			...
			if ($save_guru_smp) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_guru_smp;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/guru_smp/edit/' . $save_guru_smp, 'Edit Guru Smp'),
						anchor('administrator/guru_smp', ' Go back to list')
					]);
				} else {
					set_message(
		...
		/**
	* Update view Guru Smps
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('guru_smp_update');

		$this->data['guru_smp'] = $this->model_guru_smp->find($id);

		$this->template->title('Guru Smp Update');
		$this->render('backend/standart/administrator/guru_smp/guru_smp_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('guru_smp_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$check=$this->mymodel->withquery("select * from guru_smp where id_guru != $id and emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
				$this->db->trans_rollback();
				$this->load->library("session");
				$this->session->set_flashdata('error', "Emp Code sudah digunakan!");
			...
			if (!empty($guru_smp_foto_profil_uuid)) {
				$guru_smp_foto_profil_name_copy = date('YmdHis') . '-' . $guru_smp_foto_profil_name;
			...
			if (!empty($guru_smp_slip_gaji_uuid)) {
				$guru_smp_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_smp_slip_gaji_name;
			...
			$get_data = $this->mymodel->withquery("select * from guru_smp where id_guru = ?", 'row', [$id]);
			$save_guru_smp = $this->model_guru_smp->change($id, $save_data);
	...
	*/
	...
	{
		$this->is_allowed('guru_smp_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'guru_smp'), 'success');
        } else {
            set_message(cclang('error_delete', 'guru_smp'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('guru_smp_view');

		$this->data['guru_smp'] = $this->model_guru_smp->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Guru Smp Detail');
		$this->render('backend/standart/administrator/guru_smp/guru_smp_view', $this->data);
	}
	...
	*/
	...
	{
		$guru_smp = $this->model_guru_smp->find($id);

		if (!empty($guru_smp->foto_profil)) {
			$path = FCPATH . '/uploads/guru_smp/' . $guru_smp->foto_profil;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($guru_smp->slip_gaji)) {
			$path = FCPATH . '/uploads/guru_smp/' . $guru_smp->slip_gaji;
		...
		return $this->model_guru_smp->remove($id);
	}
	
	/**
	* Upload Image Guru Smp	* 
	* @return JSON
	*/
	...
	/**
	* Delete Image Guru Smp	* 
	* @return JSON
	*/
	public function delete_foto_profil_file($uuid)
	{
		if (!$this->is_allowed('guru_smp_delete', false)) {
			echo json_encode([
				'success' => false,
	...
	*/
	public function get_foto_profil_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_smp/',
            'delete_endpoint'   => 'administrator/guru_smp/delete_foto_profil_file'
        ]);
	}
	...
	*/
	public function get_slip_gaji_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_smp/',
            'delete_endpoint'   => 'administrator/guru_smp/delete_slip_gaji_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('guru_smp_export');

		$this->model_guru_smp->export('guru_smp', 'guru_smp');
	}
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_guru_smp->pdf('guru_smp', 'guru_smp');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_guru_smp->find($id);
		...
		if (isset($_FILES["file_guru"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					// chcek emp code
					$check=$this->mymodel->withquery("select * from guru_smp where emp_code = ".(int)$worksheet->getCellByColumnAndRow(1, $row)->getValue(), 'row');
					/* if($check){
						$this->db->trans_rollback();
						$this->load->library("session");
					...
					$check=$this->mymodel->withquery("select * from guru_smp where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					if($check){
						$insertId = $this->mymodel->update("guru_smp", $data_pendaftaran, 'email_ms_office', $worksheet->getCellByColumnAndRow(2, $row)->getValue());
						$insertId = $check->id_guru;
					}else{
						$insertId = $this->mymodel->insertid("guru_smp", $data_pendaftaran);
					}
	...
	public function upload_slip_gaji(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_slip_gaji']['name'][$i])) {
                  $trans_folder = './uploads/guru_smp/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_slip_gaji']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("guru_smp",$data, "npp", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/guru_smp/").$filename_no_extension.".pdf",
...
/* End of file guru_smp.php */
/* Location: ./application/controllers/administrator/Guru Smp.php */