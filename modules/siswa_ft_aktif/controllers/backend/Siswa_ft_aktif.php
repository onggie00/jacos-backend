...
/**
 *| --------------------------------------------------------------------------
 *| Siswa Ft Aktif Controller
 *| --------------------------------------------------------------------------
 *| Siswa Ft Aktif site
 *|
 */
class Siswa_ft_aktif extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_siswa_ft_aktif');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Siswa Ft Aktifs
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('siswa_ft_aktif_list');
		...
		if ($has_multi_filter) {
			$this->data['siswa_ft_aktifs'] = $this->model_siswa_ft_aktif->get(null, null, $this->limit_page, $offset, [], $sort, $sort_type, $multi_filters);
			$this->data['siswa_ft_aktif_counts'] = $this->model_siswa_ft_aktif->count_all(null, null, $multi_filters);
		} else {
			$this->data['siswa_ft_aktifs'] = $this->model_siswa_ft_aktif->get($filter, $field, $this->limit_page, $offset, [], $sort, $sort_type);
			$this->data['siswa_ft_aktif_counts'] = $this->model_siswa_ft_aktif->count_all($filter, $field);
		}

		$config = [
			'base_url'     => 'administrator/siswa_ft_aktif/index/',
			'total_rows'   => $has_multi_filter ? $this->model_siswa_ft_aktif->count_all(null, null, $multi_filters) : $this->model_siswa_ft_aktif->count_all($filter, $field),
		...
		$this->data['list_kelas'] = $this->mymodel->withquery("SELECT id_kelas_ft, label FROM kelas_ft ORDER BY label", "result");
		...
		$this->data['export_lengkap_cols'] = $this->model_siswa_ft_aktif->export_lengkap_columns();

		$this->template->title('Siswa Ft Aktif List');
		$this->render('backend/standart/administrator/siswa_ft_aktif/siswa_ft_aktif_list', $this->data);
	}
	...
	/**
	 * Add new siswa_ft_aktifs
	 *
	 */
	public function add()
	{
		$this->is_allowed('siswa_ft_aktif_add');

		$this->template->title('Siswa Ft Aktif New');
		$this->render('backend/standart/administrator/siswa_ft_aktif/siswa_ft_aktif_add', $this->data);
	}

	/**
	 * Add New Siswa Ft Aktifs
	 *
	 * @return JSON
	 */
	public function add_save()
	{
		if (!$this->is_allowed('siswa_ft_aktif_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_siswa_ft_aktif = $this->model_siswa_ft_aktif->store($save_data);


			
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_siswa_ft_aktif;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/siswa_ft_aktif/edit/' . $save_siswa_ft_aktif, 'Edit Siswa Ft Aktif'),
						anchor('administrator/siswa_ft_aktif', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/siswa_ft_aktif');
				}
	...
	/**
	 * Update view Siswa Ft Aktifs
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('siswa_ft_aktif_update');
		$this->model_siswa_ft_aktif->join_avaiable();
		$this->data['siswa_ft_aktif'] = $this->model_siswa_ft_aktif->find($id);

		$this->template->title('Siswa Ft Aktif Update');
		$this->render('backend/standart/administrator/siswa_ft_aktif/siswa_ft_aktif_update', $this->data);
	}

	/**
	 * Update Siswa Ft Aktifs
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('siswa_ft_aktif_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post("spp_custom") !== false) {
				$_spp_raw = $this->input->post("spp_custom");
			$save_data["spp_custom"] = ($_spp_raw !== null && strlen($_spp_raw) > 0)
				? (int) preg_replace('/[^0-9]/', '', (string) $_spp_raw)
				: null;
			}
			...
			$this->model_siswa_ft_aktif->change($id, $save_data);

			// Direct update spp_custom & spp_type (bypass MY_Model scope issue)
			if (isset($save_data['spp_custom'])) {
				$this->db->where('id_siswa_ft_aktif', $id)->update('siswa_ft_aktif', array('spp_custom' => $save_data['spp_custom']));
			}
			if (isset($save_data['spp_type'])) {
				$this->db->where('id_siswa_ft_aktif', $id)->update('siswa_ft_aktif', array('spp_type' => $save_data['spp_type']));
			}

			// --- Restore akun ---
				$restore_siswa = $this->input->post('restore_siswa');
				if ($restore_siswa === 'restore') {
					$this->mymodel->update('siswa_ft', array('deleted_at' => NULL), 'id_siswa_ft', $this->input->post('id_siswa_ft'));
					$this->mymodel->update('siswa_ft_aktif', array('deleted_at' => NULL), 'id_siswa_ft_aktif', $id);
				}
				//restore akun ortu jika dropdown = restore
				$restore_ortu = $this->input->post('restore_ortu');
				if ($restore_ortu === 'restore') {
					$this->mymodel->update('siswa_ft', array('deleted_at_ortu' => NULL), 'id_siswa_ft', $this->input->post('id_siswa_ft'));
					$this->mymodel->update('siswa_ft_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_ft_aktif', $id);
				}
				...
				$this->mymodel->update("siswa_ft", $save_data_siswa, "id_siswa_ft", $this->input->post("id_siswa_ft"));
				//simpan ke data spp
				$save_data_spp = array(
				...
				$get_spp = $this->mymodel->withquery("select * from spp_ft where tahun_ajaran = '".$get_tahun_ajaran->label."' ","result");
				$nama_kelas = $this->mymodel->withquery(
					"SELECT label FROM kelas_ft WHERE id_kelas_ft = '".$this->input->post('id_kelas')."'",
					"row"
				)->label;
				...
				$bulan = [
					...
					if ($spp_custom_changed) {
						...
						$id_tahun_ajaran = $this->input->post("id_tahun_ajaran");
						// Hanya update transaksi yang belum lunas (status_transaksi != '2')
						$this->mymodel->withquery(
							"UPDATE transaksi_spp SET total_biaya = {$total_biaya}, user_name = '{$user_name}' " .
							"WHERE no_transaksi LIKE '%ft%' " .
							"  AND id_tahun_ajaran = '{$id_tahun_ajaran}' " .
							"  AND id_siswa_aktif = {$id} " .
	...
	/**
	 * delete Siswa Ft Aktifs
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('siswa_ft_aktif_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'siswa_ft_aktif'), 'success');
		} else {
			set_message(cclang('error_delete', 'siswa_ft_aktif'), 'error');
		}
	...
	/**
	 * View view Siswa Ft Aktifs
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('siswa_ft_aktif_view');

		$this->data['siswa_ft_aktif'] = $this->model_siswa_ft_aktif->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Siswa Ft Aktif Detail');
		$this->render('backend/standart/administrator/siswa_ft_aktif/siswa_ft_aktif_view', $this->data);
	}
	...
	{
		if (!$this->is_allowed('siswa_ft_aktif_view', false)) {
			echo json_encode(['error' => 'Unauthorized']);
			exit;
		}

		$siswa = $this->model_siswa_ft_aktif->join_avaiable()->filter_avaiable()->find($id);
	...
	{
		$siswa_ft_aktif = $this->model_siswa_ft_aktif->find($id);



		return $this->model_siswa_ft_aktif->remove($id);
	}
	...
	{
		$this->is_allowed('siswa_ft_aktif_export');

		//$this->model_siswa_ft_aktif->export_siswa('siswa_ft_aktif', 'siswa_ft_aktif');
		$field = $this->input->get("f");
		...
		$case_spp = "case when sa.spp_custom = '0' then (select biaya_spp from tingkatan_ft ts where ts.id_tingkatan_ft = k.id_tingkatan) else sa.spp_custom end";
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select sa.*, s.*, k.label as kelas, t.label as tahun_ajaran, (".$case_spp.") as spp_siswa from siswa_ft_aktif sa join kelas_ft k on sa.id_kelas = k.id_kelas_ft and (k.label not like '%Keluar%' and k.label not like '%Alumni%')  join tahun_ajaran t on sa.id_tahun_ajaran = t.id_tahun_ajaran join siswa_ft s on sa.id_siswa_ft = s.id_siswa_ft","result");
		}
			...
			$get_data = $this->mymodel->withquery("select sa.*, s.*, k.label as kelas, t.label as tahun_ajaran, (".$case_spp.") as spp_siswa from siswa_ft_aktif sa join kelas_ft k on sa.id_kelas = k.id_kelas_ft join tahun_ajaran t on sa.id_tahun_ajaran = t.id_tahun_ajaran join siswa_ft s on sa.id_siswa_ft = s.id_siswa_ft where ".$where,"result");
		}
		else{
		...
		{
			if (strpos($field_search[$i], "id_siswa_ft_aktif") !== false && $i == 0) {
		...
		foreach ($get_data as $key => $value) {
			...
			for ($j=0; $j < count($field_search); $j++) {
		        ...
		        if (strpos($kolom, "id_siswa_ft_aktif") !== false && $i == 0) {
		        	$value_data = ($key+1);
		        }
				if (strpos($kolom, "file_raport") !== false) {
		        	$value_data = base_url("uploads/siswa_ft_aktif/").$value_data;
		        }
		...
		header('Content-Disposition: attachment;filename="Data Siswa FT Aktif - '.date("Y-m-d Hi").'.xls"'); 
	...
	{
		if (!$this->is_allowed('siswa_ft_aktif_export', false)) {
		...
		$where = 'fa.deleted_at IS NULL AND s.deleted_at IS NULL';
		if ($id_tingkatan) {
			$where .= ' AND k.id_tingkatan = ' . $id_tingkatan;
		}
		if ($id_kelas) {
			$where .= ' AND fa.id_kelas = ' . $id_kelas;
		}
		if (!empty($q)) {
			$qs = $this->db->escape_str($q);
			$where .= " AND (fa.nama_lengkap LIKE '%" . $qs . "%' OR fa.nis LIKE '%" . $qs . "%')";
		}

		$rows = $this->mymodel->withquery(
			"SELECT fa.id_siswa_ft_aktif AS id, CONCAT(fa.nama_lengkap, ' (', COALESCE(k.label, '-'), ')') AS text
			 FROM siswa_ft_aktif fa
			 JOIN kelas_ft k ON fa.id_kelas = k.id_kelas_ft
			 JOIN siswa_ft s ON fa.id_siswa_ft = s.id_siswa_ft
			 WHERE " . $where . "
			 ORDER BY fa.nama_lengkap LIMIT 50", 'result');
	...
	public function export_lengkap()
	{
		...
		$all_cols = $this->model_siswa_ft_aktif->export_lengkap_columns();
		...
		if (is_array($siswa) && !in_array('all', $siswa)) {
			...
			if (!empty($ids)) {
				$where .= ' AND fa.id_siswa_ft_aktif IN (' . implode(',', $ids) . ')';
			}
		}
		...
		$get_data = $this->mymodel->withquery(
			...
			' FROM siswa_ft_aktif fa
			  JOIN kelas_ft k ON fa.id_kelas = k.id_kelas_ft
			  LEFT JOIN tahun_ajaran t ON fa.id_tahun_ajaran = t.id_tahun_ajaran
			  JOIN siswa_ft s ON fa.id_siswa_ft = s.id_siswa_ft
			  WHERE ' . $where . '
			  ORDER BY k.id_tingkatan, k.label, fa.nama_lengkap', 'result');
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Data Lengkap Siswa FT Aktif - ' . date('Y-m-d Hi') . '.xls"');
		header('Cache-Control: max-age=0');
	...
	public function export_pdf()
	{
		...
		$this->model_siswa_ft_aktif->pdf('siswa_ft_aktif', 'siswa_ft_aktif');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_siswa_ft_aktif->find($id);
	...
	public function add_raport()
	{
		...
		$this->template->title('Siswa FT Tambah Raport');
		$this->render('backend/standart/administrator/siswa_ft_aktif/siswa_ft_aktif_add_raport', $this->data);
	}
	...
	public function add_raport_save()
	{
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/siswa_ft_aktif/')) {
				mkdir(FCPATH . '/uploads/siswa_ft_aktif/');
			}
			...
			if (!empty($file_raport_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/siswa_ft_aktif/' . $file_raport_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_ft_aktif/' . $file_raport_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_siswa_ft_aktif = $this->model_siswa_ft_aktif->change($item, $save_data);
		}


		if ($save_siswa_ft_aktif) {
			if ($this->input->post('save_type') == 'stay') {
				$this->data['success'] = true;
				$this->data['id'] 	   = $save_siswa_ft_aktif;
				$this->data['message'] = cclang('success_save_data_stay', [
				...
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/siswa_ft_aktif/edit/' . $save_siswa_ft_aktif, 'Edit Siswa Ft')
					]),
					'success'
				);
	...
	public function delete_file_raport_file($uuid)
	{
		...
		echo $this->delete_file([
			...
			'table_name'        => 'siswa_ft_aktif',
			'primary_key'       => 'id_siswa_ft_aktif',
			'upload_path'       => 'uploads/siswa_ft_aktif/'
		]);
	}
	...
	public function generate_nis()
	{
		...
		foreach ($array_siswa as $key => $value) {
			...
			if ($value['nis'] == "-" || $value['nis'] == "1" || empty($value['nis'])) {
				...
				//update siswa aktif
				$up = $this->mymodel->update("siswa_ft_aktif",$data_update, "id_siswa_ft_aktif", $value['id_siswa_aktif']);
				//update data siswa
				$data_update2 = array(
					"email_ms_office" => $nis."@labschoolcibubur.sch.id",
					"email_ms_office_ortu" => $nis."p@labschoolcibubur.sch.id"
				);
				$up2 = $this->mymodel->update("siswa_ft",$data_update2, "id_siswa_ft", $value['id_siswa']);
			}
		}
		redirect_back();		
	}
		...
		if (isset($_FILES["file_import"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					// Kolom (0=NO): 1=nama, 2=nis, ..., 27=tgl_lahir, 28=id_siswa_ft_aktif
					$id_siswa_aktif = $worksheet->getCellByColumnAndRow(28, $row)->getValue();
					
					if (!is_numeric($id_siswa_aktif)) continue;
					
					$get_siswa = $this->mymodel->withquery("select sa.id_siswa_ft_aktif, s.id_siswa_ft, s.nisn from siswa_ft_aktif sa join siswa_ft s on sa.id_siswa_ft = s.id_siswa_ft where sa.id_siswa_ft_aktif = ".(int)$id_siswa_aktif, "row");
					if (!$get_siswa) continue;

					$kelas_text = $cellVal(3, $row);
					$kelas_obj = $this->mymodel->withquery("select id_kelas_ft from kelas_ft where label like '%".$this->db->escape_str($kelas_text)."%'","row");
					$kelas = !empty($kelas_obj) ? $kelas_obj->id_kelas_ft : null;
					...
					$update_siswa_aktif = $this->mymodel->update("siswa_ft_aktif", $data_siswa_aktif, "id_siswa_ft_aktif", $id_siswa_aktif);
						// dd($data_pendaftaran);
						if (!$update_siswa_aktif) {
							...
							$this->mymodel->update("siswa_ft", $data_siswa, "id_siswa_ft", $get_siswa->id_siswa_ft);
							//update data spp bila tahun ajaran sama
							$this->mymodel->update("spp_ft", array("id_tahun_ajaran" => $tahun_ajaran, "kelas" => $kelas_text, "nama" => $data_siswa_aktif["nama_lengkap"], "nominal" => $data_siswa_aktif["spp_custom"]), "id_tahun_ajaran = '".$tahun_ajaran."' and id_siswa_aktif=", $get_siswa->id_siswa_ft_aktif);
							// Hanya update transaksi yang belum lunas (status_transaksi != '2')
							$this->mymodel->withquery(
								"UPDATE transaksi_spp SET id_tahun_ajaran = '{$tahun_ajaran}', user_name = '{$data_siswa_aktif[nama_lengkap]}', total_biaya = " . (int)$data_siswa_aktif["spp_custom"] . " " .
								"WHERE no_transaksi LIKE '%FT%' " .
								"  AND id_tahun_ajaran = '{$tahun_ajaran}' " .
								"  AND id_siswa_aktif = " . (int)$get_siswa->id_siswa_ft_aktif . " " .
								"  AND status_transaksi != '2'"
							);
						}
					}
	...
	public function import_raport_siswa(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_raport']['name'][$i])) {
                  $trans_folder = './uploads/siswa_ft_aktif/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_raport']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("siswa_ft_aktif",$data, "nis", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/siswa_ft_aktif/").$filename_no_extension.".pdf",
	...
	public function restore($id = null)
	{
		...
		$siswa = $this->model_siswa_ft_aktif->find($id);
		if (!$siswa) {
			set_message('Data siswa tidak ditemukan', 'error');
			redirect_back();
		}

		// Set deleted_at = NULL di tabel siswa_ft
		$this->mymodel->update('siswa_ft', array('deleted_at' => NULL), 'id_siswa_ft', $siswa->id_siswa_ft);
		// Set deleted_at = NULL di tabel siswa_ft_aktif
		$this->mymodel->update('siswa_ft_aktif', array('deleted_at' => NULL), 'id_siswa_ft_aktif', $id);
	...
	public function restore_ortu($id = null)
	{
		...
		// Set deleted_at_ortu = NULL di tabel siswa_ft
		$this->mymodel->update('siswa_ft', array('deleted_at_ortu' => NULL), 'id_siswa_ft', $siswa->id_siswa_ft);
		// Set deleted_at_ortu = NULL di tabel siswa_ft_aktif
		$this->mymodel->update('siswa_ft_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_ft_aktif', $id);
...
/* End of file siswa_ft_aktif.php */
/* Location: ./application/controllers/administrator/Siswa Ft Aktif.php */
