...
class Get_tagihan extends REST_Controller {
	...
	$user_name =$this->post('user_name');
        ...
        $data = $this->mymodel->withquery("select * from transaksi_spp where id_siswa_aktif = '".$id_siswa."' and no_transaksi like '%LISPP-".$jenjang."-".$code_tahun_ajaran."-".$id_siswa."%'" ,"result");
        
        if (!empty($data)) {
          foreach($data as $key => $item){
            if(!empty($item->file_kwitansi)){
              $item->file_kwitansi = base_url("uploads/kwitansi/").$item->file_kwitansi;
            }
            if(!empty($item->file_slip)){
