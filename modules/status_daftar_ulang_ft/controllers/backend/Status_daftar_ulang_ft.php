...
class Status_daftar_ulang_ft extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		if ($has_multi_filter) {
			$this->data['status_daftar_ulang_fts'] = $this->model_status_daftar_ulang_ft->get(null, null, $this->limit_page, $offset, array(), $sort, $sort_type, $multi_filters);
			$this->data['status_daftar_ulang_ft_counts'] = $this->model_status_daftar_ulang_ft->count_all(null, null, $multi_filters);
		} else {
			$this->data['status_daftar_ulang_fts'] = $this->model_status_daftar_ulang_ft->get($filter, $field, $this->limit_page, $offset, array(), $sort, $sort_type);
		...
		$config = [
			...
			'total_rows'   => $has_multi_filter ? $this->model_status_daftar_ulang_ft->count_all(null, null, $multi_filters) : $this->model_status_daftar_ulang_ft->count_all($filter, $field),
		...
		$this->data['list_tahun_ajaran'] = $this->mymodel->withquery("SELECT DISTINCT tahun_ajaran FROM siswa_ft WHERE deleted_at IS NULL AND tahun_ajaran IS NOT NULL AND tahun_ajaran <> '' ORDER BY tahun_ajaran DESC", "result");
		$this->data['list_gelombang']    = $this->mymodel->withquery("SELECT DISTINCT gelombang FROM siswa_ft WHERE deleted_at IS NULL AND gelombang IS NOT NULL ORDER BY gelombang ASC", "result");
	...
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('status') == 1) {
				$no_transaksi = "LDUI-FT-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_ft;
				//update tgl aktivasi
				...
				//$update_response = $this->update_billing(ENVIRONMENT, $total_biaya, $no_transaksi, array("nama" => $get_siswa->nama_lengkap, "email" => $get_siswa->email, "va_number" => $va_number));
				...
				if (!empty($data_transaksi)) {
					...
					if (empty($cek_data)) {
						...
						// $id_transaksi = $this->mymodel->update("transaksi", $data_transaksi, "id_transaksi", $cek_data->id_transaksi);
