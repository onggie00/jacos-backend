...
/**
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang Sma Controller
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang Sma site
 *|
 */
class Status_daftar_ulang_sma extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_status_daftar_ulang_sma');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Status Daftar Ulang Smas
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('status_daftar_ulang_sma_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['status_daftar_ulang_smas'] = $this->model_status_daftar_ulang_sma->get($filter, $field, $this->limit_page, $offset);
		$this->data['status_daftar_ulang_sma_counts'] = $this->model_status_daftar_ulang_sma->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/status_daftar_ulang_sma/index/',
			'total_rows'   => $this->model_status_daftar_ulang_sma->count_all($filter, $field),
		...
		$this->template->title('Status Daftar Ulang Sma List');
		$this->render('backend/standart/administrator/status_daftar_ulang_sma/status_daftar_ulang_sma_list', $this->data);
	}

	/**
	 * Add new status_daftar_ulang_smas
	 *
	 */
	public function add()
	{
		$this->is_allowed('status_daftar_ulang_sma_add');

		$this->template->title('Status Daftar Ulang Sma New');
		$this->render('backend/standart/administrator/status_daftar_ulang_sma/status_daftar_ulang_sma_add', $this->data);
	}

	/**
	 * Add New Status Daftar Ulang Smas
	 *
	 * @return JSON
	 */
	public function add_save()
	{
		if (!$this->is_allowed('status_daftar_ulang_sma_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_status_daftar_ulang_sma = $this->model_status_daftar_ulang_sma->store($save_data);


			if ($save_status_daftar_ulang_sma) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_status_daftar_ulang_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/status_daftar_ulang_sma/edit/' . $save_status_daftar_ulang_sma, 'Edit Status Daftar Ulang Sma'),
						anchor('administrator/status_daftar_ulang_sma', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/status_daftar_ulang_sma');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	/**
	 * Update view Status Daftar Ulang Smas
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('status_daftar_ulang_sma_update');

		$this->data['status_daftar_ulang_sma'] = $this->model_status_daftar_ulang_sma->find($id);

		$this->template->title('Status Daftar Ulang Sma Update');
		$this->render('backend/standart/administrator/status_daftar_ulang_sma/status_daftar_ulang_sma_update', $this->data);
	}

	/**
	 * Update Status Daftar Ulang Smas
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('status_daftar_ulang_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_status_daftar_ulang_sma = $this->model_status_daftar_ulang_sma->change($id, $save_data);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_sma", "id_daftar_ulang", $id, "row");

			if ($this->input->post('status') == 1) {
				$get_siswa = $this->mymodel->getbywhere("siswa_sma", "id_siswa_sma", $get_daftar_ulang->id_siswa_sma, "row");
				$this->mymodel->update("status_daftar_ulang_sma", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);

				if ($get_siswa->ppsbb == 1) {
					$no_transaksi = "LDUI-PPSBBSMA-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_sma;
					//get biaya pendaftaran
					$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "PPSBB SMA", "row");
					$tipe = 'PPSBB SMA';
				} else {
					$no_transaksi = "LDUI-PSBSMA-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_sma;
					//get biaya pendaftaran
				...
				if ($customPayment != '0' && $customPayment != '') {
					...
					$this->mymodel->update("status_daftar_ulang_sma", array("custom_payment" => $customPayment), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);
				} else {
					// get biaya pendaftaran
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran Ulang SMA a.n " . strtoupper($get_siswa->nama_lengkap),
				...
				if (!empty($data_transaksi)) {
					...
					$this->mymodel->update("siswa_sma", array("no_transaksi" => $no_transaksi), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);
					//$cetak_slip = $this->cetak_slip(array("id_siswa" => $get_daftar_ulang->id_siswa_sma, "tipe_siswa" => "sma"));
					...
					$this->mymodel->update("status_daftar_ulang_sma", array("slip_pembayaran" => $data_email["slip_pembayaran"]), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);
			...
			if ($this->input->post('status') == 2) {
				...
				$cetak_kartu_siswa_sementara = $this->cetak_kartu_siswa_sementara(array("tipe_siswa" => 'sma', "id_siswa" => $get_daftar_ulang->id_siswa_sma));
				...
				$cetak_kwitansi = $this->cetak_kwitansi(array(
					...
					"id_siswa" => $get_daftar_ulang->id_siswa_sma,
					"no_transaksi" => $get_transaksi->no_transaksi,
					"jalur" => $jalur,
					"jenis_kwitansi" => "uang pangkal"
				));

				$kartu = 'sma' . '-' . $get_siswa->no_peserta . '-' . $get_siswa->nama_lengkap . '.pdf';
				$kwitansi = $get_transaksi->no_transaksi . '-' . $get_siswa->nama_lengkap . '.pdf';
				$this->mymodel->update("status_daftar_ulang_sma", array("status" => 2, "kwitansi" => $kwitansi, "kartu_sementara" => $kartu, "tgl_bayar" => date("Y-m-d H:i:s")), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);
			}
	...
	/**
	 * delete Status Daftar Ulang Smas
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('status_daftar_ulang_sma_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'status_daftar_ulang_sma'), 'success');
		} else {
			set_message(cclang('error_delete', 'status_daftar_ulang_sma'), 'error');
		}
	...
	/**
	 * View view Status Daftar Ulang Smas
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('status_daftar_ulang_sma_view');

		$this->data['status_daftar_ulang_sma'] = $this->model_status_daftar_ulang_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Status Daftar Ulang Sma Detail');
		$this->render('backend/standart/administrator/status_daftar_ulang_sma/status_daftar_ulang_sma_view', $this->data);
	}
	...
	{
		$status_daftar_ulang_sma = $this->model_status_daftar_ulang_sma->find($id);



		return $this->model_status_daftar_ulang_sma->remove($id);
	}
	...
	public function update_status($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$this->mymodel->update('status_daftar_ulang_sma', array('status' => "1"), 'id_daftar_ulang', $id);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_sma", "id_daftar_ulang", $id, "row");
			$this->mymodel->update("status_daftar_ulang_sma", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_siswa_sma", $get_daftar_ulang->id_siswa_sma);
	...
	public function siswa_aktif()
	{
		...
		foreach ($ids as $key => $i) {
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_sma", "id_daftar_ulang", $i, "row");
			$get_siswa = $this->mymodel->getbywhere("siswa_sma", "id_siswa_sma", $get_daftar_ulang->id_siswa_sma, "row");
			$data_siswa_aktif = [
				"id_siswa_sma" => $get_daftar_ulang->id_siswa_sma,
				"nama_lengkap" => $get_siswa->nama_lengkap,
	...
	{
		$this->is_allowed('status_daftar_ulang_sma_export');

		$this->model_status_daftar_ulang_sma->export_siswa('status_daftar_ulang_sma', 'status_daftar_ulang_sma');
	}
	...
	public function export_pdf()
	{
		...
		$this->model_status_daftar_ulang_sma->pdf('status_daftar_ulang_sma', 'status_daftar_ulang_sma');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'status_daftar_ulang_sma';
		...
		$data = $this->model_status_daftar_ulang_sma->find($id);
...
/* End of file status_daftar_ulang_sma.php */
/* Location: ./application/controllers/administrator/Status Daftar Ulang Sma.php */
