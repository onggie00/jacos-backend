...
*|
*/
class Transaksi_lain_smp extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_transaksi_lain_smp');
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_smp_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['transaksi_lain_smps'] = $this->model_transaksi_lain_smp->get($filter, $field, $this->limit_page, $offset);
		$this->data['transaksi_lain_smp_counts'] = $this->model_transaksi_lain_smp->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/transaksi_lain_smp/index/',
			'total_rows'   => $this->model_transaksi_lain_smp->count_all($filter, $field),
		...
		// Infobox stats
		$this->data['stats'] = $this->model_transaksi_lain_smp->get_stats($filter, $field);

		// Chart data
		$this->data['chart_data'] = $this->model_transaksi_lain_smp->get_chart_data($filter, $field);

		// Pass current filter to view
		$this->data['current_filter_q'] = $filter;
		$this->data['current_filter_f'] = $field;

		$this->template->title('List Tagihan SMP List');
		$this->render('backend/standart/administrator/transaksi_lain_smp/transaksi_lain_smp_list', $this->data);
	}
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_smp_update');

		$this->data['transaksi_lain_smp'] = $this->model_transaksi_lain_smp->find($id);

		$this->template->title('List Tagihan SMP Update');
		$this->render('backend/standart/administrator/transaksi_lain_smp/transaksi_lain_smp_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_smp_update', false)) {
			echo json_encode([
				'success' => false,
		...
		$this->form_validation->set_rules('transaksi_lain_smp_file_kwitansi_name', 'File Kwitansi', 'trim');
		...
		if ($this->form_validation->run()) {
			$transaksi_lain_smp_file_kwitansi_uuid = $this->input->post('transaksi_lain_smp_file_kwitansi_uuid');
			$transaksi_lain_smp_file_kwitansi_name = $this->input->post('transaksi_lain_smp_file_kwitansi_name');
			...
			if (!is_dir(FCPATH . '/uploads/transaksi_lain_smp/')) {
				mkdir(FCPATH . '/uploads/transaksi_lain_smp/');
			}

			if (!empty($transaksi_lain_smp_file_kwitansi_uuid)) {
				$transaksi_lain_smp_file_kwitansi_name_copy = date('YmdHis') . '-' . $transaksi_lain_smp_file_kwitansi_name;

				rename(FCPATH . 'uploads/tmp/' . $transaksi_lain_smp_file_kwitansi_uuid . '/' . $transaksi_lain_smp_file_kwitansi_name, 
						FCPATH . 'uploads/transaksi_lain_smp/' . $transaksi_lain_smp_file_kwitansi_name_copy);

				if (!is_file(FCPATH . '/uploads/transaksi_lain_smp/' . $transaksi_lain_smp_file_kwitansi_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['file_kwitansi'] = $transaksi_lain_smp_file_kwitansi_name_copy;
			}
		
			
			$save_transaksi_lain_smp = $this->model_transaksi_lain_smp->change($id, $save_data);

			if ($save_transaksi_lain_smp) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/transaksi_lain_smp', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/transaksi_lain_smp');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_smp_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'transaksi_lain_smp'), 'success');
        } else {
            set_message(cclang('error_delete', 'transaksi_lain_smp'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_smp_view');

		$this->data['transaksi_lain_smp'] = $this->model_transaksi_lain_smp->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('List Tagihan SMP Detail');
		$this->render('backend/standart/administrator/transaksi_lain_smp/transaksi_lain_smp_view', $this->data);
	}
	...
	*/
	...
	{
		$transaksi_lain_smp = $this->model_transaksi_lain_smp->find($id);

		if (!empty($transaksi_lain_smp->file_kwitansi)) {
			$path = FCPATH . '/uploads/transaksi_lain_smp/' . $transaksi_lain_smp->file_kwitansi;
		...
		return $this->model_transaksi_lain_smp->remove($id);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_smp_add', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_smp_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'table_name'        => 'transaksi_lain_smp',
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/transaksi_lain_smp/'
        ]);
	}
	...
	*/
	public function get_file_kwitansi_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/transaksi_lain_smp/',
            'delete_endpoint'   => 'administrator/transaksi_lain_smp/delete_file_kwitansi_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_smp_export');

		//$this->model_transaksi_lain_smp->export('transaksi_lain_smp', 'transaksi_lain_smp');
		$f = $this->input->get('f');
		$q = $this->input->get('q');
		$where = "";
		...
		$get_data = $this->mymodel->withquery("select s.id_siswa_smp_aktif as id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label as nama_kelas, m.nama_transaksi, t.va_number, t.kode_tagihan, t.tanggal_bayar, t.nominal_bayar, 
			...
			when t.status_transaksi = '3' then 'kadaluarsa' end as status_transaksi
		 	from transaksi_lain_smp t join transaksi_lain_smp_manajemen m on t.id_transaksi_lain = m.id join siswa_smp_aktif s on t.id_siswa_aktif = s.id_siswa_smp_aktif join kelas_smp k on s.id_kelas = k.id_kelas_smp ".$where." order by s.nama_lengkap ASC","result");
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_transaksi_lain_smp->pdf('transaksi_lain_smp', 'transaksi_lain_smp');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'transaksi_lain_smp';
        ...
        $data = $this->model_transaksi_lain_smp->find($id);
...
/* End of file transaksi_lain_smp.php */
