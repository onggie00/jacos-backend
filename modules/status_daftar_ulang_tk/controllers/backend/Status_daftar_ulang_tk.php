...
/**
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang TK Controller
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang TK site
 *|
 */
class Status_daftar_ulang_tk extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_status_daftar_ulang_tk');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Status Daftar Ulang TKs
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('status_daftar_ulang_tk_list');
		...
		if ($has_multi_filter) {
			$this->data['status_daftar_ulang_tks'] = $this->model_status_daftar_ulang_tk->get(null, null, $this->limit_page, $offset, array(), $sort, $sort_type, $multi_filters);
			$this->data['status_daftar_ulang_tk_counts'] = $this->model_status_daftar_ulang_tk->count_all(null, null, $multi_filters);
		} else {
			$this->data['status_daftar_ulang_tks'] = $this->model_status_daftar_ulang_tk->get($filter, $field, $this->limit_page, $offset, array(), $sort, $sort_type);
			$this->data['status_daftar_ulang_tk_counts'] = $this->model_status_daftar_ulang_tk->count_all($filter, $field);
		}

		$config = [
			'base_url'     => 'administrator/status_daftar_ulang_tk/index/',
			'total_rows'   => $has_multi_filter ? $this->model_status_daftar_ulang_tk->count_all(null, null, $multi_filters) : $this->model_status_daftar_ulang_tk->count_all($filter, $field),
		...
		$this->data['list_tahun_ajaran'] = $this->mymodel->withquery("SELECT DISTINCT tahun_ajaran FROM siswa_tk WHERE deleted_at IS NULL AND tahun_ajaran IS NOT NULL AND tahun_ajaran <> '' ORDER BY tahun_ajaran DESC", "result");
		$this->data['list_gelombang']    = $this->mymodel->withquery("SELECT DISTINCT gelombang FROM siswa_tk WHERE deleted_at IS NULL AND gelombang IS NOT NULL ORDER BY gelombang ASC", "result");
		$this->data['multi_filters'] = $multi_filters;

		$this->template->title('Status Daftar Ulang TK List');
		$this->render('backend/standart/administrator/status_daftar_ulang_tk/status_daftar_ulang_tk_list', $this->data);
	}
	...
	/**
	 * Update view Status Daftar Ulang TKs
	 *
	 * @var $id String
	 */
	public function edit($id, $date = null)
	{
		$this->is_allowed('status_daftar_ulang_tk_update');

		$this->model_status_daftar_ulang_tk->join_avaiable();

		$this->data['status_daftar_ulang_tk'] = $this->model_status_daftar_ulang_tk->find($id);
		...
		foreach ($get_setting as $key => $item) {
			...
			if ($item->name_setting == 'bri_institution_code_close' && $this->data['status_daftar_ulang_tk']->tipe_tagihan == "fixed") {
				$institutionCode = $item->value;
			}
			else if ($item->name_setting == 'bri_institution_code' && $this->data['status_daftar_ulang_tk']->tipe_tagihan == "open") {
				$institutionCode = $item->value;
			}
		}
		...
		$brivaNo = substr($this->data['status_daftar_ulang_tk']->va_bri, 0, 5);
		$custCode = substr($this->data['status_daftar_ulang_tk']->va_bri, 5);
		$datas = array(
			'brivaNo' => $brivaNo,
			'custCode' => $custCode,
			'startDate' => $date,
		...
		$va = $this->data['status_daftar_ulang_tk']->va_bri;
		// $va='2147483647';	
		...
		$this->template->title('Status Daftar Ulang TK Update');
		$this->render('backend/standart/administrator/status_daftar_ulang_tk/status_daftar_ulang_tk_update', $this->data);
	}

	/**
	 * Update Status Daftar Ulang TKs
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('status_daftar_ulang_tk_update', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_status_daftar_ulang_tk = $this->model_status_daftar_ulang_tk->change($id, $save_data);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_tk", "id_daftar_ulang", $id, "row");

			if ($this->input->post('status') == 1) {
				$no_transaksi = "LDUI-TK-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_tk;
				$this->mymodel->update("status_daftar_ulang_tk", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_daftar_ulang", $id);

				$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $get_daftar_ulang->id_siswa_tk, "row");
				$customPayment = $this->input->post('custom_total_payment');
				// dd($customPayment);
				if ($customPayment != '0' && $customPayment != '') {
					$total_biaya = $customPayment;
					$this->mymodel->update("status_daftar_ulang_tk", array("custom_payment" => $customPayment), "id_daftar_ulang", $id);
				...
				if ($bank_aktif == 'BNI') {
					...
					$date_va = $bni['date_va'];
					$this->mymodel->update("siswa_tk", array("va_number" => $va_number), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
					$this->mymodel->update("status_daftar_ulang_tk", array("va_bri" => $va_number), "id_daftar_ulang", $id);
				}
				if ($bank_aktif != 'BNI') {
				//get no briva
				$get_setting = $this->mymodel->getall("pengaturan_akun");
				...
				if (!empty($get_siswa->va_number_bri)) {
					...
					$get_no_urut = $this->mymodel->withquery("select va_number_bri, id_siswa_tk as id_siswa from siswa_tk where va_number_bri like '%" . $brivaNo . date('y', strtotime('+1 years')) . "01%' and va_number_bri != '' and is_mutasi = '2' order by va_number_bri DESC", "row");
				...
				$this->mymodel->update("siswa_tk", array("va_number_bri" => $res_data['brivaNo'] . $res_data['custCode']), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
				$this->mymodel->update("status_daftar_ulang_tk", array("va_bri" => $res_data['brivaNo'] . $res_data['custCode']), "id_daftar_ulang", $id);
				} //end if ($bank_aktif != 'BNI')


				// $this->mymodel->update("siswa_tk", array("va_number_bri" => $va_number), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
				// $this->mymodel->update("status_daftar_ulang_tk", array("va_bri" => $va_number), "id_daftar_ulang", $id);
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran Ulang TK a.n " . strtoupper($get_siswa->nama_lengkap),
				...
				if (empty($cek_data)) {
					...
					$this->mymodel->update("siswa_tk", array("no_transaksi" => $no_transaksi), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
					//$cetak_slip = $this->cetak_slip(array("id_siswa" => $get_daftar_ulang->id_siswa_tk, "tipe_siswa" => "tk"));
					// dd($cetak_slip);
					...
					$this->mymodel->update("status_daftar_ulang_tk", array("slip_pembayaran" => $data_email["slip_pembayaran"]), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
			...
			if ($this->input->post('status') == 2) {
				...
				$cetak_kartu_siswa_sementara = $this->cetak_kartu_siswa_sementara(array("tipe_siswa" => 'sd', "id_siswa" => $get_daftar_ulang->id_siswa_tk));
				...
				$cetak_kwitansi = $this->cetak_kwitansi(array(
					...
					"id_siswa" => $get_daftar_ulang->id_siswa_tk,
				...
				$this->mymodel->update("status_daftar_ulang_tk", array("status" => 2, "kwitansi" => $kwitansi, "kartu_sementara" => $kartu, "tgl_bayar" => date("Y-m-d H:i:s")), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
			}
			...
			if ($save_status_daftar_ulang_tk) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $id;
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/status_daftar_ulang_tk', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/status_daftar_ulang_tk');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
	...
	/**
	 * delete Status Daftar Ulang TKs
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('status_daftar_ulang_tk_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'status_daftar_ulang_tk'), 'success');
		} else {
			set_message(cclang('error_delete', 'status_daftar_ulang_tk'), 'error');
		}
	...
	/**
	 * View view Status Daftar Ulang TKs
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('status_daftar_ulang_tk_view');

		$this->data['status_daftar_ulang_tk'] = $this->model_status_daftar_ulang_tk->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Status Daftar Ulang TK Detail');
		$this->render('backend/standart/administrator/status_daftar_ulang_tk/status_daftar_ulang_tk_view', $this->data);
	}

	private function _remove($id)
	{
		$siswa_tk = $this->model_status_daftar_ulang_tk->find($id);

		return $this->model_status_daftar_ulang_tk->remove($id);
	}
	...
	public function update_status($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$this->mymodel->update('status_daftar_ulang_tk', array('status' => '1'), 'id_daftar_ulang', $id);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_tk", "id_daftar_ulang", $id, "row");

			//update tgl aktivasi
			$this->mymodel->update("status_daftar_ulang_tk", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);

			$no_transaksi = "LDUI-TK-" . date("YmdHis") . "-" . $get_daftar_ulang->id_siswa_tk;
			// get biaya pendaftaran
			$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "TK", "row");
			$total_biaya = $get_biaya->nominal_daftar_ulang;
			$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $get_daftar_ulang->id_siswa_tk, "row");
	...
	public function update_status_new($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			$this->mymodel->update('status_daftar_ulang_tk', ['status' => '1', 'tgl_aktivasi' => $now], 'id_daftar_ulang', $id);
			...
			if ($bank_aktif == 'BNI') {
				...
				$full_va = $va_number;
				$date_va = $bni['date_va'];
				$this->mymodel->update("siswa_tk", array("va_number" => $va_number, "no_transaksi" => $no_transaksi), "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);
				$this->mymodel->update("status_daftar_ulang_tk", array("va_bri" => $va_number), "id_daftar_ulang", $id);
			}
			...
			$this->mymodel->update("siswa_tk", [
				"va_number_bri" => $full_va,
				"no_transaksi" => $no_transaksi
			], "id_siswa_tk", $get_daftar_ulang->id_siswa_tk);

			$this->mymodel->update("status_daftar_ulang_tk", [
				"va_bri" => $full_va,
				"slip_pembayaran" => $no_transaksi . '-' . $get_siswa->nama_lengkap . '.pdf'
			], "id_daftar_ulang", $id);
			} //end if ($bank_aktif != 'BNI')
	...
	public function siswa_aktif()
	{
		...
		foreach ($ids as $key => $i) {
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_tk", "id_daftar_ulang", $i, "row");
			$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $get_daftar_ulang->id_siswa_tk, "row");
			$data_siswa_aktif = [
				"id_siswa_tk" => $get_daftar_ulang->id_siswa_tk,
				"nama_lengkap" => $get_siswa->nama_lengkap,
				"id_tahun_ajaran" => $this->input->post('tahun_ajaran'),
				"id_kelas" => $this->input->post('kelas_tk'),
				"spp_custom" => $this->input->post('spp_custom')
			];

			$insertAktif = $this->mymodel->insertid("siswa_tk_aktif", $data_siswa_aktif);
			// dd($insertAktif);
			if ($insertAktif == 0) {
	...
	{
		$this->is_allowed('status_daftar_ulang_tk_export');

		$this->model_status_daftar_ulang_tk->export_siswa('status_daftar_ulang_tk', 'status_daftar_ulang_tk');
	}
	...
	public function export_pdf()
	{
		...
		$this->model_status_daftar_ulang_tk->pdf('status_daftar_ulang_tk', 'status_daftar_ulang_tk');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'status_daftar_ulang_tk';
		...
		$data = $this->model_status_daftar_ulang_tk->find($id);
	 ...
	 * Pakai VA BNI PSB siswa (siswa_tk.va_number) bila sudah ada, bila belum generate baru (kode 01).
...
/* End of file status_daftar_ulang_tk.php */
/* Location: ./application/controllers/administrator/Status Daftar Ulang TK.php */
