...
assert_source_contains(
	$source,
	"->where('jenjang', 'smp')",
	'Query catatan harus dibatasi untuk jenjang SMP.',
	$failures
);
assert_source_contains(
	$source,
...
if (!$failures) {
	...
	$base_sheet_method->invoke($controller, $excel, 0, 'VII A');
	...
	assert_same('VII A-Catatan', $sheet->getTitle(), 'Nama sheet detail tidak sesuai.', $failures);
	assert_same('0012345', $sheet->getCell('B4')->getValue(), 'NIS catatan tidak dipertahankan sebagai string.', $failures);
	...
	$base_sheet_method->invoke($controller, $excel, 2, 'VIII A');
	...
	assert_same('VII A', $excel->getSheet(0)->getTitle(), 'Sheet presensi kelas pertama salah urutan.', $failures);
	assert_same('VIII A', $excel->getSheet(2)->getTitle(), 'Sheet presensi kelas kedua salah urutan.', $failures);
	assert_same('VIII A-Catatan', $excel->getSheet(3)->getTitle(), 'Sheet catatan kelas kedua salah urutan.', $failures);

	$collision_excel = new PHPExcel();
	$base_sheet_method->invoke($controller, $collision_excel, 0, $title_method->invoke($controller, 'VII/A', false));
	$sheet_method->invoke($controller, $collision_excel, 1, $collision_excel->getSheet(0)->getTitle(), array(), array());
	$base_sheet_method->invoke($controller, $collision_excel, 2, $title_method->invoke($controller, 'VII:A', false));
	$sheet_method->invoke($controller, $collision_excel, 3, $collision_excel->getSheet(2)->getTitle(), array(), array());
	assert_same('VII-A-2', $collision_excel->getSheet(2)->getTitle(), 'Collision sheet presensi harus diberi suffix angka.', $failures);
	assert_same('VII-A-2-Catatan', $collision_excel->getSheet(3)->getTitle(), 'Collision sheet catatan harus tetap berakhiran -Catatan.', $failures);
	$base_sheet_method->invoke($controller, $collision_excel, 4, $title_method->invoke($controller, 'vii/a', false), false);
	$sheet_method->invoke($controller, $collision_excel, 5, $collision_excel->getSheet(4)->getTitle(), array(), array());
	assert_same('vii-a-3', $collision_excel->getSheet(4)->getTitle(), 'Collision nama sheet harus case-insensitive.', $failures);
	assert_same('vii-a-3-Catatan', $collision_excel->getSheet(5)->getTitle(), 'Pasangan collision case-insensitive harus konsisten.', $failures);
	$base_sheet_method->invoke($controller, $collision_excel, 6, $title_method->invoke($controller, 'VII-A-Catatan', false), false);
	$sheet_method->invoke($controller, $collision_excel, 7, $collision_excel->getSheet(6)->getTitle(), array(), array());
	assert_same('VII-A-Catatan-2', $collision_excel->getSheet(6)->getTitle(), 'Nama kelas berakhiran -Catatan harus diperlakukan sebagai base.', $failures);
	assert_same('VII-A-Catatan-2-Catatan', $collision_excel->getSheet(7)->getTitle(), 'Pasangan nama kelas berakhiran -Catatan harus deterministik.', $failures);
