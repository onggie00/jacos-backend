...
$controller_path = dirname(__DIR__) . '/controllers/backend/Presensi_sd.php';
...
assert_source_contains(
	$source,
	"->where('jenjang', 'sd')",
	'Query catatan harus dibatasi untuk jenjang SD.',
	$failures
);
assert_source_contains(
	$source,
...
if (!$failures) {
	...
	$reflection = new ReflectionClass('Presensi_sd');
	...
	$base_sheet_method->invoke($controller, $excel, 0, 'VI A');
	...
	assert_same('VI A-Catatan', $sheet->getTitle(), 'Nama sheet detail tidak sesuai.', $failures);
	assert_same('0012345', $sheet->getCell('B4')->getValue(), 'NIS catatan tidak ditulis sebagai string.', $failures);
	...
	$base_sheet_method->invoke($controller, $excel, 2, 'V A');
	...
	assert_same('VI A', $excel->getSheet(0)->getTitle(), 'Sheet presensi kelas pertama salah urutan.', $failures);
	assert_same('V A', $excel->getSheet(2)->getTitle(), 'Sheet presensi kelas kedua salah urutan.', $failures);
	assert_same('V A-Catatan', $excel->getSheet(3)->getTitle(), 'Sheet catatan kelas kedua salah urutan.', $failures);

	$collision_excel = new PHPExcel();
	$base_sheet_method->invoke($controller, $collision_excel, 0, $title_method->invoke($controller, 'VI/A', false));
	$sheet_method->invoke($controller, $collision_excel, 1, $collision_excel->getSheet(0)->getTitle(), array(), array());
	$base_sheet_method->invoke($controller, $collision_excel, 2, $title_method->invoke($controller, 'VI:A', false));
	$sheet_method->invoke($controller, $collision_excel, 3, $collision_excel->getSheet(2)->getTitle(), array(), array());
	assert_same('VI-A-2', $collision_excel->getSheet(2)->getTitle(), 'Collision sheet presensi harus diberi suffix angka.', $failures);
	assert_same('VI-A-2-Catatan', $collision_excel->getSheet(3)->getTitle(), 'Collision sheet catatan harus tetap berakhiran -Catatan.', $failures);
	$base_sheet_method->invoke($controller, $collision_excel, 4, $title_method->invoke($controller, 'vi/a', false), false);
	$sheet_method->invoke($controller, $collision_excel, 5, $collision_excel->getSheet(4)->getTitle(), array(), array());
	assert_same('vi-a-3', $collision_excel->getSheet(4)->getTitle(), 'Collision nama sheet harus case-insensitive.', $failures);
	assert_same('vi-a-3-Catatan', $collision_excel->getSheet(5)->getTitle(), 'Pasangan collision case-insensitive harus konsisten.', $failures);
	$base_sheet_method->invoke($controller, $collision_excel, 6, $title_method->invoke($controller, 'VI-A-Catatan', false), false);
	$sheet_method->invoke($controller, $collision_excel, 7, $collision_excel->getSheet(6)->getTitle(), array(), array());
	assert_same('VI-A-Catatan-2', $collision_excel->getSheet(6)->getTitle(), 'Nama kelas berakhiran -Catatan harus diperlakukan sebagai base.', $failures);
	assert_same('VI-A-Catatan-2-Catatan', $collision_excel->getSheet(7)->getTitle(), 'Pasangan nama kelas berakhiran -Catatan harus deterministik.', $failures);
