...
class Model_siswa_sd extends MY_Model
{

    private $primary_key    = 'id_siswa_sd';
    private $table_name     = 'siswa_sd';
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_sd." . $field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_sd." . $field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
        ...
        $this->db->where($where);
        $this->db->where('siswa_sd.is_show', 1);
        $this->db->group_by('siswa_sd.id_siswa_sd');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null)
    {
        ...
        $sort_by = (empty($sort)) ? 'id_siswa_sd' : $sort;
        ...
        $this->db->order_by('siswa_sd.' . $sort_by, $sort_type);
        $this->db->group_by('siswa_sd.id_siswa_sd');
        $query = $this->db->get($this->table_name);
    ...
    {
        $this->db->join('provinces', 'provinces.id = siswa_sd.provinsi', 'LEFT');
        $this->db->join('areas', 'areas.id = siswa_sd.kelurahan', 'LEFT');
        $this->db->join('districts', 'districts.id = siswa_sd.kecamatan', 'LEFT');
        $this->db->join('regencies', 'regencies.id = siswa_sd.kota', 'LEFT');
        $this->db->join('status_lulus', 'status_lulus.id_status_lulus = siswa_sd.status_lulus', 'LEFT');
/*        $this->db->join('provinces provinces1', 'provinces1.id = siswa_sd.provinsi_sekolah', 'LEFT');
        $this->db->join('regencies regencies1', 'regencies1.id = siswa_sd.kota_sekolah', 'LEFT');
        $this->db->join('districts districts1', 'districts1.id = siswa_sd.kecamatan_sekolah', 'LEFT');
        $this->db->join('areas areas1', 'areas1.id = siswa_sd.kelurahan_sekolah', 'LEFT');*/
        $this->db->join('status_daftar_ulang_sd daftar_ulang', 'daftar_ulang.id_siswa_sd = siswa_sd.id_siswa_sd', 'LEFT');
        $this->db->join('transaksi', 'transaksi.user_email = siswa_sd.email', 'LEFT');
        $this->db->group_start();
        $this->db->where('transaksi.nama_bank', 'BNI');
        $this->db->or_where('transaksi.nama_bank', 'BRI');
        $this->db->group_end();
        $this->db->like(array('transaksi.no_transaksi'=>'LI-SD'));
        $this->db->select('siswa_sd.*,
                           ...
                           (CASE
                            ...
                            (CASE WHEN siswa_sd.is_mutasi = 1 THEN "YA" ELSE "TIDAK" END) as is_mutasi
                        ');
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->where('siswa_sd.is_show', 1);
        $this->db->order_by('id_siswa_sd', 'ASC');
        return $this;
    }
    ...
    public function export_siswa($table, $subject = 'file',$value=null,$col=null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_sd." . $field . " LIKE '%" . $value . "%' ";
                } else {
                    $where .= "OR " . "siswa_sd." . $field . " LIKE '%" . $value . "%' ";
                }
                $iterasi++;
            }
...
/* End of file Model_siswa_sd.php */
/* Location: ./application/models/Model_siswa_sd.php */
