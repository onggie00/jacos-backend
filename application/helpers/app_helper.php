		   	...
		   	foreach ($result as $row) {
				...
				$segments = array_slice($ci->uri->segment_array(), 0, count($links));
		   		...
		   		if (implode('/', $segments) == implode('/', $links)) {
		   			...
		   			// Fallback: scoped URL administrator/program_anggaran/{sd|smp|sma|ft}
		   			// harus match menu item dengan link administrator/program_anggaran_{scope}
		   			// (untuk konsistensi active state antara submodule asli dan route scoped).
		   			$active = '';
		   			$full_segments = $ci->uri->segment_array();
		   			if (count($links) >= 2
		   				&& isset($full_segments[0], $full_segments[1], $full_segments[2])
		   				&& $links[0] === 'administrator'
		   				&& $full_segments[0] === 'administrator'
		   				&& $full_segments[1] === 'program_anggaran'
		   				&& in_array($full_segments[2], array('sd', 'smp', 'sma', 'ft'))
		   				&& isset($links[1])
		   				&& $links[1] === 'program_anggaran_' . $full_segments[2]) {
		   				$active = 'active';
		   			}
		   		}
...
if(!function_exists('db_get_all_data_join')) {
	function db_get_all_data_join($table_name = null, $where = false, $join, $query_join) {
		$ci =& get_instance();
		if($join){
			$ci->db->join($join , $query_join, 'LEFT');
		}
		if ($where) {
			$ci->db->where($where);
		}
...
if(!function_exists('get_all_siswa_aktif_sd')) {
	function get_all_siswa_aktif_sd() {
		return db_get_all_data_join('siswa_sd_aktif', ['is_active' => '1'],'kelas_sd','kelas_sd.id_kelas_sd = siswa_sd_aktif.id_kelas');
	}
}

if(!function_exists('get_all_siswa_aktif_smp')) {
	function get_all_siswa_aktif_smp() {
		return db_get_all_data_join('siswa_smp_aktif', ['is_active' => '1'],'kelas_smp','kelas_smp.id_kelas_smp = siswa_smp_aktif.id_kelas');
	}
}

if(!function_exists('get_all_siswa_aktif_sma')) {
	function get_all_siswa_aktif_sma() {
		return db_get_all_data_join('siswa_sma_aktif', ['is_active' => '1'],'kelas_sma','kelas_sma.id_kelas_sma = siswa_sma_aktif.id_kelas');
	}
}

if(!function_exists('get_all_siswa_aktif_ft')) {
	function get_all_siswa_aktif_ft() {
		return db_get_all_data_join('siswa_ft_aktif', ['is_active' => '1'],'kelas_ft','kelas_ft.id_kelas_ft = siswa_ft_aktif.id_kelas');
	}
}
...
if(!function_exists('get_terminal')) {
	function get_terminal($id = null) {
		$id-=1;
		$array=array("Teller","ATM","IB","SMSB","CMS","EDC","RIGS","OTHER","API");
...
if(!function_exists('get_bulan2')) {
	function get_bulan2($id = null) {
		$id-=1;
		$array=array("januari","februari","maret","april","mei","juni","juli","agustus","september","oktober","november","desember");
...
if(!function_exists('get_status_kpi')) {
	function get_status_kpi($id = null) {
		$array=array(0=>"Menunggu Persetujuan",1=>"Disetujui",2=>"Ditolak");

		return $array[$id];
	}
}

if(!function_exists('get_status_anggaran')) {
	function get_status_anggaran($id = null) {
		$array=array(0=>"Belum Proses",1=>"Sudah Proses",2=>"Selesai");
...
if(!function_exists('get_day')) {
	function get_day($id = null) {
		$array=array(0=>"minggu",1=>"senin",2=>"selasa",3=>"rabu",4=>"kamis",5=>"jumat",6=>"sabtu");

		return $array[$id];
	}
}

if(!function_exists('status_lulus')) {
	function status_lulus($id = null) {
		$array=array('Menunggu Konfirmasi'=>1,"Lulus"=>2,"Tidak Lulus"=>3,"Cadangan"=>4);

		return $array[$id];
	}
}

if(!function_exists('is_mutasi')) {
	function is_mutasi($id = null) {
		$array=array('TIDAK'=>2,"IYA"=>0);

		return $array[$id];
	}
}

if(!function_exists('ppsbb')) {
	function ppsbb($id = null) {
		$array=array('TIDAK'=>2,"IYA"=>1);

		return $array[$id];
	}
}

if(!function_exists('is_ppsbb')) {
	function is_ppsbb($id = null) {
		$array=array(2=>'TIDAK',1=>"IYA");

		return $array[$id];
	}
}

if(!function_exists('jenis_ppsbb')) {
	function jenis_ppsbb($id = null) {
		$array=array(1=>'AKADEMIK',2=>"NON AKADEMIK",0=>"NON PPSBB");

		return $array[$id];
	}
}

if(!function_exists('jenis_ppsbb_id')) {
	function jenis_ppsbb_id($id = null) {
		$array=array('AKADEMIK'=>1,"NON AKADEMIK"=>2,"NON PPSBB"=>0);

		return $array[$id];
	}
}

if(!function_exists('is_approve_status')) {
	function is_approve_status($q) {
		if($q=="disetujui"){
			$q=1;
		}elseif($q=="ditolak"){
			$q=2;
...
if(!function_exists('approve_status')) {
	function approve_status($id) {
		$array=array(1=>'Disetujui',2=>"Ditolak",0=>"Menunggu Persetujuan");
...
if(!function_exists('get_api_key')) {
	function get_api_key() {
		$token='708B6E99028D8E71E7A7B0BDCAA88E70';

		return $token;
	}
}

if(!function_exists('tingkatan_romawi')) {
	function tingkatan_romawi($data, $tingkat ) {
		if(strtolower($tingkat) == 'smp'){
			if($data == 1){
				$data = "VII";
			}else if($data == 1){
				$data = "VIII";
		...
		}else if(strtolower($tingkat) == 'sma'){
			if($data == 1){
				$data = "X";
			}else if($data == 1){
				$data = "XI";
