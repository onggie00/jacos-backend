...
class Jadwal_ujian_sd extends REST_Controller
{
  ...
  public function index_get()
  {
    ...
    /*$get_siswa=$this->second_db->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,sa.nama_lengkap as nama_lengkap from siswa_sd_aktif sa join siswa_sd s on s.id_siswa_sd = sa.id_siswa_sd join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran join kelas_sd k on k.id_kelas_sd = sa.id_kelas where id_siswa_sd_aktif = $id_siswa_aktif","row");
    if(!$get_siswa){
      ...
      if (!empty($get_siswa)) {
        ...
        if (!empty($get_ujian)) {
          ...
          foreach ($get_siswa as $key => $value) {
            $value->judul_ujian = $this->second_db->withquery("select ju.nama_ujian from jenis_ujian ju join jadwal_ujian_sd j on ju.id_jenis_ujian = j.id_jenis_ujian join ujian_ruang_kelas k on j.id_tingkatan = k.id_tingkatan where k.id_tingkatan = '".$get_siswa[0]->id_tingkatan."'","row")->nama_ujian;
          }
