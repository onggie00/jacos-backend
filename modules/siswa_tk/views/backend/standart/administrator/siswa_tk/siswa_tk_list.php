...
<section class="content-header">
   <h1>
      <i class="fa fa-users"></i> <?= cclang('siswa_tk') ?> <small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('siswa_tk') ?></li>
   </ol>
</section>
         ...
         <div class="box box-warning">
            <div class="box-header with-border">
               ...
               <div class="box-tools pull-right">
                  <?php is_allowed('siswa_tk_add', function () { ?>
                     <a class="btn btn-sm btn-success" title="Tambah Siswa (Ctrl+a)" href="<?= site_url('administrator/siswa_tk/add'); ?>"><i class="fa fa-plus"></i> Tambah</a>
                  <?php }) ?>
                  <?php is_allowed('siswa_tk_export', function () { ?>
                     <a class="btn btn-sm btn-success" title="Export XLS" href="<?= site_url('administrator/siswa_tk/export?' . http_build_query($_GET)); ?>"><i class="fa fa-file-excel-o"></i> XLS</a>
                  <?php }) ?>
            ...
            <div class="box-body">
               <!-- Filter & Search -->
               <form name="form_filter_siswa_tk" id="form_filter_siswa_tk" action="<?= base_url('administrator/siswa_tk/index'); ?>">
                  ...
                  <div class="row">
                     ...
                     <div class="col-md-2">
                        <div class="form-group" style="margin-bottom:8px">
                           <select class="form-control chosen chosen-select" name="s">
                              ...
                              <option <?= $this->input->get('s') == 'id_siswa_tk' ? 'selected' : ''; ?> value="id_siswa_tk">ID</option>
                     ...
                     <div class="col-md-3">
                        <div class="form-group" style="margin-bottom:8px">
                           ...
                           <a class="btn btn-default" href="<?= base_url('administrator/siswa_tk'); ?>"><i class="fa fa-times"></i> Reset</a>
                           <?php endif; ?>
                        </div>
                     </div>
                  </div>
                  ...
                  <form name="form_siswa_tk" id="form_siswa_tk" action="<?= base_url('administrator/siswa_tk/index'); ?>">
                     ...
                     <div class="table-responsive">
                        <table class="table table-bordered table-striped dataTable">
                           ...
                           <tbody id="tbody_siswa_tk">
                              <?php foreach ($siswa_tks as $siswa_tk) : ?>
                                 <tr>
                                    <td width="5">
                                       <input type="checkbox" class="flat-red check" name="id[]" value="<?= $siswa_tk->id_siswa_tk; ?>">
                                    </td>
                                    <td width="180" style="white-space:nowrap">
                                       <?php is_allowed('siswa_tk_view', function () use ($siswa_tk) { ?>
                                          <a href="<?= site_url('administrator/siswa_tk/view/' . $siswa_tk->id_siswa_tk); ?>" class="btn-action btn-action-view" title="Lihat Detail"><i class="fa fa-eye"></i> Detail</a>
                                          <a href="<?= site_url('administrator/siswa_tk/single_pdf/' . $siswa_tk->id_siswa_tk); ?>" class="btn-action btn-action-pdf" title="Download PDF"><i class="fa fa-file-pdf-o"></i> PDF</a>
                                       <?php }) ?>
                                       <?php is_allowed('siswa_tk_update', function () use ($siswa_tk) { ?>
                                          <a href="<?= site_url('administrator/siswa_tk/edit/' . $siswa_tk->id_siswa_tk); ?>" class="btn-action btn-action-edit" title="Edit"><i class="fa fa-pencil"></i> Edit</a>
                                       <?php }) ?>
                                       <?php is_allowed('siswa_tk_delete', function () use ($siswa_tk) { ?>
                                          <a href="javascript:void(0);" data-href="<?= site_url('administrator/siswa_tk/delete/' . $siswa_tk->id_siswa_tk); ?>" class="btn-action btn-action-delete remove-data" title="Hapus"><i class="fa fa-trash"></i> Hapus</a>
                                       <?php }) ?>
                                    </td>
                                    <td><?= _ent(ucwords($siswa_tk->nama_lengkap)); ?></td>
                                    <td><?= _ent($siswa_tk->tingkatan_label); ?></td>
                                    <td><?= _ent($siswa_tk->no_peserta); ?></td>
                                    <td><?= _ent($siswa_tk->email); ?></td>
                                    <td><?= _ent($siswa_tk->status_lulus) ?></td>
                                    <td><?= _ent($siswa_tk->email_ms_office); ?></td>
                                    <td><?= _ent($siswa_tk->nisn); ?></td>
                                    <td><?= _ent($siswa_tk->email_ms_office_ortu); ?></td>
                                    <td><?= _ent($siswa_tk->notelp_ibu); ?></td>
                                    <td><?= _ent($siswa_tk->notelp_ayah); ?></td>
                                    <td><?= _ent($siswa_tk->sekolah_asal); ?></td>
                                    <td><?= "<a target='_blank' href='".BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->foto_peserta."'>Lihat Foto Peserta</a>"; ?></td>
                                    <td><?= "<a target='_blank' href='".BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->akte_lahir."'>Lihat Akte Lahir</a>"; ?></td>
                                    <td><?= "<a target='_blank' href='".BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->kartu_keluarga."'>Lihat KK</a>"; ?></td>
                                  <!-- <td>
                                       <?php if (!empty($siswa_tk->foto_peserta)) : ?>
                                          ...
                                          <?php else : ?>
                                             <a href="<?= BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->foto_peserta; ?>">
                                                <img src="<?= get_icon_file($siswa_tk->foto_peserta); ?>" class="image-responsive image-icon" alt="image siswa_tk" title="foto_peserta <?= $siswa_tk->foto_peserta; ?>" width="40px">
                                             </a>
                                          <?php endif; ?>
                                       <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                       <?php if (!empty($siswa_tk->akte_lahir)) : ?>
                                          ...
                                          <?php else : ?>
                                             <a href="<?= BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->akte_lahir; ?>">
                                                <img src="<?= get_icon_file($siswa_tk->akte_lahir); ?>" class="image-responsive image-icon" alt="image siswa_tk" title="akte_lahir <?= $siswa_tk->akte_lahir; ?>" width="40px">
                                             </a>
                                          <?php endif; ?>
                                       <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                       <?php if (!empty($siswa_tk->kartu_keluarga)) : ?>
                                          ...
                                          <?php else : ?>
                                             <a href="<?= BASE_URL . 'uploads/siswa_tk/' . $siswa_tk->kartu_keluarga; ?>">
                                                <img src="<?= get_icon_file($siswa_tk->kartu_keluarga); ?>" class="image-responsive image-icon" alt="image siswa_tk" title="kartu_keluarga <?= $siswa_tk->kartu_keluarga; ?>" width="40px">
                                             </a>
                                          <?php endif; ?>
                                       <?php endif; ?>
                                    </td> -->
                                    <td><?= _ent($siswa_tk->is_mutasi); ?></td>
                                    <td><?= _ent($siswa_tk->no_transaksi); ?></td>
                                    <td><?= _ent($siswa_tk->va_number); ?></td>
                                    <td><?= _ent($siswa_tk->tanggal_daftar); ?></td>
                                    <td><?= _ent($siswa_tk->tanggal_pembayaran); ?></td>
                                    <td><?= _ent($siswa_tk->tanggal_aktivasi); ?></td>
                                    <td><?= _ent($siswa_tk->tanggal_bayar_daftar_ulang); ?></td>
                                    <td><?= _ent($siswa_tk->status_daftar_ulang); ?></td>
...
<div class="modal fade" id="modal_add_new" tabindex="-1" role="dialog">
   <div class="modal-dialog">
      <div class="modal-content" style="border-radius:8px;overflow:hidden">
         ...
         <form action="<?= base_url('administrator/siswa_tk/import/'); ?>" method="post" enctype="multipart/form-data">
            <div class="modal-body" style="padding:20px">
               <div style="background:#f8f9fa;border-radius:6px;padding:15px;margin-bottom:15px">
                  <p style="margin:0;color:#666;font-size:13px">
                     <i class="fa fa-info-circle" style="color:#3498db"></i> Import siswa baru ke tabel <strong>siswa_tk_aktif</strong>.<br>
               ...
               <div class="form-group">
                  ...
                  <p style="margin-top:8px;font-size:12px;color:#999">
                     ...
                     <a href="<?= base_url('uploads/example/format_siswa_tk.xlsx'); ?>" style="color:#3498db"><i class="fa fa-download"></i> Download template</a>
                  </p>
               </div>
            </div>
...
<script>
   $(document).ready(function() {
      ...
      $('#apply').click(function() {
         ...
         var bulk = $('#bulk');
         var serialize_bulk = $('#form_siswa_tk').serialize();
         ...
         if (bulk.val() == '1') {
            swal({
             ...
             function(isConfirm){
               if (isConfirm) {
                  document.location.href = BASE_URL + '/administrator/siswa_tk/delete?' + serialize_bulk;      
               }
             });
           ...
           return false;
            ...
            swal({
               ...
               function(inputValue) {
                  ...
                  document.location.href = BASE_URL + '/administrator/siswa_tk/update_status_daftar_ulang?' + $('#form_siswa_tk').serialize();
               })
            return false;

         } else if (bulk.val() == '3') {
            $('#st').val('3');
            document.location.href = BASE_URL + '/administrator/siswa_tk/update_status?' + $('#form_siswa_tk').serialize();
            ...
            document.location.href = BASE_URL + 'administrator/siswa_tk/download_file?' + $('#form_siswa_tk').serialize();
            return false;
         } else if (bulk.val() == '6') {
            document.location.href = BASE_URL + 'administrator/siswa_tk/download_kartu_peserta?' + $('#form_siswa_tk').serialize();
            return false;
         } else if (bulk.val() == '') {
            swal({
               title: "Upss",
