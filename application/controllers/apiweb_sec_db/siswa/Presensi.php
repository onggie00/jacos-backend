...
class Presensi extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        $get_siswa_aktif = $this->second_db->withquery("select id_siswa_".$jenjang.", nama_lengkap, device_id_siswa, device_id_ortu from siswa_".$jenjang."_aktif where id_siswa_".$jenjang."_aktif = ".$id_siswa_aktif,"row");
        $get_siswa = null;
        $nama_kolom = "id_siswa_".$jenjang;
        $notelp = "";
        $nama_siswa = "";
        if (!empty($get_siswa_aktif)) {
            $get_siswa = $this->second_db->withquery("select notelp_ibu, notelp_ayah, nama_lengkap from siswa_".$jenjang." where id_siswa_".$jenjang." =".$get_siswa_aktif->$nama_kolom,"row");
        ...
        $check_if_presented = $this->second_db->withquery("select * from presensi_".$jenjang." $where $where_tanggalabsen order by id desc","result");
        ...
        if ($check_if_presented){
            ...
            //$cek_ssid = $this->second_db->withquery("select * from pengaturan_whitelist_ssid where nama_ssid = '".$this->post('wifi_ssid')."' or id_pengaturan = '".$this->post('wifi_ssid')."'","row");
        ...
        if (!empty($data)) {
            $insert=$this->second_db->insertid("presensi_".$jenjang,$data);
            if($insert){
