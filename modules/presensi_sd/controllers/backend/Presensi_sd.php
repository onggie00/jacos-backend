...
class Presensi_sd extends Admin	
{
	...
	*/
	public function index($offset = 0)
	{
		...
		FROM presensi_sd p
		JOIN siswa_sd_aktif s ON p.id_siswa_aktif = s.id_siswa_sd_aktif
		...
		$this->data['list_tingkatan'] = $this->mymodel->withquery("SELECT * FROM tingkatan_sd ORDER BY label ASC", 'result');
		$this->data['list_kelas'] = $this->mymodel->withquery("SELECT * FROM kelas_sd ORDER BY label ASC", 'result');
	...
	private function _build_filter_where($get)
	{
		...
		if (!empty($get['id_kelas'])) {
			$where .= (empty($where) ? 'WHERE ' : ' AND ') . "k.id_kelas_sd = '" . $this->db->escape_str($get['id_kelas']) . "'";
		}
	...
	*/
	...
	/**
	 * Ambil catatan pelajaran SD dan kelompokkan per siswa serta tanggal.
	 */
	...
	private function _get_export_filter_label($id_tingkatan, $id_kelas)
	{
		...
		if (!empty($id_kelas)) {
			$filter = $this->db
				->select('label')
				->where('id_kelas_sd', $id_kelas)
				->get('kelas_sd')
				->row();
		} elseif (!empty($id_tingkatan)) {
			$filter = $this->db
				->select('label')
				->where('id_tingkatan_sd', $id_tingkatan)
				->get('tingkatan_sd')
				->row();
		}
	...
	private function _build_export_filename($start_date, $end_date, $filter_label)
	{
		...
		return 'Presensi SD - ' . $start_date . '-' . $end_date . '-' . $filter_label . '.xls';
	}
	...
	*/
	public function export_bulanan()
	{
		...
		if ($id_tingkatan) {
			...
			$where_kelas = "WHERE id_kelas_sd = '$id_kelas'";
		}
		$get_kelas = $this->mymodel->withquery("SELECT * FROM kelas_sd $where_kelas", "result");

		$date_range = range(1, $total_day_in_month);
		$presensi_rows = $this->mymodel->withquery("SELECT * FROM presensi_sd WHERE MONTH(tanggal_absen) = '$bulan' AND YEAR(tanggal_absen) = '$tahun'", "result");
		$presensi_map = [];
		...
		if ($get_kelas) {
			foreach ($get_kelas as $key_kelas => $kelas) {
				...
				if ($kelas->id_kelas_sd) $where[] = "s.id_kelas = '$kelas->id_kelas_sd'";
				$where_clause = $where ? "WHERE " . implode(" AND ", $where) : "";

				$get_siswa = $this->mymodel->withquery("SELECT s.id_siswa_sd_aktif AS id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label AS nama_kelas FROM siswa_sd_aktif s JOIN kelas_sd k ON s.id_kelas = k.id_kelas_sd $where_clause ORDER BY k.label ASC, s.nama_lengkap ASC", "result");
	...
	public function export_harian()
	{
		...
		if (!empty($id_tingkatan)) {
			$kelas_query = "SELECT * FROM kelas_sd WHERE id_tingkatan = '$id_tingkatan'";
		} elseif (!empty($id_kelas)) {
			$kelas_query = "SELECT * FROM kelas_sd WHERE id_kelas_sd = '$id_kelas'";
		}
		...
		if (!empty($get_kelas)) {
			foreach ($get_kelas as $key_kelas => $value_kelas) {
				...
				if (!empty($value_kelas->id_kelas_sd)) $where[] = "s.id_kelas = '{$value_kelas->id_kelas_sd}'";
	
				$sql_where = $where ? "WHERE " . implode(" AND ", $where) : "";
				$get_siswa = $this->mymodel->withquery("SELECT s.id_siswa_sd_aktif AS id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label AS nama_kelas FROM siswa_sd_aktif s JOIN kelas_sd k ON s.id_kelas = k.id_kelas_sd $sql_where ORDER BY k.label ASC, s.nama_lengkap ASC", "result");
	
				$presensi_map = [];
				$presensi_rows = $this->mymodel->withquery("SELECT * FROM presensi_sd WHERE tanggal_absen BETWEEN '$tgl_start' AND '$tgl_end'", "result");
	...
	public function export_periode()
	{
		...
		foreach ($get_kelas as $i => $kelas) {
			...
			$siswa = $this->mymodel->withquery("
				SELECT s.id_siswa_sd_aktif AS id_siswa, s.nama_lengkap, s.nis,
					k.label AS nama_kelas
				FROM siswa_sd_aktif s
				JOIN kelas_sd k ON s.id_kelas = k.id_kelas_sd
				$where_clause
	...
	*/
	...
	{
		if (!$this->is_allowed('presensi_sd_list', false)) {
		...
		$this->load->view('backend/standart/administrator/presensi_sd/presensi_sd_detail_catatan', $this->data);
	}
	...
	public function get_siswa_terlambat()
	{
		...
		$sql = "SELECT 
				s.id_siswa_sd_aktif AS id_siswa_aktif,
				s.nama_lengkap,
				s.nis,
				k.label AS nama_kelas,
		...
		if (!empty($id_kelas)) {
			$sql .= " AND k.id_kelas_sd = " . $this->db->escape_str($id_kelas);
		}

		$sql .= " GROUP BY s.id_siswa_sd_aktif, s.nama_lengkap, s.nis, k.label
	...
	public function get_riwayat_terlambat()
	{
		...
		echo json_encode(array(
			'success' => true,
			'data' => $riwayat,
			'nama_siswa' => $this->db->select('nama_lengkap')->where('id_siswa_sd_aktif', $siswa)->get('siswa_sd_aktif')->row()->nama_lengkap
		));
	}
