...
<section class="content-header">
   <h1><i class="fa fa-calendar-check-o"></i> Presensi FT <small>Daftar Presensi Harian Siswa</small></h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">Presensi FT</li>
   </ol>
</section>
         ...
         <div class="box box-warning">
            <div class="box-header with-border">
               ...
               <div class="box-tools pull-right">
                  <?php is_allowed('presensi_ft_add', function(){ ?>
                  <a class="btn btn-sm btn-success" title="<?= cclang('add_new_button', [cclang('presensi_ft')]); ?> (Ctrl+a)" href="<?= site_url('administrator/presensi_ft/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Presensi
                  </a>
                  <?php }) ?>
                  <?php is_allowed('presensi_ft_export', function() use ($filter){
                     $qfilter = isset($filter) ? http_build_query($filter) : '';
                  ?>
                  <a class="btn btn-sm btn-info" title="Export Harian" href="<?= site_url('administrator/presensi_ft/export_harian?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Harian</a>
                  <a class="btn btn-sm btn-info" title="Export Bulanan" href="<?= site_url('administrator/presensi_ft/export_bulanan?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Bulanan</a>
                  <a class="btn btn-sm btn-info" title="Export Periode" href="<?= site_url('administrator/presensi_ft/export_periode?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Periode</a>
                  <?php }) ?>
               </div>
            </div>
            ...
            <div class="box-body">
                           ...
                           <div class="form-group">
                              ...
                              <select name="id_tingkatan" id="select_tingkatan_terlambat" class="form-control input-sm">
                                 ...
                                 <?php foreach($list_tingkatan as $t): ?>
                                    <option value="<?= $t->id_tingkatan_ft; ?>"><?= htmlspecialchars($t->label); ?></option>
                                 <?php endforeach; ?>
                              </select>
                           </div>
                           <div class="form-group">
                              ...
                              <select name="id_kelas" id="select_kelas_terlambat" class="form-control input-sm">
                                 ...
                                 <?php foreach($list_kelas as $k): ?>
                                    <option value="<?= $k->id_kelas_ft; ?>"><?= htmlspecialchars($k->label); ?></option>
                                 <?php endforeach; ?>
                              </select>
                           </div>
               ...
               <form method="get" action="<?= base_url('administrator/presensi_ft/index'); ?>" class="form-inline filter-bar">
                  <div class="form-group">
                     <label>Cari:</label>
                  ...
                  <div class="form-group">
                     ...
                     <select name="id_kelas" class="form-control input-sm">
                        <option value="">- Semua -</option>
                        <?php foreach($list_kelas as $k): ?>
                        <option value="<?= $k->id_kelas_ft; ?>" <?= (isset($filter['id_kelas']) && $filter['id_kelas']==$k->id_kelas_ft) ? 'selected' : ''; ?>><?= htmlspecialchars($k->label); ?></option>
                        <?php endforeach; ?>
                     </select>
                  </div>
                  <div class="form-group">
                  ...
                  <a href="<?= base_url('administrator/presensi_ft'); ?>" class="btn btn-default btn-sm"><i class="fa fa-times"></i> Reset</a>
               </form>
               ...
               if (!empty($filter['id_kelas'])) {
                  $k_label = '';
                  foreach($list_kelas as $k) { if ($k->id_kelas_ft==$filter['id_kelas']) { $k_label = $k->label; break; } }
                        ...
                        <tr>
                           ...
                           <td>
                              ...
                              <?php if (!empty($p->id_siswa_aktif)): ?>
                                 <?= anchor('administrator/siswa_ft_aktif/view/'.$p->id_siswa_aktif.'?popup=show', $p->nama_lengkap, ['class' => 'popup-view']); ?>
                              <?php endif; ?>
                           </td>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($p->id_izin)): ?>
                                 <?= anchor('administrator/izin_siswa_ft/view/'.$p->id_izin.'?popup=show', $p->jenis_izin, ['class' => 'popup-view']); ?>
                              <?php else: ?>
                                 <span class="text-muted">-</span>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php is_allowed('presensi_ft_update', function() use ($p){?>
                              <a href="<?= site_url('administrator/presensi_ft/edit/' . $p->id); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                              <?php }) ?>
                              <?php is_allowed('presensi_ft_delete', function() use ($p){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/presensi_ft/delete/' . $p->id); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                              <?php }) ?>
                           </td>
                        </tr>
                     <?php endforeach; else: ?>
...
$(document).ready(function(){
   ...
   $('.btn-catatan').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_ft/detail_catatan'); ?>',
         type: 'POST',
         data: {
   ...
   $('#btn_load_terlambat').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_ft/get_siswa_terlambat'); ?>',
         type: 'POST',
         data: {
   ...
   $(document).on('click', '.btn-riwayat-terlambat', function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_ft/get_riwayat_terlambat'); ?>',
         type: 'POST',
         data: {
            id_siswa_aktif: id_siswa,
            id_tahun_ajaran: ta,
