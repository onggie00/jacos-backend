...
class User extends Admin	
{
	...
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'user_insert_id' => $this->session->userdata('id'),
			];
			...
			$save_user = $this->aauth->create_user($this->input->post('email'), 
			$this->input->post('password'), $this->input->post('username'), $save_data);
	...
	*/
	...
	public function import()
	{
		...
		if (isset($_FILES["file_user"]["name"])) {
			$path = $_FILES["file_user"]["tmp_name"];
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					// dd($worksheet->getCellByColumnAndRow(1, $row)->getValue());
					$save_data = [
						'full_name' 	=> $worksheet->getCellByColumnAndRow(3, $row)->getValue(),
						'avatar' 		=> 'default.png',
						'date_created'	=> date('Y-m-d H:i:s')
					];
					...
					$save_user = $this->aauth->create_user($email, $password,$username,$save_data);

					// dd($save_user);
					if ($save_user == false) {
						$this->db->trans_rollback();
						$this->load->library("session");
						$this->session->set_flashdata('error', 'Import siswa gagal atas nama '.$save_data["full_name"]);
	...
	*/
	public function profile()
	{
		...
		// echo "access_token : ".$this->session->userdata('access_token'). " - microsoft id : " . $this->session->userdata('microsoft_id');
