...
class Pendaftaran_kb extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->mymodel->withquery("select id_siswa_kb from siswa_kb where email like '%".$this->post("email")."%' and nama_lengkap like '%".$this->post("nama_lengkap")."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");
        ...
        if (empty($cek_email)) {
          ...
          $gelombang = $this->mymodel->withquery("select gelombang from web_periode_daftar where periode_pendaftaran_mulai <= '".date("Y-m-d H:i:s")."' and periode_pendaftaran_selesai >= '".date("Y-m-d H:i:s")."' and kelas = 'KB' ","row")->gelombang;
          ...
          if (!empty($_FILES['foto_peserta']['name'])) {
            $uploaddir = './uploads/siswa_kb/'; if(!is_dir($uploaddir)){ mkdir($uploaddir, 0777); }
          ...
          $get_tingkatan_kb = $this->mymodel->getbywhere("tingkatan_kb", "id_tingkatan_kb", $id_tingkatan_post, "row");
          if (empty($get_tingkatan_kb)) {
            $msg = array('status' => 0, 'message' => 'Tingkatan tidak ditemukan', 'data' => array(), 'transaksi' => array());
            $this->response($msg, '200');
          }
          if (!psb_cek_usia($this->post("tgl_lahir"), $get_tingkatan_kb->usia_min, $get_tingkatan_kb->usia_max)) {
            $msg = array('status' => 0, 'message' => 'Usia calon siswa tidak sesuai dengan tingkatan ' . $get_tingkatan_kb->label, 'data' => array(), 'transaksi' => array());
            $this->response($msg, '200');
          }

          if (!empty($data) && empty($cek_email)) {
            $id_siswa = $this->mymodel->insertid("siswa_kb",$data);
            //echo $this->db->last_query();
            if (!empty($id_siswa)) {
              $data['id_siswa'] = $id_siswa;
              //insert transaksi
              $no_transaksi = "LI-KB-".date("Ymd")."-".$id_siswa;
              ...
              $data_transaksi = array(
                ...
                "nama_bank" => 'BRI',//$this->db->escape($this->post('nama_bank')),
                "user_email" => $this->post('email'),
                "user_name" => $this->post('nama_lengkap'),
                "user_phone" => "",
                "description" => "Tagihan Pendaftaran KB a.n ".strtoupper($this->post('nama_lengkap')),
              ...
              if (!empty($data_transaksi)) {
                ...
                //create billing
                                //create billing VA BNI eCollection (kode 11 = KB) — jenjang baru, langsung BNI tanpa flag BRI
                $client_id = '';
                $prefix = '';
                ...
                $this->mymodel->update("siswa_kb", array("no_transaksi" => $no_transaksi, "va_number" => $payment_response['virtual_account']), "id_siswa_kb", $id_siswa);
