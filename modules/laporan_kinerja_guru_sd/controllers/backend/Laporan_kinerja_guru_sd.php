...
/**
*| --------------------------------------------------------------------------
*| Laporan Kinerja Guru Sd Controller
*| --------------------------------------------------------------------------
*| Laporan Kinerja Guru Sd site
*|
*/
class Laporan_kinerja_guru_sd extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_laporan_kinerja_guru_sd');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Laporan Kinerja Guru Sds
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('laporan_kinerja_guru_sd_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['laporan_kinerja_guru_sds'] = $this->model_laporan_kinerja_guru_sd->get($filter, $field, $this->limit_page, $offset);
		$this->data['laporan_kinerja_guru_sd_counts'] = $this->model_laporan_kinerja_guru_sd->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/laporan_kinerja_guru_sd/index/',
			'total_rows'   => $this->model_laporan_kinerja_guru_sd->count_all($filter, $field),
		...
		$this->template->title('Laporan Kinerja Guru SD List');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sd/laporan_kinerja_guru_sd_list', $this->data);
	}
	
	/**
	* Add new laporan_kinerja_guru_sds
	*
	*/
	public function add()
	{
		$this->is_allowed('laporan_kinerja_guru_sd_add');

		$this->template->title('Laporan Kinerja Guru SD New');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sd/laporan_kinerja_guru_sd_add', $this->data);
	}

	/**
	* Add New Laporan Kinerja Guru Sds
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('laporan_kinerja_guru_sd_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_kinerja_guru_sd = $this->model_laporan_kinerja_guru_sd->store($save_data);
            

			if ($save_laporan_kinerja_guru_sd) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_laporan_kinerja_guru_sd;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/laporan_kinerja_guru_sd/edit/' . $save_laporan_kinerja_guru_sd, 'Edit Laporan Kinerja Guru Sd'),
						anchor('administrator/laporan_kinerja_guru_sd', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/laporan_kinerja_guru_sd');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Laporan Kinerja Guru Sds
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('laporan_kinerja_guru_sd_update');

		$this->data['laporan_kinerja_guru_sd'] = $this->model_laporan_kinerja_guru_sd->find($id);

		$this->template->title('Laporan Kinerja Guru SD Update');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sd/laporan_kinerja_guru_sd_update', $this->data);
	}

	/**
	* Update Laporan Kinerja Guru Sds
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('laporan_kinerja_guru_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_kinerja_guru_sd = $this->model_laporan_kinerja_guru_sd->change($id, $save_data);
	...
	/**
	* delete Laporan Kinerja Guru Sds
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('laporan_kinerja_guru_sd_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'laporan_kinerja_guru_sd'), 'success');
        } else {
            set_message(cclang('error_delete', 'laporan_kinerja_guru_sd'), 'error');
        }
		...
		/**
	* View view Laporan Kinerja Guru Sds
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('laporan_kinerja_guru_sd_view');

		$this->data['laporan_kinerja_guru_sd'] = $this->model_laporan_kinerja_guru_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Laporan Kinerja Guru SD Detail');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sd/laporan_kinerja_guru_sd_view', $this->data);
	}
	...
	*/
	...
	{
		$laporan_kinerja_guru_sd = $this->model_laporan_kinerja_guru_sd->find($id);

		
		
		return $this->model_laporan_kinerja_guru_sd->remove($id);
	}
	...
	*/
	...
	{
		$this->is_allowed('laporan_kinerja_guru_sd_export');

		//$this->model_laporan_kinerja_guru_sd->export('laporan_kinerja_guru_sd', 'laporan_kinerja_guru_sd');
		$f = $this->input->get('f');
		$q = $this->input->get('q');
		$where = "";
		...
		$list_kinerja = array();
		if (!empty($f) && !empty($q)) {
			$query = "select p.id_guru, p.nama_lengkap, p.npp, k.unit_kerja, k.mata_pelajaran, k.umur, k.masa_kerja, k.golongan, k.kompetensi_pedagogik, k.kompetensi_profesional, k.kompetensi_kepribadian, k.kompetensi_sosial, k.leadership, k.nilai_prestasi, k.nilai_presensi, k.total_skor from laporan_kinerja_guru_sd k 
			...
			$list_kinerja = $this->mymodel->withquery("select p.id_guru, p.nama_lengkap, p.npp, k.unit_kerja, k.mata_pelajaran, k.umur, k.masa_kerja, k.golongan, k.kompetensi_pedagogik, k.kompetensi_profesional, k.kompetensi_kepribadian, k.kompetensi_sosial, k.leadership, k.nilai_prestasi, k.nilai_presensi, k.total_skor from laporan_kinerja_guru_sd k 
			join guru_sd p on k.id_guru = p.id_guru order by k.tahun_ajaran DESC, p.nama_lengkap ASC ","result");
		}
		...
		header('Content-Disposition: attachment;filename="Laporan Kinerja Guru SD - '.date("Y-m-d").'.xls"');
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_laporan_kinerja_guru_sd->pdf('laporan_kinerja_guru_sd', 'laporan_kinerja_guru_sd');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'laporan_kinerja_guru_sd';
        ...
        $data = $this->model_laporan_kinerja_guru_sd->find($id);
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 5; $row <= $highestRow; $row++) {
					...
					if(empty($check)){
						...
						$check2=$this->mymodel->withquery("select * from guru_sd where nama_lengkap like '%".$nama."%'", 'row');
						if(empty($check2)){
							$this->db->trans_rollback();
							$this->load->library("session");
							$this->session->set_flashdata('failed', "Input NPP guru_sd ".$check2->nama_lengkap." ".$nama." salah. (NPP : ".$check2->npp." ".$npp.")");
							redirect($_SERVER['HTTP_REFERER']);
						}
						else{
							$check=$check2;
					...
					/* foreach ($data_laporan as $key => $item) {
						if(!$item){
							$this->db->trans_rollback();
							$this->load->library("session");
							$this->session->set_flashdata('failed', "Data ada yang kosong -".$key."-".$item);
							redirect($_SERVER['HTTP_REFERER']);
						}
					} */
				
					//check apakah npp tersebut di tahun ajaran yg sama sudah ada / belum
					$check_nilai = $this->mymodel->withquery("select * from laporan_kinerja_guru_sd where id_guru = '".$check->id_guru."' and tahun_ajaran = '".$this->input->post('tahun_ajaran')."'", 'row');
					
					if(!empty($check_nilai)){
						$data_laporan['created_at'] = date("Y-m-d H:i:s");
						$this->mymodel->update("laporan_kinerja_guru_sd", $data_laporan, 'id_guru', $check->id_guru);
						$insertId = $check_nilai->id_laporan;
					}else{
						$insertId = $this->mymodel->insertid("laporan_kinerja_guru_sd", $data_laporan);
					}
...
/* End of file laporan_kinerja_guru_sd.php */
/* Location: ./application/controllers/administrator/Laporan Kinerja Guru Sd.php */