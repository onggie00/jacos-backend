...
class Model_siswa_tk extends MY_Model
{

    private $primary_key    = 'id_siswa_tk';
    private $table_name     = 'siswa_tk';
    private $field_search   = ['nama_lengkap', 'email', 'email_ms_office', 'nisn', 'tempat_lahir', 'tgl_lahir', 'jenis_kelamin', 'agama', 'email_ms_office_ortu', 'nama_ibu', 'pekerjaan_ibu', 'notelp_ibu', 'nama_ayah', 'pekerjaan_ayah', 'notelp_ayah', 'alamat', 'provinsi', 'kelurahan', 'kecamatan', 'kota', 'kode_pos', 'sekolah_asal', 'foto_peserta', 'akte_lahir', 'kartu_keluarga', 'sumber_informasi', 'alasan_tertarik', 'is_mutasi', 'kelas_mutasi', 'status_lulus', 'no_peserta', 'no_transaksi', 'provinsi_sekolah', 'kota_sekolah', 'kecamatan_sekolah', 'kelurahan_sekolah', 'va_number', 'is_show'];
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_tk." . $field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_tk." . $field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
        ...
        $this->db->where($where);
        $this->db->where('siswa_tk.is_show', 1);
        $this->db->group_by('siswa_tk.id_siswa_tk');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                // if($field=="status_lulus"){
                //     $q=status_lulus($q);
                // }
                // if($field=="is_mutasi"){
                //     $q=is_mutasi(strtoupper($q));
                // }
                if ($iterasi == 1) {
                    $where .= "siswa_tk." . $field . " LIKE '%" . $q . "%' ";
                } else {
        ...
        $sort_by = (empty($sort)) ? 'id_siswa_tk' : $sort;
        ...
        $this->db->order_by('siswa_tk.' . $sort_by, $sort_type);
        $this->db->group_by('siswa_tk.id_siswa_tk');
        $query = $this->db->get($this->table_name);
    ...
    {
        $this->db->join('provinces', 'provinces.id = siswa_tk.provinsi', 'LEFT');
        $this->db->join('areas', 'areas.id = siswa_tk.kelurahan', 'LEFT');
        $this->db->join('districts', 'districts.id = siswa_tk.kecamatan', 'LEFT');
        $this->db->join('regencies', 'regencies.id = siswa_tk.kota', 'LEFT');
        $this->db->join('status_lulus', 'status_lulus.id_status_lulus = siswa_tk.status_lulus', 'LEFT');
        $this->db->join('tingkatan_tk', 'tingkatan_tk.id_tingkatan_tk = siswa_tk.id_tingkatan', 'LEFT');
/*        $this->db->join('provinces provinces1', 'provinces1.id = siswa_tk.provinsi_sekolah', 'LEFT');
        $this->db->join('regencies regencies1', 'regencies1.id = siswa_tk.kota_sekolah', 'LEFT');
        $this->db->join('districts districts1', 'districts1.id = siswa_tk.kecamatan_sekolah', 'LEFT');
        $this->db->join('areas areas1', 'areas1.id = siswa_tk.kelurahan_sekolah', 'LEFT');*/
        $this->db->join('status_daftar_ulang_tk daftar_ulang', 'daftar_ulang.id_siswa_tk = siswa_tk.id_siswa_tk', 'LEFT');
        $this->db->join('transaksi', 'transaksi.user_email = siswa_tk.email', 'LEFT');
        $this->db->group_start();
        $this->db->where('transaksi.nama_bank', 'BNI');
        $this->db->or_where('transaksi.nama_bank', 'BRI');
        $this->db->group_end();
        $this->db->like(array('transaksi.no_transaksi'=>'LI-TK'));
        $this->db->select('siswa_tk.*,
                           ...
                           tingkatan_tk.label as tingkatan_label
                        ');
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->where('siswa_tk.is_show', 1);
        $this->db->order_by('id_siswa_tk', 'ASC');
        return $this;
    }
    ...
    public function export_siswa($table, $subject = 'file',$value=null,$col=null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_tk." . $field . " LIKE '%" . $value . "%' ";
                } else {
                    $where .= "OR " . "siswa_tk." . $field . " LIKE '%" . $value . "%' ";
                }
                $iterasi++;
            }
        ...
        foreach ($result->result() as $data) {
            ...
            foreach ($fields as $f) {
                ...
                if ($f == 'nama_lengkap') {
                    $data_field = ucwords(strtolower($data->$f));
                } else {
                    $data_field = $data->$f;
                }
...
/* End of file Model_siswa_tk.php */
/* Location: ./application/models/Model_siswa_tk.php */
