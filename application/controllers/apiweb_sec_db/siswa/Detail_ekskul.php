...
class Detail_ekskul extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        if (!empty($jenjang)) {
            ...
            $data = $this->second_db->withquery("select e.* from ekskul e where e.jenjang = '".$jenjang."' and id_ekskul = '".$id_ekskul."' order by e.nama ASC", "result");
        }
        else{
        ...
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                $cek_member = $this->second_db->withquery("select * from ekskul_member_".$jenjang." where id_siswa = '".$id_siswa."' and id_ekskul = '".$id_ekskul."' ","row");
                if (!empty($value->foto)) {
