      ...
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-header with-border">
               <h3 class="box-title"><i class="fa fa-list"></i> Data Kelas <span class="label bg-yellow" style="margin-left:8px"><?= (int)$kelas_smp_counts; ?> Data</span></h3>
               <div class="box-tools pull-right">
                  <?php is_allowed('kelas_smp_add', function(){ ?>
                  <a class="btn btn-sm btn-success" title="Tambah Kelas (Ctrl+a)" href="<?= site_url('administrator/kelas_smp/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Kelas
                  </a>
                  <?php }) ?>
                  <?php is_allowed('kelas_smp_export', function(){ ?>
                  <a class="btn btn-sm btn-info" href="<?= site_url('administrator/kelas_smp/export'); ?>">
            ...
            <div class="box-body">
               ...
               <form method="get" action="<?= base_url('administrator/kelas_smp/index'); ?>" class="form-inline filter-bar">
                  ...
                  <a href="<?= base_url('administrator/kelas_smp'); ?>" class="btn btn-default btn-sm"><i class="fa fa-times"></i> Reset</a>
               </form>
               ...
               <div class="row" style="margin-bottom:15px">
                  <div class="col-md-4">
                     <div class="info-box-custom bg-grad-blue">
                        ...
                        <div><span class="info-label">Total Kelas</span><span class="info-value"><?= (int)$kelas_smp_counts; ?></span></div>
                     </div>
                  </div>
                  <div class="col-md-4">
               ...
               <div class="table-responsive">
                  <table class="table table-bordered table-striped table-hover">
                     ...
                     <tbody>
                     <?php if (!empty($kelas_smps)): $no = (int)$offset + 1; foreach($kelas_smps as $ks): ?>
                        <tr>
                           <td style="text-align:center"><?= $no++; ?></td>
                           <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $ks->id_kelas_smp; ?>"></td>
                           <td>
                              <?php if (!empty($ks->id_tingkatan)): ?>
                                 <a href="javascript:void(0);" class="popup-view" data-id="<?= $ks->id_tingkatan; ?>">
                                    <i class="fa fa-layer-group text-muted"></i> <?= htmlspecialchars($ks->tingkatan_smp_label); ?>
                                 </a>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php is_allowed('kelas_smp_view', function() use ($ks){?>
                              <a href="javascript:void(0);" class="btn btn-xs btn-info btn-detail-modal" data-id="<?= $ks->id_kelas_smp; ?>" title="Lihat detail"><i class="fa fa-eye"></i></a>
                              <?php }) ?>
                              <?php is_allowed('kelas_smp_update', function() use ($ks){?>
                              <a href="<?= site_url('administrator/kelas_smp/edit/' . $ks->id_kelas_smp); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                              <?php }) ?>
                              <?php is_allowed('kelas_smp_delete', function() use ($ks){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/kelas_smp/delete/' . $ks->id_kelas_smp); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                              <?php }) ?>
                           </td>
                        </tr>
                     <?php endforeach; else: ?>
               ...
               <div class="row" style="margin-top:12px">
                  <div class="col-md-6">
                     ...
                     <small class="text-muted" style="margin-left:15px">Menampilkan <?= count($kelas_smps); ?> dari <?= (int)$kelas_smp_counts; ?> data</small>
                  </div>
...
$(document).ready(function(){
   ...
   $('a.popup-view').click(function(e){
      e.preventDefault();
      var id = $(this).data('id');
      window.open('<?= base_url('administrator/tingkatan_smp/view/'); ?>' + id + '?popup=show', '_blank');
   });
