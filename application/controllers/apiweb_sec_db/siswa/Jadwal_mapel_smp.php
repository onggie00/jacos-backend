...
class Jadwal_mapel_smp extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if($id_siswa){
        $get_siswa=$this->second_db->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,sa.nama_lengkap as nama_lengkap from siswa_smp_aktif sa join siswa_smp s on s.id_siswa_smp = sa.id_siswa_smp join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran join kelas_smp k on k.id_kelas_smp = sa.id_kelas where id_siswa_smp_aktif = $id_siswa","row");
        if(!$get_siswa){
