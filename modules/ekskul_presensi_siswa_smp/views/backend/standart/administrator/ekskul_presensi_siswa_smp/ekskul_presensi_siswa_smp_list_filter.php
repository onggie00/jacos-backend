         ...
         <div class="labs-card">
            ...
            <div class="labs-card__header">
               <h3 class="labs-card__title">
                  ...
                  <span class="labs-badge labs-badge--default" style="margin-left:8px;"><?= $value_counts; ?> <?= cclang('items'); ?></span>
               </h3>
               <div>
                  ...
                  <a class="labs-btn labs-btn--success labs-btn--sm" id="btn_export" title="<?= cclang('export'); ?> <?= cclang('ekskul_presensi_siswa_smp') ?>" href="<?= site_url('administrator/ekskul_presensi_siswa_smp/export?').'id_ekskul='.$this->input->get('id_ekskul').'&bulan='.$this->input->get('bulan').'&tahun='.$this->input->get('tahun'); ?>"><i class="fa fa-file-excel-o"></i> <?= cclang('export'); ?> XLS</a>
                  <?php }) ?>
               </div>
            </div>
                        ...
                        <table class="labs-table">
                           ...
                           <?php foreach($data_presensi as $key => $value): ?>
                              <?php
                                 ...
                                 $status_val = strtolower(trim($value->status_absen));
                              ...
                              <tr>
                                 ...
                                 <td><?php if ($value->id_siswa_aktif) {
                                       echo anchor('administrator/siswa_smp_aktif/view/'.$value->id_siswa_aktif.'?popup=show', $value->nama_lengkap, ['class' => 'popup-view']); } ?>
                                 </td>
                                 <td><?php if ($value->id_ekskul) {
                                       echo anchor('administrator/ekskul/view/'.$value->id_ekskul.'?popup=show', $value->nama_ekskul, ['class' => 'popup-view']); } ?>
                                 </td>
                                 <td><?= _ent($value->hari_absen); ?></td>
                                 <td><?= _ent($value->tanggal_absen); ?></td>
                                 <td><?= _ent($value->waktu_absen); ?></td>
                                 <td><span class="labs-badge <?= $status_cls; ?>"><?= _ent($value->status_absen); ?></span></td>
                                 <td><?= _ent($value->keterangan_presensi); ?></td>
                                 <td>
                                    <?php is_allowed('ekskul_presensi_siswa_smp_view', function() use ($value){?>
                                    <a href="<?= site_url('administrator/ekskul_presensi_siswa_smp/view/' . $value->id); ?>" class="labs-btn labs-btn--icon labs-btn--xs labs-btn--info toltip" title="Detail"><i class="fa fa-eye"></i></a>
                                    <?php }) ?>
                                    <?php is_allowed('ekskul_presensi_siswa_smp_update', function() use ($value){?>
                                    <a href="<?= site_url('administrator/ekskul_presensi_siswa_smp/edit/' . $value->id); ?>" class="labs-btn labs-btn--icon labs-btn--xs labs-btn--primary toltip" title="Edit"><i class="fa fa-pencil"></i></a>
                                    <?php }) ?>
                                    <?php is_allowed('ekskul_presensi_siswa_smp_delete', function() use ($value){?>
                                    <a href="javascript:void(0);" data-href="<?= site_url('administrator/ekskul_presensi_siswa_smp/delete/' . $value->id); ?>" class="labs-btn labs-btn--icon labs-btn--xs labs-btn--danger remove-data toltip" title="Hapus"><i class="fa fa-trash"></i></a>
                                    <?php }) ?>
                                 </td>
                              </tr>
                           <?php endforeach; ?>
