...
<section class="content-header">
   <h1><i class="fa fa-users"></i> <?= cclang('siswa_kb_aktif') ?> <small><?= cclang('list_all'); ?></small></h1>
   <ol class="breadcrumb"><li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li><li class="active"><?= cclang('siswa_kb_aktif') ?></li></ol>
</section>
         ...
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               ...
               <div class="box-tools pull-right">
                  ...
                  <a class="btn btn-sm btn-action btn-action-add btn-top" title="<?= cclang('add_new_button', [cclang('siswa_kb_aktif')]); ?> (Ctrl+a)" href="<?= site_url('administrator/siswa_kb_aktif/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Siswa
                  </a>
                  <a class="btn btn-sm btn-action btn-action-add btn-top" title="Tambah Raport" href="<?= site_url('administrator/siswa_kb_aktif/add_raport'); ?>">
                  ...
                  <?php }) ?>
                  <?php is_allowed('siswa_kb_aktif_export', function(){?>
                  <a class="btn btn-sm btn-action btn-action-export btn-top" title="<?= cclang('export'); ?> XLS" href="<?= site_url('administrator/siswa_kb_aktif/export') . '?' . http_build_query($_GET); ?>">
                  ...
                  <a class="btn btn-sm btn-action btn-action-pdf btn-top" title="<?= cclang('export'); ?> PDF" href="<?= site_url('administrator/siswa_kb_aktif/export_pdf'); ?>">
            ...
            <div class="box-body">
               ...
               <form name="form_siswa_kb_aktif" id="form_siswa_kb_aktif" action="<?= base_url('administrator/siswa_kb_aktif/index'); ?>">
               <div class="filter-builder" style="margin-bottom:15px;padding:12px;background:#f8f9fa;border-radius:5px;border:1px solid #e0e0e0">
                  ...
                  <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                     ...
                     <a class="btn btn-flat btn-default" href="<?= base_url('administrator/siswa_kb_aktif'); ?>"><i class="fa fa-times"></i> Reset Semua</a>
                     <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
                        <select class="form-control input-sm" name="s" id="sort" style="width:130px">
                           ...
                           <option <?= $this->input->get('s') == 'id_siswa_kb_aktif' ? 'selected' : ''; ?> value="id_siswa_kb_aktif">ID</option>
                        </select>
               ...
               <div class="table-responsive"> 
                  <table class="table table-bordered table-striped table-hover dataTable">
                     ...
                     <tbody id="tbody_siswa_kb_aktif">
                     <?php if($siswa_kb_aktif_counts > 0): ?>
                     <?php foreach($siswa_kb_aktifs as $siswa_kb_aktif): ?>
                        <tr>
                           <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $siswa_kb_aktif->id_siswa_kb_aktif; ?>"></td>
                           <td>
                              <i class="fa fa-user"></i> <?= _ent($siswa_kb_aktif->nama_lengkap); ?>
                           </td>
                           <td style="text-align:center"><?= _ent($siswa_kb_aktif->nis); ?></td>
                           <td style="text-align:center">
                              <?php if ($siswa_kb_aktif->id_tahun_ajaran): ?>
                                 <a href="<?= site_url('administrator/tahun_ajaran/view/'.$siswa_kb_aktif->id_tahun_ajaran.'?popup=show'); ?>" class="popup-view" title="Lihat tahun ajaran">
                                    <?= $siswa_kb_aktif->tahun_ajaran_label; ?>
                                 </a>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php if ($siswa_kb_aktif->id_kelas): ?>
                                 <a href="<?= site_url('administrator/kelas_kb/view/'.$siswa_kb_aktif->id_kelas.'?popup=show'); ?>" class="popup-view" title="Lihat kelas">
                                    <?= $siswa_kb_aktif->kelas_kb_label; ?>
                                 </a>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php 
                              $spp_type = $siswa_kb_aktif->spp_type ?: 'FULL';
                              if ($spp_type == 'FULL'): ?>
                           ...
                           <td style="text-align:right">
                              <?php if (!empty($siswa_kb_aktif->spp_custom) && $siswa_kb_aktif->spp_custom > 0): ?>
                                 Rp <?= number_format($siswa_kb_aktif->spp_custom, 0, ',', '.'); ?>
                              <?php else: ?>
                                 <small class="text-muted">-</small>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php if ($siswa_kb_aktif->acc_ujian == 1): ?>
                           ...
                           <td style="text-align:center">
                              <?php if ($siswa_kb_aktif->is_active == 1): ?>
                           ...
                           <td style="text-align:center;font-size:11px">
                              <?php if (!empty($siswa_kb_aktif->deleted_at_siswa)): ?>
                                 <span class="label label-nonaktif"><i class="fa fa-user-times"></i> Siswa Terhapus</span>
                                 <br><small class="text-danger"><i class="fa fa-calendar"></i> <?= date('d M Y', strtotime($siswa_kb_aktif->deleted_at_siswa)); ?></small>
                                 <?php is_allowed('siswa_kb_aktif_update', function() use ($siswa_kb_aktif){ ?>
                                 <br><a href="<?= site_url('administrator/siswa_kb_aktif/restore/'.$siswa_kb_aktif->id_siswa_kb_aktif); ?>" class="btn btn-xs btn-success restore-link" style="margin-top:3px" title="Pulihkan akun siswa"><i class="fa fa-undo"></i> Pulihkan</a>
                                 <?php }) ?>
                              <?php else: ?>
                                 <span class="label label-aktif"><i class="fa fa-user"></i> Siswa Aktif</span>
                              <?php endif; ?>
                              <hr style="margin:5px 0">
                              <?php if (!empty($siswa_kb_aktif->deleted_at_ortu)): ?>
                                 <span class="label label-nonaktif"><i class="fa fa-user-times"></i> Ortu Terhapus</span>
                                 <br><small class="text-danger"><i class="fa fa-calendar"></i> <?= date('d M Y', strtotime($siswa_kb_aktif->deleted_at_ortu)); ?></small>
                                 <?php is_allowed('siswa_kb_aktif_update', function() use ($siswa_kb_aktif){ ?>
                                 <br><a href="<?= site_url('administrator/siswa_kb_aktif/restore_ortu/'.$siswa_kb_aktif->id_siswa_kb_aktif); ?>" class="btn btn-xs btn-success restore-link" style="margin-top:3px" title="Pulihkan akun ortu"><i class="fa fa-undo"></i> Pulihkan</a>
                                 <?php }) ?>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($siswa_kb_aktif->file_raport)): ?>
                                 <?php if (is_image($siswa_kb_aktif->file_raport)): ?>
                                    <a class="fancybox" rel="group" href="<?= BASE_URL . 'uploads/siswa_kb_aktif/' . $siswa_kb_aktif->file_raport; ?>">
                                       <img src="<?= BASE_URL . 'uploads/siswa_kb_aktif/' . $siswa_kb_aktif->file_raport; ?>" alt="raport" width="40px" style="border-radius:3px">
                                    </a>
                                 <?php else: ?>
                                    <a href="<?= BASE_URL . 'uploads/siswa_kb_aktif/' . $siswa_kb_aktif->file_raport; ?>" title="Download raport">
                                       <img src="<?= get_icon_file($siswa_kb_aktif->file_raport); ?>" alt="raport" width="30px">
                                    </a>
                                 <?php endif; ?>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <div style="margin-bottom:3px">
                                 <?php is_allowed('siswa_kb_aktif_view', function() use ($siswa_kb_aktif){?>
                                 <button type="button" class="btn btn-sm btn-info btn-aksi btn_detail_siswa" data-id="<?= $siswa_kb_aktif->id_siswa_kb_aktif; ?>" title="Lihat detail">
                                    <i class="fa fa-eye"></i> Detail
                                 </button>
                                 <?php }) ?>
                                 <?php is_allowed('siswa_kb_aktif_update', function() use ($siswa_kb_aktif){?>
                                 <a href="<?= site_url('administrator/siswa_kb_aktif/edit/'.$siswa_kb_aktif->id_siswa_kb_aktif); ?>" class="btn btn-sm btn-warning btn-aksi" title="Edit data">
                                    <i class="fa fa-edit"></i> Edit
                                 </a>
                                 <?php }) ?>
                              </div>
                              <div style="margin-bottom:3px">
                                 <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-aksi btn_ubah_password" title="Ubah Password Siswa" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $siswa_kb_aktif->id_siswa_kb_aktif; ?>" data-email="<?= $siswa_kb_aktif->email_ms_office; ?>">
                                    <i class="fa fa-key"></i> Ubah Pass Siswa
                                 </a>
                                 <a href="javascript:void(0)" class="btn btn-sm btn-success btn-aksi btn_ubah_password" title="Ubah Password Ortu" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $siswa_kb_aktif->id_siswa_kb_aktif; ?>" data-email="<?= $siswa_kb_aktif->email_ms_office_ortu; ?>">
                                    <i class="fa fa-key"></i> Ubah Pass Ortu
                                 </a>
                              </div>
                              <?php is_allowed('siswa_kb_aktif_delete', function() use ($siswa_kb_aktif){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/siswa_kb_aktif/delete/'.$siswa_kb_aktif->id_siswa_kb_aktif); ?>" class="btn btn-sm btn-danger btn-aksi-full remove-data" title="Hapus data">
...
<div class="modal fade" id="modal_generate" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_kb_aktif/generate_nis/'); ?>" method="post">
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_kb_aktif/import_siswa/'); ?>" method="post" enctype="multipart/form-data">
         <div class="modal-body">
            <div class="form-group">
...
<div class="modal fade" id="modal_import_raport" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_kb_aktif/import_raport_siswa/'); ?>" method="post" enctype="multipart/form-data">
         <div class="modal-body">
            <div class="form-group">
...
<div class="modal fade" id="modal_update_password" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_kb_aktif/update_password/'); ?>" method="post">
         <div class="modal-body">
            <div class="form-group">
...
<div class="modal fade labs-modal" id="modal_export_lengkap" tabindex="-1" role="dialog">
   <div class="modal-dialog modal-lg"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_kb_aktif/export_lengkap'); ?>" method="post">
         <div class="modal-body">
            <div class="row">
               <div class="col-sm-6">
                  <div class="form-group">
...
<script>
// ===== EXPORT DATA LENGKAP =====
var exportKelasData = <?= json_encode(db_get_all_data('kelas_kb')); ?>;
...
$('#export_siswa').select2({
   ...
   allowClear: true,
   ajax: {
      url: '<?= base_url('administrator/siswa_kb_aktif/get_siswa_options'); ?>',
      dataType: 'json',
      delay: 250,
...
$('#export_tingkatan').on('change', function () {
   ...
   if (!idTingkatan) {
      ...
      for (var j = 0; j < kelasList.length; j++) {
         $kelas.append($('<option>', { value: kelasList[j].id_kelas_kb, text: kelasList[j].label }));
      }
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_siswa_kb_aktif').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
            ...
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/siswa_kb_aktif/delete?' + serialize_bulk;      
            }
         });
         return false;
      } else if(bulk.val() == '') {
   ...
   $('.btn_ubah_password').click(function(){
      ...
      $.ajax({
         ...
         success: function(res) {
            ...
            $('#reset_password').attr('href', `<?php echo base_url('administrator/siswa_kb_aktif/reset_password'); ?>?user_microsoft_id=${res.id}`);
         },
         error: function(err) {
            console.error(err);
         }
   ...
   $('.btn_detail_siswa').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/siswa_kb_aktif/get_detail/' . ''); ?>' + id,
         method: 'GET',
         dataType: 'json',
         success: function(data) {
            ...
            var fotoProfil = data.foto_profil ? '<img src="<?= BASE_URL; ?>uploads/siswa_kb/' + data.foto_profil + '" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #3498db">' : '<div style="width:80px;height:80px;border-radius:50%;background:#e8f4fc;display:flex;align-items:center;justify-content:center"><i class="fa fa-user" style="font-size:35px;color:#3498db"></i></div>';
            ...
            html += '<tr><th style="background:#e8f4fc;width:200px">Kelas</th><td>' + (data.kelas_kb_label || '-') + '</td></tr>';
