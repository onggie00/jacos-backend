...
class Spp_tk extends Admin
{
	...
	{
		$this->is_allowed('spp_tk_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['spp_tks'] = $this->model_spp_tk->get($filter, $field, $this->limit_page, $offset);
		$this->data['spp_tk_counts'] = $this->model_spp_tk->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/spp_tk/index/',
			'total_rows'   => $this->model_spp_tk->count_all($filter, $field),
		...
		$this->template->title('Spp TK List');
		$this->render('backend/standart/administrator/spp_tk/spp_tk_list', $this->data);
	}
	...
	public function add_save()
	{
		...
		if (!$this->is_allowed('spp_tk_list', false)) {
			echo json_encode([
				'success' => false,
		...
		$id_siswa = $this->input->post('id_siswa_tk_aktif');
		if (empty($id_siswa)) {
			//get data siswa aktif per kelas tersebut
			$list_siswa = $this->mymodel->getbywhere("siswa_tk_aktif", "id_kelas", $this->input->post("id_kelas"), "result");
			$id_siswa = array();
			foreach ($list_siswa as $key => $value) {
				array_push($id_siswa, $value->id_siswa_tk_aktif);
			}
		...
		foreach ($id_siswa as $key_siswa => $value_siswa) {
			...
			$get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.* from siswa_tk_aktif sa join siswa_tk s on s.id_siswa_tk = sa.id_siswa_tk join kelas_tk k on k.id_kelas_tk = sa.id_kelas where sa.id_siswa_tk_aktif = " . (int)$value_siswa, 'row');
			if (!$get_siswa) {
			...
			if (!empty($custom_nominal_spp) && $custom_nominal_spp > 0) {
				...
				$this->mymodel->update('siswa_tk_aktif', ['spp_custom' => $biaya], 'id_siswa_tk_aktif', $value_siswa);
				$spp_custom = $biaya;
			...
			$cek_detail = $this->mymodel->getbywhere("spp_tk", array("id_siswa_aktif" => $value_siswa, "id_tahun_ajaran" => $get_tahun_ajaran_aktif->id_tahun_ajaran), "row");
			...
			$is_update = false;
			// Bulan yang di-skip untuk siswa ini
			...
			if (!$cek_detail) {
				$detail_spp = array(
					...
					"id_kelas" => $get_siswa->id_kelas_tk,
				...
				// Update nominal, nama, kelas jika berubah
				$update_spp = array();
				...
				if (empty($cek_detail[0]->id_kelas) || $cek_detail[0]->id_kelas != $get_siswa->id_kelas_tk) $update_spp['id_kelas'] = $get_siswa->id_kelas_tk;
				if (!empty($update_spp)) {
					$this->mymodel->update("spp_tk", $update_spp, "id", $id_detail_spp);
					$is_update = true;
				}
			}

			// Update kolom bulan di spp_tk
			foreach ($arrMonth as $in) {
				...
				// Skip jika sudah ada isi (bukan null dan bukan '-')
				if ($current_value !== null && $current_value !== '' && $current_value !== '-') {
					continue;
				}
				
				// Jika bulan dipilih (centang), set null; jika tidak, set '-'
				if ($bln && in_array((int)$bulan[1], $bln)) {
					$this->mymodel->update("spp_tk", array($kolom_bulan => null), "id", $id_detail_spp);
				} else if (!$bln) {
					// Generate semua bulan, set null
					$this->mymodel->update("spp_tk", array($kolom_bulan => null), "id", $id_detail_spp);
				} else {
					// Bulan tidak dipilih, set '-'
					$this->mymodel->update("spp_tk", array($kolom_bulan => '-'), "id", $id_detail_spp);
				}
			}
			...
			foreach ($arrMonth as $i) {
				...
				// Skip jika bulan tidak dipilih
				if ($bln && !in_array((int)$bulan[1], $bln)) {
					continue;
				}

				// Cek transaksi berdasarkan id_siswa_aktif + id_tahun_ajaran + bulan + jenjang
				// Filter no_transaksi LIKE '%tk%' untuk hindari konflik dengan SMP/SMA
				...
				if (!$cek_transaksi_bln) {
					...
					// UPDATE user_name, total_biaya jika berubah
	...
	{
		$this->is_allowed('spp_tk_update');

		$this->data['spp_tk'] = $this->model_spp_tk->find($id);

		$this->template->title('Spp sd Update');
		$this->render('backend/standart/administrator/spp_tk/spp_tk_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('spp_tk_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_spp_tk = $this->model_spp_tk->change($id, $save_data);
			...
			if ($save_spp_tk) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/spp_tk', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/spp_tk');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	public function delete($id = null)
	{
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'spp_tk'), 'success');
		} else {
			set_message(cclang('error_delete', 'spp_tk'), 'error');
		}
	...
	{
		$this->is_allowed('spp_tk_view');

		$this->data['spp_tk'] = $this->model_spp_tk->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Spp TK Detail');
		$this->render('backend/standart/administrator/spp_tk/spp_tk_view', $this->data);
	}
	...
	{
		$spp_tk = $this->model_spp_tk->find($id);
		...
		return $this->model_spp_tk->remove($id);
	}
	...
	{
		$this->is_allowed('spp_tk_export');

		//$this->model_spp_tk->export('spp_tk', 'spp_tk');
		$field = $this->input->get("f");
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_tk spp join siswa_tk_aktif s on spp.id_siswa_aktif = s.id_siswa_tk_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
			...
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_tk spp join siswa_tk_aktif s on spp.id_siswa_aktif = s.id_siswa_tk_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran where ".$where." order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
		else{
		...
		header('Content-Disposition: attachment;filename="Rekap SPP sd_'.date("Y-m-d Hi").'.xls"'); 
	...
	public function export_tunggakan()
	{
		...
		// =====================
		$queries = [
			'TK'  => "SELECT spp.*, s.nama_lengkap, s.nis, 'TK' as nama_unit
					FROM spp_tk spp
					JOIN siswa_tk_aktif s ON spp.id_siswa_aktif = s.id_siswa_tk_aktif
		...
		// $sheet->setCellValue(($col_total_nominal-1) . $row, "Total Nominal");
	...
	public function export_pdf()
	{
		...
		$this->model_spp_tk->pdf('spp_tk', 'spp_tk');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_spp_tk->find($id);
	...
	public function sync_spp_data(){
		...
		foreach ($get_spp as $key => $value) {
			...
			foreach ($get_transaksi as $key_transaksi => $value_transaksi) {
				if (!empty($value_transaksi->updated_at)) {
					...
					$this->mymodel->update("spp_tk",$data_update,"id",$value->id_spp);
					//echo $bulan." - ".$value_transaksi->id_spp." - ".$value->id_siswa_aktif."<br/>";
				}
			}

			/*$get_transaksi2 = $this->mymodel->withquery("select id_spp, id_siswa_aktif, no_transaksi, id_tahun_ajaran, updated_at, bulan, status_transaksi from transaksi_spp where id_spp = '".$value->id_spp."' and id_siswa_aktif = '".$value->id_siswa_aktif."' and id_tahun_ajaran = '".$value->id_tahun_ajaran."' and status_transaksi = '0' and no_transaksi like '%"."SD"."%' ","result");
			//cek status transaksi dan spp sd
			foreach ($get_transaksi2 as $key_transaksi => $value_transaksi) {
				if (empty($value_transaksi->updated_at)) {
					$bulan = strtolower($value_transaksi->bulan);
					$data_update = array(
						$bulan => null,
		...
		redirect('administrator/spp_tk');
	}
}


/* End of file spp_tk.php */
/* Location: ./application/controllers/administrator/Spp TK.php */
