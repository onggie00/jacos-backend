...
/**
*| --------------------------------------------------------------------------
*| Ekskul Member Sd Controller
*| --------------------------------------------------------------------------
*| Ekskul Member Sd site
*|
*/
class Ekskul_member_sd extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_ekskul_member_sd');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Ekskul Member Sds
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('ekskul_member_sd_list');
		$this->limit_page = 20;
		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['data_list'] = $this->model_ekskul_member_sd->get($filter, $field, $this->limit_page, $offset);
		$this->data['value_counts'] = $this->model_ekskul_member_sd->count_all($filter, $field);

		// Stats
		$this->data['stats'] = new stdClass();
		$this->data['stats']->total = $this->model_ekskul_member_sd->count_all();
		$this->data['stats']->aktif = $this->db->where('status_member', 1)->where('jenjang', 'sd')->count_all_results('ekskul_member_sd');
		$this->data['stats']->tidak_aktif = $this->db->where('status_member', 0)->where('jenjang', 'sd')->count_all_results('ekskul_member_sd');
		$this->data['stats']->ekskul_count = $this->db->where('jenjang', 'sd')->count_all_results('ekskul');
		$this->data['stats']->sudah_bayar = $this->db->where('jenjang', 'sd')->where('file_pembayaran !=', '')->count_all_results('ekskul_member_sd');

		$config = array(
			'base_url'	     => 'administrator/ekskul_member_sd/index/',
		...
		$this->template->title('Anggota Ekskul SD');
		$this->render('backend/standart/administrator/ekskul_member_sd/ekskul_member_sd_list', $this->data);
	}
	...
	public function filter($offset = 0)
	{
		...
		if (!empty($get['id_ekskul']) && $get['id_ekskul'][0] !== '') {
			...
			$get_all_ekskul = $this->mymodel->withquery("select id_ekskul, nama from ekskul where jenjang = 'sd' order by nama ASC","result");
		...
		$get_data = $this->mymodel->withquery("select m.*, s.nama_lengkap, s.nis, k.id_kelas_sd as id_kelas, k.label as nama_kelas, e.nama as nama_ekskul, 
			case when m.status_member = '0' then 'Tidak Aktif' 
			when m.status_member = '1' then 'Aktif' end as status_member_text
			from ekskul_member_sd m join ekskul e on m.id_ekskul = e.id_ekskul 
			join siswa_sd_aktif s on m.id_siswa = s.id_siswa_sd_aktif 
			join kelas_sd k on s.id_kelas = k.id_kelas_sd ".$where." order by s.nama_lengkap ASC limit ".$offset.",".$limit." ","result");
		//echo $this->db->last_query();
		$get_data_count = $this->mymodel->withquery("select m.id_member from ekskul_member_sd m join ekskul e on m.id_ekskul = e.id_ekskul join siswa_sd_aktif s on m.id_siswa = s.id_siswa_sd_aktif join kelas_sd k on s.id_kelas = k.id_kelas_sd ".$where." order by s.nama_lengkap ASC ","result");
		$total_rows = count($get_data_count);

		$config = array(
			'base_url'     => 'administrator/ekskul_member_sd/filter/',
	...
	/**
	* Add new ekskul_member_sds
	*
	*/
	public function add()
	{
		$this->is_allowed('ekskul_member_sd_add');

		$this->template->title('Anggota Ekskul SD New');
		$this->render('backend/standart/administrator/ekskul_member_sd/ekskul_member_sd_add', $this->data);
	}

	/**
	* Add New Ekskul Member Sds
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('ekskul_member_sd_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			$ekskul_member_sd_file_pembayaran_uuid = $this->input->post('ekskul_member_sd_file_pembayaran_uuid');
			$ekskul_member_sd_file_pembayaran_name = $this->input->post('ekskul_member_sd_file_pembayaran_name');
			...
			if (!is_dir(FCPATH . '/uploads/ekskul_member_sd/')) {
				mkdir(FCPATH . '/uploads/ekskul_member_sd/');
			}

			if (!empty($ekskul_member_sd_file_pembayaran_name)) {
				$ekskul_member_sd_file_pembayaran_name_copy = date('YmdHis') . '-' . $ekskul_member_sd_file_pembayaran_name;

				rename(FCPATH . 'uploads/tmp/' . $ekskul_member_sd_file_pembayaran_uuid . '/' . $ekskul_member_sd_file_pembayaran_name, 
						FCPATH . 'uploads/ekskul_member_sd/' . $ekskul_member_sd_file_pembayaran_name_copy);

				if (!is_file(FCPATH . '/uploads/ekskul_member_sd/' . $ekskul_member_sd_file_pembayaran_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['file_pembayaran'] = $ekskul_member_sd_file_pembayaran_name_copy;
			}
		
			
			$save_ekskul_member_sd = $this->model_ekskul_member_sd->store($save_data);
            

			if ($save_ekskul_member_sd) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_ekskul_member_sd;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/ekskul_member_sd/edit/' . $save_ekskul_member_sd, 'Edit Ekskul Member Sd'),
						anchor('administrator/ekskul_member_sd', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/ekskul_member_sd');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Ekskul Member Sds
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('ekskul_member_sd_update');

		$this->data['ekskul_member_sd'] = $this->model_ekskul_member_sd->find($id);

		$this->template->title('Anggota Ekskul SD Update');
		$this->render('backend/standart/administrator/ekskul_member_sd/ekskul_member_sd_update', $this->data);
	}

	/**
	* Update Ekskul Member Sds
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('ekskul_member_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			if (!empty($ekskul_member_sd_file_pembayaran_uuid)) {
				$ekskul_member_sd_file_pembayaran_name_copy = date('YmdHis') . '-' . $ekskul_member_sd_file_pembayaran_name;
			...
			$save_ekskul_member_sd = $this->model_ekskul_member_sd->change($id, $save_data);
	...
	/**
	* delete Ekskul Member Sds
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('ekskul_member_sd_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'ekskul_member_sd'), 'success');
        } else {
            set_message(cclang('error_delete', 'ekskul_member_sd'), 'error');
        }
	...
	public function activate_member($id = null)
	{
		...
		$activate = false;
		if (!empty($id)) {
			$activate = $this->mymodel->update("ekskul_member_sd", array("status_member" => 1), "id_member", $id);
		...
		if ($activate) {
            set_message("Aktivasi anggota ekstrakurikuler SD berhasil", 'success');
        } else {
            set_message("Aktivasi anggota ekstrakurikuler SD gagal", 'error');
        }
	...
	public function deactivate_member($id = null)
	{
		...
		if (!empty($id)) {
			$deactivate = $this->mymodel->update("ekskul_member_sd", array("status_member" => 0), "id_member", $id);
		} elseif (is_array($arr_id) && count($arr_id) > 0) {
			foreach ($arr_id as $id) {
				$deactivate = $this->mymodel->update("ekskul_member_sd", array("status_member" => 0), "id_member", $id);
			}
		...
		if ($deactivate) {
            set_message("Deaktivasi anggota ekstrakurikuler SD berhasil", 'success');
        } else {
            set_message("Deaktivasi anggota ekstrakurikuler SD gagal", 'error');
        }
		...
		/**
	* View view Ekskul Member Sds
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('ekskul_member_sd_view');

		$this->data['ekskul_member_sd'] = $this->model_ekskul_member_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Anggota Ekskul SD Detail');
		$this->render('backend/standart/administrator/ekskul_member_sd/ekskul_member_sd_view', $this->data);
	}
	...
	*/
	...
	{
		$ekskul_member_sd = $this->model_ekskul_member_sd->find($id);

		if (!empty($ekskul_member_sd->file_pembayaran)) {
			$path = FCPATH . '/uploads/ekskul_member_sd/' . $ekskul_member_sd->file_pembayaran;
		...
		return $this->model_ekskul_member_sd->remove($id);
	}
	
	/**
	* Upload Image Ekskul Member Sd	* 
	* @return JSON
	*/
	public function upload_file_pembayaran_file()
	{
		...
		echo $this->upload_file([
			'uuid' 		 	=> $uuid,
			'table_name' 	=> 'ekskul_member_sd',
		]);
	}

	/**
	* Delete Image Ekskul Member Sd	* 
	* @return JSON
	*/
	public function delete_file_pembayaran_file($uuid)
	{
		if (!$this->is_allowed('ekskul_member_sd_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'upload_path'       => 'uploads/ekskul_member_sd/'
        ]);
	}

	/**
	* Get Image Ekskul Member Sd	* 
	* @return JSON
	*/
	public function get_file_pembayaran_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_member',
            'upload_path'       => 'uploads/ekskul_member_sd/',
            'delete_endpoint'   => 'administrator/ekskul_member_sd/delete_file_pembayaran_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('ekskul_member_sd_export');
		...
		if (isset($get['status_member']) && $get['status_member'] !== '') {
			$student_status_filter = " AND EXISTS (SELECT 1 FROM ekskul_member_sd em WHERE em.id_siswa = s.id_siswa_sd_aktif AND em.status_member = '".$get['status_member']."')";
		}
		if (isset($get['file_pembayaran']) && $get['file_pembayaran'] !== '') {
			$student_status_filter .= $get['file_pembayaran'] === 'sudah_upload'
				? " AND EXISTS (SELECT 1 FROM ekskul_member_sd em WHERE em.id_siswa = s.id_siswa_sd_aktif AND em.file_pembayaran IS NOT NULL AND em.file_pembayaran != '')"
				: " AND EXISTS (SELECT 1 FROM ekskul_member_sd em WHERE em.id_siswa = s.id_siswa_sd_aktif AND (em.file_pembayaran IS NULL OR em.file_pembayaran = ''))";
		}
		if (!empty($get['id_kelas'])) {
			$get_siswa = $this->mymodel->withquery("
				SELECT s.id_siswa_sd_aktif AS id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label AS nama_kelas
				FROM siswa_sd_aktif s JOIN kelas_sd k ON s.id_kelas = k.id_kelas_sd
			...
			$get_siswa = $this->mymodel->withquery("
				SELECT s.id_siswa_sd_aktif AS id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label AS nama_kelas
				FROM ekskul_member_sd m
				JOIN ekskul e ON m.id_ekskul = e.id_ekskul AND e.jenjang = 'sd'
				JOIN siswa_sd_aktif s ON m.id_siswa = s.id_siswa_sd_aktif
				JOIN kelas_sd k ON s.id_kelas = k.id_kelas_sd
				$where
				GROUP BY m.id_siswa
				ORDER BY k.label ASC, s.nama_lengkap ASC
			", "result");
		...
		header('Content-Disposition: attachment;filename="Data Pilihan Ekstrakurikuler Siswa SD - '.date("Y-m-d").'.xls"'); 
	...
	public function export_list_anggota()
	{
		...
		if (!empty($get['id_ekskul'])) {
			...
			$conditions[] = "m.id_ekskul IN (SELECT id_ekskul FROM ekskul WHERE jenjang = 'sd')";
		}
		...
		$all_data = $this->mymodel->withquery("
			SELECT 
				...
				s.id_siswa_sd_aktif, s.nis, s.nama_lengkap, 
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Daftar Anggota Ekstrakurikuler Siswa SD - ' . date("Y-m-d") . '.xls"');
		header('Cache-Control: max-age=0');
	...
	public function export_rekap_pembayaran()
	{
		...
		$all_data = $this->mymodel->withquery("
			...
			FROM ekskul e
			JOIN ekskul_member_sd m ON e.id_ekskul = m.id_ekskul 
			JOIN siswa_sd_aktif s ON m.id_siswa = s.id_siswa_sd_aktif
			{$where}
		...
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Rekap Pembayaran Ekstrakurikuler SD - ' . date("Y-m-d") . '.xls"');
		header('Cache-Control: max-age=0');
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$objWriter->save('php://output');
	}
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_ekskul_member_sd->pdf('ekskul_member_sd', 'ekskul_member_sd');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_ekskul_member_sd->find($id);
	...
	public function convert_heic()
	{
		...
		$file = basename($file);
		$original_path = FCPATH . 'uploads/ekskul_member_sd/' . $file;
		...
		if ($jpeg_path && is_file($jpeg_path)) {
			...
			echo json_encode(array(
				'success' => true,
				'url' => base_url('uploads/ekskul_member_sd/' . $jpeg_name)
			));
		} else {
...
/* End of file ekskul_member_sd.php */
/* Location: ./application/controllers/administrator/Ekskul Member Sd.php */