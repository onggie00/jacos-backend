...
<section class="content-header">
   <h1><i class="fa fa-calendar-check-o"></i> Presensi SD <small>Daftar Presensi Harian Siswa</small></h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">Presensi SD</li>
   </ol>
</section>
         ...
         <div class="box box-warning">
            <div class="box-header with-border">
               ...
               <div class="box-tools pull-right">
                  <?php is_allowed('presensi_sd_add', function(){ ?>
                  <a class="btn btn-sm btn-success" title="<?= cclang('add_new_button', [cclang('presensi_sd')]); ?> (Ctrl+a)" href="<?= site_url('administrator/presensi_sd/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Presensi
                  </a>
                  <?php }) ?>
                  <?php is_allowed('presensi_sd_export', function() use ($filter){
                     $qfilter = isset($filter) ? http_build_query($filter) : '';
                  ?>
                  <a class="btn btn-sm btn-info" title="Export Harian" href="<?= site_url('administrator/presensi_sd/export_harian?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Harian</a>
                  <a class="btn btn-sm btn-info" title="Export Bulanan" href="<?= site_url('administrator/presensi_sd/export_bulanan?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Bulanan</a>
                  <a class="btn btn-sm btn-info" title="Export Periode" href="<?= site_url('administrator/presensi_sd/export_periode?'.$qfilter); ?>"><i class="fa fa-file-excel-o"></i> Periode</a>
                  <?php }) ?>
               </div>
            </div>
            ...
            <div class="box-body">
                           ...
                           <div class="form-group">
                              ...
                              <select name="id_tingkatan" id="select_tingkatan_terlambat" class="form-control input-sm">
                                 ...
                                 <?php foreach($list_tingkatan as $t): ?>
                                    <option value="<?= $t->id_tingkatan_sd; ?>"><?= htmlspecialchars($t->label); ?></option>
                                 <?php endforeach; ?>
                              </select>
                           </div>
                           <div class="form-group">
                              ...
                              <select name="id_kelas" id="select_kelas_terlambat" class="form-control input-sm">
                                 ...
                                 <?php foreach($list_kelas as $k): ?>
                                    <option value="<?= $k->id_kelas_sd; ?>"><?= htmlspecialchars($k->label); ?></option>
                                 <?php endforeach; ?>
                              </select>
                           </div>
               ...
               <form method="get" action="<?= base_url('administrator/presensi_sd/index'); ?>" class="form-inline filter-bar">
                  <div class="form-group">
                     <label>Cari:</label>
                  ...
                  <div class="form-group">
                     ...
                     <select name="id_tingkatan" class="form-control input-sm">
                        <option value="">- Semua -</option>
                        <?php foreach($list_tingkatan as $t): ?>
                        <option value="<?= $t->id_tingkatan_sd; ?>" <?= (isset($filter['id_tingkatan']) && $filter['id_tingkatan']==$t->id_tingkatan_sd) ? 'selected' : ''; ?>><?= htmlspecialchars($t->label); ?></option>
                        <?php endforeach; ?>
                     </select>
                  </div>
                  <div class="form-group">
                     ...
                     <select name="id_kelas" class="form-control input-sm">
                        <option value="">- Semua -</option>
                        <?php foreach($list_kelas as $k): ?>
                        <option value="<?= $k->id_kelas_sd; ?>" <?= (isset($filter['id_kelas']) && $filter['id_kelas']==$k->id_kelas_sd) ? 'selected' : ''; ?>><?= htmlspecialchars($k->label); ?></option>
                        <?php endforeach; ?>
                     </select>
                  </div>
                  <div class="form-group">
                  ...
                  <a href="<?= base_url('administrator/presensi_sd'); ?>" class="btn btn-default btn-sm"><i class="fa fa-times"></i> Reset</a>
               </form>
               ...
               if (!empty($filter['id_tingkatan'])) {
                  $t_label = '';
                  foreach($list_tingkatan as $t) { if ($t->id_tingkatan_sd==$filter['id_tingkatan']) { $t_label = $t->label; break; } }
                  if ($t_label) $chips[] = 'Tingkat: '.$t_label;
               }
               if (!empty($filter['id_kelas'])) {
                  $k_label = '';
                  foreach($list_kelas as $k) { if ($k->id_kelas_sd==$filter['id_kelas']) { $k_label = $k->label; break; } }
                        ...
                        <tr>
                           ...
                           <td>
                              ...
                              <?php if (!empty($p->id_siswa_aktif)): ?>
                                 <?= anchor('administrator/siswa_sd_aktif/view/'.$p->id_siswa_aktif.'?popup=show', $p->nama_lengkap, ['class' => 'popup-view']); ?>
                              <?php endif; ?>
                           </td>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($p->id_izin)): ?>
                                 <?= anchor('administrator/izin_siswa_sd/view/'.$p->id_izin.'?popup=show', $p->jenis_izin, ['class' => 'popup-view']); ?>
                              <?php else: ?>
                                 <span class="text-muted">-</span>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php is_allowed('presensi_sd_update', function() use ($p){?>
                              <a href="<?= site_url('administrator/presensi_sd/edit/' . $p->id); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                              <?php }) ?>
                              <?php is_allowed('presensi_sd_delete', function() use ($p){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/presensi_sd/delete/' . $p->id); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                              <?php }) ?>
                           </td>
                        </tr>
                     <?php endforeach; else: ?>
...
$(document).ready(function(){
   ...
   $('.btn-catatan').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_sd/detail_catatan'); ?>',
         type: 'POST',
         data: {
   ...
   $('#btn_load_terlambat').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_sd/get_siswa_terlambat'); ?>',
         type: 'POST',
         data: {
   ...
   $(document).on('click', '.btn-riwayat-terlambat', function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/presensi_sd/get_riwayat_terlambat'); ?>',
         type: 'POST',
         data: {
            id_siswa_aktif: id_siswa,
            id_tahun_ajaran: ta,
