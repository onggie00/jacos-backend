...
class Jadwal_mapel_ft extends REST_Controller
{
  ...
  public function index_post()
  {
    ...
    if ($id_siswa) {
      $get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,sa.nama_lengkap as nama_lengkap from siswa_ft_aktif sa join siswa_ft s on s.id_siswa_ft = sa.id_siswa_ft join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran join kelas_ft k on k.id_kelas_ft = sa.id_kelas where id_siswa_ft_aktif = $id_siswa", "row");
      if (!$get_siswa) {
    ...
    $data = $this->mymodel->withquery(
      "
        SELECT j.*,t.*,k.*,m.*,j.id_tahun_ajaran as id_tahun_ajaran 
