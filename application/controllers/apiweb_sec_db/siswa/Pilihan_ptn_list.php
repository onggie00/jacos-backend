    ...
    public function index_post()
    {
      ...
      foreach (getallheaders() as $name => $value) {
        ...
        if (empty($tahun_ajaran)) {
          $data_siswa = $this->second_db->withquery("select pt.nilai_utbk1, pt.nilai_utbk2, pt.nilai_utbk3, pt.nilai_utbk4, pt.nilai_nasional, pt.tahun_ajaran from pt_to_siswa pt where pt.id_siswa_aktif = '".$id_siswa."' and pt.jenjang='".$jenjang."' order by id DESC","row");
        }
        else{
          $data_siswa = $this->second_db->withquery("select pt.nilai_utbk1, pt.nilai_utbk2, pt.nilai_utbk3, pt.nilai_utbk4, pt.nilai_nasional, pt.tahun_ajaran from pt_to_siswa pt where pt.id_siswa_aktif = '".$id_siswa."' and pt.jenjang='".$jenjang."' and pt.tahun_ajaran = '".$tahun_ajaran."' order by id DESC","row");
        }

        $data = $this->second_db->withquery("select pp.*, pt.nama_pt, pt.logo_ptn, j.jurusan, j.passing_grade from pt_pilihan_ptn pp join pt_perguruan_tinggi pt on pp.id_pt = pt.id join pt_jurusan j on pp.id_jurusan = j.id where pp.jenjang = '".$jenjang."' and pp.id_siswa_aktif = '".$id_siswa."' order by pp.id ASC","result");
        if (!empty($data)) {
