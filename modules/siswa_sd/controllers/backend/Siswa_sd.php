 ...
 *|
 */
class Siswa_sd extends Admin
{

	public function __construct()
	{
		parent::__construct();
		$this->load->model('model_siswa_sd');
	...
	public function index($offset = 0)
	{
		...
		$limit = 20;
		// dd($offset);
		$this->data['siswa_sds'] = $this->model_siswa_sd->get($filter, $field, $limit, $offset, [], $sort, $sort_type);
		$this->data['siswa_sd_counts'] = $this->model_siswa_sd->count_all($filter, $field);
		
		// Info box data
		$this->data['total_daftar'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_sd', 'row')->cnt;
		$this->data['total_aktif'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_sd_aktif', 'row')->cnt;
		...
		if ($tahun_ajaran_aktif) {
			...
			$this->data['total_daftar_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_sd WHERE id_siswa_sd IN (SELECT id_siswa_sd FROM siswa_sd_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta . ')', 'row')->cnt;
			$this->data['total_aktif_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_sd_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta, 'row')->cnt;
		} else {
		...
		$config = [
			'base_url'     => 'administrator/siswa_sd/index/',
			'total_rows'   => $this->model_siswa_sd->count_all($filter, $field),
			'per_page'     => $limit,
			'uri_segment'  => 4,
		];
		...
		$this->render('backend/standart/administrator/siswa_sd/siswa_sd_list', $this->data);
	}
	...
	public function add()
	{
		$this->is_allowed('siswa_sd_add');

		$this->template->title('Siswa SD New');
		$this->render('backend/standart/administrator/siswa_sd/siswa_sd_add', $this->data);
	}
	...
	{
		if (!$this->is_allowed('siswa_sd_add', false)) {
			echo json_encode([
				'success' => false,
		...
		//$this->form_validation->set_rules('siswa_sd_foto_peserta_name', 'Foto Peserta', 'trim|required');
		//$this->form_validation->set_rules('siswa_sd_akte_lahir_name', 'Akte Lahir', 'trim|required');
		//$this->form_validation->set_rules('siswa_sd_kartu_keluarga_name', 'Kartu Keluraga', 'trim|required');
		...
		if ($this->form_validation->run()) {
			$siswa_sd_foto_peserta_uuid = $this->input->post('siswa_sd_foto_peserta_uuid');
			$siswa_sd_foto_peserta_name = $this->input->post('siswa_sd_foto_peserta_name');
			$siswa_sd_akte_lahir_uuid = $this->input->post('siswa_sd_akte_lahir_uuid');
			$siswa_sd_akte_lahir_name = $this->input->post('siswa_sd_akte_lahir_name');
			$siswa_sd_kartu_keluarga_uuid = $this->input->post('siswa_sd_kartu_keluarga_uuid');
			$siswa_sd_kartu_keluarga_name = $this->input->post('siswa_sd_kartu_keluarga_name');
			...
			if (!is_dir(FCPATH . '/uploads/siswa_sd/')) {
				mkdir(FCPATH . '/uploads/siswa_sd/');
			}


			if (!empty($siswa_sd_foto_peserta_name)) {
				$siswa_sd_foto_peserta_name_copy = date('YmdHis') . '-' . $siswa_sd_foto_peserta_name;

				rename(
					FCPATH . 'uploads/tmp/' . $siswa_sd_foto_peserta_uuid . '/' . $siswa_sd_foto_peserta_name,
					FCPATH . 'uploads/siswa_sd/' . $siswa_sd_foto_peserta_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_sd/' . $siswa_sd_foto_peserta_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['foto_peserta'] = $siswa_sd_foto_peserta_name_copy;
			}

			if (!empty($siswa_sd_akte_lahir_name)) {
				$siswa_sd_akte_lahir_name_copy = date('YmdHis') . '-' . $siswa_sd_akte_lahir_name;

				rename(
					FCPATH . 'uploads/tmp/' . $siswa_sd_akte_lahir_uuid . '/' . $siswa_sd_akte_lahir_name,
					FCPATH . 'uploads/siswa_sd/' . $siswa_sd_akte_lahir_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_sd/' . $siswa_sd_akte_lahir_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['akte_lahir'] = $siswa_sd_akte_lahir_name_copy;
			}

			if (!empty($siswa_sd_kartu_keluarga_name)) {
				$siswa_sd_kartu_keluarga_name_copy = date('YmdHis') . '-' . $siswa_sd_kartu_keluarga_name;

				rename(
					FCPATH . 'uploads/tmp/' . $siswa_sd_kartu_keluarga_uuid . '/' . $siswa_sd_kartu_keluarga_name,
					FCPATH . 'uploads/siswa_sd/' . $siswa_sd_kartu_keluarga_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_sd/' . $siswa_sd_kartu_keluarga_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['kartu_keluarga'] = $siswa_sd_kartu_keluarga_name_copy;
			}

			$save_siswa_sd = $this->model_siswa_sd->store($save_data);

			if ($save_siswa_sd) {
				//CREATE TRANSAKSI LUNAS
				$no_transaksi = "LI-SD-" . date("Ymd") . "-" . $save_siswa_sd;
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran SD a.n " . strtoupper($this->input->post('nama_lengkap')),
				...
				$this->mymodel->update("siswa_sd", array("no_transaksi" => $no_transaksi), "id_siswa_sd", $save_siswa_sd);
			}

			if ($save_siswa_sd) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_siswa_sd;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/siswa_sd/edit/' . $save_siswa_sd, 'Edit Siswa Sd'),
						anchor('administrator/siswa_sd', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/siswa_sd');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	/**
	 * Update view Siswa Sds
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('siswa_sd_update');
		$this->db->join('status_daftar_ulang_sd daftar_ulang', 'daftar_ulang.id_siswa_sd = siswa_sd.id_siswa_sd', 'LEFT');
		$this->data['siswa_sd'] = $this->model_siswa_sd->find($id);

		// dd($this->create_billing('development', 50000, '19212192-tesA', array("nama" => 'COBA PARTIAL PAY', "email" => $this->data['siswa_sd']->email, "va_number" => '9881611323010669')));
		// dd($this->inquiry_billing('development','19212192-tesA'));
		$this->template->title('Siswa SD Update');
		$this->render('backend/standart/administrator/siswa_sd/siswa_sd_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('siswa_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		//$this->form_validation->set_rules('siswa_sd_foto_peserta_name', 'Foto Peserta', 'trim');
		//$this->form_validation->set_rules('siswa_sd_akte_lahir_name', 'Akte Lahir', 'trim');
		///$this->form_validation->set_rules('siswa_sd_kartu_keluarga_name', 'Kartu Keluraga', 'trim');
		...
		if ($this->form_validation->run()) {
			...
			if (!empty($siswa_sd_foto_peserta_uuid)) {
				$siswa_sd_foto_peserta_name_copy = date('YmdHis') . '-' . $siswa_sd_foto_peserta_name;
			...
			if (!empty($siswa_sd_akte_lahir_uuid)) {
				$siswa_sd_akte_lahir_name_copy = date('YmdHis') . '-' . $siswa_sd_akte_lahir_name;
			...
			if (!empty($siswa_sd_kartu_keluarga_uuid)) {
				$siswa_sd_kartu_keluarga_name_copy = date('YmdHis') . '-' . $siswa_sd_kartu_keluarga_name;
			...
			$old_data = $this->mymodel->withquery('select * from siswa_sd where id_siswa_sd=' . $id, 'row');
			...
			if (count($update_transaksi)) {
				$this->mymodel->update3('transaksi', $update_transaksi, array('user_email' => $old_data->email), array('no_transaksi' => 'SD'));
			}

			$save_siswa_sd = $this->model_siswa_sd->change($id, $save_data);
			$new_data = $this->mymodel->withquery('select * from siswa_sd where id_siswa_sd=' . $id, 'row');
			if ($save_siswa_sd) {
				...
				if ($get_transaksi->status_transaksi == 1) { //jika sudah membayar 
					//if ($this->input->post('no_peserta') != $old_data->no_peserta || $old_data->foto_peserta != $siswa_sd_foto_peserta_name_copy || $this->input->post('nama_lengkap') != $old_data->nama_lengkap) { // cek jika ada perubahan no peserta
			...
			if (!empty($save_siswa_sd) && $this->input->post('status_lulus') == "2") {
				if ($old_data->status_lulus != 2) {
					...
					$check_exist_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_sd", "id_siswa_sd", $id, "row");
					...
					$this->mymodel->insertid("status_daftar_ulang_sd", $data_daftar_ulang);
				}
			}
			if ($save_siswa_sd) {
				if ($this->input->post('save_type') == 'stay') {
	...
	{
		$this->is_allowed('siswa_sd_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'siswa_sd'), 'success');
		} else {
			set_message(cclang('error_delete', 'siswa_sd'), 'error');
		}
		...
		if (isset($_FILES["file_siswa"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					$check=$this->mymodel->withquery("select * from siswa_sd where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					...
					$check2=$this->mymodel->withquery("select * from siswa_sd where email_ms_office_ortu = '".$worksheet->getCellByColumnAndRow(3, $row)->getValue()."'", 'row');
					if(!empty($check2)){
					...
					$check = $this->mymodel->withquery("select * from siswa_sd where nama_lengkap like '%".ucwords($worksheet->getCellByColumnAndRow(0, $row)->getValue())."%'", 'row');
					
					if($check){
						$insertId = $this->mymodel->update("siswa_sd", $data_pendaftaran, 'id_siswa_sd', $check->id_siswa_sd);
						$insertId = $check->id_siswa_sd;
					}else{
						$insertId = $this->mymodel->insertid("siswa_sd", $data_pendaftaran);
					}
					...
					if(!$check){
						//CREATE TRANSAKSI LUNAS
						$no_transaksi = "LI-SD-" . date("Ymd") . "-" . $insertId;
						//get biaya pendaftaran
						$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "SD", "row");
						$total_biaya = $get_biaya->nominal_pendaftaran;
						$data_transaksi = array(
							...
							"user_phone" => "",
							"description" => "Tagihan Pendaftaran SD a.n " . strtoupper($data_pendaftaran['nama_lengkap']),
							"id_biaya_pendaftaran" => $get_biaya->id_biaya_pendaftaran,
							"total_biaya" => $total_biaya,
							"status_transaksi" => "1",
							"created_at" => date("Y-m-d H:i:s"),
						...
						$id_transaksi = $this->mymodel->insertid("transaksi", $data_transaksi);
						$this->mymodel->update("siswa_sd", array("no_transaksi" => $no_transaksi), "id_siswa_sd", $insertId);
					}
				}
			}
	...
	public function update_status_daftar_ulang($id = null)
	{
		...
		$arr_id = $this->input->get('id');
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->withquery("select id_siswa_sd, no_peserta, nama_lengkap, va_number_bri from siswa_sd where id_siswa_sd = '" . $id ."'" ,"row");
			$data_daftar_ulang = array(
				"id_siswa_sd" => $id,
				"tanggal_lulus" =>  date("Y-m-d H:i:s"),
				"tgl_daftar_ulang" =>  $this->input->get('tgl_du'),
				"va_bri" => $get_siswa->va_number_bri
			);
			...
			$this->mymodel->update('siswa_sd', array('status_lulus' => $this->input->get('st')), 'id_siswa_sd', $id);
			$insert = $this->mymodel->insertid("status_daftar_ulang_sd", $data_daftar_ulang);
			// dd(!$insert);
			if (!$insert) {
				$this->db->trans_rollback();
	...
	public function update_status($id = null)
	{
		...
		$arr_id = $this->input->get('id');
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_sd", "id_siswa_sd", $id, "row");
			if ($get_siswa->status_lulus == 2) {
				$get_status_du = $this->mymodel->getbywhere("status_daftar_ulang_sd", "id_siswa_sd", $id, "row");
	...
	public function download_file($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_sd", "id_siswa_sd", $id, "row");
			$data[] = [
				"file_foto_path" => FCPATH . 'uploads/siswa_sd/' . $get_siswa->foto_peserta,
				"file_foto" => $get_siswa->foto_peserta,
				"file_kk_path" => FCPATH . 'uploads/siswa_sd/' . $get_siswa->kartu_keluarga,
				"file_kk" => $get_siswa->kartu_keluarga,
				"file_akte_path" => FCPATH . 'uploads/siswa_sd/' . $get_siswa->akte_lahir,
				"file_akte" => $get_siswa->akte_lahir,
				"nama_folder" => $get_siswa->id_siswa_sd . "_" . str_replace(" ", "_", $get_siswa->nama_lengkap)
			];
		}
		// dd($data);
	...
	public function download_kartu_peserta($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_sd", "id_siswa_sd", $id, "row");
			$kartu_peserta = 'sd' . '-' . $get_siswa->no_peserta . '-' . $get_siswa->nama_lengkap . ".pdf";
	...
	{
		$this->is_allowed('siswa_sd_view');

		$this->data['siswa_sd'] = $this->model_siswa_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Siswa SD Detail');
		$this->render('backend/standart/administrator/siswa_sd/siswa_sd_view', $this->data);
	}
	public function view_no_peserta($no_peserta)
	{
		$this->is_allowed('siswa_sd_view');

		$this->data['siswa_sd'] = $this->mymodel->getbywhere("siswa_sd", "no_peserta", $no_peserta, "row");
	...
	{
		$siswa_sd = $this->model_siswa_sd->find($id);

		if (!empty($siswa_sd->foto_peserta)) {
			$path = FCPATH . '/uploads/siswa_sd/' . $siswa_sd->foto_peserta;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($siswa_sd->akte_lahir)) {
			$path = FCPATH . '/uploads/siswa_sd/' . $siswa_sd->akte_lahir;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($siswa_sd->kartu_keluarga)) {
			$path = FCPATH . '/uploads/siswa_sd/' . $siswa_sd->kartu_keluarga;
		...
		return $this->model_siswa_sd->remove($id);
	}

	/**
	 * Upload Image Siswa Sd	* 
	 * @return JSON
	 */
	...
	/**
	 * Delete Image Siswa Sd	* 
	 * @return JSON
	 */
	public function delete_foto_peserta_file($uuid)
	{
		if (!$this->is_allowed('siswa_sd_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
			...
			'table_name'        => 'siswa_sd',
			'primary_key'       => 'id_siswa_sd',
			'upload_path'       => 'uploads/siswa_sd/'
		]);
	}
	...
	public function get_foto_peserta_file($id)
	{
		...
		echo $this->get_file([
			...
			'primary_key'       => 'id_siswa_sd',
			'upload_path'       => 'uploads/siswa_sd/',
			'delete_endpoint'   => 'administrator/siswa_sd/delete_foto_peserta_file'
		]);
	}
	...
	public function get_akte_lahir_file($id)
	{
		...
		echo $this->get_file([
			...
			'primary_key'       => 'id_siswa_sd',
			'upload_path'       => 'uploads/siswa_sd/',
			'delete_endpoint'   => 'administrator/siswa_sd/delete_akte_lahir_file'
		]);
	}
	...
	public function get_kartu_keluarga_file($id)
	{
		...
		echo $this->get_file([
			...
			'primary_key'       => 'id_siswa_sd',
			'upload_path'       => 'uploads/siswa_sd/',
			'delete_endpoint'   => 'administrator/siswa_sd/delete_kartu_keluarga_file'
		]);
	}
	...
	{
		$this->is_allowed('siswa_sd_export');
		$value 	= $this->input->get('q');
		$col 	= $this->input->get('f');
		// dd($field);
		$this->model_siswa_sd->export_siswa('siswa_sd', 'siswa_sd', $value, $col);
	}
	...
	public function export_pdf()
	{
		...
		$this->model_siswa_sd->export_siswa_pdf('siswa_sd', 'siswa_sd');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_siswa_sd->find($id);
...
/* End of file siswa_sd.php */
/* Location: ./application/controllers/administrator/Siswa Sd.php */
