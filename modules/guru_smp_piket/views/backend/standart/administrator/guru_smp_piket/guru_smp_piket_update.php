...
<section class="content-header">
    <h1>
        Guru Piket SMP        <small>Edit Guru Piket SMP</small>
    </h1>
    <ol class="breadcrumb">
                ...
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <div class="widget-user-header ">
                            ...
                            <h5 class="widget-user-desc">Edit Guru Piket SMP</h5>
                            <hr>
                        </div>
                        <?= form_open(base_url('administrator/guru_smp_piket/edit_save/'.$this->uri->segment(4)), [
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="id_guru" id="id_guru" data-placeholder="Select Guru" >
                                    ...
                                    <option <?=  $row->id_guru ==  $guru_smp_piket->id_guru ? 'selected' : ''; ?> value="<?= $row->id_guru ?>"><?= $row->nama_lengkap; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="id_kelas" id="id_kelas" data-placeholder="Select Kelas" >
                                    ...
                                    <option <?=  $row->id_kelas_smp ==  $guru_smp_piket->id_kelas ? 'selected' : ''; ?> value="<?= $row->id_kelas_smp ?>"><?= $row->label; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                                <small class="info help-block">
                                </small>
...
<script>
    $(document).ready(function(){
      ...
      $('.btn_save').click(function(){
        ...
        $.ajax({
          url: form_guru_smp_piket.attr('action'),
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
        .done(function(res) {
          ...
          if(res.success) {
            var id = $('#guru_smp_piket_image_galery').find('li').attr('qq-file-id');
            if (save_type == 'back') {
