      ...
      if(isset($headers['user_role']))
        ...
        $data = null;
          ...
          if (!empty($tipe_siswa == "ft")) {
            $data = $this->mymodel->withquery("select s.id_siswa_ft as id_siswa, s.email, s.nama_lengkap, s.va_number, s.no_peserta, s.sekolah_asal , s.status_lulus as status_lulus_int, st.status_lulus from siswa_ft s join status_lulus st on s.status_lulus = st.id_status_lulus where email like '%".$email."%' ","row");
            if (!empty($data)) {
              $data->tipe_siswa = "ft";
              $data->label_tipe = "SMA";
