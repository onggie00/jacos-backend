...
class Pendaftaran_sd extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_sd from calon_siswa_sd where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");

        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->second_db->getbywhere("list_sekolah_tk", "id_list_sekolah_tk",$this->db->escape($this->post("sekolah_asal")),"row")->nama_sekolah;
          ...
          $cek_tgl_lahir = $this->second_db->getbywhere("pengaturan_tanggal_lahir","jenjang","sd","row");
          ...
          if (!empty($_FILES['foto_peserta']['name'])) {
            $uploaddir = './uploads/calon_siswa_sd/';
          ...
          if(!empty($cek_email)){
            ...
            $id_siswa = $this->second_db->insertid("calon_siswa_sd",$data);
            if (!empty($id_siswa)) {
              $data['id_siswa'] = $id_siswa;
              //insert transaksi
              $no_transaksi = "LI-SD-".date("Ymd")."-".$id_siswa;
              //get biaya pendaftaran
              $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","SD","row");
              ...
              if (!empty($data_transaksi)) {
                ...
                $get_no_urut = $this->second_db->withquery("select va_number_bri, id_siswa_sd as id_siswa from calon_siswa_sd where va_number_bri like '".$this->db->escape_like_str($brivaNo.date('y', strtotime('+1 years')))."01%' and va_number_bri != '' and is_mutasi = '2' order by id_siswa_sd DESC","row");
                if (empty($get_no_urut)) {
                  //$no_urut = "0001";
                  $no_urut = rand(1000,9999);
                }
                else{
                ...
                $id_transaksi = $this->second_db->insertid("transaksi",$data_transaksi);
                $this->second_db->update("calon_siswa_sd", array("no_transaksi" => $no_transaksi, "va_number_bri" => $va_number_bri), "id_siswa_sd", $id_siswa);
