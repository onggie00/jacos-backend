...
class Pendaftaran_smp extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->mymodel->withquery("select id_siswa_smp from siswa_smp where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");
          ...
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              if ($ppsbb == "1") {
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_smp as id_siswa from siswa_smp where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')))."03%' and va_number != '' order by id_siswa_smp DESC","row");
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_smp as id_siswa from siswa_smp where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')))."06%' and va_number != '' order by id_siswa_smp DESC","row");
                if (empty($get_no_urut)) {
                  $no_urut = "0001";
                }
                else{
              ...
              if (!empty($data_transaksi)) {
                ...
                if(empty($payment_response['virtual_account'])){
                  // ponytail: fix bug — kondisi sebelumnya !empty() kebalik, VA sukses malah dianggap error
