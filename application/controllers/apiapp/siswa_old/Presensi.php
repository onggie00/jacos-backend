...
class Presensi extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        $waktu_absen = date("H:i:s");//$this->post('waktu_absen');
        ...
        if (!empty($check_if_presented)){
            $msg = array('status' => 200, 'message'=>'Anda sudah melakukan absen untuk hari ini!');
        }else{
            ...
            if (!empty($data) && empty($check_if_presented)) {
                $insert=$this->mymodel->insertid("presensi_".$jenjang,$data);
                if($insert){
                    $data["notelp_ibu"] = $get_siswa->notelp_ibu;
                    $data["notelp_formatted"] = $notelp;
                    ...
                    if (!empty($notelp)) {
                        // FIX: pakai send_wa2 (Wapanels) menggantikan send_wa_omni (OmniChat).
                        // - send_wa2 generate pesan internal dari $status_absen + $alasan_terlambat.
                        // - Sebelumnya: send_wa_omni echo response mentah -> mencemari HTTP body.
                ...
                $msg = array('status' => 200, 'message'=>'Anda sudah melakukan absen untuk hari ini / Data tdk valid!', 'data' => $data);
            }
        }
        $this->response($msg);
  ...
  public function send_wa2($notelp, $nama_siswa, $status_absen, $alasan_terlambat = null){
    ...
    // FIX: log instead of echo - mencegah pencemaran response body.
    if ($err) {
      log_message('error', 'send_wa2 cURL Error: ' . $err);
    } else {
      log_message('debug', 'send_wa2 response: ' . $response);
    }
  }
    ...
    public function send_wa_omni($notelp, $pesan){
        ...
        // FIX: log instead of echo - mencegah pencemaran response body.
    // Catatan: setelah commit ganti ke send_wa2, send_wa_omni jadi dead code
    // tapi tetap difix agar aman jika dipanggil dari tempat lain.
    if ($err) {
        log_message('error', 'send_wa_omni cURL Error: ' . $err);
    } else {
        log_message('debug', 'send_wa_omni response: ' . $response);
        return $response;
    }
    }
