...
/**
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang KB Controller
 *| --------------------------------------------------------------------------
 *| Status Daftar Ulang KB site
 *|
 */
class Status_daftar_ulang_kb extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_status_daftar_ulang_kb');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Status Daftar Ulang KBs
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('status_daftar_ulang_kb_list');
		...
		if ($has_multi_filter) {
			$this->data['status_daftar_ulang_kbs'] = $this->model_status_daftar_ulang_kb->get(null, null, $this->limit_page, $offset, array(), $sort, $sort_type, $multi_filters);
			$this->data['status_daftar_ulang_kb_counts'] = $this->model_status_daftar_ulang_kb->count_all(null, null, $multi_filters);
		} else {
			$this->data['status_daftar_ulang_kbs'] = $this->model_status_daftar_ulang_kb->get($filter, $field, $this->limit_page, $offset, array(), $sort, $sort_type);
			$this->data['status_daftar_ulang_kb_counts'] = $this->model_status_daftar_ulang_kb->count_all($filter, $field);
		}

		$config = [
			'base_url'     => 'administrator/status_daftar_ulang_kb/index/',
			'total_rows'   => $has_multi_filter ? $this->model_status_daftar_ulang_kb->count_all(null, null, $multi_filters) : $this->model_status_daftar_ulang_kb->count_all($filter, $field),
		...
		$this->data['list_tahun_ajaran'] = $this->mymodel->withquery("SELECT DISTINCT tahun_ajaran FROM siswa_kb WHERE deleted_at IS NULL AND tahun_ajaran IS NOT NULL AND tahun_ajaran <> '' ORDER BY tahun_ajaran DESC", "result");
		$this->data['list_gelombang']    = $this->mymodel->withquery("SELECT DISTINCT gelombang FROM siswa_kb WHERE deleted_at IS NULL AND gelombang IS NOT NULL ORDER BY gelombang ASC", "result");
		$this->data['multi_filters'] = $multi_filters;

		$this->template->title('Status Daftar Ulang KB List');
		$this->render('backend/standart/administrator/status_daftar_ulang_kb/status_daftar_ulang_kb_list', $this->data);
	}

	/**
	 * Parse multi-filter from GET params (ff[], fo[], fv[])
	 */
	private function _parse_filters()
	{
		...
		$count = count($fields);
		for ($i = 0; $i < $count; $i++) {
			if (!empty($fields[$i]) && $values[$i] !== '' && $values[$i] !== null) {
				$filters[] = array(
					'field'    => $fields[$i],
	...
	/**
	 * Update view Status Daftar Ulang KBs
	 *
	 * @var $id String
	 */
	public function edit($id, $date = null)
	{
		$this->is_allowed('status_daftar_ulang_kb_update');

		$this->model_status_daftar_ulang_kb->join_avaiable();

		$this->data['status_daftar_ulang_kb'] = $this->model_status_daftar_ulang_kb->find($id);
		...
		foreach ($get_setting as $key => $item) {
			...
			if ($item->name_setting == 'bri_institution_code_close' && $this->data['status_daftar_ulang_kb']->tipe_tagihan == "fixed") {
				$institutionCode = $item->value;
			}
			else if ($item->name_setting == 'bri_institution_code' && $this->data['status_daftar_ulang_kb']->tipe_tagihan == "open") {
				$institutionCode = $item->value;
			}
		}
		...
		$brivaNo = substr($this->data['status_daftar_ulang_kb']->va_bri, 0, 5);
		$custCode = substr($this->data['status_daftar_ulang_kb']->va_bri, 5);
		$datas = array(
			'brivaNo' => $brivaNo,
			'custCode' => $custCode,
			'startDate' => $date,
		...
		$va = $this->data['status_daftar_ulang_kb']->va_bri;
		// $va='2147483647';	
		...
		$this->template->title('Status Daftar Ulang KB Update');
		$this->render('backend/standart/administrator/status_daftar_ulang_kb/status_daftar_ulang_kb_update', $this->data);
	}

	/**
	 * Update Status Daftar Ulang KBs
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('status_daftar_ulang_kb_update', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_status_daftar_ulang_kb = $this->model_status_daftar_ulang_kb->change($id, $save_data);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_kb", "id_daftar_ulang", $id, "row");

			if ($this->input->post('status') == 1) {
				$no_transaksi = "LDUI-KB-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_kb;
				$this->mymodel->update("status_daftar_ulang_kb", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_daftar_ulang", $id);

				$get_siswa = $this->mymodel->getbywhere("siswa_kb", "id_siswa_kb", $get_daftar_ulang->id_siswa_kb, "row");
				$customPayment = $this->input->post('custom_total_payment');
				// dd($customPayment);
				if ($customPayment != '0' && $customPayment != '') {
					$total_biaya = $customPayment;
					$this->mymodel->update("status_daftar_ulang_kb", array("custom_payment" => $customPayment), "id_daftar_ulang", $id);
				...
				if ($bank_aktif == 'BNI') {
					...
					$date_va = $bni['date_va'];
					$this->mymodel->update("siswa_kb", array("va_number" => $va_number), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
					$this->mymodel->update("status_daftar_ulang_kb", array("va_bri" => $va_number), "id_daftar_ulang", $id);
				}
				if ($bank_aktif != 'BNI') {
				//get no briva
				$get_setting = $this->mymodel->getall("pengaturan_akun");
				...
				foreach ($get_setting as $key => $value) {
					if (ENVIRONMENT == "production") {
						if ($value->name_setting == "bri_no_briva_prod_close" && $tipe_tagihan == 'fixed') {
							$brivaNo = $value->value;
						}
						else if($value->name_setting == "bri_no_briva_prod" && $tipe_tagihan == 'open'){
							$brivaNo = $value->value;
						}
				...
				if (!empty($get_siswa->va_number_bri)) {
					...
					$va_number = $brivaNo . substr($va_number, 5);
				} else {
					$get_no_urut = $this->mymodel->withquery("select va_number_bri, id_siswa_kb as id_siswa from siswa_kb where va_number_bri like '%" . $brivaNo . date('y', strtotime('+1 years')) . "01%' and va_number_bri != '' and is_mutasi = '2' order by va_number_bri DESC", "row");
				...
				if(!empty($this->input->post("end_date_va"))){
					$date_va = $this->input->post("end_date_va");
				}
				else{
				...
				//print_r($datas);
				$delete_response = $this->delete_va_bri(array("brivaNo" => $datas["brivaNo"], "custCode" => $datas["custCode"], "tipe_tagihan" => $tipe_tagihan));
				...
				$this->mymodel->update("siswa_kb", array("va_number_bri" => $res_data['brivaNo'] . $res_data['custCode']), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
				$this->mymodel->update("status_daftar_ulang_kb", array("va_bri" => $res_data['brivaNo'] . $res_data['custCode']), "id_daftar_ulang", $id);
				} //end if ($bank_aktif != 'BNI')


				// $this->mymodel->update("siswa_kb", array("va_number_bri" => $va_number), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
				// $this->mymodel->update("status_daftar_ulang_kb", array("va_bri" => $va_number), "id_daftar_ulang", $id);
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran Ulang KB a.n " . strtoupper($get_siswa->nama_lengkap),
					"id_biaya_pendaftaran" => $get_biaya->id_biaya_pendaftaran,
					"total_biaya" => $total_biaya,
					"status_transaksi" => "0",
					"created_at" => date("Y-m-d H:i:s"),
					"tipe_tagihan" => $tipe_tagihan,
					"end_date_va" => date("Y-m-d H:i:s", strtotime($date_va)),
				...
				if (empty($cek_data)) {
					...
					$this->mymodel->update("siswa_kb", array("no_transaksi" => $no_transaksi), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
					//$cetak_slip = $this->cetak_slip(array("id_siswa" => $get_daftar_ulang->id_siswa_kb, "tipe_siswa" => "kb"));
					// dd($cetak_slip);
					...
					$data_email = array(
						...
						"nama_panitia" => "Panitia PSB KB Labschool Cibubur " . date("Y", strtotime("+1 years")) . "-" . date("Y", strtotime("+2 years")),
						"slip_pembayaran" => $no_transaksi . '-' . $get_siswa->nama_lengkap . '.pdf',
					);
					$this->mymodel->update("status_daftar_ulang_kb", array("slip_pembayaran" => $data_email["slip_pembayaran"]), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
			...
			if ($this->input->post('status') == 2) {
				...
				$this->mymodel->update("transaksi", array("status_transaksi" => 1, "updated_at" => date("Y-m-d H:i:s")), "va_number", $get_daftar_ulang->va_bri);
				...
				$cetak_kartu_siswa_sementara = $this->cetak_kartu_siswa_sementara(array("tipe_siswa" => 'sd', "id_siswa" => $get_daftar_ulang->id_siswa_kb));
				...
				$cetak_kwitansi = $this->cetak_kwitansi(array(
					...
					"id_siswa" => $get_daftar_ulang->id_siswa_kb,
				...
				$this->mymodel->update("status_daftar_ulang_kb", array("status" => 2, "kwitansi" => $kwitansi, "kartu_sementara" => $kartu, "tgl_bayar" => date("Y-m-d H:i:s")), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
			}
			...
			if ($save_status_daftar_ulang_kb) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $id;
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/status_daftar_ulang_kb', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/status_daftar_ulang_kb');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
	...
	/**
	 * delete Status Daftar Ulang KBs
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('status_daftar_ulang_kb_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'status_daftar_ulang_kb'), 'success');
		} else {
			set_message(cclang('error_delete', 'status_daftar_ulang_kb'), 'error');
		}
	...
	/**
	 * View view Status Daftar Ulang KBs
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('status_daftar_ulang_kb_view');

		$this->data['status_daftar_ulang_kb'] = $this->model_status_daftar_ulang_kb->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Status Daftar Ulang KB Detail');
		$this->render('backend/standart/administrator/status_daftar_ulang_kb/status_daftar_ulang_kb_view', $this->data);
	}

	private function _remove($id)
	{
		$siswa_kb = $this->model_status_daftar_ulang_kb->find($id);

		return $this->model_status_daftar_ulang_kb->remove($id);
	}
	...
	public function update_status($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$this->mymodel->update('status_daftar_ulang_kb', array('status' => '1'), 'id_daftar_ulang', $id);
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_kb", "id_daftar_ulang", $id, "row");

			//update tgl aktivasi
			$this->mymodel->update("status_daftar_ulang_kb", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);

			$no_transaksi = "LDUI-KB-" . date("YmdHis") . "-" . $get_daftar_ulang->id_siswa_kb;
			// get biaya pendaftaran
			$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "KB", "row");
			$total_biaya = $get_biaya->nominal_daftar_ulang;
			$get_siswa = $this->mymodel->getbywhere("siswa_kb", "id_siswa_kb", $get_daftar_ulang->id_siswa_kb, "row");
			...
			$get_pengaturan_masa_aktif = $this->mymodel->getall("pengaturan_masa_aktif_va");
			if(!empty($this->input->get("end_date_va"))){
				$date_va = $this->input->get("end_date_va");
			}
			else{
				foreach ($get_pengaturan_masa_aktif as $key => $item) {
					if ($item->label == 'daftar_ulang') {
			...
			$delete_response = $this->delete_va_bri(array("brivaNo" => $datas["brivaNo"], "custCode" => $datas["custCode"], "tipe_tagihan" => $datas["tipe_tagihan"]));
			$payment_response = $this->create_va_bri($datas);
			...
			if ($payment_response['responseCode'] != '00') {
				...
				$payment_response = $this->update_va_bri($datas);
      			...
      			if ($payment_response['responseCode'] != '00') {
      				$this->data['success'] = false;
					$this->data['message'] = $payment_response['errDesc'];
					$this->data['response'] = $payment_response;
					echo json_encode($this->data);
					exit;
      			}
			}
		...
		set_message('Update status VA berhasil', 'success');

		redirect_back();
	}

	public function update_status_new($id = null)
	{
		$arr_id = $this->input->get('id');
		if (count($arr_id) >= 20) {
			set_message('Update aktivasi va maksimal per 10 data', 'error');
		...
		// Cache tanggal dan data tetap agar tidak query berulang
		$now = date("Y-m-d H:i:s");
		$year_next = date('y', strtotime('+1 years'));

		// Ambil 1x semua data global yg sama
		$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "KB", "row");
		$total_biaya = $get_biaya->nominal_daftar_ulang;

		// Ambil pengaturan akun dan VA sekalian
		$pengaturan_akun = $this->mymodel->getall("pengaturan_akun");
		$pengaturan_masa_aktif = $this->mymodel->getall("pengaturan_masa_aktif_va");

		// Tentukan brivaNo hanya sekali
		$brivaNo = '';
		foreach ($pengaturan_akun as $value) {
			if (ENVIRONMENT === "production" && $value->name_setting === "bri_no_briva_prod_close") {
				$brivaNo = $value->value;
			} elseif ((ENVIRONMENT === "development" || ENVIRONMENT === "testing") && $value->name_setting === "bri_no_briva_dev_close") {
				$brivaNo = $value->value;
			}
		}

		// Tentukan masa aktif VA
		$date_va = 24; // default fallback
		foreach ($pengaturan_masa_aktif as $item) {
			if ($item->label === 'daftar_ulang') {
		...
		foreach ($arr_id as $id) {
			// update status awal + tanggal aktivasi
			$this->mymodel->update('status_daftar_ulang_kb', ['status' => '1', 'tgl_aktivasi' => $now], 'id_daftar_ulang', $id);

			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_kb", "id_daftar_ulang", $id, "row");
			if (!$get_daftar_ulang) continue;
			...
			if ($bank_aktif == 'BNI') {
				...
				$full_va = $va_number;
				$date_va = $bni['date_va'];
				$this->mymodel->update("siswa_kb", array("va_number" => $va_number, "no_transaksi" => $no_transaksi), "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);
				$this->mymodel->update("status_daftar_ulang_kb", array("va_bri" => $va_number), "id_daftar_ulang", $id);
			}
			...
			if (!empty($get_siswa->va_number_bri)) {
				...
				$get_no_urut = $this->mymodel->withquery("
					SELECT va_number_bri 
					FROM siswa_kb 
					WHERE va_number_bri LIKE '%" . $brivaNo . $year_next . "01%' 
					AND va_number_bri != '' 
					AND is_mutasi = '2' 
					ORDER BY va_number_bri DESC LIMIT 1", 
				"row");

				$no_urut = empty($get_no_urut)
					? "0001"
					: sprintf("%04d", ((int) substr($get_no_urut->va_number_bri, -4)) + 1);

				$va_number = $brivaNo . $year_next . "01" . $no_urut;
			}
			...
			// Eksekusi VA API (hapus, buat, atau update)
			$this->delete_va_bri(["brivaNo" => $datas["brivaNo"], "custCode" => $datas["custCode"], "tipe_tagihan" => $datas["tipe_tagihan"]]);
			$payment_response = $this->create_va_bri($datas);

			if ($payment_response['responseCode'] !== '00') {
				$payment_response = $this->update_va_bri($datas);
				$this->update_va_bri_status($datas);
				if ($payment_response['responseCode'] !== '00') {
					echo json_encode([
						'success' => false,
			...
			$res_data = $payment_response['data'];
			$full_va = $res_data['brivaNo'] . $res_data['custCode'];

			// Update siswa & daftar ulang sekali saja
			$this->mymodel->update("siswa_kb", [
				"va_number_bri" => $full_va,
				"no_transaksi" => $no_transaksi
			], "id_siswa_kb", $get_daftar_ulang->id_siswa_kb);

			$this->mymodel->update("status_daftar_ulang_kb", [
				"va_bri" => $full_va,
				"slip_pembayaran" => $no_transaksi . '-' . $get_siswa->nama_lengkap . '.pdf'
			], "id_daftar_ulang", $id);
			} //end if ($bank_aktif != 'BNI')
			...
			$cek_data = $this->mymodel->withquery("
				SELECT id_transaksi 
				FROM transaksi 
				WHERE va_number = '$va_number' 
				AND no_transaksi LIKE '%LDUI%' 
			...
			// Kirim email slip
			$get_transaksi = $this->mymodel->getbywhere("transaksi", "id_transaksi", $id_transaksi, "row");
			$data_email = [
				"email" => $get_siswa->email,
				"nama_lengkap" => $get_siswa->nama_lengkap,
				"tipe_pendaftaran" => "PSB SD",
	...
	public function siswa_aktif()
	{
		...
		foreach ($ids as $key => $i) {
			$get_daftar_ulang = $this->mymodel->getbywhere("status_daftar_ulang_kb", "id_daftar_ulang", $i, "row");
			$get_siswa = $this->mymodel->getbywhere("siswa_kb", "id_siswa_kb", $get_daftar_ulang->id_siswa_kb, "row");
			$data_siswa_aktif = [
				"id_siswa_kb" => $get_daftar_ulang->id_siswa_kb,
				"nama_lengkap" => $get_siswa->nama_lengkap,
				"id_tahun_ajaran" => $this->input->post('tahun_ajaran'),
				"id_kelas" => $this->input->post('kelas_kb'),
	...
	{
		$this->is_allowed('status_daftar_ulang_kb_export');

		$this->model_status_daftar_ulang_kb->export_siswa('status_daftar_ulang_kb', 'status_daftar_ulang_kb');
	}
	...
	public function export_pdf()
	{
		...
		$this->model_status_daftar_ulang_kb->pdf('status_daftar_ulang_kb', 'status_daftar_ulang_kb');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'status_daftar_ulang_kb';
		...
		$data = $this->model_status_daftar_ulang_kb->find($id);
	...
	{
		// KB/TK = BNI langsung (jenjang baru, tanpa histori BRI)
		return 'BNI';
	}

	/**
	 * Buat VA BNI utk tagihan daftar ulang SD.
	 * Pakai VA BNI PSB siswa (siswa_kb.va_number) bila sudah ada, bila belum generate baru (kode 01).
	...
	private function _bni_va_daftar_ulang($get_siswa, $no_transaksi, $total_biaya)
	{
		...
		if (!empty($get_siswa->va_number)) {
			...
			$prefix_cari = $prefix . $client_id . date('y', strtotime('+1 years')) . "11";
			$get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_kb as id_siswa from siswa_kb where va_number like '" . $prefix_cari . "%' and va_number != '' and is_mutasi = '2' order by id_siswa_kb DESC", "row");
			if (empty($get_no_urut)) {
				$no_urut = "0001";
			} else {
	...
	function create_va_bri($datas)
	{
		...
		foreach ($get_setting as $key => $item) {
			...
			if ($item->name_setting == 'bri_institution_code' && $datas['tipe_tagihan'] == "open") {
				$institutionCode = $item->value;
			}
			else if ($item->name_setting == 'bri_institution_code_close' && $datas['tipe_tagihan'] == "fixed") {
				$institutionCode = $item->value;
			}
		}
...
/* End of file status_daftar_ulang_kb.php */
/* Location: ./application/controllers/administrator/Status Daftar Ulang KB.php */
