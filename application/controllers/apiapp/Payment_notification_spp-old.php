        ...
        if (!$data_asli) {
          ...
          if (!empty($data_asli['payment_ntb'])) {
            if ($jenis_pembayaran == "LI") {
              ...
              //$get_transaksi = $this->mymodel->withquery("select bulan from transaksi_spp where id_spp = '".$get_transaksi_spp->id_spp."' and status_transaksi = '2' order by id_transaksi DESC","row");
              $bulan_ini = formatBulan(date("m", strtotime("-1 months")));
              if ($get_transaksi_spp->bulan == $bulan_ini) {
                $this->mymodel->update("ujian_ruang_detail", array("nama_siswa" => $get_transaksi_spp->nama_lengkap, "boleh_ujian" => "YA"), "nomor_peserta_ujian", $get_siswa->nomor_peserta_ujian);
              }
