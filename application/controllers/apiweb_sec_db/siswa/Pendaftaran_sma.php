...
class Pendaftaran_sma extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_sma from siswa_sma where email like '%".$this->post("email")."%' and is_show = 1","row");
        // dd($cek_email);
        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->second_db->getbywhere("list_sekolah_smp", "id_list_sekolah_smp",$this->post("sekolah_asal"),"row")->nama_sekolah;
          ...
          if ($this->post("pekerjaan_ayah") > 0) {
            $pekerjaan_ayah = $this->second_db->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->post("pekerjaan_ayah")."'","row")->nama_pekerjaan;
          ...
          if ($this->post("pekerjaan_ibu") > 0) {
            $pekerjaan_ibu = $this->second_db->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->post("pekerjaan_ibu")."'","row")->nama_pekerjaan;
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select * from siswa_sma where email = '".$this->post("email")."'", "row");
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              $peminatan = $this->post('peminatan_sma');
              if ($ppsbb == "1") {
                if ($peminatan == "1") {
                  $va_class = "04";
                }
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_sma as id_siswa from siswa_sma where va_number like '".$prefix.$client_id.date('y', strtotime('+1 years')).$va_class."%' and va_number != '' order by id_siswa_sma DESC","row");
