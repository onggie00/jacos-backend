	...
	<div class="col-sm-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title"><i class="fa fa-upload"></i> Import Buat Baru — Siswa KB Aktif (Data Historis)</h3>
				<div class="box-tools pull-right">
					<a class="btn btn-sm btn-default" href="<?= site_url('administrator/siswa_kb_aktif'); ?>"><i class="fa fa-arrow-left"></i> Kembali</a>
				</div>
			</div>
			<div class="box-body">
				...
				<div class="alert alert-warning">
					...
					<b>Untuk migrasi data historis:</b> baris akan dibuat BARU di <code>siswa_kb_aktif</code>
					dengan <code>id_siswa_kb = 0</code> (tidak terhubung record PSB).
					Tahun ajaran dipilih per-batch. Selalu cek preview sebelum konfirmasi.
				</div>

				<?php if (empty($preview)): ?>
					<form action="<?= base_url('administrator/siswa_kb_aktif/import_create_parse'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<a class="btn btn-sm btn-default" href="<?= site_url('administrator/siswa_kb_aktif/template_import_create'); ?>"><i class="fa fa-download"></i> Download Template</a>
							</div>
						</div>
						<div class="form-group">
				...
				<?php else: ?>
					...
					<form action="<?= base_url('administrator/siswa_kb_aktif/import_create_commit'); ?>" method="post" class="form-inline" style="margin-top:10px">
						<button type="submit" class="btn btn-success" onclick="return confirm('Insert <?= count($preview['rows']); ?> baris ke siswa_kb_aktif? Data historis — pastikan preview sudah benar.');"><i class="fa fa-check"></i> Konfirmasi &amp; Insert <?= count($preview['rows']); ?> Baris</button>
						<a class="btn btn-default" href="<?= site_url('administrator/siswa_kb_aktif/import_create_cancel'); ?>">Batal</a>
					</form>
				<?php endif; ?>
			</div>
		</div>
