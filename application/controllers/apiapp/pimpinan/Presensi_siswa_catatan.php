 ...
 * Akses   : Pimpinan dengan token pimpinan pada jenjang yang sama.
 ...
 *
 * Header:
 *   - x-token                  : token login pimpinan_<jenjang>
 *
...
class Presensi_siswa_catatan extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        // --- 2a. Validasi jenjang ---
        if (empty($jenjang) || !in_array($jenjang, array('sd', 'smp', 'sma'))) {
            $this->response(array(
                'status'  => 0,
                'message' => 'jenjang tidak valid. Pilihan: sd, smp, sma',
                'data'    => array(),
            ));
            return;
        }
        ...
        // === 3. Validasi token & ambil nama pimpinan ===
        $pimpinan = $this->mymodel->withquery(
            "SELECT id_pimpinan, nama_pimpinan FROM pimpinan_$jenjang
        ...
        if (empty($pimpinan)) {
            $this->response(array(
                'status'  => 0,
                'message' => 'Token tidak valid atau pimpinan tidak ditemukan di jenjang ' . $jenjang,
                'data'    => array(),
            ));
            return;
        }
