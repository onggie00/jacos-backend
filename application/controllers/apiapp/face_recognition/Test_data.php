...
class Test_data extends MY_Controller {
    ...
    public function index()
    {
      ...
      if(isset($headers['x-token']))
        ...
        if (!empty($token)) {
          $msg = array('status' => 1, 'message'=>'Success Request Token' ,'token'=>$token, 'host'=>$host, 'port'=>$port, 'data'=>$data);
          $status="200";
          //Get all device list
          /* if(empty($this->input->post('device_id'))){
            $list_device = $this->get_all_device($host, $port, $token);
            $data->all_device = $list_device;
            $msg = array('status' => 1, 'message'=>'Get All Device Success' ,'token'=>$token, 'data'=>$data);
            $status="200";
          }
          else{
            $device_id = $this->input->post('device_id');
            $detail_device = $this->get_device_by_id($host, $port, $token, $device_id);
            $data->all_device = [];
            $data->detail_device = $detail_device;
            $msg = array('status' => 1, 'message'=>'Get Detail Device Success' ,'token'=>$token, 'data'=>$data);
            $status="200";
          } */

          //get transaction by time
          $start_time = (!empty($this->input->post('start_time'))) ? $this->input->post('start_time') : date('Y-m-d');
          $end_time = (!empty($this->input->post('end_time'))) ? $this->input->post('end_time') : date('Y-m-d', strtotime($start_time . ' +1 day'));
          $emp_code = (!empty($this->input->post('emp_code'))) ? $this->input->post('emp_code') : "";
          $page_size = (!empty($this->input->post('page_size'))) ? $this->input->post('page_size') : 1000;
          if (!empty($start_time) && !empty($end_time) && !empty($emp_code) && !empty($page_size)) {
            $data->transaction_by_time = $this->get_transaction_by_time($host, $port, $token, $emp_code, $start_time, $end_time, $page_size);
          }

          //get transaction by id
          /* $transaction_id = (!empty($this->input->post('transaction_id'))) ? $this->input->post('transaction_id') : "";
          if (!empty($transaction_id)) {
            $data->transaction = $this->get_transaction_by_id($host, $port, $token, $transaction_id);
            $msg = array('status' => 1, 'message'=>'Get Data Success' ,'token'=>$token, 'data'=>$data);
            $status="200";
          } */
        }
        else{
          $msg = array('status' => 0, 'message'=>'Request Token Failed' ,'token'=>$token, 'host'=>$host, 'port'=>$port, 'data'=>$data);
          $status="200";
        }
    ...
    private function get_device_by_id($host, $port, $token, $id){ //Work
      $url = $host . ":" . $port . "/iclock/api/terminals/" . $id . "/";
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    private function get_transaction_by_id($host, $port, $token, $id){ //Works
      $url = $host . ":" . $port . "/iclock/api/transactions/" . $id . "/";
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $url);
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
