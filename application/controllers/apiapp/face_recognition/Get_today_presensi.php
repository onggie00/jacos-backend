...
class Get_today_presensi extends MY_Controller {
    ...
    public function index()
    {
      ...
      if(isset($headers['x-token']))
        ...
        if (!empty($token)) {
            ...
            if (!empty($emp_code) && !empty($start_time) && !empty($end_time) && !empty($page_size)) {
                $data_get = array(
                    'emp_code' => $emp_code,
                ...
                $riwayat_presensi = $this->get_transaction_by_time($host, $port, $token, $emp_code, $start_time, $end_time, $page_size);
                $data->riwayat_presensi = $riwayat_presensi;
