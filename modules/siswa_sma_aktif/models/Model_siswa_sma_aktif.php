...
class Model_siswa_sma_aktif extends MY_Model {

    private $primary_key    = 'id_siswa_sma_aktif';
    private $table_name     = 'siswa_sma_aktif';
    private $field_search   = ['nama_lengkap', 'nis', 'id_kelas', 'id_siswa_sma', 'id_tahun_ajaran', 'kewarganegaraan', 'nik', 'golongan_darah', 'telp', 'pendidikan_ayah', 'pendidikan_ibu', 'penghasilan_ayah', 'penghasilan_ibu', 'tgl_lahir_ayah', 'tgl_lahir_ibu', 'spp_custom', 'spp_type', 'acc_ujian'];
    private $field_search_join   = ['kelas_sma.label','tahun_ajaran.label'];
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_sma_aktif.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_sma_aktif.".$field . " LIKE '%" . $q . "%' ";
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $sort_by = (empty($sort)) ? 'id_siswa_sma_aktif' : $sort;
            $sort_type = (empty($sort_type)) ? 'desc' : $sort_type;
            $this->db->order_by('siswa_sma_aktif.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_sma', 'siswa_sma.id_siswa_sma = siswa_sma_aktif.id_siswa_sma', 'LEFT');
        $this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun_ajaran = siswa_sma_aktif.id_tahun_ajaran', 'LEFT');

        $this->db->select('siswa_sma_aktif.*,
        kelas_sma.label as kelas_sma_label,
        siswa_sma.nama_lengkap as siswa_sma_nama_lengkap,
        tahun_ajaran.label as tahun_ajaran_label,
        email_ms_office,
        email_ms_office_ortu,
        foto_profil,
        siswa_sma.deleted_at as deleted_at_siswa,
        siswa_sma.deleted_at_ortu as deleted_at_ortu
        ');
    ...
    public function join_avaiable_export() {
        ...
        $this->db->select('siswa_sma_aktif.nama_lengkap,
        kelas_sma.label as kelas,
        tahun_ajaran.label as tahun_ajaran,
        siswa_sma.email,
        siswa_sma.email_ms_office,
        siswa_sma.email_ms_office_ortu,
        siswa_sma.notelp_ibu,
        siswa_sma.notelp_ayah,
        siswa_sma.nisn,
        siswa_sma_aktif.file_raport,
        siswa_sma_aktif.kewarganegaraan,
        siswa_sma_aktif.nik,
        siswa_sma_aktif.golongan_darah,
        siswa_sma_aktif.telp,
        siswa_sma_aktif.pendidikan_ayah,
        siswa_sma_aktif.pendidikan_ibu,
        siswa_sma_aktif.penghasilan_ayah,
        siswa_sma_aktif.penghasilan_ibu,
        siswa_sma_aktif.tgl_lahir_ayah,
        siswa_sma_aktif.tgl_lahir_ibu,
        siswa_sma_aktif.spp_custom
        ');
    ...
    private function _build_multi_where($filters)
    {
        ...
        $field_map = [
            'nama_lengkap'   => 'siswa_sma_aktif.nama_lengkap',
            'nis'            => 'siswa_sma_aktif.nis',
            'kelas'          => 'kelas_sma.label',
            'tahun_ajaran'   => 'tahun_ajaran.label',
            'spp_type'       => 'siswa_sma_aktif.spp_type',
            'acc_ujian'      => 'siswa_sma_aktif.acc_ujian',
            'is_active'      => 'siswa_sma_aktif.is_active',
            'spp_custom'     => 'siswa_sma_aktif.spp_custom',
            'nik'            => 'siswa_sma_aktif.nik',
            'kewarganegaraan'=> 'siswa_sma_aktif.kewarganegaraan',
            'nomor_peserta_ujian' => 'siswa_sma_aktif.nomor_peserta_ujian',
        ];
        ...
        foreach ($filters as $filter) {
            ...
            $db_field = isset($field_map[$field]) ? $field_map[$field] : 'siswa_sma_aktif.' . $field;
        ...
        public function export_siswa($table, $subject = 'file',$value=null,$col=null)
        {
            ...
            if (empty($field)) {
                foreach ($this->field_search as $field) {
                    if ($iterasi == 1) {
                        $where .= "siswa_sma_aktif." . $field . " LIKE '%" . $value . "%' ";
                    } else {
                        $where .= "OR " . "siswa_sma_aktif." . $field . " LIKE '%" . $value . "%' ";
                    }
                    $iterasi++;
                }
            ...
            foreach ($result->result() as $data) {
                ...
                foreach ($fields as $field) {
                    if ($field == 'nama_lengkap') {
                        ...
                        $data_field = ($data->$field)?base_url('uploads/siswa_sma_aktif/') .$data->$field:$data->$field;
                    } else {
...
/* End of file Model_siswa_sma_aktif.php */
/* Location: ./application/models/Model_siswa_sma_aktif.php */