...
class Notification_test extends MY_Controller
{
  ...
  public function index()
  {
    ...
    if (!$data_json) {
      ...
      if ($data_json['brivaNo']) {
        ...
        $get_transaksi = $this->mymodel->getbywhere("transaksi", array("va_number" => $data_json['brivaNo'], "status_transaksi" => 0), "row");
        if ($get_transaksi) {
        ...
        $get_transaksi_spp = $this->mymodel->getbywhere("transaksi_spp", array("va_number" => $data_json['brivaNo']), "row");
        if($get_transaksi_spp){
          ...
          $data_transaksi = array(
            ...
            "file_kwitansi" => $data_json['brivaNo'] . '-' . str_replace(' ', '_', $get_transaksi_spp->user_name) . '.pdf'
          );

          //UPDATE TRANSAKSI SPP
          $transaksi = $this->mymodel->update("transaksi_spp", $data_transaksi, "va_number", $data_json['brivaNo']);
