...
class Create_tagihan_noslip extends REST_Controller
{
  ...
  public function index_post()
  {
    ...
    // Fix: identitas siswa diambil dari baris transaksi (server-side),
    // bukan dipercaya mentah-mentah dari POST. Kalau POST id_siswa_aktif
    // tidak match dengan transaksi, semua validasi (tahun lalu & tagihan
    // lain) bisa di-bypass padahal transaksi tetap di-update.
    $get_transaksi_0 = $this->mymodel->withquery("select * from transaksi_spp where id_transaksi = ". $id_trans[0] , "row");
    if (empty($get_transaksi_0)) {
      $msg = array('status' => 0, 'message' => 'Transaksi tidak ditemukan', 'data' => null, 'detail_charge' => null);
      $status = "200";
      $this->response($msg, $status);
      exit;
    }
    if ($id_siswa != $get_transaksi_0->id_siswa_aktif) {
      $msg = array('status' => 0, 'message' => 'id_siswa_aktif tidak sesuai dengan transaksi', 'data' => null, 'detail_charge' => null);
      $status = "200";
      $this->response($msg, $status);
      exit;
    }
    $id_siswa = $get_transaksi_0->id_siswa_aktif;
    ...
    foreach ($id_trans as $key => $item) {
      ...
      foreach($get_tahun_ajaran_lalu as $key => $item){
        $cek_exist_tagihan=$this->mymodel->withquery("select * from transaksi_spp where status_transaksi in(0,1) and user_name = '".str_replace("'", "", $name)."' and id_siswa_aktif = $id_siswa and id_tahun_ajaran = $item->id_tahun_ajaran", "result");
    ...
    //cek tagihan lain
    $tagihan_lain = $this->mymodel->withquery("select ts.* from transaksi_lain_".strtolower($jenjang)." ts 
    join transaksi_lain_".strtolower($jenjang)."_manajemen t on t.id = ts.id_transaksi_lain  
    where ts.status_transaksi in(0,1) and t.id_tahun_ajaran = ".$this->db->escape($id_thn_ajar)." and ts.id_siswa_aktif = ".$this->db->escape($id_siswa),"result");
    ...
    if($jenjang=='sd'){
      ...
      $code='60';
      $get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,s.nama_lengkap as nama_lengkap,s.id_siswa_smp as id_siswa from siswa_smp_aktif sa join siswa_smp s on s.id_siswa_smp = sa.id_siswa_smp join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran left join kelas_smp k on k.id_kelas_smp = sa.id_kelas where id_siswa_smp_aktif = ".$id_siswa."","row");
    }else if($jenjang=='sma'){
      $code='70';
      $get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,s.nama_lengkap as nama_lengkap,s.id_siswa_sma as id_siswa from siswa_sma_aktif sa join siswa_sma s on s.id_siswa_sma = sa.id_siswa_sma join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran left join kelas_sma k on k.id_kelas_sma = sa.id_kelas where id_siswa_sma_aktif = ".$id_siswa."","row");
    }else if($jenjang=='ft'){
      $code='70';
      $get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,s.nama_lengkap as nama_lengkap,s.id_siswa_ft as id_siswa from siswa_ft_aktif sa join siswa_ft s on s.id_siswa_ft = sa.id_siswa_ft join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran left join kelas_ft k on k.id_kelas_ft = sa.id_kelas where id_siswa_ft_aktif = ".$id_siswa."","row");
    }
    ...
    $get_last_va = $this->mymodel->withquery("select va_number from transaksi_spp where user_name = '".$name."' and status_transaksi = '2' and no_transaksi like '%".strtoupper($jenjang)."%'" , "row");
    ...
    if ($va_number == "") {
      ...
      $payment_response = $this->create_billing(ENVIRONMENT, $total_biaya, $tagihan_code,$jenjang, array("nama" => str_replace("'", "", $get_siswa->nama_lengkap), "email" => $get_siswa->email, "va_number" => $va_number));
    }
    else{
      $tagihan_code="SP".date("imy")."-".strtoupper($jenjang)."-".$va_number;
      $payment_response = $this->create_billing(ENVIRONMENT, $total_biaya, $tagihan_code,$jenjang, array("nama" => str_replace("'", "", $get_siswa->nama_lengkap), "email" => $get_siswa->email, "va_number" => $va_number));
    ...
    if(!$payment_response['virtual_account']){
      $msg = array('status' => 0, 'message'=>'Terjadi Kesalahan Ketika Pembuatan VA' ,'data'=>'', 'errResponse' => $payment_response);
      $status="200";
      $this->mymodel->insertid("error_log_bni",array("status"=>$payment_response['status'],"message"=>$payment_response['message'],"va_number"=>$va_number, "keterangan" => json_encode(array("nama" => $get_siswa->nama_lengkap, "email" => $get_siswa->email, "va_number" => $va_number, "jenjang" => $jenjang, "total_biaya" => $total_biaya, "tagihan_code" => $tagihan_code, "id_siswa" => $get_siswa->id_siswa, "id_tahun_ajaran" => $get_siswa->id_tahun_ajaran))));
      $this->response($msg,$status);
    }
    $detail_bulan="";
    //$total_biaya = 0;
    foreach ($id_trans as $key => $item) {
      ...
      //$total_biaya = $total_biaya+$get_transaksi->total_biaya;
    }
    ...
    foreach ($id_trans as $item) {
      ...
      // Kosongkan cell bulan di tabel spp_{jenjang} agar konsisten dengan status_transaksi=1.
      // Cell lama (jejak proses lain) tidak boleh menolak pelunasan push berikutnya.
      $trx_parts = explode('-', $get_transaksi->no_transaksi);
      $trx_jenjang = isset($trx_parts[1]) ? strtolower($trx_parts[1]) : '';
      $bulan_cell = strtolower(trim($get_transaksi->bulan));
      $bulan_valid = array('juli','agustus','september','oktober','november','desember','januari','februari','maret','april','mei','juni');
      if (in_array($trx_jenjang, array('sd','smp','sma','ft')) && in_array($bulan_cell, $bulan_valid) && !empty($get_transaksi->id_spp)) {
        $this->db->where(array(
          'id' => $get_transaksi->id_spp,
          'id_siswa_aktif' => $get_transaksi->id_siswa_aktif,
          'id_tahun_ajaran' => $get_transaksi->id_tahun_ajaran
        ))->update('spp_'.$trx_jenjang, array($bulan_cell => NULL));
      }
    ...
    $msg = array('status' => 1, 'message' => 'Tagihan Berhasil Dibuat', 'data' => $payment_response['virtual_account'], 'detail_charge' => $data, 'payment_response' => $payment_response);
    $status = "200";
