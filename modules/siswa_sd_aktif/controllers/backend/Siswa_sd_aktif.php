...
/**
 *| --------------------------------------------------------------------------
 *| Siswa Sd Aktif Controller
 *| --------------------------------------------------------------------------
 *| Siswa Sd Aktif site
 *|
 */
class Siswa_sd_aktif extends Admin
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_siswa_sd_aktif');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	 * show all Siswa Sd Aktifs
	 *
	 * @var $offset String
	 */
	public function index($offset = 0)
	{
		$this->is_allowed('siswa_sd_aktif_list');
		...
		if ($has_multi_filter) {
			$this->data['siswa_sd_aktifs'] = $this->model_siswa_sd_aktif->get(null, null, $this->limit_page, $offset, [], $sort, $sort_type, $multi_filters);
			$this->data['siswa_sd_aktif_counts'] = $this->model_siswa_sd_aktif->count_all(null, null, $multi_filters);
		} else {
			$this->data['siswa_sd_aktifs'] = $this->model_siswa_sd_aktif->get($filter, $field, $this->limit_page, $offset, [], $sort, $sort_type);
			$this->data['siswa_sd_aktif_counts'] = $this->model_siswa_sd_aktif->count_all($filter, $field);
		}

		$config = [
			'base_url'     => 'administrator/siswa_sd_aktif/index/',
			'total_rows'   => $has_multi_filter ? $this->model_siswa_sd_aktif->count_all(null, null, $multi_filters) : $this->model_siswa_sd_aktif->count_all($filter, $field),
		...
		$this->data['list_kelas'] = $this->mymodel->withquery("SELECT id_kelas_sd, label FROM kelas_sd ORDER BY label", "result");
		...
		$this->data['export_lengkap_cols'] = $this->model_siswa_sd_aktif->export_lengkap_columns();

		$this->template->title('Siswa Sd Aktif List');
		$this->render('backend/standart/administrator/siswa_sd_aktif/siswa_sd_aktif_list', $this->data);
	}
	...
	/**
	 * Add new siswa_sd_aktifs
	 *
	 */
	public function add()
	{
		$this->is_allowed('siswa_sd_aktif_add');

		$this->template->title('Siswa Sd Aktif New');
		$this->render('backend/standart/administrator/siswa_sd_aktif/siswa_sd_aktif_add', $this->data);
	}
	...
	public function add_raport()
	{
		...
		$this->render('backend/standart/administrator/siswa_sd_aktif/siswa_sd_aktif_add_raport', $this->data);
	}

	public function add_raport_save()
	{
		if (!$this->is_allowed('siswa_sd_aktif_add', false)) {
			echo json_encode([
				'success' => false,
		...
		$ids = $this->input->post('id_siswa_sd');
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/siswa_sd_aktif/')) {
				mkdir(FCPATH . '/uploads/siswa_sd_aktif/');
			}
			...
			if (!empty($file_raport_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/siswa_sd_aktif/' . $file_raport_name_copy
				);

				if (!is_file(FCPATH . '/uploads/siswa_sd_aktif/' . $file_raport_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_siswa_sd_aktif = $this->model_siswa_sd_aktif->change($item, $save_data);
		}


		if ($save_siswa_sd_aktif) {
			if ($this->input->post('save_type') == 'stay') {
				$this->data['success'] = true;
				$this->data['id'] 	   = $save_siswa_sd_aktif;
				$this->data['message'] = cclang('success_save_data_stay', [

					anchor('administrator/siswa_sd_aktif', ' Go back to list')
				]);
			} else {
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/siswa_sd_aktif/edit/' . $save_siswa_sd_aktif, 'Edit Siswa Ft')
					]),
					'success'
				);

				$this->data['success'] = true;
				$this->data['redirect'] = base_url('administrator/siswa_sd_aktif');
			}
		} else {
			if ($this->input->post('save_type') == 'stay') {
	...
	public function delete_file_raport_file($uuid)
	{
		...
		echo $this->delete_file([
			...
			'table_name'        => 'siswa_sd_aktif',
			'primary_key'       => 'id_siswa_sd_aktif',
			'upload_path'       => 'uploads/siswa_sd_aktif/'
		]);
	}

	/**
	 * Add New Siswa Sd Aktifs
	 *
	 * @return JSON
	 */
	public function add_save()
	{
		...
		$this->form_validation->set_rules('id_siswa_sd', 'Id Siswa Sd', 'trim|required|max_length[11]');
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'id_siswa_sd' => $this->input->post('id_siswa_sd'),
			...
			$save_siswa_sd_aktif = $this->model_siswa_sd_aktif->store($save_data);
			...
			if ($save_siswa_sd_aktif) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_siswa_sd_aktif;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/siswa_sd_aktif/edit/' . $save_siswa_sd_aktif, 'Edit Siswa Sd Aktif'),
						anchor('administrator/siswa_sd_aktif', ' Go back to list')
					]);
				} else {
					set_message(
	...
	/**
	 * Update view Siswa Sd Aktifs
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('siswa_sd_aktif_update');
		$this->model_siswa_sd_aktif->join_avaiable();
		$this->data['siswa_sd_aktif'] = $this->model_siswa_sd_aktif->find($id);

		$this->template->title('Siswa Sd Aktif Update');
		$this->render('backend/standart/administrator/siswa_sd_aktif/siswa_sd_aktif_update', $this->data);
	}

	/**
	 * Update Siswa Sd Aktifs
	 *
	 * @var $id String
	 */
	public function edit_save($id)
	{
		if (!$this->is_allowed('siswa_sd_aktif_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$this->model_siswa_sd_aktif->change($id, $save_data);
			...
			if (isset($save_data['spp_custom'])) {
				$this->db->where('id_siswa_sd_aktif', $id)->update('siswa_sd_aktif', array('spp_custom' => $save_data['spp_custom']));
			}
			if (isset($save_data['spp_type'])) {
				$this->db->where('id_siswa_sd_aktif', $id)->update('siswa_sd_aktif', array('spp_type' => $save_data['spp_type']));
			}

			// --- Restore akun ---
			$restore_siswa = $this->input->post('restore_siswa');
			if ($restore_siswa === 'restore') {
				$this->mymodel->update('siswa_sd', array('deleted_at' => NULL), 'id_siswa_sd', $this->input->post('id_siswa_sd'));
				$this->mymodel->update('siswa_sd_aktif', array('deleted_at' => NULL), 'id_siswa_sd_aktif', $id);
			}
			$restore_ortu = $this->input->post('restore_ortu');
			if ($restore_ortu === 'restore') {
				$this->mymodel->update('siswa_sd', array('deleted_at_ortu' => NULL), 'id_siswa_sd', $this->input->post('id_siswa_sd'));
				$this->mymodel->update('siswa_sd_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_sd_aktif', $id);
			}
			...
			$this->mymodel->update('siswa_sd', $save_data_siswa, 'id_siswa_sd', $this->input->post('id_siswa_sd'));
			...
			$nama_kelas = $this->mymodel->withquery(
				"SELECT label FROM kelas_sd WHERE id_kelas_sd = '".$this->input->post('id_kelas')."'",
				"row"
			)->label;
	...
	/**
	 * delete Siswa Sd Aktifs
	 *
	 * @var $id String
	 */
	public function delete($id = null)
	{
		$this->is_allowed('siswa_sd_aktif_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'siswa_sd_aktif'), 'success');
		} else {
			set_message(cclang('error_delete', 'siswa_sd_aktif'), 'error');
		}
	...
	/**
	 * View view Siswa Sd Aktifs
	 *
	 * @var $id String
	 */
	public function view($id)
	{
		$this->is_allowed('siswa_sd_aktif_view');

		$this->data['siswa_sd_aktif'] = $this->model_siswa_sd_aktif->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Siswa Sd Aktif Detail');
		$this->render('backend/standart/administrator/siswa_sd_aktif/siswa_sd_aktif_view', $this->data);
	}
	...
	{
		if (!$this->is_allowed('siswa_sd_aktif_view', false)) {
			echo json_encode(['error' => 'Unauthorized']);
			exit;
		}

		$siswa = $this->model_siswa_sd_aktif->join_avaiable()->filter_avaiable()->find($id);
	...
	{
		$siswa_sd_aktif = $this->model_siswa_sd_aktif->find($id);



		return $this->model_siswa_sd_aktif->remove($id);
	}
	...
	public function restore($id = null)
	{
		...
		$siswa = $this->model_siswa_sd_aktif->find($id);
		if (!$siswa) {
			set_message('Data siswa tidak ditemukan', 'error');
			redirect_back();
		}

		// Set deleted_at = NULL di tabel siswa_sd
		$this->mymodel->update('siswa_sd', array('deleted_at' => NULL), 'id_siswa_sd', $siswa->id_siswa_sd);
		// Set deleted_at = NULL di tabel siswa_sd_aktif
		$this->mymodel->update('siswa_sd_aktif', array('deleted_at' => NULL), 'id_siswa_sd_aktif', $id);
	...
	public function restore_ortu($id = null)
	{
		...
		// Set deleted_at_ortu = NULL di tabel siswa_sd
		$this->mymodel->update('siswa_sd', array('deleted_at_ortu' => NULL), 'id_siswa_sd', $siswa->id_siswa_sd);
		// Set deleted_at_ortu = NULL di tabel siswa_sd_aktif
		$this->mymodel->update('siswa_sd_aktif', array('deleted_at_ortu' => NULL), 'id_siswa_sd_aktif', $id);
	...
	public function alumni($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			$this->mymodel->update('siswa_sd_aktif', array('is_active' => '0'), 'id_siswa_sd_aktif', $id);
	...
	{
		$this->is_allowed('siswa_sd_aktif_export');

		//$this->model_siswa_sd_aktif->export_siswa('siswa_sd_aktif', 'siswa_sd_aktif');
		$field = $this->input->get("f");
		$inputan = $this->input->get("q");
		$where = null;
		// Export columns: fields 1..N untuk import data, kolom terakhir = id_siswa_sd_aktif (primary key untuk import).
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select sa.*, s.*, k.label as kelas, t.label as tahun_ajaran, (".$case_spp.") as spp_siswa from siswa_sd_aktif sa join kelas_sd k on sa.id_kelas = k.id_kelas_sd join tahun_ajaran t on sa.id_tahun_ajaran = t.id_tahun_ajaran join siswa_sd s on sa.id_siswa_sd = s.id_siswa_sd","result");
		}
		...
		{
			// Kolom terakhir (id_siswa_sd_aktif) → label "ID SISWA SD AKTIF"
			if ($i == count($field_search) - 1) {
				$objPHPExcel->getActiveSheet()->setCellValue($column.$rowCount, 'ID SISWA SD AKTIF');
			}
	...
	{
		if (!$this->is_allowed('siswa_sd_aktif_export', false)) {
		...
		$rows = $this->mymodel->withquery(
			"SELECT sa.id_siswa_sd_aktif AS id, CONCAT(sa.nama_lengkap, ' (', COALESCE(k.label, '-'), ')') AS text
			 FROM siswa_sd_aktif sa
			 JOIN kelas_sd k ON sa.id_kelas = k.id_kelas_sd
			 JOIN siswa_sd s ON sa.id_siswa_sd = s.id_siswa_sd
			 WHERE " . $where . "
	...
	public function export_lengkap()
	{
		...
		$all_cols = $this->model_siswa_sd_aktif->export_lengkap_columns();
		...
		if (is_array($siswa) && !in_array('all', $siswa)) {
			...
			if (!empty($ids)) {
				$where .= ' AND sa.id_siswa_sd_aktif IN (' . implode(',', $ids) . ')';
			}
		}
	...
	public function export_pdf()
	{
		...
		$this->model_siswa_sd_aktif->pdf('siswa_sd_aktif', 'siswa_sd_aktif');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_siswa_sd_aktif->find($id);
	...
	public function generate_nis()
	{
		...
		$get_data = $this->mymodel->withquery("select * from siswa_sd_aktif where id_tahun_ajaran = '".$id_tahun_ajaran."' order by nama_lengkap ASC","result");
		...
		foreach ($get_data as $key => $value) {
			...
			$cek_master_siswa = $this->mymodel->withquery("select id_siswa_sd, email, email_ms_office, email_ms_office_ortu, is_mutasi, kelas_mutasi from siswa_sd where id_siswa_sd = '".$value->id_siswa_sd."'", "row");
			if ($value->nis == "-" || empty($value->nis)) {
				...
				if(!empty($cek_master_siswa) && !empty($cek_master_siswa->email_ms_office)){
					...
					if ((int)$nis_temp[0] > 0) {
						...
						for ($i=0; $i < count($get_data); $i++) { 
							$cek = $this->mymodel->withquery("select id_siswa_sd_aktif, nis from siswa_sd_aktif where nis = '".$nis."'","result");
							if (empty($cek)) {
								break;
							}
							else{
				...
				//update siswa aktif
				$up = $this->mymodel->update("siswa_sd_aktif",$data_update, "id_siswa_sd_aktif", $value->id_siswa_sd_aktif);
				//update data siswa
				$data_update2 = array(
					"email_ms_office" => $nis."@labschoolcibubur.sch.id",
					"email_ms_office_ortu" => $nis."p@labschoolcibubur.sch.id"
				);
				$up2 = $this->mymodel->update("siswa_sd",$data_update2, "id_siswa_sd", $value->id_siswa_sd);
			}
		}
		redirect_back();		
	}
		...
		if (isset($_FILES["file_import"]["name"]) && !empty($_FILES["file_import"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					// 29=id_siswa_sd_aktif (primary key untuk import)
					...
					// Cari siswa berdasarkan id_siswa_sd_aktif (kolom 29) jika ada
					if (!empty($id_siswa_aktif_import)) {
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_sd_aktif, s.id_siswa_sd from siswa_sd_aktif sa join siswa_sd s on sa.id_siswa_sd = s.id_siswa_sd where sa.id_siswa_sd_aktif = " . $this->db->escape_str($id_siswa_aktif_import),
							"row"
						);
					}
					
					// Fallback: cari berdasarkan NIS
					if (!$get_siswa || empty($get_siswa->id_siswa_sd)) {
						$nis = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_sd_aktif, s.id_siswa_sd from siswa_sd_aktif sa join siswa_sd s on sa.id_siswa_sd = s.id_siswa_sd where sa.nis = " . $this->db->escape_str($nis),
							"row"
						);
					}
					...
					if (!$get_siswa || empty($get_siswa->id_siswa_sd)) {
						...
						$get_siswa = $this->mymodel->withquery(
							"select sa.id_siswa_sd_aktif, s.id_siswa_sd from siswa_sd_aktif sa join siswa_sd s on sa.id_siswa_sd = s.id_siswa_sd where lower(sa.nama_lengkap) = lower(" . $this->db->escape_str($nama_lower) . ")",
							"row"
						);
					}
					...
					$id_siswa_aktif = (int)$get_siswa->id_siswa_sd_aktif;
					$id_siswa_sd    = (int)$get_siswa->id_siswa_sd;
					...
					$kelas_obj   = $this->mymodel->withquery(
						"select id_kelas_sd from kelas_sd where label like '%" . $this->db->escape_str($kelas_text) . "%'",
						"row"
					);
					...
					$kelas  = (!empty($kelas_obj))   ? $kelas_obj->id_kelas_sd   : null;
					...
					// Update siswa_sd_aktif berdasarkan id_siswa_sd_aktif
					$update_siswa_aktif = $this->mymodel->update("siswa_sd_aktif", $data_siswa_aktif, "id_siswa_sd_aktif", $id_siswa_aktif);
					...
					// Update siswa_sd
					$this->mymodel->update("siswa_sd", $data_siswa, "id_siswa_sd", $id_siswa_sd);
	...
	public function import_raport_siswa(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_raport']['name'][$i])) {
                  $trans_folder = './uploads/siswa_sd_aktif/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_raport']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("siswa_sd_aktif",$data, "nis", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/siswa_sd_aktif/").$filename_no_extension.".pdf",
...
/* End of file siswa_sd_aktif.php */
/* Location: ./application/controllers/administrator/Siswa Sd Aktif.php */
