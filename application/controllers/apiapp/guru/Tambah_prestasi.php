...
class Tambah_prestasi extends REST_Controller {
    ...
    function index_post() {
      ...
      // Whitelist guru lebih longgar dari siswa (termasuk heic/webp/gif untuk
      // kompatibilitas mobile app guru yang sudah ada).
      ...
      if (!empty($_FILES['file_prestasi']['name'])) {
        ...
        if($jenjang=='sd'){
          $uploaddir = './uploads/prestasi_guru_sd/';
        }elseif($jenjang=='smp'){
          $uploaddir = './uploads/prestasi_guru_smp/';
        }elseif($jenjang=='sma'){
          $uploaddir = './uploads/prestasi_guru_sma/';
        }else{
          $uploaddir = './uploads/prestasi_guru_ft/';
        }
        $img = explode('.', $_FILES['file_prestasi']['name']);
        $extension = end($img);
      ...
      if (!empty($data)) {
        if($jenjang=='sd'){
          $insert=$this->mymodel->insertid("prestasi_guru_sd",$data);
        }elseif($jenjang=='smp'){
          $insert=$this->mymodel->insertid("prestasi_guru_smp",$data);
        }elseif($jenjang=='sma'){
          $insert=$this->mymodel->insertid("prestasi_guru_sma",$data);
        }else{
          $insert=$this->mymodel->insertid("prestasi_guru_ft",$data);
        }
        if($insert){
