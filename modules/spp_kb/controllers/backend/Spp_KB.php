...
class Spp_kb extends Admin
{
	...
	{
		$this->is_allowed('spp_kb_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['spp_kbs'] = $this->model_spp_kb->get($filter, $field, $this->limit_page, $offset);
		$this->data['spp_kb_counts'] = $this->model_spp_kb->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/spp_kb/index/',
			'total_rows'   => $this->model_spp_kb->count_all($filter, $field),
		...
		$this->template->title('Spp KB List');
		$this->render('backend/standart/administrator/spp_kb/spp_kb_list', $this->data);
	}
	...
	public function add_save()
	{
		...
		if (!$this->is_allowed('spp_kb_list', false)) {
			echo json_encode([
				'success' => false,
		...
		$id_siswa = $this->input->post('id_siswa_kb_aktif');
		if (empty($id_siswa)) {
			//get data siswa aktif per kelas tersebut
			$list_siswa = $this->mymodel->getbywhere("siswa_kb_aktif", "id_kelas", $this->input->post("id_kelas"), "result");
			$id_siswa = array();
			foreach ($list_siswa as $key => $value) {
				array_push($id_siswa, $value->id_siswa_kb_aktif);
			}
		...
		foreach ($id_siswa as $key_siswa => $value_siswa) {
			...
			$get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.* from siswa_kb_aktif sa join siswa_kb s on s.id_siswa_kb = sa.id_siswa_kb join kelas_kb k on k.id_kelas_kb = sa.id_kelas where sa.id_siswa_kb_aktif = " . (int)$value_siswa, 'row');
			if (!$get_siswa) {
			...
			if (!empty($custom_nominal_spp) && $custom_nominal_spp > 0) {
				...
				$this->mymodel->update('siswa_kb_aktif', ['spp_custom' => $biaya], 'id_siswa_kb_aktif', $value_siswa);
				$spp_custom = $biaya;
			...
			$cek_detail = $this->mymodel->getbywhere("spp_kb", array("id_siswa_aktif" => $value_siswa, "id_tahun_ajaran" => $get_tahun_ajaran_aktif->id_tahun_ajaran), "row");
			...
			if (!$cek_detail) {
				$detail_spp = array(
					...
					"id_kelas" => $get_siswa->id_kelas_kb,
				...
				if (empty($cek_detail[0]->id_kelas) || $cek_detail[0]->id_kelas != $get_siswa->id_kelas_kb) $update_spp['id_kelas'] = $get_siswa->id_kelas_kb;
				if (!empty($update_spp)) {
					$this->mymodel->update("spp_kb", $update_spp, "id", $id_detail_spp);
					$is_update = true;
				}
			}

			// Update kolom bulan di spp_kb
			foreach ($arrMonth as $in) {
				...
				if ($bln && in_array((int)$bulan[1], $bln)) {
					$this->mymodel->update("spp_kb", array($kolom_bulan => null), "id", $id_detail_spp);
				} else if (!$bln) {
					// Generate semua bulan, set null
					$this->mymodel->update("spp_kb", array($kolom_bulan => null), "id", $id_detail_spp);
				} else {
					// Bulan tidak dipilih, set '-'
					$this->mymodel->update("spp_kb", array($kolom_bulan => '-'), "id", $id_detail_spp);
				}
			}
			...
			foreach ($arrMonth as $i) {
				...
				// Filter no_transaksi LIKE '%kb%' untuk hindari konflik dengan SMP/SMA
	...
	{
		$this->is_allowed('spp_kb_update');

		$this->data['spp_kb'] = $this->model_spp_kb->find($id);

		$this->template->title('Spp sd Update');
		$this->render('backend/standart/administrator/spp_kb/spp_kb_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('spp_kb_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_spp_kb = $this->model_spp_kb->change($id, $save_data);
			...
			if ($save_spp_kb) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/spp_kb', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/spp_kb');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	public function delete($id = null)
	{
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'spp_kb'), 'success');
		} else {
			set_message(cclang('error_delete', 'spp_kb'), 'error');
		}
	...
	{
		$this->is_allowed('spp_kb_view');

		$this->data['spp_kb'] = $this->model_spp_kb->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Spp KB Detail');
		$this->render('backend/standart/administrator/spp_kb/spp_kb_view', $this->data);
	}
	...
	{
		$spp_kb = $this->model_spp_kb->find($id);
		...
		return $this->model_spp_kb->remove($id);
	}
	...
	{
		$this->is_allowed('spp_kb_export');

		//$this->model_spp_kb->export('spp_kb', 'spp_kb');
		$field = $this->input->get("f");
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_kb spp join siswa_kb_aktif s on spp.id_siswa_aktif = s.id_siswa_kb_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
			...
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_kb spp join siswa_kb_aktif s on spp.id_siswa_aktif = s.id_siswa_kb_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran where ".$where." order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
		else{
	...
	public function export_tunggakan()
	{
		...
		// =====================
		$queries = [
			'KB'  => "SELECT spp.*, s.nama_lengkap, s.nis, 'KB' as nama_unit
					FROM spp_kb spp
					JOIN siswa_kb_aktif s ON spp.id_siswa_aktif = s.id_siswa_kb_aktif
	...
	public function export_pdf()
	{
		...
		$this->model_spp_kb->pdf('spp_kb', 'spp_kb');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_spp_kb->find($id);
	...
	public function sync_spp_data(){
		$get_spp = $this->mymodel->withquery("select id as id_spp, nama, id_siswa_aktif, id_tahun_ajaran from spp_kb order by id_spp ASC","result");
		foreach ($get_spp as $key => $value) {
			...
			foreach ($get_transaksi as $key_transaksi => $value_transaksi) {
				if (!empty($value_transaksi->updated_at)) {
					...
					$this->mymodel->update("spp_kb",$data_update,"id",$value->id_spp);
		...
		redirect('administrator/spp_kb');
	}
}


/* End of file spp_kb.php */
/* Location: ./application/controllers/administrator/Spp KB.php */
