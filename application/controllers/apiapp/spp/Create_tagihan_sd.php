...
class Create_tagihan_sd extends REST_Controller
{
  ...
  public function index_post()
  {
    ...
    foreach ($id_trans as $key => $item) {
      ...
      foreach($get_tahun_ajaran_lalu as $key => $item){
        $cek_exist_tagihan=$this->mymodel->withquery("select * from transaksi_spp where status_transaksi in(0,1) and user_name = '".$name."' and id_siswa_aktif = $id_siswa ", "result");
    ...
    /* $get_transaksi = $this->mymodel->withquery("select id_transaksi, va_number, bulan from transaksi_spp where user_name = '".$name."' and status_transaksi = '1' and expired_datetime >= now() order by id_transaksi DESC" , "row");
    ...
    if ($payment_response['responseCode'] != '00') {
      $msg = array('status' => 0, 'message'=>'Terjadi Kesalahan Ketika Pembuatan VA' ,'data'=>$datas, 'res' => $payment_response);
      $status="200";
      $this->response($msg,$status);
    }
    ...
    $msg = array('status' => 1, 'message' => 'Tagihan Berhasil Dibuat', 'data' => $payment_response, 'detail_charge' => $data);
    $status = "200";
