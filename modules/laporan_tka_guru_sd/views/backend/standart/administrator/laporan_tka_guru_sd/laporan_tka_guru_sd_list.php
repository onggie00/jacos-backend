...
<section class="content-header">
   <h1>
      <?= cclang('laporan_tka_guru_sd') ?> <small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('laporan_tka_guru_sd') ?></li>
   </ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               <h3 class="box-title">
                  <i class="fa fa-list"></i> Data Laporan TKA Guru SD
                  <span class="label bg-yellow" style="margin-left:10px"><?= $laporan_tka_guru_sd_counts; ?> Data</span>
                  <?php if(!empty($_GET['tahun'])): ?>
                  <span class="filter-badge">
                     <i class="fa fa-calendar"></i> Tahun: <?= $_GET['tahun']; ?>
                     <a href="<?= base_url('administrator/laporan_tka_guru_sd'); ?>" title="Hapus Filter"><i class="fa fa-times"></i></a>
                  </span>
                  <?php endif; ?>
               </h3>
               <div class="box-tools pull-right">
                  <a class="btn btn-sm btn-filter btn-top" title="Filter Tahun" data-toggle="modal" data-target="#modal_filter_tahun">
                     <i class="fa fa-filter"></i> Filter
                  </a>
                  <?php if(!empty($_GET['tahun'])): ?>
                  <a class="btn btn-sm btn-danger btn-top" id="btn_kosongkan" title="Hapus semua data tahun <?= $_GET['tahun']; ?>">
                     <i class="fa fa-trash"></i> Kosongkan Data
                  </a>
                  <?php endif; ?>
                  <a class="btn btn-sm btn-success btn-top" title="Import Data" data-toggle="modal" data-target="#modal_import">
                     <i class="fa fa-upload"></i> Import
                  </a>
                  <a class="btn btn-sm btn-warning btn-top" title="Import Detail Nilai" data-toggle="modal" data-target="#modal_import_detail">
                     <i class="fa fa-file-excel-o"></i> Import Detail
                  </a>
                  <?php is_allowed('laporan_tka_guru_sd_export', function(){?>
                  <a class="btn btn-sm btn-info btn-top btn-export <?= empty($_GET['tahun']) ? 'btn-export-disabled' : ''; ?>" 
                     title="Export Excel" id="btn_export" 
                     href="<?= site_url('administrator/laporan_tka_guru_sd/export?q='.$this->input->get('q').'&f='.$this->input->get('f').'&tahun='.$this->input->get('tahun')); ?>">
            ...
            <div class="box-body">
               ...
               <!-- Search Form -->
               <form name="form_search" id="form_search" action="<?= base_url('administrator/laporan_tka_guru_sd/index'); ?>" method="get">
                  ...
                  <div class="col-md-5">
                     <div class="input-group">
                        ...
                        <span class="input-group-btn">
                           ...
                           <a class="btn btn-flat btn-default" href="<?= base_url('administrator/laporan_tka_guru_sd?tahun='.$this->input->get('tahun')); ?>"><i class="fa fa-undo"></i></a>
                           <?php endif; ?>
                        </span>
                     </div>
                  </div>
               ...
               <!-- Data Table -->
               <form name="form_laporan_tka_guru_sd" id="form_laporan_tka_guru_sd" action="">
               <input type="hidden" name="tahun" value="<?= $this->input->get('tahun'); ?>">
               ...
               <table class="table table-bordered table-striped table-hover">
                  ...
                  <tbody>
                  <?php if($laporan_tka_guru_sd_counts > 0): ?>
                     <?php foreach($laporan_tka_guru_sds as $row): ?>
                     <tr>
                        <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $row->id_laporan; ?>"></td>
                        <td><?= $row->guru_sd_nama_lengkap ?? '-'; ?></td>
                        <td><?= _ent($row->npp ?? '-'); ?></td>
                        ...
                        <td style="white-space:nowrap">
                           <a target="_blank" href="<?= site_url('apiapp/export_tka_guru?tipe_guru=guru_sd&id='.$row->id_laporan); ?>" class="btn btn-action btn-action-pdf btn-xs"><i class="fa fa-file-pdf-o"></i> PDF</a>
                           <?php is_allowed('laporan_tka_guru_sd_view', function() use ($row){?>
                           ...
                           <?php }) ?>
                           <?php is_allowed('laporan_tka_guru_sd_update', function() use ($row){?>
                           <a href="<?= site_url('administrator/laporan_tka_guru_sd/edit/'.$row->id_laporan); ?>" class="btn btn-action btn-action-edit btn-xs"><i class="fa fa-edit"></i> Edit</a>
                           <?php }) ?>
                           <?php is_allowed('laporan_tka_guru_sd_delete', function() use ($row){?>
                           <a href="javascript:void(0);" data-href="<?= site_url('administrator/laporan_tka_guru_sd/delete/'.$row->id_laporan); ?>" class="btn btn-action btn-action-delete btn-xs remove-data"><i class="fa fa-trash"></i></a>
                           <?php }) ?>
                        </td>
                     </tr>
                     <?php endforeach; ?>
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/laporan_tka_guru_sd/import'); ?>" method="post" enctype="multipart/form-data">
            <div class="modal-body">
               <div class="form-group">
                  <label>Upload File Excel</label>
                  <input type="file" class="form-control" name="file_upload" required accept=".xlsx,.xls">
                  <small class="help-block">Format: .xlsx atau .xls. Pastikan sesuai template export.</small>
               </div>
               <div class="form-group">
                  <label>Tahun</label>
                  <input type="text" class="form-control" name="tahun" placeholder="Contoh: <?= date('Y') ?>" required>
                  <small class="help-block">Tahun ajaran untuk data yang diimport.</small>
               </div>
            </div>
            <div class="modal-footer">
               <button type="button" class="btn btn-default btn-flat" data-dismiss="modal">Batal</button>
      ...
      <div class="modal-content">
         ...
         <form id="formImportDetail" enctype="multipart/form-data">
            <div class="modal-body">
               <div class="form-group">
                  <label>Upload File Excel</label>
                  <input type="file" class="form-control" name="file_import_detail" id="file_import_detail" accept=".xlsx,.xls" required>
                  <small class="help-block">Format: .xlsx atau .xls. Kolom header harus mengandung No-1, No-2, dst.</small>
               </div>
               <div class="form-group">
                  <label>Tahun</label>
                  <input type="text" class="form-control" name="tahun_detail" id="tahun_detail" placeholder="Contoh: <?= date('Y') ?>" required>
                  <small class="help-block">Tahun ajaran untuk data yang diimport.</small>
               </div>
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         ...
         <div class="modal-body">
            ...
            <div id="detailNilaiEmpty" style="display:none" class="text-center" style="padding:30px">
               <i class="fa fa-inbox" style="font-size:40px;color:#ddd"></i><br>
               <span class="text-muted">Belum ada data detail nilai untuk laporan ini.</span>
            </div>
         </div>
         <div class="modal-footer">
...
$(document).ready(function(){
   ...
   $('#formImportDetail').on('submit', function(e){
      ...
      $.ajax({
         url: BASE_URL + '/administrator/laporan_tka_guru_sd/import_detail',
         type: 'POST',
         data: formData,
         processData: false,
         contentType: false,
         ...
         error: function(){
            result.show().html('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Terjadi kesalahan server.</div>');
         },
         complete: function(){
            btn.prop('disabled', false).html('<i class="fa fa-upload"></i> Import Detail');
         }
      });
   });
   ...
   $(document).on('click', '.btn-detail-nilai', function(){
      ...
      $.ajax({
         url: BASE_URL + '/administrator/laporan_tka_guru_sd/detail_nilai/' + id,
         type: 'GET',
         dataType: 'json',
         success: function(res){
            ...
            if(res.success && res.details && res.details.length > 0){
               ...
               html += '<thead><tr><th width="50">No</th><th width="100">No Urut</th><th>Kategori</th><th>Indikator Soal</th><th width="100">Keterangan</th></tr></thead>';
               html += '<tbody>';
               var no = 0;
               for(var i = 0; i < res.details.length; i++){
                  ...
                  html += '<td class="text-center">' + (d.no_urut || '-') + '</td>';
                  html += '<td><span class="label label-primary">' + (d.judul_kategori || '-') + '</span></td>';
                  html += '<td>' + (d.soal || '-') + '</td>';
               ...
               $('#detailNilaiEmpty').show();
            }
         },
         error: function(){
            $('#detailNilaiLoading').hide();
            $('#detailNilaiContent').html('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Gagal memuat data.</div>').show();
         }
      });
   });
   ...
   $('#btn_apply_filter').on('click', function() {
      ...
      var url = '<?= base_url('administrator/laporan_tka_guru_sd'); ?>';
      if (tahun) {
         url += '?tahun=' + tahun;
      }
   ...
   if (!currentTahun) {
      $('#btn_export').addClass('btn-export-disabled');
   }
   ...
   $('#btn_export').on('click', function(e) {
      if (!currentTahun) {
         ...
         swal({
            title: "Perhatian",
            text: "Pilih filter tahun terlebih dahulu sebelum export.",
            type: "warning",
   ...
   $('#btn_kosongkan').on('click', function() {
      var tahun = currentTahun;
      swal({
         title: "Hapus Semua Data " + tahun + "?",
         ...
         if (isConfirm) {
            var form = $('<form>', {
               method: 'POST',
               action: '<?= base_url('administrator/laporan_tka_guru_sd/delete_by_tahun'); ?>'
            });
            form.append($('<input>', {type: 'hidden', name: 'tahun', value: tahun}));
   ...
   $('#apply').click(function(){
      ...
      if (bulk == 'delete') {
         ...
         swal({
            ...
            if (isConfirm) {
               $('#form_laporan_tka_guru_sd').attr('action', '<?= base_url('administrator/laporan_tka_guru_sd/delete'); ?>').submit();
            }
         });
      }
      return false;
