...
<section class="content-header">
   <h1>
      <?= cclang('guru_smp') ?><small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('guru_smp') ?></li>
   </ol>
</section>
<!-- Main content -->
            ...
            <div class="box-body ">
               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('guru_smp_add', function(){?>
                        <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('guru_smp')]); ?>  (Ctrl+a)" href="<?=  site_url('administrator/guru_smp/add'); ?>"><i class="fa fa-plus-square-o" ></i> <?= cclang('add_new_button', [cclang('guru_smp')]); ?></a>
                        <a style="display:none" class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('guru_smp')]); ?>  (Ctrl+a)" href="<?=  site_url('administrator/guru_smp/add_slip_gaji'); ?>"><i class="fa fa-plus-square-o" ></i> Tambah Slip Gaji</a>
                        ...
                        <?php }) ?>
                        <?php is_allowed('guru_smp_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('guru_smp') ?>']); ?>" href="<?= site_url('administrator/guru_smp/export'); ?>"><i class="fa fa-file-excel-o" ></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('guru_smp_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('guru_smp') ?>']); ?>" href="<?= site_url('administrator/guru_smp/export_pdf'); ?>"><i class="fa fa-file-pdf-o" ></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                     </div>
                     ...
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username"><?= cclang('guru_smp') ?></h3>
                     <h5 class="widget-user-desc"><?= cclang('list_all', [cclang('guru_smp')]); ?>  <i class="label bg-yellow"><?= $guru_smp_counts; ?>  <?= cclang('items'); ?></i></h5>
                  </div>

                  <form name="form_guru_smp" id="form_guru_smp" action="<?= base_url('administrator/guru_smp/index'); ?>">
                  ...
                  <table class="table table-bordered table-striped dataTable">
                     ...
                     <tbody id="tbody_guru_smp">
                     <?php foreach($guru_smps as $guru_smp): ?>
                        <tr>
                                                       <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $guru_smp->id_guru; ?>">
                           </td>
                           <td width="200">
                           
                           <?php is_allowed('guru_smp_view', function() use ($guru_smp){?>
                              <a href="<?= site_url('administrator/guru_smp/view/' . $guru_smp->id_guru); ?>" class="btn btn-success"><i class="fa fa-eye"></i> </a>
                           <?php }) ?>
                           <?php is_allowed('guru_smp_update', function() use ($guru_smp){?>
                              <a href="<?= site_url('administrator/guru_smp/edit/' . $guru_smp->id_guru); ?>" class="btn btn-info"><i class="fa fa-edit "></i> </a>
                           <?php }) ?> 
                           <?php is_allowed('guru_smp_delete', function() use ($guru_smp){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/guru_smp/delete/' . $guru_smp->id_guru); ?>" class="btn btn-danger remove-data"><i class="fa fa-close"></i> </a>
                           <?php }) ?>
                           <a style="margin-top:5px;" class="btn btn-primary btn_ubah_password" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $guru_smp->id_guru; ?>" data-email="<?= $guru_smp->email_ms_office; ?>"><i class="fa fa-key"></i >Ubah Password</a>

                           </td>
                           <td><?= _ent($guru_smp->nama_lengkap); ?></td> 
                           <td><?= _ent($guru_smp->nik); ?></td> 
                           <td><?= _ent($guru_smp->nuptk); ?></td> 
                           <td><?= _ent($guru_smp->npp); ?></td> 
                           <td><?= _ent($guru_smp->npwp); ?></td> 
                           <td><?= _ent($guru_smp->alamat); ?></td> 
                           <td><?= _ent($guru_smp->agama); ?></td> 
                           <td><?= _ent($guru_smp->jenis_kelamin); ?></td> 
                           <td><?= _ent($guru_smp->status_menikah); ?></td> 
                           <td><?= _ent($guru_smp->jumlah_anak); ?></td> 
                           <td><?= _ent($guru_smp->no_telp); ?></td> 
                           <td><?= _ent($guru_smp->email); ?></td> 
                           <td><?php if  ($guru_smp->id_posisi) {

                              echo anchor('administrator/posisi_guru/view/'.$guru_smp->id_posisi.'?popup=show', $guru_smp->posisi_guru_nama_posisi, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($guru_smp->satuan_pendidikan); ?></td> 
                           <td><?= _ent($guru_smp->unit); ?></td> 
                           <td><?php if  ($guru_smp->id_mapel) {

                              echo anchor('administrator/mata_pelajaran_smp/view/'.$guru_smp->id_mapel.'?popup=show', $guru_smp->mata_pelajaran_smp_nama_mapel, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($guru_smp->status_kepegawaian); ?></td> 
                           <td><?= _ent($guru_smp->informasi_kepala_pimpinan); ?></td> 
                           <td><?= _ent($guru_smp->emp_code); ?></td> 
                           <td><?= _ent($guru_smp->token); ?></td> 
                           <td><?= _ent($guru_smp->token_expired); ?></td> 
                           <td><?= _ent($guru_smp->email_ms_office); ?></td> 
                           <td>
                              <?php if (!empty($guru_smp->foto_profil)): ?>
                                ...
                                <?php else: ?>
                                  <a href="<?= BASE_URL . 'uploads/guru_smp/' . $guru_smp->foto_profil; ?>">
                                   <img src="<?= get_icon_file($guru_smp->foto_profil); ?>" class="image-responsive image-icon" alt="image guru_smp" title="foto_profil <?= $guru_smp->foto_profil; ?>" width="40px"> 
                                 </a>
                                <?php endif; ?>
                              <?php endif; ?>
                           </td>
                            
                           <td><?= _ent($guru_smp->no_kk); ?></td> 
                           <td><?= _ent($guru_smp->tempat_lahir); ?></td> 
                           <td><?= _ent($guru_smp->tgl_lahir); ?></td> 
                           
                           </tr>
                      <?php endforeach; ?>
                      <?php if ($guru_smp_counts == 0) :?>
                         <tr>
                           <td colspan="100">
                           Guru Smp data is not available
                           </td>
                         </tr>
                      <?php endif; ?>
                     </tbody>
               ...
               <div class="row">
                  <div class="col-md-8">
                     ...
                     <div class="col-sm-1 padd-left-0 ">
                        <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= base_url('administrator/guru_smp');?>" title="<?= cclang('reset_filter'); ?>">
...
<div class="modal fade" id="modal_add_new" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_smp/import/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
               ...
               <div class="form-group">
                  ...
                  <div class="col-xs-8">
                     ...
                     <span>*Silahkan isikan excel sesuai dengan format yang tersedia, berikut contoh format excel
                        <a class="text-danger" href="<?= base_url('uploads/example/format_guru_smp.xlsx'); ?>">download disini</a> </span>
                  </div>
               </div>
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_smp/upload_slip_gaji/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
                  <label class="control-label col-xs-3">File</label>
                  <div class="col-xs-8">
...
<div class="modal fade" id="modal_update_password" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_smp/update_password/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
...
<script>
  $(document).ready(function(){
    ...
    $('#apply').click(function(){
      ...
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_guru_smp').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/guru_smp/delete?' + serialize_bulk;      
            }
          });
   ...
   $('.btn_ubah_password').click(function(){
      ...
      $.ajax({
         ...
         success: function(res) {
            ...
            $('#reset_password').attr('href', `<?php echo base_url('administrator/guru_smp/reset_password'); ?>?user_microsoft_id=${res.id}`);
