...
class Status_daftar_ulang_sd extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		if ($has_multi_filter) {
			$this->data['status_daftar_ulang_sds'] = $this->model_status_daftar_ulang_sd->get(null, null, $this->limit_page, $offset, array(), $sort, $sort_type, $multi_filters);
			$this->data['status_daftar_ulang_sd_counts'] = $this->model_status_daftar_ulang_sd->count_all(null, null, $multi_filters);
		} else {
			$this->data['status_daftar_ulang_sds'] = $this->model_status_daftar_ulang_sd->get($filter, $field, $this->limit_page, $offset, array(), $sort, $sort_type);
		...
		$config = [
			...
			'total_rows'   => $has_multi_filter ? $this->model_status_daftar_ulang_sd->count_all(null, null, $multi_filters) : $this->model_status_daftar_ulang_sd->count_all($filter, $field),
		...
		$this->data['list_tahun_ajaran'] = $this->mymodel->withquery("SELECT DISTINCT tahun_ajaran FROM siswa_sd WHERE deleted_at IS NULL AND tahun_ajaran IS NOT NULL AND tahun_ajaran <> '' ORDER BY tahun_ajaran DESC", "result");
		$this->data['list_gelombang']    = $this->mymodel->withquery("SELECT DISTINCT gelombang FROM siswa_sd WHERE deleted_at IS NULL AND gelombang IS NOT NULL ORDER BY gelombang ASC", "result");
	...
	public function edit($id, $date = null)
	{
		...
		foreach ($get_setting as $key => $item) {
			...
			if ($item->name_setting == 'bri_institution_code_close' && $this->data['status_daftar_ulang_sd']->tipe_tagihan == "fixed") {
				$institutionCode = $item->value;
			}
			else if ($item->name_setting == 'bri_institution_code' && $this->data['status_daftar_ulang_sd']->tipe_tagihan == "open") {
				$institutionCode = $item->value;
			}
		}
	...
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('status') == 1) {
				$no_transaksi = "LDUI-SD-" . date("YmdHi") . "-" . $get_daftar_ulang->id_siswa_sd;
				$this->mymodel->update("status_daftar_ulang_sd", array("tgl_aktivasi" => date("Y-m-d H:i:s")), "id_daftar_ulang", $id);
				...
				if ($customPayment != '0' && $customPayment != '') {
					...
					$this->mymodel->update("status_daftar_ulang_sd", array("custom_payment" => $customPayment), "id_daftar_ulang", $id);
	...
	public function update_status_new($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			$this->mymodel->update('status_daftar_ulang_sd', ['status' => '1', 'tgl_aktivasi' => $now], 'id_daftar_ulang', $id);
			...
			$this->mymodel->update("siswa_sd", [
				"va_number_bri" => $full_va,
				"no_transaksi" => $no_transaksi
			], "id_siswa_sd", $get_daftar_ulang->id_siswa_sd);

			$this->mymodel->update("status_daftar_ulang_sd", [
				"va_bri" => $full_va,
				"slip_pembayaran" => $no_transaksi . '-' . $get_siswa->nama_lengkap . '.pdf'
			], "id_daftar_ulang", $id);
			} //end if ($bank_aktif != 'BNI')
