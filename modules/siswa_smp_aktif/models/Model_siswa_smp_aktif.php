...
class Model_siswa_smp_aktif extends MY_Model {

    private $primary_key    = 'id_siswa_smp_aktif';
    private $table_name     = 'siswa_smp_aktif';
    private $field_search   = ['nama_lengkap', 'nis', 'id_kelas', 'id_siswa_smp', 'id_tahun_ajaran', 'kewarganegaraan', 'nik', 'golongan_darah', 'telp', 'pendidikan_ayah', 'pendidikan_ibu', 'penghasilan_ayah', 'penghasilan_ibu', 'tgl_lahir_ayah', 'tgl_lahir_ibu', 'spp_custom', 'spp_type', 'acc_ujian'];
    private $field_search_join   = ['kelas_smp.label','tahun_ajaran.label'];
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_smp_aktif.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_smp_aktif.".$field . " LIKE '%" . $q . "%' ";
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $sort_by = (empty($sort)) ? 'id_siswa_smp_aktif' : $sort;
            $sort_type = (empty($sort_type)) ? 'desc' : $sort_type;
            $this->db->order_by('siswa_smp_aktif.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_smp', 'siswa_smp.id_siswa_smp = siswa_smp_aktif.id_siswa_smp', 'LEFT');
        $this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun_ajaran = siswa_smp_aktif.id_tahun_ajaran', 'LEFT');

        $this->db->select('siswa_smp_aktif.*,
        kelas_smp.label as kelas_smp_label,
        siswa_smp.nama_lengkap as siswa_smp_nama_lengkap,
        tahun_ajaran.label as tahun_ajaran_label,
        email_ms_office,
        email_ms_office_ortu,
        foto_profil,
        siswa_smp.deleted_at as deleted_at_siswa,
        siswa_smp.deleted_at_ortu as deleted_at_ortu
        ');
    ...
    public function join_avaiable_export() {
        ...
        $this->db->select('siswa_smp_aktif.nama_lengkap,
        kelas_smp.label as kelas,
        tahun_ajaran.label as tahun_ajaran,
        siswa_smp.email,
        siswa_smp.email_ms_office,
        siswa_smp.email_ms_office_ortu,
        siswa_smp.notelp_ibu,
        siswa_smp.notelp_ayah,
        siswa_smp.nisn,
        siswa_smp_aktif.file_raport,
        siswa_smp_aktif.kewarganegaraan,
        siswa_smp_aktif.nik,
        siswa_smp_aktif.golongan_darah,
        siswa_smp_aktif.telp,
        siswa_smp_aktif.pendidikan_ayah,
        siswa_smp_aktif.pendidikan_ibu,
        siswa_smp_aktif.penghasilan_ayah,
        siswa_smp_aktif.penghasilan_ibu,
        siswa_smp_aktif.tgl_lahir_ayah,
        siswa_smp_aktif.tgl_lahir_ibu,
        siswa_smp_aktif.spp_custom
        ');
     ...
     * @param array $filters [['field'=>'nama_lengkap','operator'=>'contains','value'=>'Ahmad'], ...]
     * @return string|null
     */
    private function _build_multi_where($filters)
    {
        ...
        $field_map = [
            'nama_lengkap'   => 'siswa_smp_aktif.nama_lengkap',
            'nis'            => 'siswa_smp_aktif.nis',
            'kelas'          => 'kelas_smp.label',
            'tahun_ajaran'   => 'tahun_ajaran.label',
            'spp_type'       => 'siswa_smp_aktif.spp_type',
            'acc_ujian'      => 'siswa_smp_aktif.acc_ujian',
            'is_active'      => 'siswa_smp_aktif.is_active',
            'spp_custom'     => 'siswa_smp_aktif.spp_custom',
            'nik'            => 'siswa_smp_aktif.nik',
            'kewarganegaraan'=> 'siswa_smp_aktif.kewarganegaraan',
            'nomor_peserta_ujian' => 'siswa_smp_aktif.nomor_peserta_ujian',
        ];
        ...
        foreach ($filters as $filter) {
            ...
            $db_field = isset($field_map[$field]) ? $field_map[$field] : 'siswa_smp_aktif.' . $field;
        ...
        public function export_siswa($table, $subject = 'file',$value=null,$col=null)
        {
            ...
            if (empty($field)) {
                foreach ($this->field_search as $field) {
                    if ($iterasi == 1) {
                        $where .= "siswa_smp_aktif." . $field . " LIKE '%" . $value . "%' ";
                    } else {
                        $where .= "OR " . "siswa_smp_aktif." . $field . " LIKE '%" . $value . "%' ";
                    }
                    $iterasi++;
                }
            ...
            foreach ($result->result() as $data) {
                ...
                foreach ($fields as $field) {
                    if ($field == 'nama_lengkap') {
                        ...
                        $data_field = ($data->$field)?base_url('uploads/siswa_smp_aktif/') .$data->$field:$data->$field;
                    } else {
...
/* End of file Model_siswa_smp_aktif.php */
/* Location: ./application/models/Model_siswa_smp_aktif.php */