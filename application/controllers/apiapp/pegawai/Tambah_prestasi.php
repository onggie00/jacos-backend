...
class Tambah_prestasi extends REST_Controller {
    ...
    function index_post() {
      ...
      // id_siswa diganti id_pegawai). Tabel prestasi_pegawai single — tanpa jenjang.
      $required = array(
      ...
      // Whitelist pegawai mengikuti guru (termasuk heic/webp/gif untuk
      ...
      if (!empty($_FILES['file_prestasi']['name'])) {
        ...
        $uploaddir = './uploads/prestasi_pegawai/';
        $img = explode('.', $_FILES['file_prestasi']['name']);
        $extension = end($img);
      ...
      if (!empty($data)) {
        $insert=$this->mymodel->insertid("prestasi_pegawai",$data);
        if($insert){
