...
<section class="content-header">
  <h1>
    Manajemen Tagihan SMA <small>Daftar Semua Tagihan</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active">Manajemen Tagihan SMA</li>
  </ol>
</section>
    ...
    <div class="col-md-12">
      <div class="infobox-row">
        ...
        <div class="infobox-card bg-infobox-purple">
          ...
          <div class="infobox-value"><?= number_format($transaksi_lain_sma_manajemen_counts, 0, ',', '.'); ?></div>
      ...
      <div class="box box-warning">
        <div class="box-header with-border">
          ...
          <div class="box-tools pull-right">
            <?php is_allowed('transaksi_lain_sma_manajemen_add', function(){ ?>
            <a class="btn btn-flat btn-success btn-sm" id="btn_add_new" title="Tambah Tagihan Baru (Ctrl+a)" href="<?= site_url('administrator/transaksi_lain_sma_manajemen/add'); ?>">
        ...
        <div class="box-body">
          ...
          <!-- Filter Section -->
          <form name="form_transaksi_lain_sma_manajemen" id="form_transaksi_lain_sma_manajemen" action="<?= base_url('administrator/transaksi_lain_sma_manajemen/index'); ?>">
          <div class="filter-section">
            <div class="row">
              <div class="col-md-5 col-sm-12">
                <div class="input-group">
                  ...
                  <span class="input-group-btn">
                    ...
                    <a class="btn btn-flat btn-default" href="<?= base_url('administrator/transaksi_lain_sma_manajemen'); ?>" title="Reset Filter"><i class="fa fa-undo"></i></a>
                  </span>
                </div>
              </div>
              ...
              <div class="col-md-4 col-sm-6 text-right">
                <div class="btn-group">
                  <?php is_allowed('transaksi_lain_sma_manajemen_export', function(){ ?>
                  <a class="btn btn-flat btn-success btn-sm" title="Export Excel" id="btnExportXls" href="<?= site_url('administrator/transaksi_lain_sma_manajemen/export'); ?>">
                    <i class="fa fa-file-excel-o"></i> Export XLS
                  </a>
                  <a class="btn btn-flat btn-danger btn-sm" title="Export PDF" href="<?= site_url('administrator/transaksi_lain_sma_manajemen/export_pdf'); ?>">
          ...
          <div class="filter-badge-bar">
            ...
            <a href="<?= base_url('administrator/transaksi_lain_sma_manajemen'); ?>" class="filter-badge-clear" title="Hapus filter"><i class="fa fa-times"></i> Hapus</a>
          </div>
          <?php endif; ?>
          ...
          <div class="row" style="margin-bottom:10px;">
            ...
            <div class="col-md-4 text-right">
              <span class="item-count-badge"><?= $transaksi_lain_sma_manajemen_counts; ?> item</span>
            </div>
          </div>
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-striped table-modern dataTable">
              ...
              <tbody id="tbody_transaksi_lain_sma_manajemen">
              <?php foreach($transaksi_lain_sma_manajemens as $row): ?>
                <tr>
                  ...
                  <td><?= _ent($row->tingkatan_sma_label); ?></td>
                  <td><?= join_multi_select($row->id_kelas, 'kelas_sma', 'id_kelas_sma', 'nama_kelas'); ?></td>
                  <td><?= join_multi_select($row->id_siswa, 'siswa_sma_aktif', 'id_siswa_sma_aktif', 'nama_lengkap'); ?></td>
                  ...
                  <td>
                    <div class="action-btn-group">
                      <?php is_allowed('transaksi_lain_sma_manajemen_view', function() use ($row){ ?>
                        <a href="<?= site_url('administrator/transaksi_lain_sma_manajemen/single_pdf/' . $row->id); ?>" class="btn btn-xs btn-default" title="PDF"><i class="fa fa-file-pdf-o"></i></a>
                        <a href="<?= site_url('administrator/transaksi_lain_sma_manajemen/view/' . $row->id); ?>" class="btn btn-xs btn-info" title="Detail"><i class="fa fa-eye"></i></a>
                      <?php }) ?>
                      <?php is_allowed('transaksi_lain_sma_manajemen_update', function() use ($row){ ?>
                        <a href="<?= site_url('administrator/transaksi_lain_sma_manajemen/edit/' . $row->id); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                      <?php }) ?>
                      <?php is_allowed('transaksi_lain_sma_manajemen_delete', function() use ($row){ ?>
                        <a href="javascript:void(0);" data-href="<?= site_url('administrator/transaksi_lain_sma_manajemen/delete/' . $row->id); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                      <?php }) ?>
                    </div>
                  </td>
                </tr>
              <?php endforeach; ?>
              <?php if ($transaksi_lain_sma_manajemen_counts == 0): ?>
                <tr>
          ...
          <div class="row" style="margin-top:15px;">
            <div class="col-md-8">
              <small class="text-muted">Menampilkan <?= $transaksi_lain_sma_manajemen_counts > 0 ? ($this->uri->segment(4) + 1) : 0; ?> - <?= min($this->uri->segment(4) + $this->limit_page, $transaksi_lain_sma_manajemen_counts); ?> dari <?= $transaksi_lain_sma_manajemen_counts; ?> data</small>
            </div>
            <div class="col-md-4">
...
$(document).ready(function(){
  ...
  $('#apply').click(function(){
    var bulk = $('#bulk');
    var serialize_bulk = $('#form_transaksi_lain_sma_manajemen').serialize();
    if (bulk.val() == 'delete') {
      swal({
        title: "<?= cclang('are_you_sure'); ?>",
        text: "<?= cclang('data_to_be_deleted_can_not_be_restored'); ?>",
        ...
        if (isConfirm) {
          document.location.href = BASE_URL + '/administrator/transaksi_lain_sma_manajemen/delete?' + serialize_bulk;
        }
      });
      return false;
    } else if(bulk.val() == '') {
