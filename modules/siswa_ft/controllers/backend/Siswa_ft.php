...
class Siswa_ft extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_siswa_ft');
	...
	*/
	public function index($offset = 0)
	{
		...
		$this->data['siswa_fts'] = $this->model_siswa_ft->get($filter, $field, $this->limit_page, $offset,[],$sort,$sort_type);
		$this->data['siswa_ft_counts'] = $this->model_siswa_ft->count_all($filter, $field);
		
		// Info box data
		$this->data['total_daftar'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_ft', 'row')->cnt;
		$this->data['total_aktif'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_ft_aktif', 'row')->cnt;
		...
		if ($tahun_ajaran_aktif) {
			...
			$this->data['total_daftar_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_ft WHERE id_siswa_ft IN (SELECT id_siswa_ft FROM siswa_ft_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta . ')', 'row')->cnt;
			$this->data['total_aktif_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_ft_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta, 'row')->cnt;
		} else {
		...
		$config = [
			'base_url'     => 'administrator/siswa_ft/index/',
			'total_rows'   => $this->model_siswa_ft->count_all($filter, $field),
		...
		$this->template->title('Siswa France Track List');
		$this->render('backend/standart/administrator/siswa_ft/siswa_ft_list', $this->data);
	}
	...
	*/
	public function add()
	{
		$this->is_allowed('siswa_ft_add');

		$this->template->title('Siswa France Track New');
		$this->render('backend/standart/administrator/siswa_ft/siswa_ft_add', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('siswa_ft_add', false)) {
			echo json_encode([
				'success' => false,
		...
		$this->form_validation->set_rules('email_ms_office', 'Email Ms. Office', 'trim|required|max_length[255]');
		...
		$this->form_validation->set_rules('siswa_ft_foto_peserta_name', 'Foto Peserta', 'trim|required');
		$this->form_validation->set_rules('siswa_ft_akte_lahir_name', 'Akte Lahir', 'trim|required');
		$this->form_validation->set_rules('siswa_ft_kartu_keluarga_name', 'Kartu Keluraga', 'trim|required');
		...
		//$this->form_validation->set_rules('peminatan_ft', 'Peminatan', 'trim|required|max_length[100]');
		...
		if ($this->form_validation->run()) {
			$siswa_ft_foto_peserta_uuid = $this->input->post('siswa_ft_foto_peserta_uuid');
			$siswa_ft_foto_peserta_name = $this->input->post('siswa_ft_foto_peserta_name');
			$siswa_ft_akte_lahir_uuid = $this->input->post('siswa_ft_akte_lahir_uuid');
			$siswa_ft_akte_lahir_name = $this->input->post('siswa_ft_akte_lahir_name');
			$siswa_ft_kartu_keluarga_uuid = $this->input->post('siswa_ft_kartu_keluarga_uuid');
			$siswa_ft_kartu_keluarga_name = $this->input->post('siswa_ft_kartu_keluarga_name');
			...
			$save_data = [
				...
				//'peminatan_ft' => $this->input->post('peminatan_ft'),
				'status_lulus' => $this->input->post('status_lulus'),
				'tahun_ajaran' => $this->input->post('tahun_ajaran'),
				'gelombang' => $this->input->post('gelombang'),
			];

			if (!is_dir(FCPATH . '/uploads/siswa_ft/')) {
				mkdir(FCPATH . '/uploads/siswa_ft/');
			}

			if (!empty($siswa_ft_foto_peserta_name)) {
				$siswa_ft_foto_peserta_name_copy = date('YmdHis') . '-' . $siswa_ft_foto_peserta_name;

				rename(FCPATH . 'uploads/tmp/' . $siswa_ft_foto_peserta_uuid . '/' . $siswa_ft_foto_peserta_name, 
						FCPATH . 'uploads/siswa_ft/' . $siswa_ft_foto_peserta_name_copy);

				// if (!is_file(FCPATH . '/uploads/siswa_ft/' . $siswa_ft_foto_peserta_name_copy)) {
				// 	echo json_encode([
				// 		'success' => false,
				...
				$save_data['foto_peserta'] = $siswa_ft_foto_peserta_name_copy;
			}
		
			if (!empty($siswa_ft_akte_lahir_name)) {
				$siswa_ft_akte_lahir_name_copy = date('YmdHis') . '-' . $siswa_ft_akte_lahir_name;

				rename(FCPATH . 'uploads/tmp/' . $siswa_ft_akte_lahir_uuid . '/' . $siswa_ft_akte_lahir_name, 
						FCPATH . 'uploads/siswa_ft/' . $siswa_ft_akte_lahir_name_copy);

				// if (!is_file(FCPATH . '/uploads/siswa_ft/' . $siswa_ft_akte_lahir_name_copy)) {
				// 	echo json_encode([
				// 		'success' => false,
				...
				$save_data['akte_lahir'] = $siswa_ft_akte_lahir_name_copy;
			}
		
			if (!empty($siswa_ft_kartu_keluarga_name)) {
				$siswa_ft_kartu_keluarga_name_copy = date('YmdHis') . '-' . $siswa_ft_kartu_keluarga_name;

				rename(FCPATH . 'uploads/tmp/' . $siswa_ft_kartu_keluarga_uuid . '/' . $siswa_ft_kartu_keluarga_name, 
						FCPATH . 'uploads/siswa_ft/' . $siswa_ft_kartu_keluarga_name_copy);

				// if (!is_file(FCPATH . '/uploads/siswa_ft/' . $siswa_ft_kartu_keluarga_name_copy)) {
				// 	echo json_encode([
				// 		'success' => false,
				...
				$save_data['kartu_keluarga'] = $siswa_ft_kartu_keluarga_name_copy;
			}
		
			
			$save_siswa_ft = $this->model_siswa_ft->store($save_data);
			if ($save_siswa_ft) {
				//CREATE TRANSAKSI LUNAS
				//get biaya pendaftaran
				$no_transaksi = "LI-FT-" . date("Ymd") . "-" . $save_siswa_ft;
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran FT a.n " . strtoupper($this->input->post('nama_lengkap')),
				...
				$this->mymodel->update("siswa_ft", array("no_transaksi" => $no_transaksi), "id_siswa_ft", $save_siswa_ft);
			}
			// dd($save_data);
			if ($save_siswa_ft) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_siswa_ft;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/siswa_ft/edit/' . $save_siswa_ft, 'Edit Siswa Ft'),
						anchor('administrator/siswa_ft', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/siswa_ft');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Siswa Fts
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('siswa_ft_update');

		$this->data['siswa_ft'] = $this->model_siswa_ft->find($id);

		$this->template->title('Siswa France Track Update');
		$this->render('backend/standart/administrator/siswa_ft/siswa_ft_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('siswa_ft_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		$this->form_validation->set_rules('siswa_ft_foto_peserta_name', 'Foto Peserta', 'trim');
		$this->form_validation->set_rules('siswa_ft_akte_lahir_name', 'Akte Lahir', 'trim');
		$this->form_validation->set_rules('siswa_ft_kartu_keluarga_name', 'Kartu Keluraga', 'trim');
		...
		//$this->form_validation->set_rules('peminatan_ft', 'Peminatan', 'trim|max_length[100]');
		$this->form_validation->set_rules('status_lulus', 'Status Lulus', 'trim');
		$this->form_validation->set_rules('tahun_ajaran', 'Tahun Ajaran', 'trim');
		$this->form_validation->set_rules('gelombang', 'Gelombang', 'trim');
		...
		if ($this->form_validation->run()) {
			...
			if (!empty($siswa_ft_foto_peserta_uuid)) {
				$siswa_ft_foto_peserta_name_copy = date('YmdHis') . '-' . $siswa_ft_foto_peserta_name;
			...
			if (!empty($siswa_ft_akte_lahir_uuid)) {
				$siswa_ft_akte_lahir_name_copy = date('YmdHis') . '-' . $siswa_ft_akte_lahir_name;
			...
			if (!empty($siswa_ft_kartu_keluarga_uuid)) {
				$siswa_ft_kartu_keluarga_name_copy = date('YmdHis') . '-' . $siswa_ft_kartu_keluarga_name;
			...
			$old_data=$this->mymodel->withquery('select * from siswa_ft where id_siswa_ft='.$id,'row');
			...
			if($old_transaksi->status_transaksi==0){
				if(empty($old_data->no_peserta) && !empty($this->input->post('no_peserta'))){
					$this->data['success'] = false;
					$this->data['message'] = 'Siswa yang tagihannya belum lunas tidak dapat menginput no peserta';
					echo json_encode($this->data);
					exit;
				}
			}
			...
			if (count($update_transaksi)) {
				$this->mymodel->update3('transaksi', $update_transaksi, array('user_email' => $old_data->email),array('no_transaksi' => 'FT'));
			}

			$save_siswa_ft = $this->model_siswa_ft->change($id, $save_data);
			$new_data=$this->mymodel->withquery('select * from siswa_ft where id_siswa_ft='.$id,'row');
			$nama_ruang = "";
			if ($save_siswa_ft) {
				...
				if($get_transaksi->status_transaksi==1){
					...
					(select count(*) from ujian_ruang_pendaftaran_detail_ft 
					where r.id_ruang_pendaftaran = ujian_ruang_pendaftaran_detail_ft.id_ruang_pendaftaran) as total_peserta_now 
					from ujian_ruang_pendaftaran_ft r order by nama_ruang ASC","result");
					foreach ($cek_ruangan as $key => $item) {
						if ($item->total_peserta_now < $item->maks_peserta) {
							...
							$cek_registered = $this->mymodel->withquery("select * from ujian_ruang_pendaftaran_detail_ft 
							...
							if(empty($cek_registered)){
								$ruangan_peserta = $this->mymodel->insertid("ruang_pendaftaran_detail_ft", $data_ruangan);
								$this->data['success'] = true;
					...
					//if($this->input->post('no_peserta')!=$old_data->no_peserta){ // cek jika ada perubahan no peserta
						...
						$jenis_pembayaran = $trx_id[0];
							if($jenis_pembayaran =='LI'){ 
								//cetak kartu peserta dan kwitansi ulang
								$this->cetak_kartu(array("tipe_siswa" => 'ft', "id_siswa" => $id));
								...
								//cetak kartu peserta sementara dan kwitansi ulang
								$this->cetak_kartu_siswa_sementara(array("tipe_siswa" => 'ft', "id_siswa" => $id));
								$this->cetak_kwitansi(
									array(
										"tipe_siswa" => 'ft', 
										"jenjang" => 'FT', 
				...
				if($new_data->nama_lengkap!=$old_data->nama_lengkap){
					$this->cetak_slip(array("id_siswa" => $id, "tipe_siswa" => "ft"));
				}
			}


			if (!empty($save_siswa_ft) && $save_data['status_lulus'] == "2") {
				if($old_data->status_lulus!=2){
					...
					$check_exist_daftar_ulang=$this->mymodel->getbywhere("status_daftar_ulang_ft","id_siswa_ft",$id,"row");
					...
					$this->mymodel->insertid("status_daftar_ulang_ft", $data_daftar_ulang);
				}
			}
					...
					//if($this->input->post('no_peserta')!=$old_data->no_peserta){ // cek jika ada perubahan no peserta
						...
						$jenis_pembayaran = $trx_id[0];
							if($jenis_pembayaran =='LI'){ //cetak kartu peserta dan kwitansi ulang
								...
								$this->cetak_kwitansi(array("tipe_siswa" => 'ft', 
															"jenjang" => 'FT', 
															"nama_ortu" => $nama_ortu, 
															"nama_lengkap" => $new_data->nama_lengkap, 
															"va_number" => $get_transaksi->va_number, 
	...
	*/
	...
	{
		$this->is_allowed('siswa_ft_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'siswa_ft'), 'success');
        } else {
            set_message(cclang('error_delete', 'siswa_ft'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('siswa_ft_view');

		$this->data['siswa_ft'] = $this->model_siswa_ft->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Siswa France Track Detail');
		$this->render('backend/standart/administrator/siswa_ft/siswa_ft_view', $this->data);
	}
	...
	public function view_no_peserta($no_peserta)
	{
		...
		$this->data['siswa_ft'] = $this->mymodel->getbywhere("siswa_ft", "no_peserta", $no_peserta, "row");
	...
	*/
	...
	{
		$siswa_ft = $this->model_siswa_ft->find($id);

		if (!empty($siswa_ft->foto_peserta)) {
			$path = FCPATH . '/uploads/siswa_ft/' . $siswa_ft->foto_peserta;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($siswa_ft->akte_lahir)) {
			$path = FCPATH . '/uploads/siswa_ft/' . $siswa_ft->akte_lahir;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($siswa_ft->kartu_keluarga)) {
			$path = FCPATH . '/uploads/siswa_ft/' . $siswa_ft->kartu_keluarga;
		...
		return $this->model_siswa_ft->remove($id);
	}
	...
	public function update_status_daftar_ulang($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			$this->mymodel->update('siswa_ft',array('status_lulus'=>$this->input->get('st')),'id_siswa_ft',$id);
			$insert=$this->mymodel->insertid("status_daftar_ulang_ft", $data_daftar_ulang);
			// dd(!$insert);
			if(!$insert){
				$this->db->trans_rollback();
	...
	{
		// $this->db->trans_begin();
		// dd($this->input->get('st'));
		$arr_id = $this->input->get('id');
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_ft", "id_siswa_ft", $id, "row");
			if($get_siswa->status_lulus==2){
				$get_status_du = $this->mymodel->getbywhere("status_daftar_ulang_ft", "id_siswa_ft", $id, "row");
			...
			if($this->input->get('st')==4){
				$cadangan_no = $this->input->get('cadangan_no');
				$this->mymodel->update('siswa_ft',array('cadangan_no'=>$cadangan_no),'id_siswa_ft',$id);
			}
			$this->mymodel->update('siswa_ft',array('status_lulus'=>$this->input->get('st')),'id_siswa_ft',$id);
		}
	...
	public function download_file($id = null){
		...
		foreach($arr_id as $id){
			$get_siswa = $this->mymodel->getbywhere("siswa_ft", "id_siswa_ft", $id, "row");
			$data[]=[
				"file_foto_path"=>FCPATH.'/uploads/siswa_ft/'.$get_siswa->foto_peserta,
				"file_foto"=>$get_siswa->foto_peserta,
				"file_kk_path"=>FCPATH.'/uploads/siswa_ft/'.$get_siswa->kartu_keluarga,
				"file_kk"=>$get_siswa->kartu_keluarga,
				"file_akte_path"=>FCPATH.'/uploads/siswa_ft/'.$get_siswa->akte_lahir,
				"file_akte"=>$get_siswa->akte_lahir,
				"nama_folder"=>$get_siswa->id_siswa_ft."_".str_replace(" ","_",$get_siswa->nama_lengkap)
			];
		}
		...
		foreach($data as $item){
		 ...
		 // Download
		 $filename = "export_ft_".rand(10,100)."_".date('YmdHis').".zip";
	...
	public function download_kartu_peserta($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_ft", "id_siswa_ft", $id, "row");
			$kartu_peserta = 'ft' . '-' . $get_siswa->no_peserta . '-' . $get_siswa->nama_lengkap . ".pdf";
		...
		// Download
		$filename = "export-kartu_ft_" . rand(10, 100) . "_" . date('YmdHis') . ".zip";
		$this->zip->download($filename);
	}
	/**
	...
	*/
	...
	{
		if (!$this->is_allowed('siswa_ft_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'table_name'        => 'siswa_ft',
            'primary_key'       => 'id_siswa_ft',
            'upload_path'       => 'uploads/siswa_ft/'
        ]);
	}
	...
	*/
	public function get_foto_peserta_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_siswa_ft',
            'upload_path'       => 'uploads/siswa_ft/',
            'delete_endpoint'   => 'administrator/siswa_ft/delete_foto_peserta_file'
        ]);
	}
	...
	*/
	public function get_akte_lahir_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_siswa_ft',
            'upload_path'       => 'uploads/siswa_ft/',
            'delete_endpoint'   => 'administrator/siswa_ft/delete_akte_lahir_file'
        ]);
	}
	...
	*/
	public function get_kartu_keluarga_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_siswa_ft',
            'upload_path'       => 'uploads/siswa_ft/',
            'delete_endpoint'   => 'administrator/siswa_ft/delete_kartu_keluarga_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('siswa_ft_export');
		$value 	= $this->input->get('q');
		$col 	= $this->input->get('f');
		// dd($col);
		$this->model_siswa_ft->export_siswa('siswa_ft', 'siswa_ft',$value,$col);
	}
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_siswa_ft->pdf('siswa_ft', 'siswa_ft');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_siswa_ft->find($id);
		...
		if (isset($_FILES["file_siswa"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					$check=$this->mymodel->withquery("select * from siswa_ft where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					if(!empty($check)){
						/* $this->db->trans_rollback();
						$this->load->library("session");
					...
					$check2=$this->mymodel->withquery("select * from siswa_ft where email_ms_office_ortu = '".$worksheet->getCellByColumnAndRow(3, $row)->getValue()."'", 'row');
					if(!empty($check2)){
						/* $this->db->trans_rollback();
						$this->load->library("session");
					...
					$data_pendaftaran = array(
						...
						"tempat_lahir" => $worksheet->getCellByColumnAndRow(21, $row)->getValue(),
						"tgl_lahir" => parse_excel_date($worksheet->getCellByColumnAndRow(22, $row)),
						"nisn" => $worksheet->getCellByColumnAndRow(23, $row)->getValue(),
						"peminatan_ft" => $worksheet->getCellByColumnAndRow(24, $row)->getValue(),
						'no_peserta' => '',
						'foto_peserta' => '',
						'akte_lahir' => '',
						'kartu_keluarga' => '',
...
//					}
					...
					$check=$this->mymodel->withquery("select * from siswa_ft where nama_lengkap = '".$worksheet->getCellByColumnAndRow(0, $row)->getValue()."'", 'row');
					if($check){
						$insertId = $this->mymodel->update("siswa_ft", $data_pendaftaran, 'id_siswa_ft', $check->id);
						$insertId = $check->id;
					}else{
						$insertId = $this->mymodel->insertid("siswa_ft", $data_pendaftaran);
					}
					...
					if(!$check){
						//CREATE TRANSAKSI LUNAS
						$no_transaksi = "LI-FT-" . date("Ymd") . "-" . $insertId;
						//get biaya pendaftaran
						$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "FRANCE TRACK", "row");
						$total_biaya = $get_biaya->nominal_pendaftaran;
						$data_transaksi = array(
							...
							"user_phone" => "",
							"description" => "Tagihan Pendaftaran FT a.n " . strtoupper($data_pendaftaran['nama_lengkap']),
							"id_biaya_pendaftaran" => $get_biaya->id_biaya_pendaftaran,
							"total_biaya" => $total_biaya,
							"status_transaksi" => "1",
							"created_at" => date("Y-m-d H:i:s"),
						...
						$id_transaksi = $this->mymodel->insertid("transaksi", $data_transaksi);
						$this->mymodel->update("siswa_ft", array("no_transaksi" => $no_transaksi), "id_siswa_ft", $insertId);
	
						$data_siswa_aktif = [
							"id_siswa_ft" => $insertId,
...
/* End of file siswa_ft.php */
/* Location: ./application/controllers/administrator/Siswa Ft.php */