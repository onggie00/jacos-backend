...
class Get_ekskul extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        if (!empty($jenjang)) {
            ...
            $data = $this->second_db->withquery("select e.* from ekskul e where e.jenjang = '".$jenjang."' order by e.nama ASC", "result");
        }
        else{
            $data = $this->second_db->withquery("select e.* from ekskul e order by e.nama ASC", "result");
        }

        if (!empty($data)) {
            foreach ($data as $key => $value) {
                $cek_member = $this->second_db->withquery("select * from ekskul_member_".$jenjang." where id_siswa = '".$id_siswa."' and id_ekskul = '".$value->id_ekskul."' and semester = '".$semester."' and tahun_ajaran = '".$tahun_ajaran."' ","row");
                if (!empty($value->foto)) {
