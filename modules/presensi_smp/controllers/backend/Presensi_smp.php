...
/**
*| --------------------------------------------------------------------------
*| Presensi Smp Controller
*| --------------------------------------------------------------------------
*| Presensi Smp site
*|
*/
class Presensi_smp extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_presensi_smp');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Presensi Smps
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('presensi_smp_list');

		$get = $this->input->get();
		$limit = 40;
		$where = $this->_build_filter_where($get);

		$data_sql = "SELECT p.*, (SELECT COUNT(*) FROM presensi_catatan_pelajaran pcp WHERE pcp.id_siswa_aktif = p.id_siswa_aktif AND pcp.jenjang = 'smp' AND pcp.tanggal_waktu >= p.tanggal_absen AND pcp.tanggal_waktu < DATE_ADD(p.tanggal_absen, INTERVAL 1 DAY)) AS jml_catatan, s.nama_lengkap, s.nis, k.id_tingkatan, k.label AS nama_kelas, i.jenis_izin FROM presensi_smp p JOIN siswa_smp_aktif s ON p.id_siswa_aktif = s.id_siswa_smp_aktif JOIN kelas_smp k ON s.id_kelas = k.id_kelas_smp LEFT JOIN izin_siswa_smp i ON p.id_izin = i.id " . $where . " ORDER BY p.tanggal_absen DESC, s.nama_lengkap ASC LIMIT " . (int)$offset . "," . (int)$limit;
		$data_presensi = $this->mymodel->withquery($data_sql, 'result');

		$count_sql = "SELECT p.id FROM presensi_smp p JOIN siswa_smp_aktif s ON p.id_siswa_aktif = s.id_siswa_smp_aktif JOIN kelas_smp k ON s.id_kelas = k.id_kelas_smp LEFT JOIN izin_siswa_smp i ON p.id_izin = i.id " . $where;
		...
		FROM presensi_smp p
		JOIN siswa_smp_aktif s ON p.id_siswa_aktif = s.id_siswa_smp_aktif
		...
		$this->data['list_tingkatan'] = $this->mymodel->withquery("SELECT * FROM tingkatan_smp ORDER BY label ASC", 'result');
		$this->data['list_kelas'] = $this->mymodel->withquery("SELECT * FROM kelas_smp ORDER BY label ASC", 'result');

		$config = array(
			'base_url' => 'administrator/presensi_smp/index',
		...
		$this->template->title('Presensi SMP List');
		$this->render('backend/standart/administrator/presensi_smp/presensi_smp_list', $this->data);
	}
	...
	private function _build_filter_where($get)
	{
		...
		if (!empty($get['id_kelas'])) {
			$where .= (empty($where) ? 'WHERE ' : ' AND ') . "k.id_kelas_smp = '" . $this->db->escape_str($get['id_kelas']) . "'";
		}
	...
	/**
	* Add new presensi_smps
	*
	*/
	public function add()
	{
		$this->is_allowed('presensi_smp_add');

		$this->template->title('Presensi SMP New');
		$this->render('backend/standart/administrator/presensi_smp/presensi_smp_add', $this->data);
	}

	/**
	* Add New Presensi Smps
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('presensi_smp_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_presensi_smp = $this->model_presensi_smp->store($save_data);
            

			if ($save_presensi_smp) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_presensi_smp;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/presensi_smp/edit/' . $save_presensi_smp, 'Edit Presensi Smp'),
						anchor('administrator/presensi_smp', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/presensi_smp');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Presensi Smps
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('presensi_smp_update');

		$this->data['presensi_smp'] = $this->model_presensi_smp->find($id);

		$this->template->title('Presensi SMP Update');
		$this->render('backend/standart/administrator/presensi_smp/presensi_smp_update', $this->data);
	}

	/**
	* Update Presensi Smps
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('presensi_smp_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$save_presensi_smp = $this->model_presensi_smp->change($id, $save_data);
	...
	/**
	* delete Presensi Smps
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('presensi_smp_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'presensi_smp'), 'success');
        } else {
            set_message(cclang('error_delete', 'presensi_smp'), 'error');
        }
		...
		/**
	* View view Presensi Smps
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('presensi_smp_view');

		$this->data['presensi_smp'] = $this->model_presensi_smp->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Presensi SMP Detail');
		$this->render('backend/standart/administrator/presensi_smp/presensi_smp_view', $this->data);
	}
	...
	*/
	...
	{
		$presensi_smp = $this->model_presensi_smp->find($id);

		
		
		return $this->model_presensi_smp->remove($id);
	}

	/**
	 * Ambil catatan pelajaran SMP dan kelompokkan per siswa serta tanggal.
	 */
	...
	private function _get_export_filter_label($id_tingkatan, $id_kelas)
	{
		...
		if (!empty($id_kelas)) {
			$filter = $this->db
				->select('label')
				->where('id_kelas_smp', $id_kelas)
				->get('kelas_smp')
				->row();
		} elseif (!empty($id_tingkatan)) {
			$filter = $this->db
				->select('label')
				->where('id_tingkatan_smp', $id_tingkatan)
				->get('tingkatan_smp')
				->row();
		}
	...
	private function _build_export_filename($start_date, $end_date, $filter_label)
	{
		...
		return 'Presensi SMP - ' . $start_date . '-' . $end_date . '-' . $filter_label . '.xls';
	}
	...
	*/
	...
	{
		$this->is_allowed('presensi_smp_export');
		...
		if ($id_tingkatan) {
			...
			$where_kelas = "WHERE id_kelas_smp = '$id_kelas'";
		}
		$get_kelas = $this->mymodel->withquery("SELECT * FROM kelas_smp $where_kelas", "result");

		$date_range = range(1, $total_day_in_month);
		$presensi_rows = $this->mymodel->withquery("SELECT * FROM presensi_smp WHERE MONTH(tanggal_absen) = '$bulan' AND YEAR(tanggal_absen) = '$tahun'", "result");
		$presensi_map = [];
		...
		if ($get_kelas) {
			foreach ($get_kelas as $key_kelas => $kelas) {
				...
				if ($kelas->id_kelas_smp) $where[] = "s.id_kelas = '$kelas->id_kelas_smp'";
				$where_clause = $where ? "WHERE " . implode(" AND ", $where) : "";

				$get_siswa = $this->mymodel->withquery("SELECT s.id_siswa_smp_aktif AS id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label AS nama_kelas FROM siswa_smp_aktif s JOIN kelas_smp k ON s.id_kelas = k.id_kelas_smp $where_clause ORDER BY k.label ASC, s.nama_lengkap ASC", "result");
	...
	public function export_periode()
	{
		...
		foreach ($get_kelas as $i => $kelas) {
			...
			$siswa = $this->mymodel->withquery("
				SELECT s.id_siswa_smp_aktif AS id_siswa, s.nama_lengkap, s.nis,
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_presensi_smp->pdf('presensi_smp', 'presensi_smp');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_presensi_smp->find($id);
	...
	{
		if (!$this->is_allowed('presensi_smp_list', false)) {
		...
		$this->load->view('backend/standart/administrator/presensi_smp/presensi_smp_detail_catatan', $this->data);
	}
	...
	public function get_siswa_terlambat()
	{
		...
		$sql = "SELECT 
				s.id_siswa_smp_aktif AS id_siswa_aktif,
				s.nama_lengkap,
				s.nis,
				k.label AS nama_kelas,
		...
		if (!empty($id_kelas)) {
			$sql .= " AND k.id_kelas_smp = " . $this->db->escape_str($id_kelas);
		}

		$sql .= " GROUP BY s.id_siswa_smp_aktif, s.nama_lengkap, s.nis, k.label
	...
	public function get_riwayat_terlambat()
	{
		...
		echo json_encode(array(
			'success' => true,
			'data' => $riwayat,
			'nama_siswa' => $this->db->select('nama_lengkap')->where('id_siswa_smp_aktif', $siswa)->get('siswa_smp_aktif')->row()->nama_lengkap
		));
	}
...
/* End of file presensi_smp.php */
/* Location: ./application/controllers/administrator/Presensi Smp.php */
