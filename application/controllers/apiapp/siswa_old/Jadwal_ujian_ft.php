...
class Jadwal_ujian_ft extends REST_Controller
{
  ...
  public function index_get()
  {
    ...
    /*$get_siswa=$this->mymodel->withquery("select sa.*,s.*,k.*,t.*,t.label as tahun_ajaran,sa.nama_lengkap as nama_lengkap from siswa_ft_aktif sa join siswa_ft s on s.id_siswa_ft = sa.id_siswa_ft join tahun_ajaran t on t.id_tahun_ajaran = sa.id_tahun_ajaran join kelas_ft k on k.id_kelas_ft = sa.id_kelas where id_siswa_ft_aktif = $id_siswa_aktif","row");
    if(!$get_siswa){
    ...
    $data = $this->mymodel->withquery(
      "select u.*,m.nama_mapel as mata_pelajaran_ft_nama_mapel,j.nama_ujian as jenis_ujian_nama_ujian from jadwal_ujian_ft u join mata_pelajaran_ft m on m.id_mapel = u.id_mapel join jenis_ujian j on j.id_jenis_ujian = u.id_jenis_ujian $where $where_and",
      "result"
    );*/
      ...
      if (!empty($get_siswa)) {
        ...
        if (!empty($get_ujian)) {
          ...
          foreach ($get_siswa as $key => $value) {
            $value->judul_ujian = $this->mymodel->withquery("select ju.nama_ujian from jenis_ujian ju join jadwal_ujian_ft j on ju.id_jenis_ujian = j.id_jenis_ujian join ujian_ruang_kelas k on j.id_tingkatan = k.id_tingkatan where k.id_tingkatan = '".$get_siswa[0]->id_tingkatan."'","row")->nama_ujian;
          }
