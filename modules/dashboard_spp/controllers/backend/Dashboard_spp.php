...
class Dashboard_spp extends Admin	
{
	...
	public function chart_spp(){
		...
		if (!empty($start_date) && !empty($end_date)) {
			...
			if($where == ""){
				$where .= "where tr.description like '%".$start_date_text."%' and (sp.kelas not like '%lulus%' and sp.kelas not like '%mutasi%') ";
			}
		}
		...
		$get_data = $this->mymodel->withquery("select (select count(distinct tr.no_transaksi) from transaksi_spp tr join spp_".strtolower($jenjang)." sp on tr.id_spp = sp.id join siswa_".$strtolower($jenjang)."_aktif sa on sp.id_siswa_aktif = sa.id_siswa_".strtolower($jenjang)."_aktif ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and (tr.status_transaksi = '0' or tr.status_transaksi = '1' or tr.status_transaksi = '2') and sa.is_active = '1') as total_spp, 
		(select count(distinct tr.no_transaksi) from transaksi_spp tr join spp_".strtolower($jenjang)." sp on tr.id_spp = sp.id join siswa_".$strtolower($jenjang)."_aktif sa on sp.id_siswa_aktif = sa.id_siswa_".strtolower($jenjang)."_aktif ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and (tr.status_transaksi = '0' or tr.status_transaksi = '1') and sa.is_active = '1') as total_spp_belum_dibayar, 
		(select count(distinct tr.no_transaksi) from transaksi_spp tr join spp_".strtolower($jenjang)." sp on tr.id_spp = sp.id join siswa_".$strtolower($jenjang)."_aktif sa on sp.id_siswa_aktif = sa.id_siswa_".strtolower($jenjang)."_aktif ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and tr.status_transaksi = '2' and sa.is_active = '1') as total_spp_lunas from siswa_".strtolower($jenjang)."_aktif s group by total_spp","result");
	...
	public function export_chart_spp(){
		...
		//export all data based on input where all field
			$iterasi = 1;
	        $get_data = $this->mymodel->withquery("select tr.*, s.nis, spp.kelas, spp.tahun_ajaran from transaksi_spp tr left join spp_".strtolower($jenjang)." spp on tr.id_spp = spp.id left join siswa_".strtolower($jenjang)."_aktif s on spp.id_siswa_aktif = s.id_siswa_".strtolower($jenjang)."_aktif  ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and (tr.status_transaksi = '0' or tr.status_transaksi = '1' or tr.status_transaksi = '2') order by spp.kelas ASC, s.nama_lengkap ASC, s.nis ASC","result");
	        $get_data2 = $this->mymodel->withquery("select tr.*, s.nis, spp.kelas, spp.tahun_ajaran from transaksi_spp tr left join spp_".strtolower($jenjang)." spp on tr.id_spp = spp.id left join siswa_".strtolower($jenjang)."_aktif s on spp.id_siswa_aktif = s.id_siswa_".strtolower($jenjang)."_aktif  ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and (tr.status_transaksi = '0' or tr.status_transaksi = '1') order by spp.kelas ASC, s.nama_lengkap ASC, s.nis ASC","result");
	        $get_data3 = $this->mymodel->withquery("select tr.*, s.nis, spp.kelas, spp.tahun_ajaran from transaksi_spp tr left join spp_".strtolower($jenjang)." spp on tr.id_spp = spp.id left join siswa_".strtolower($jenjang)."_aktif s on spp.id_siswa_aktif = s.id_siswa_".strtolower($jenjang)."_aktif  ".$where." and tr.no_transaksi like '%LISPP-".strtoupper($jenjang)."%' and (tr.status_transaksi = '2') order by spp.kelas ASC, s.nama_lengkap ASC, s.nis ASC","result");
