...
<section class="content-header">
   ...
   <ol class="breadcrumb">
      ...
      <li class="active">Anggota Ekskul FT</li>
   </ol>
</section>
         ...
         <div class="labs-card">
            ...
            <div class="labs-card__header">
               <h3 class="labs-card__title">
                  ...
                  Daftar Anggota
                  <span class="labs-badge labs-badge--default labs-ml-2"><?= $ekskul_member_ft_counts; ?> data</span>
               </h3>
               <div>
                  <?php is_allowed('ekskul_member_ft_add', function(){ ?>
                  <a class="labs-btn labs-btn--success labs-btn--sm" href="<?= site_url('administrator/ekskul_member_ft/add'); ?>" title="Tambah Anggota (Ctrl+a)">
            ...
            <div class="labs-card__body">
               ...
               <!-- INLINE FILTER -->
               <form action="<?= base_url('administrator/ekskul_member_ft/filter'); ?>" method="GET" class="labs-filter-inline">
                  <select class="form-control" name="id_ekskul[]" title="Ekstrakurikuler" id="filterEkskul">
                     <option value="">Semua Ekskul</option>
                     <?php
                        $list_ekskul = $this->mymodel->withquery("select id_ekskul, nama from ekskul where jenjang = 'ft' order by nama ASC", "result");
                  ...
                  <select class="form-control" name="id_kelas" title="Kelas" id="filterKelas">
                     ...
                     <?php
                        $list_kelas = $this->mymodel->withquery("select id_kelas_ft, label from kelas_ft order by label ASC", "result");
                        $selected_kelas = isset($_GET['id_kelas']) ? $_GET['id_kelas'] : '';
                        foreach ($list_kelas as $k) {
                            $sel = ($k->id_kelas_ft == $selected_kelas) ? 'selected' : '';
                            echo '<option value="' . $k->id_kelas_ft . '" ' . $sel . '>' . $k->label . '</option>';
                        }
                     ?>
                  </select>
                  ...
                  <a href="<?= base_url('administrator/ekskul_member_ft'); ?>" class="labs-btn labs-btn--default labs-btn--sm" title="Reset"><i class="fa fa-undo"></i></a>
               </form>
               ...
               <div class="labs-filter-active labs-mb-3">
                  ...
                  <?php
                     ...
                     if (!empty($_GET['id_kelas'])) foreach ($list_kelas as $k) if ((string) $k->id_kelas_ft === (string) $_GET['id_kelas']) $active_filters[] = 'Kelas: ' . $k->label;
               ...
               <div style="margin-bottom:12px;display:flex;gap:8px;flex-wrap:wrap">
                  <?php is_allowed('ekskul_member_ft_export', function() use ($filter_param) { ?>
                  <a class="labs-btn labs-btn--success labs-btn--sm" href="<?= site_url('administrator/ekskul_member_ft/export') . $filter_param; ?>">
                     <i class="fa fa-file-excel-o"></i> Export Pilihan Siswa
                  </a>
                  <a class="labs-btn labs-btn--warning labs-btn--sm" href="<?= site_url('administrator/ekskul_member_ft/export_list_anggota') . $filter_param; ?>">
                     <i class="fa fa-file-excel-o"></i> Export List Anggota
                  </a>
                  <a class="labs-btn labs-btn--info labs-btn--sm" href="<?= site_url('administrator/ekskul_member_ft/export_rekap_pembayaran') . $filter_param; ?>">
               ...
               <!-- TABLE -->
               <form name="form_ekskul_member_ft" id="form_ekskul_member_ft">
                        ...
                        <tr>
                           ...
                           <td>
                              <?php if (!empty($row->file_pembayaran)): ?>
                                 <?php
                                    $file_url = BASE_URL . 'uploads/ekskul_member_ft/' . $row->file_pembayaran;
                           ...
                           <td class="labs-cell-numeric">
                              <?php is_allowed('ekskul_member_ft_update', function() use ($row){ ?>
                              <a href="<?= site_url('administrator/ekskul_member_ft/edit/' . $row->id_member); ?>" class="labs-btn labs-btn--warning labs-btn--sm" title="Edit">
                                 <i class="fa fa-edit"></i>
                              </a>
                              <?php }) ?>
                              <?php is_allowed('ekskul_member_ft_delete', function() use ($row){ ?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/ekskul_member_ft/delete/' . $row->id_member); ?>" class="labs-btn labs-btn--danger labs-btn--sm remove-data" title="Hapus">
...
$(document).ready(function(){
   ...
   function convertHeicServer(url, container) {
      ...
      var convertUrl = '<?= site_url('administrator/ekskul_member_ft/convert_heic'); ?>?file=' + encodeURIComponent(filename);
      $.getJSON(convertUrl)
         .done(function(res) {
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_ekskul_member_ft').serialize();
      ...
      var urls = {
         'delete': 'administrator/ekskul_member_ft/delete?',
         'aktif': 'administrator/ekskul_member_ft/activate_member?',
         'tdk_aktif': 'administrator/ekskul_member_ft/deactivate_member?'
      };
      var labels = {
