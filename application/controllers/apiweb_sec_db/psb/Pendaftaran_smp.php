...
class Pendaftaran_smp extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_smp from calon_siswa_smp where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");

        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->second_db->getbywhere("list_sekolah_sd", "id_list_sekolah_sd",$this->post("sekolah_asal"),"row")->nama_sekolah;
          ...
          $notelp_ibu = $this->cek_notelp($this->db->escape($this->post("notelp_ibu")));
          $notelp_ayah = $this->cek_notelp($this->db->escape($this->post("notelp_ayah"))); 
          $data = array(
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select id_siswa_smp from calon_siswa_sd where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");
          if(!empty($cek_email)){
            ...
            $id_siswa = $this->second_db->insertid("siswa_smp",$data);
            if (!empty($id_siswa)) {
              ...
              $id_raport = $this->second_db->insertid("nilai_raport_smp",$data_raport);

              if ($ppsbb == "1") {
                $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","PPSBB SMP","row");
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_smp as id_siswa from calon_siswa_smp where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')))."03%' and va_number != '' order by id_siswa_smp DESC","row");
                ...
                $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","PSB SMP","row");
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_smp as id_siswa from calon_siswa_smp where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')))."06%' and va_number != '' order by id_siswa_smp DESC","row");
                if (empty($get_no_urut)) {
                  $no_urut = "0001";
                }
                else{
              ...
              if (!empty($data_transaksi)) {
                ...
                $this->second_db->update("siswa_smp", array("no_transaksi" => $no_transaksi, "va_number" => $va_number), "id_siswa_smp", $id_siswa);
