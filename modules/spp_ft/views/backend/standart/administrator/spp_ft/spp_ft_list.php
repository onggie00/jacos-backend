...
<section class="content-header">
   <h1><i class="fa fa-money"></i> <?= cclang('spp_ft') ?> <small><?= cclang('list_all'); ?></small></h1>
   <ol class="breadcrumb"><li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li><li class="active"><?= cclang('spp_ft') ?></li></ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-header with-border">
               <h3 class="box-title"><i class="fa fa-credit-card"></i> Data SPP FT <span class="label bg-yellow" style="margin-left:10px"><?= $spp_ft_counts; ?> Data</span></h3>
               <div class="box-tools pull-right">
                  ...
                  <?php is_allowed('spp_ft_export', function(){?>
                  <a class="btn btn-sm btn-action btn-action-export btn-top" href="<?= site_url('administrator/spp_ft/export') . '?' . http_build_query($_GET); ?>"><i class="fa fa-file-excel-o"></i> Export XLS</a>
                  ...
                  <a class="btn btn-sm btn-action btn-action-pdf btn-top" href="<?= site_url('administrator/spp_ft/export_pdf'); ?>"><i class="fa fa-file-pdf-o"></i> PDF</a>
                  <?php }) ?>
               </div>
            </div>
            <div class="box-body">
               <form name="form_spp_ft" id="form_spp_ft" action="<?= base_url('administrator/spp_ft/index'); ?>">
                  ...
                  <div class="col-md-4">
                     <div class="input-group">
                        ...
                        <span class="input-group-btn">
                           ...
                           <a class="btn btn-flat btn-default" href="<?= base_url('administrator/spp_ft'); ?>"><i class="fa fa-times"></i></a>
                           <?php endif; ?>
                        </span>
                     </div>
                  </div>
               ...
               <div class="table-responsive"> 
                  <table class="table table-bordered table-striped table-hover dataTable">
                     ...
                     <tbody>
                     <?php if($spp_ft_counts > 0): ?>
                     <?php foreach($spp_fts as $spp_ft): ?>
                        <tr>
                           <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $spp_ft->id; ?>"></td>
                           <td>
                              <?php if ($spp_ft->id_siswa_aktif): ?>
                                 <a href="<?= site_url('administrator/siswa_ft_aktif/view/'.$spp_ft->id_siswa_aktif.'?popup=show'); ?>" class="popup-view"><i class="fa fa-user"></i> <?= _ent($spp_ft->siswa_ft_aktif_nama_lengkap); ?></a>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center"><?= _ent($spp_ft->nis); ?></td>
                           <td style="text-align:center"><?= _ent($spp_ft->kelas); ?></td>
                           <td style="text-align:center"><?= _ent($spp_ft->tahun_ajaran); ?></td>
                           <td style="text-align:right">Rp <?= number_format($spp_ft->nominal, 0, ',', '.'); ?></td>
                           <?php 
                           ...
                           <td style="white-space:nowrap;text-align:center">
                              <?php is_allowed('spp_ft_view', function() use ($spp_ft){?>
                              <a href="<?= site_url('administrator/spp_ft/single_pdf/'.$spp_ft->id); ?>" class="btn btn-action btn-action-pdf btn-sm"><i class="fa fa-file-pdf-o"></i> PDF</a>
                              <a href="<?= site_url('administrator/spp_ft/view/'.$spp_ft->id); ?>" class="btn btn-action btn-action-view btn-sm"><i class="fa fa-eye"></i> Detail</a><br>
                              <?php }) ?>
                              <?php is_allowed('spp_ft_update', function() use ($spp_ft){?>
                              <a href="<?= site_url('administrator/spp_ft/edit/'.$spp_ft->id); ?>" class="btn btn-action btn-action-edit btn-sm"><i class="fa fa-edit"></i> Edit</a>
                              <?php }) ?>
                              <?php is_allowed('spp_ft_delete', function() use ($spp_ft){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/spp_ft/delete/'.$spp_ft->id); ?>" class="btn btn-action btn-action-delete btn-sm remove-data"><i class="fa fa-trash"></i> Hapus</a>
                              <?php }) ?>
                           </td>
                        </tr>
                     <?php endforeach; ?>
                     <?php else: ?>
                        <tr><td colspan="19" class="text-center" style="padding:30px"><i class="fa fa-inbox" style="font-size:40px;color:#ddd"></i><br><span style="color:#999">Data SPP FT belum tersedia</span></td></tr>
                     <?php endif; ?>
                     </tbody>
                  </table>
               </div>
...
<div class="modal fade" id="modal_add_new" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      <div class="modal-header" style="background:#27ae60;color:#fff"><button type="button" class="close" data-dismiss="modal" style="color:#fff">&times;</button><h4 class="modal-title"><i class="fa fa-plus-square-o"></i> Generate SPP FT</h4></div>
      <form action="<?= base_url('administrator/spp_ft/add_save/'); ?>" method="post" class="form-horizontal">
         <div class="modal-body">
            <div class="form-group"><label class="control-label col-xs-3">Siswa</label><div class="col-xs-8"><select class="form-control chosen chosen-select-deselect" name="id_siswa_ft_aktif[]" multiple><option value=""></option><?php foreach (db_get_all_data('siswa_ft_aktif') as $row): ?><option value="<?= $row->id_siswa_ft_aktif ?>"><?= $row->nama_lengkap; ?></option><?php endforeach; ?></select><small class="text-muted">Pilih siswa atau kelas (salah satu)</small></div></div>
            <div class="form-group"><label class="control-label col-xs-3">Kelas</label><div class="col-xs-8"><select class="form-control chosen chosen-select-deselect" name="id_kelas"><option value=""></option><?php foreach (db_get_all_data('kelas_ft') as $row): ?><option value="<?= $row->id_kelas_ft ?>"><?= $row->label; ?></option><?php endforeach; ?></select><small class="text-muted">Pilih siswa atau kelas (salah satu)</small></div></div>
...
<div class="modal fade" id="modal_confirm_sync" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      <div class="modal-header" style="background:#1abc9c;color:#fff"><button type="button" class="close" data-dismiss="modal" style="color:#fff">&times;</button><h4 class="modal-title"><i class="fa fa-refresh"></i> Sinkronisasi Data SPP FT</h4></div>
      <form action="<?= base_url('administrator/spp_ft/sync_spp_data/'); ?>" method="post">
...
function doExportTunggakan() {
   ...
   window.location.href='<?= site_url("administrator/spp_ft/export_tunggakan") ?>?bulan='+bln.join(',')+'&id_tahun_ajaran='+ta;
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){ var bulk=$('#bulk'); var s=$('#form_spp_ft').serialize(); if(bulk.val()=='delete'){swal({title:"<?= cclang('are_you_sure'); ?>",text:"<?= cclang('data_to_be_deleted_can_not_be_restored'); ?>",type:"warning",showCancelButton:true,confirmButtonColor:"#DD6B55",confirmButtonText:"<?= cclang('yes_delete_it'); ?>",cancelButtonText:"<?= cclang('no_cancel_plx'); ?>"},function(isConfirm){if(isConfirm)document.location.href=BASE_URL+'/administrator/spp_ft/delete?'+s;});}else if(bulk.val()==''){swal({title:"Upss",text:"<?= cclang('please_choose_bulk_action_first'); ?>",type:"warning"});} return false; });
