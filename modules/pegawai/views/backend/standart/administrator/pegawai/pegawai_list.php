               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        ...
                        <a style="display:none" class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('pegawai')]); ?>  (Ctrl+a)" href="<?=  site_url('administrator/pegawai/add_slip_gaji'); ?>"><i class="fa fa-plus-square-o" ></i> Tambah Slip Gaji</a>
                        ...
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('pegawai') ?>']); ?>" href="<?= site_url('administrator/pegawai/export'); ?>"><i class="fa fa-file-excel-o" ></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('pegawai_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('pegawai') ?>']); ?>" href="<?= site_url('administrator/pegawai/export_pdf'); ?>"><i class="fa fa-file-pdf-o" ></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                        <a class="btn btn-flat btn-success" title="Import Excel" data-toggle="modal" data-target="#modal_add_new"><i class="fa fa-file-pdf-o"></i> <?= Import ?> Pegawai</a>
                  ...
                  <table class="table table-bordered table-striped dataTable">
                     ...
                     <?php foreach($pegawais as $pegawai): ?>
                        <tr>
                                                       <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $pegawai->id_pegawai; ?>">
                           </td>
                           <td width="200">
                           
                           <?php is_allowed('pegawai_view', function() use ($pegawai){?>
                              <a href="<?= site_url('administrator/pegawai/view/' . $pegawai->id_pegawai); ?>" class="btn btn-success"><i class="fa fa-eye"></i> </a>
                           <?php }) ?>
                           <?php is_allowed('pegawai_update', function() use ($pegawai){?>
                              <a href="<?= site_url('administrator/pegawai/edit/' . $pegawai->id_pegawai); ?>" class="btn btn-info"><i class="fa fa-edit "></i> </a>
                           <?php }) ?> 
                           <?php is_allowed('pegawai_delete', function() use ($pegawai){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/pegawai/delete/' . $pegawai->id_pegawai); ?>" class="btn btn-danger remove-data"><i class="fa fa-close"></i> </a>
                           <?php }) ?>
                           <a style="margin-top:5px;" class="btn btn-primary btn_ubah_password" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $pegawai->id_pegawai; ?>" data-email="<?= $pegawai->email_ms_office; ?>"><i class="fa fa-key"></i >Ubah Password</a>

                           </td>
                           <td><?= _ent($pegawai->nama_lengkap); ?></td> 
                           <td><?= _ent($pegawai->nik); ?></td> 
                           <td><?= _ent($pegawai->nuptk); ?></td> 
                           <td><?= _ent($pegawai->npp); ?></td> 
                           <td><?= _ent($pegawai->kk); ?></td> 
                           <td><?= _ent($pegawai->npwp); ?></td> 
                           <td><?= _ent($pegawai->alamat); ?></td> 
                           <td><?php if  ($pegawai->agama) {

                              echo anchor('administrator/agama/view/'.$pegawai->agama.'?popup=show', $pegawai->agama_label, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($pegawai->jenis_kelamin); ?></td> 
                           <td><?= _ent($pegawai->status_menikah); ?></td> 
                           <td><?= _ent($pegawai->jumlah_anak); ?></td> 
                           <td><?= _ent($pegawai->no_telp); ?></td> 
                           <td><?= _ent($pegawai->email_ms_office); ?></td> 
                           <td><?php if  ($pegawai->id_posisi) {

                              echo anchor('administrator/posisi_pegawai/view/'.$pegawai->id_posisi.'?popup=show', $pegawai->posisi_pegawai_nama, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($pegawai->unit); ?></td> 
                           <td><?= _ent($pegawai->status_kepegawaian); ?></td> 
                           <td><?= _ent($pegawai->informasi_kepala_pimpinan); ?></td> 
                           <td><?= _ent($pegawai->emp_code); ?></td> 
                           <td><?= _ent($pegawai->token); ?></td> 
                           <td><?= _ent($pegawai->token_expired); ?></td> 
                           <td><?= _ent($pegawai->email_ms_office); ?></td> 
                           <td><?= _ent($pegawai->no_kk); ?></td> 
                           <td><?= _ent($pegawai->tempat_lahir); ?></td> 
                           <td><?= _ent($pegawai->tgl_lahir); ?></td> 
                           <td>
                              <?php if (!empty($pegawai->foto_profil)): ?>
                                ...
                                <?php else: ?>
                                  <a href="<?= BASE_URL . 'uploads/pegawai/' . $pegawai->foto_profil; ?>">
                                   <img src="<?= get_icon_file($pegawai->foto_profil); ?>" class="image-responsive image-icon" alt="image pegawai" title="foto_profil <?= $pegawai->foto_profil; ?>" width="40px"> 
                                 </a>
                                <?php endif; ?>
                              <?php endif; ?>
                           </td>
                  ...
                  <div class="col-md-8">
                     ...
                     <div class="col-sm-2 padd-left-0 ">
                        <select type="text" class="form-control chosen chosen-select" name="s" id="sort">
                           ...
                           <option <?= $this->input->get('s') == 'id_pegawai' ? 'selected' : ''; ?> value="id_pegawai">ID Pegawai</option>
   ...
   <div class="modal-dialog">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h3 class="modal-title" id="myModalLabel">Tambah Pegawai Baru</h3>
         </div>
         <form action="<?= base_url('administrator/pegawai/import/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">

               <div class="form-group">
                  <label class="control-label col-xs-3">File</label>
                  <div class="col-xs-8">
                     <input name="file_pegawai" class="form-control" type="file"  accept=".xls,.xlsx" required>
                     <span>*Silahkan isikan excel sesuai dengan format yang tersedia, berikut contoh format excel
                        <a class="text-danger" href="<?= base_url('uploads/example/format_pegawai.xlsx'); ?>">download disini</a> </span>
                  </div>
               </div>
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/pegawai/upload_slip_gaji/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
                  <label class="control-label col-xs-3">File</label>
                  <div class="col-xs-8">
...
<div class="modal fade" id="modal_update_password" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/pegawai/update_password/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
   ...
   $('.btn_ubah_password').click(function(){
      ...
      $.ajax({
         ...
         success: function(res) {
            ...
            $('#reset_password').attr('href', `<?php echo base_url('administrator/pegawai/reset_password'); ?>?user_microsoft_id=${res.id}`);
