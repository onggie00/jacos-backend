      ...
      if (!empty($get_siswa)) {
        ...
        if (!empty($get_ujian)) {
          ...
          foreach ($get_siswa as $key => $value) {
            $value->judul_ujian = $this->second_db->withquery("select ju.nama_ujian from jenis_ujian ju join jadwal_ujian_ft j on ju.id_jenis_ujian = j.id_jenis_ujian join ujian_ruang_kelas k on j.id_tingkatan = k.id_tingkatan where k.id_tingkatan = '".$get_siswa[0]->id_tingkatan."'","row")->nama_ujian;
          }
