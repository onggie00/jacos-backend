...
<section class="content-header">
   <h1><i class="fa fa-th-large"></i> Kelas SMA <small>Daftar Kelas Siswa</small></h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">Kelas SMA</li>
   </ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-header with-border">
               <h3 class="box-title"><i class="fa fa-list"></i> Data Kelas <span class="label bg-yellow" style="margin-left:8px"><?= (int)$kelas_sma_counts; ?> Data</span></h3>
               <div class="box-tools pull-right">
                  <?php is_allowed('kelas_sma_add', function(){ ?>
                  <a class="btn btn-sm btn-success" title="Tambah Kelas (Ctrl+a)" href="<?= site_url('administrator/kelas_sma/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Kelas
                  </a>
                  <?php }) ?>
                  <?php is_allowed('kelas_sma_export', function(){ ?>
                  <a class="btn btn-sm btn-info" href="<?= site_url('administrator/kelas_sma/export'); ?>">
            ...
            <div class="box-body">
               ...
               <!-- Filter & Search -->
               <form method="get" action="<?= base_url('administrator/kelas_sma/index'); ?>" class="form-inline filter-bar">
                  ...
                  <a href="<?= base_url('administrator/kelas_sma'); ?>" class="btn btn-default btn-sm"><i class="fa fa-times"></i> Reset</a>
               </form>
                  ...
                  <div class="col-md-4">
                     <div class="info-box-custom bg-grad-blue">
                        ...
                        <div>
                           ...
                           <span class="info-value"><?= (int)$kelas_sma_counts; ?></span>
                        </div>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="info-box-custom bg-grad-green">
                        ...
                        <div>
                           ...
                           <span class="info-value"><?= count($list_tingkatan); ?></span>
                        </div>
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="info-box-custom bg-grad-purple">
                        ...
                        <div>
                           <span class="info-label">Rata-rata/Kelas</span>
                  ...
                  <table class="table table-bordered table-striped table-hover">
                     <thead>
                        <tr>
                           <th width="40">No</th>
                           <th width="5">
                              <input type="checkbox" class="flat-red" id="check_all" title="Pilih semua">
                           </th>
                           <th>Tingkatan</th>
                           <th>Nama Kelas</th>
                           <th>Label</th>
                     ...
                     <tbody>
                     <?php if (!empty($kelas_smas)): $no = (int)$offset + 1; foreach($kelas_smas as $ks): ?>
                        <tr>
                           <td style="text-align:center"><?= $no++; ?></td>
                           <td>
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $ks->id_kelas_sma; ?>">
                           </td>
                           <td>
                              <?php if (!empty($ks->id_tingkatan)): ?>
                                 <a href="javascript:void(0);" class="popup-view" data-id="<?= $ks->id_tingkatan; ?>">
                                    <i class="fa fa-layer-group text-muted"></i> <?= htmlspecialchars($ks->tingkatan_sma_label); ?>
                                 </a>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php is_allowed('kelas_sma_view', function() use ($ks){?>
                              <a href="javascript:void(0);" class="btn btn-xs btn-info btn-detail-modal" data-id="<?= $ks->id_kelas_sma; ?>" title="Lihat detail"><i class="fa fa-eye"></i></a>
                              <?php }) ?>
                              <?php is_allowed('kelas_sma_update', function() use ($ks){?>
                              <a href="<?= site_url('administrator/kelas_sma/edit/' . $ks->id_kelas_sma); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                              <?php }) ?>
                              <?php is_allowed('kelas_sma_delete', function() use ($ks){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/kelas_sma/delete/' . $ks->id_kelas_sma); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                              <?php }) ?>
                           </td>
                        </tr>
                     <?php endforeach; else: ?>
                        <tr>
                           <td colspan="6" class="text-center" style="padding:30px">
                              ...
                              <span style="color:#999">
                                 <?php if (!empty($filter)): ?>
                                    Data kelas tidak ditemukan untuk filter yang dipilih.
                                 <?php else: ?>
                                    Data kelas SMA belum tersedia.
                                 <?php endif; ?>
                              </span>
                           </td>
                        </tr>
               ...
               <div class="row" style="margin-top:12px">
                  <div class="col-md-6">
                     ...
                     <small class="text-muted" style="margin-left:15px">Menampilkan <?= count($kelas_smas); ?> dari <?= (int)$kelas_sma_counts; ?> data</small>
                  </div>
      ...
      <div class="modal-content" style="border-radius:5px;overflow:hidden">
         ...
         <div class="modal-body" id="detail_kelas_content" style="padding:20px;background:#f8f9fa">
            <div class="text-center" style="padding:40px">
               ...
               <p style="margin-top:10px;color:#666">Memuat detail kelas...</p>
            </div>
         </div>
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){
      ...
      if (bulk.val() == 'delete') {
         ...
         if (checked.length === 0) {
            swal({ title: "Upss", text: "Pilih data dulu.", type: "warning", confirmButtonColor: "#DD6B55", confirmButtonText: "Okay!" });
            return false;
         }
         var ids = [];
         ...
         swal({
            ...
            if (isConfirm) {
               var url = BASE_URL + '/administrator/kelas_sma/delete';
               $.get(url, { id: ids }, function(){ location.reload(); });
            }
         });
      } else if (bulk.val() === '') {
   ...
   // Popup tingkatan link
   $('a.popup-view').click(function(e){
      e.preventDefault();
      var id = $(this).data('id');
      window.open('<?= base_url('administrator/tingkatan_sma/view/'); ?>' + id + '?popup=show', '_blank');
   });

   // Detail modal
   $('.btn-detail-modal').click(function(){
      var id = $(this).data('id');
      var content = $('#detail_kelas_content');
      content.html('<div class="text-center" style="padding:40px"><i class="fa fa-spinner fa-spin" style="font-size:40px;color:#f39c12"></i><p style="margin-top:10px;color:#666">Memuat detail...</p></div>');
      $('#modalDetailKelas').modal('show');
      $.ajax({
         url: '<?= site_url("administrator/kelas_sma/single_pdf/"); ?>' + id,
         type: 'GET',
         error: function(){
            content.html('<div class="alert alert-danger" style="margin:0"><i class="fa fa-exclamation-triangle"></i> Gagal memuat data.</div>');
         }
      });
   });
});
