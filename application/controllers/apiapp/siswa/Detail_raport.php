...
class Detail_raport extends REST_Controller {
    ...
    if(isset($headers['x-token']))
        ...
        $data = $this->mymodel->withquery("select * from siswa_".strtolower($jenjang)."_aktif_raport where tahun_ajaran = '".$get_tahun_ajaran->label."' and id_siswa_aktif = ".$this->db->escape($id),"result");
        if (!empty($data)) {
