...
class Model_siswa_kb_aktif extends MY_Model {

    private $primary_key    = 'id_siswa_kb_aktif';
    private $table_name     = 'siswa_kb_aktif';
    private $field_search   = ['nama_lengkap', 'nis', 'id_kelas', 'id_siswa_kb', 'id_tahun_ajaran', 'kewarganegaraan', 'nomor_peserta_ujian', 'nik', 'golongan_darah', 'telp', 'pendidikan_ayah', 'pendidikan_ibu', 'penghasilan_ayah', 'penghasilan_ibu', 'tgl_lahir_ayah', 'tgl_lahir_ibu', 'spp_custom', 'spp_type', 'acc_ujian'];
    private $field_search_join   = ['kelas_kb.label','tahun_ajaran.label'];
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_kb_aktif.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_kb_aktif.".$field . " LIKE '%" . $q . "%' ";
                    $where .= "OR " . "kelas_kb.label LIKE '%" . $q . "%' ";
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $sort_by = (empty($sort)) ? 'id_siswa_kb_aktif' : $sort;
            $sort_type = (empty($sort_type)) ? 'desc' : $sort_type;
            $this->db->order_by('siswa_kb_aktif.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        $this->db->join('kelas_kb', 'kelas_kb.id_kelas_kb = siswa_kb_aktif.id_kelas', 'LEFT');
        $this->db->join('siswa_kb', 'siswa_kb.id_siswa_kb = siswa_kb_aktif.id_siswa_kb', 'LEFT');
        $this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun_ajaran = siswa_kb_aktif.id_tahun_ajaran', 'LEFT');

        $this->db->select('siswa_kb_aktif.*,
        kelas_kb.label as kelas_kb_label,
        siswa_kb.nama_lengkap as siswa_kb_nama_lengkap,
        tahun_ajaran.label as tahun_ajaran_label,
        email_ms_office,
        email_ms_office_ortu,
        foto_profil,
        siswa_kb.deleted_at as deleted_at_siswa,
        siswa_kb.deleted_at_ortu as deleted_at_ortu
        ');
    ...
    public function join_avaiable_export() {
        ...
        $this->db->select('siswa_kb_aktif.nama_lengkap,
        kelas_kb.label as kelas,
        tahun_ajaran.label as tahun_ajaran,
        siswa_kb.email,
        siswa_kb.email_ms_office,
        siswa_kb.email_ms_office_ortu,
        siswa_kb.notelp_ibu,
        siswa_kb.notelp_ayah,
        siswa_kb.nisn,
        siswa_kb_aktif.file_raport,
        siswa_kb_aktif.kewarganegaraan,
        siswa_kb_aktif.nik,
        siswa_kb_aktif.golongan_darah,
        siswa_kb_aktif.telp,
        siswa_kb_aktif.pendidikan_ayah,
        siswa_kb_aktif.pendidikan_ibu,
        siswa_kb_aktif.penghasilan_ayah,
        siswa_kb_aktif.penghasilan_ibu,
        siswa_kb_aktif.tgl_lahir_ayah,
        siswa_kb_aktif.tgl_lahir_ibu,
        siswa_kb_aktif.spp_custom
        ');
    ...
    private function _build_multi_where($filters)
    {
        ...
        $field_map = [
            'nama_lengkap'   => 'siswa_kb_aktif.nama_lengkap',
            'nis'            => 'siswa_kb_aktif.nis',
            'kelas'          => 'kelas_kb.label',
            'tahun_ajaran'   => 'tahun_ajaran.label',
            'spp_type'       => 'siswa_kb_aktif.spp_type',
            'acc_ujian'      => 'siswa_kb_aktif.acc_ujian',
            'is_active'      => 'siswa_kb_aktif.is_active',
            'spp_custom'     => 'siswa_kb_aktif.spp_custom',
            'nik'            => 'siswa_kb_aktif.nik',
            'kewarganegaraan'=> 'siswa_kb_aktif.kewarganegaraan',
            'nomor_peserta_ujian' => 'siswa_kb_aktif.nomor_peserta_ujian',
        ];
        ...
        foreach ($filters as $filter) {
            ...
            $db_field = isset($field_map[$field]) ? $field_map[$field] : 'siswa_kb_aktif.' . $field;
        ...
        public function export_siswa($table, $subject = 'file',$value=null,$col=null)
        {
            ...
            if (empty($field)) {
                foreach ($this->field_search as $field) {
                    if ($iterasi == 1) {
                        $where .= "siswa_kb_aktif." . $field . " LIKE '%" . $value . "%' ";
                    } else {
                        $where .= "OR " . "siswa_kb_aktif." . $field . " LIKE '%" . $value . "%' ";
                    }
                    $iterasi++;
                }
            ...
            foreach ($result->result() as $data) {
                ...
                foreach ($fields as $field) {
                    if ($field == 'nama_lengkap') {
                        ...
                        $data_field = ($data->$field)?base_url('uploads/siswa_kb_aktif/') .$data->$field:$data->$field;
                    } else {
	...
	/**
	 * Definisi kolom untuk Export Data Lengkap (siswa_kb_aktif + siswa_kb).
	...
	public function export_lengkap_columns()
	{
		return array(
			'siswa_aktif' => array(
				'title' => 'Kolom Siswa Aktif (siswa_kb_aktif)',
				'cols' => array(
			...
			'pendaftar' => array(
				'title' => 'Kolom Pendaftar (siswa_kb)',
				'cols' => array(
...
/* End of file Model_siswa_kb_aktif.php */
/* Location: ./application/models/Model_siswa_kb_aktif.php */