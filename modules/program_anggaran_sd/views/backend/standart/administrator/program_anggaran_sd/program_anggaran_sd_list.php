...
/* Modal Detail */
#modal_detail_sd .modal-header {
   background: linear-gradient(135deg, #3498db, #2980b9);
   color: #fff;
   padding: 15px 20px;
}
#modal_detail_sd .modal-title { font-weight: 600; }
#modal_detail_sd .modal-body {
   padding: 20px;
   background: #f8f9fa;
   max-height: 75vh;
   overflow-y: auto;
...
<section class="content-header">
   <h1>
      <i class="fa fa-money"></i> <?= cclang('program_anggaran_sd') ?>
      <small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('program_anggaran_sd') ?></li>
   </ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               <h3 class="box-title">
                  <i class="fa fa-list-alt"></i> Data Program Anggaran SD
                  <span class="label bg-yellow" style="margin-left:10px"><?= $program_anggaran_sd_counts; ?> Data</span>
               </h3>
               <div class="box-tools pull-right">
                  <?php
                     if ($total_proposal < 50) {
                        is_allowed('program_anggaran_sd_add', function(){
                  ?>
                  <a class="btn btn-sm btn-success btn_add_new btn-top" title="<?= cclang('add_new_button', [cclang('program_anggaran_sd')]); ?> (Ctrl+a)" href="<?= site_url('administrator/program_anggaran_sd/add'); ?>">
                     <i class="fa fa-plus-square-o"></i> <?= cclang('add_new_button', [cclang('program_anggaran_sd')]); ?>
                  </a>
                  <?php
                        });
                     }
                  ?>
                  <?php is_allowed('program_anggaran_sd_export', function(){?>
                  <a class="btn btn-sm btn-success btn-top" title="<?= cclang('export'); ?> XLS" href="<?= site_url('administrator/program_anggaran_sd/export'); ?>">
            ...
            <div class="box-body">
               ...
               <form name="form_program_anggaran_sd" id="form_program_anggaran_sd" action="<?= base_url('administrator/program_anggaran_sd/index'); ?>" method="get">
               <div class="filter-builder" style="margin-bottom:15px;padding:12px;background:#f8f9fa;border-radius:5px;border:1px solid #e0e0e0">
                  ...
                  <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                     ...
                     <a class="btn btn-flat btn-default" href="<?= base_url('administrator/program_anggaran_sd'); ?>"><i class="fa fa-times"></i> Reset Semua</a>
               ...
               <div class="table-responsive">
                  <table class="table table-bordered table-striped table-hover">
                     ...
                     <tbody id="tbody_program_anggaran_sd">
                     <?php if($program_anggaran_sd_counts > 0): ?>
                     <?php foreach($program_anggaran_sds as $program_anggaran_sd): ?>
                        <tr>
                           <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $program_anggaran_sd->id; ?>">
                           </td>
                           <td class="td-aksi">
                              <div class="btn-aksi-stack">
                                 <?php is_allowed('program_anggaran_sd_view', function() use ($program_anggaran_sd){?>
                                 <button type="button" class="btn btn-info btn-detail-sd" data-id="<?= $program_anggaran_sd->id; ?>" title="Lihat detail">
                                    <i class="fa fa-eye"></i> Detail
                                 </button>
                                 <?php }) ?>
                                 <?php
                                    if ($program_anggaran_sd->status_pengajuan == 1 || $program_anggaran_sd->status_pengajuan == 5
                                       || ($program_anggaran_sd->status_pengajuan == 6 && $program_anggaran_sd->status_laporan == 1)
                                       || ($program_anggaran_sd->status_pengajuan == 6 && $program_anggaran_sd->status_laporan == 5)
                                       || $program_anggaran_sd->status_laporan == 5
                                       || $program_anggaran_sd->status_laporan == 1
                                       || ($program_anggaran_sd->status_pengajuan == 4 && $this->session->userdata('user_keuangan') == true)) {
                                 ?>
                                 <?php is_allowed('program_anggaran_sd_update', function() use ($program_anggaran_sd){?>
                                 <a href="<?= site_url('administrator/program_anggaran_sd/edit/' . $program_anggaran_sd->id); ?>" class="btn btn-warning" title="Edit data">
                                    <i class="fa fa-edit"></i> Edit
                                 </a>
                                 <?php }) ?>
                                 <?php } ?>
                                 <?php if ($program_anggaran_sd->status_pengajuan != 4 && $program_anggaran_sd->status_pengajuan != 6): ?>
                                    <?php is_allowed('program_anggaran_sd_delete', function() use ($program_anggaran_sd){?>
                                    <a href="javascript:void(0);" data-href="<?= site_url('administrator/program_anggaran_sd/delete/' . $program_anggaran_sd->id); ?>" class="btn btn-danger remove-data" title="Hapus data">
                                       <i class="fa fa-trash"></i> Hapus
                                    </a>
                                    <?php }) ?>
                                 <?php endif; ?>
                                 <?php if (!empty($program_anggaran_sd->token_confirmation)): ?>
                                    <a href="<?= site_url('administrator/program_anggaran_sd/resend_email/' . $program_anggaran_sd->id); ?>" class="btn btn-primary" title="Kirim Ulang Email Konfirmasi">
                           ...
                           ?>
                           <td>
                              <?php if (!empty($program_anggaran_sd->nomor_program)): ?>
                                 <a href="<?= site_url('administrator/program_anggaran/view/'.$program_anggaran_sd->nomor_program.'?popup=show'); ?>" class="popup-view"><?= $program_anggaran_sd->nomor_program; ?></a>
                              <?php endif; ?>
                           </td>
                           <td><?= _ent($program_anggaran_sd->jenis_kegiatan); ?></td>
                           <td><?= _ent($program_anggaran_sd->sub_jenis_kegiatan); ?></td>
                           <td><?= _ent($program_anggaran_sd->nama_program); ?></td>
                           <td style="text-align:center"><?= _ent($program_anggaran_sd->tahun_ajaran); ?></td>
                           <td style="text-align:center"><?= !empty($program_anggaran_sd->tanggal_program) ? date("d-m-Y", strtotime($program_anggaran_sd->tanggal_program)) : '-'; ?></td>
                           <td style="text-align:right">Rp <?= number_format($program_anggaran_sd->nominal_okr, 0, ',', '.'); ?></td>
                           <td style="text-align:right">
                              Rp <?= number_format($program_anggaran_sd->nominal_pengajuan, 0, ',', '.'); ?>
                              <span class="sisa-okr"><strong>Sisa OKR: Rp <?= number_format($okr, 0, ',', '.'); ?></strong></span>
                           </td>
                           <td style="text-align:center">
                              <?php if (!empty($program_anggaran_sd->file_proposal_pengajuan)): ?>
                                 <a href="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_proposal_pengajuan; ?>" class="fancybox" rel="group" title="Lihat file proposal pengajuan">
                                    <span class="file-thumb-wrap">
                                       <?php if (is_image($program_anggaran_sd->file_proposal_pengajuan)): ?>
                                          <img src="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_proposal_pengajuan; ?>" class="file-thumb" alt="file proposal">
                                       <?php else: ?>
                                          <img src="<?= get_icon_file($program_anggaran_sd->file_proposal_pengajuan); ?>" class="file-thumb" alt="file proposal">
                                       <?php endif; ?>
                                    </span>
                                 </a>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($program_anggaran_sd->file_proposal_keuangan)): ?>
                                 <a href="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_proposal_keuangan; ?>" class="fancybox" rel="group" title="Lihat file proposal keuangan">
                                    <span class="file-thumb-wrap">
                                       <?php if (is_image($program_anggaran_sd->file_proposal_keuangan)): ?>
                                          <img src="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_proposal_keuangan; ?>" class="file-thumb" alt="file proposal keuangan">
                                       <?php else: ?>
                                          <img src="<?= get_icon_file($program_anggaran_sd->file_proposal_keuangan); ?>" class="file-thumb" alt="file proposal keuangan">
                                       <?php endif; ?>
                                    </span>
                                 </a>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($program_anggaran_sd->status_pengajuan)): ?>
                                 <?php
                                 ...
                                 <span style="background-color:<?= $bg; ?>;color:<?= $col; ?>;padding:5px 10px;border-radius:3px;font-size:11px;display:inline-block;">
                                    <?= $program_anggaran_sd->program_anggaran_status_pengajuan_status; ?>
                                 </span>
                                 <?php if (!empty($program_anggaran_sd->file_kwitansi)): ?>
                                    <div style="margin-top:5px;text-align:center;">
                                       <a href="<?= base_url('uploads/program_anggaran_sd/').$program_anggaran_sd->file_kwitansi; ?>" target="_blank">
                           ...
                           <td style="text-align:center"><?= !empty($program_anggaran_sd->tanggal_pencairan) ? date("d-m-Y", strtotime($program_anggaran_sd->tanggal_pencairan)) : '<small class="text-muted">-</small>'; ?></td>
                           <td><?= _ent($program_anggaran_sd->catatan_pengajuan); ?></td>
                           <td style="text-align:center">
                              <?php if (!empty($program_anggaran_sd->file_laporan_kegiatan)): ?>
                                 <a href="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_laporan_kegiatan; ?>" class="fancybox" rel="group" title="Lihat file laporan kegiatan">
                                    <span class="file-thumb-wrap">
                                       <?php if (is_image($program_anggaran_sd->file_laporan_kegiatan)): ?>
                                          <img src="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $program_anggaran_sd->file_laporan_kegiatan; ?>" class="file-thumb" alt="file laporan kegiatan">
                                       <?php else: ?>
                                          <img src="<?= get_icon_file($program_anggaran_sd->file_laporan_kegiatan); ?>" class="file-thumb" alt="file laporan kegiatan">
                                       <?php endif; ?>
                                    </span>
                                 </a>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php foreach (explode(',', $program_anggaran_sd->file_laporan_keuangan) as $file): ?>
                                 <?php if (!empty($file)): ?>
                                    <a href="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $file; ?>" class="fancybox" rel="group" title="Lihat file laporan keuangan">
                                       <span class="file-thumb-wrap">
                                          <?php if (is_image($file)): ?>
                                             <img src="<?= BASE_URL . 'uploads/program_anggaran_sd/' . $file; ?>" class="file-thumb" alt="file laporan keuangan">
                                          <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($program_anggaran_sd->status_laporan)): ?>
                                 <?php
                                 ...
                                 ?>
                                 <span style="background-color:<?= $bg; ?>;color:<?= $col; ?>;padding:5px 10px;border-radius:3px;font-size:11px;display:inline-block;">
                                    <?= $program_anggaran_sd->program_anggaran_status_laporan_status; ?>
                                 </span>
                              <?php else: ?>
                                 <small class="text-muted">-</small>
                              <?php endif; ?>
                           </td>
                           <td><?= _ent($program_anggaran_sd->catatan_laporan); ?></td>
                           <td style="text-align:center"><small><?= !empty($program_anggaran_sd->created_at) ? date("d-m-Y H:i", strtotime($program_anggaran_sd->created_at)) : '-'; ?></small></td>
                        </tr>
                     <?php endforeach; ?>
                     <?php else: ?>
                        <tr>
                           <td colspan="20" class="text-center" style="padding:30px">
                              ...
                              <span style="color:#999">
                                 ...
                                 <?php else: ?>
                                    Data program anggaran SD belum tersedia
                                 <?php endif; ?>
                              </span>
                           </td>
                        </tr>
...
<!-- ============ MODAL DETAIL SD =============== -->
<div class="modal fade" id="modal_detail_sd" tabindex="-1" role="dialog">
   <div class="modal-dialog modal-lg">
      <div class="modal-content" style="border-radius:5px;overflow:hidden">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" style="color:#fff;opacity:0.8">&times;</button>
            <h4 class="modal-title" style="font-weight:600"><i class="fa fa-eye"></i> Detail Program Anggaran SD</h4>
         </div>
         <div class="modal-body" id="detail_sd_content">
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_program_anggaran_sd').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/program_anggaran_sd/delete?' + serialize_bulk;
            }
          });
        return false;
      } else if(bulk.val() == '')  {
   ...
   // ===== DETAIL MODAL =====
   $('.btn-detail-sd').click(function(){
      var id = $(this).data('id');
      var content = $('#detail_sd_content');
      ...
      $('#modal_detail_sd').modal('show');

      $.ajax({
         url: '<?= site_url('administrator/program_anggaran_sd/get_detail/' . ''); ?>' + id,
         method: 'GET',
         dataType: 'json',
         success: function(data) {
            ...
            var baseUrl = '<?= BASE_URL . 'uploads/program_anggaran_sd/'; ?>';
            ...
            function renderImageFile(name, label) {
               if (!name) return fileEmpty;
               return '<a href="' + baseUrl + name + '" class="fancybox" rel="group" title="Lihat ' + label + '"><span class="file-thumb-wrap"><img src="' + baseUrl + name + '" class="file-thumb" alt="' + label + '"></span></a>';
            }
            ...
            // Re-init fancybox untuk file di modal
