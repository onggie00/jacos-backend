...
<section class="content-header">
   <h1><i class="fa fa-check-circle"></i> <?= cclang('status_daftar_ulang_tk') ?> <small><?= cclang('list_all'); ?></small></h1>
   <ol class="breadcrumb"><li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li><li class="active"><?= cclang('status_daftar_ulang_tk') ?></li></ol>
</section>
         ...
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               <h3 class="box-title">
                  ...
                  <span class="label bg-yellow" style="margin-left:10px"><?= $status_daftar_ulang_tk_counts; ?> Data</span>
               </h3>
               <div class="box-tools pull-right">
                  <?php is_allowed('status_daftar_ulang_tk_export', function(){?>
                  <a class="btn btn-sm btn-action btn-action-export btn-top" title="<?= cclang('export'); ?> XLS" href="<?= site_url('administrator/status_daftar_ulang_tk/export') . '?' . http_build_query($_GET); ?>">
                     <i class="fa fa-file-excel-o"></i> XLS
                  </a>
                  <a class="btn btn-sm btn-action btn-action-pdf btn-top" title="<?= cclang('export'); ?> PDF" href="<?= site_url('administrator/status_daftar_ulang_tk/export_pdf') . '?' . http_build_query($_GET); ?>">
            ...
            <div class="box-body">
               ...
               <form name="form_status_daftar_ulang_tk" id="form_status_daftar_ulang_tk" action="<?= base_url('administrator/status_daftar_ulang_tk/index'); ?>">
               <div class="filter-builder" style="margin-bottom:15px;padding:12px;background:#f8f9fa;border-radius:5px;border:1px solid #e0e0e0">
                  ...
                  <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                     ...
                     <a class="btn btn-flat btn-default" href="<?= base_url('administrator/status_daftar_ulang_tk'); ?>"><i class="fa fa-times"></i> Reset Semua</a>
               ...
               <div class="table-responsive">
                  <table class="table table-bordered table-striped table-hover dataTable">
                     ...
                     <tbody id="tbody_status_daftar_ulang_tk">
                     <?php if($status_daftar_ulang_tk_counts > 0): ?>
                     <?php foreach($status_daftar_ulang_tks as $row): ?>
                        <tr>
                           ...
                           <td style="text-align:center">
                              <div style="margin-bottom:3px">
                                 <?php is_allowed('status_daftar_ulang_tk_view', function() use ($row){?>
                                 <a href="<?= site_url('administrator/status_daftar_ulang_tk/view/'.$row->id_daftar_ulang); ?>" class="btn btn-sm btn-info btn-aksi" title="Lihat detail">
                                    <i class="fa fa-eye"></i> Detail
                                 </a>
                                 <a href="<?= site_url('administrator/status_daftar_ulang_tk/single_pdf/'.$row->id_daftar_ulang); ?>" class="btn btn-sm btn-default btn-aksi" title="Cetak PDF" target="_blank">
                                    <i class="fa fa-file-pdf-o"></i> PDF
                                 </a>
                                 <?php }) ?>
                              </div>
                              <div style="margin-bottom:3px">
                                 <?php is_allowed('status_daftar_ulang_tk_update', function() use ($row){?>
                                 <a href="<?= site_url('administrator/status_daftar_ulang_tk/edit/'.$row->id_daftar_ulang); ?>" class="btn btn-sm btn-warning btn-aksi" title="Edit / Aktivasi">
                                    <i class="fa fa-edit"></i> Edit
                                 </a>
                                 <?php }) ?>
                                 <?php if ($row->status == 0 || $row->status == 2): ?>
                                 <?php is_allowed('status_daftar_ulang_tk_delete', function() use ($row){?>
                                 <a href="javascript:void(0);" data-href="<?= site_url('administrator/status_daftar_ulang_tk/delete/'.$row->id_daftar_ulang); ?>" class="btn btn-sm btn-danger btn-aksi remove-data" title="Hapus data">
...
<div class="modal fade" id="modal_add_new" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/status_daftar_ulang_tk/siswa_aktif/'); ?>" method="post" class="form-horizontal" id="form-aktif">
            <div class="modal-body">
               <div class="form-group mb-3">
                  <label class="control-label col-xs-3">Kelas</label>
                  <div class="col-xs-8">
                     <select class="form-control chosen chosen-select-deselect" name="kelas_tk" id="kelas_tk" data-placeholder="Pilih Kelas" required>
                        <option value=""></option>
                        <?php foreach (db_get_all_data('kelas_tk') as $r) : ?>
                           <option value="<?= $r->id_kelas_tk ?>"><?= $r->label; ?></option>
                        <?php endforeach; ?>
                     </select>
                  </div>
               </div>
...
<div class="modal fade" id="modal_update_expired" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/status_daftar_ulang_tk/update_expired_va/'); ?>" method="post" class="form-horizontal" id="form-expired">
            <div class="modal-body">
               <div class="form-group mb-3">
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_status_daftar_ulang_tk').serialize();
      ...
      if (bulk.val() == '1') {
         swal({
            ...
            if (isConfirm) {
               $('#st').remove();
               document.location.href = BASE_URL + '/administrator/status_daftar_ulang_tk/update_status?' + serialize_bulk;
            }
         });
         return false;
         ...
         var serialize = $('#form_status_daftar_ulang_tk').serializeArray();
