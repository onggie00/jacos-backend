...
<section class="content-header">
    <h1>
        Detail Peserta Ujian PSB SD        <small>Edit Detail Peserta Ujian PSB SD</small>
    </h1>
    <ol class="breadcrumb">
                ...
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <div class="widget-user-header ">
                            ...
                            <h5 class="widget-user-desc">Edit Detail Peserta Ujian PSB SD</h5>
                            <hr>
                        </div>
                        <?= form_open(base_url('administrator/ujian_ruang_pendaftaran_detail_sd/edit_save/'.$this->uri->segment(4)), [
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="id_ruang_pendaftaran" id="id_ruang_pendaftaran" data-placeholder="Select Ruang" >
                                    ...
                                    <option <?=  $row->id_ruang_pendaftaran ==  $ujian_ruang_pendaftaran_detail_sd->id_ruang_pendaftaran ? 'selected' : ''; ?> value="<?= $row->id_ruang_pendaftaran ?>"><?= $row->nama_ruang; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select-deselect" name="nomor_peserta" id="nomor_peserta" data-placeholder="Select Nomor Peserta" >
                                    ...
                                    <option <?=  $row->no_peserta ==  $ujian_ruang_pendaftaran_detail_sd->nomor_peserta ? 'selected' : ''; ?> value="<?= $row->no_peserta ?>"><?= $row->nama_lengkap; ?></option>
                                    <?php endforeach; ?>  
                                </select>
                                <small class="info help-block">
...
<script>
    $(document).ready(function(){
      ...
      $('.btn_save').click(function(){
        ...
        $.ajax({
          url: form_ujian_ruang_pendaftaran_detail_sd.attr('action'),
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
        .done(function(res) {
          ...
          if(res.success) {
            var id = $('#ujian_ruang_pendaftaran_detail_sd_image_galery').find('li').attr('qq-file-id');
            if (save_type == 'back') {
