...
<section class="content-header">
   <h1><i class="fa fa-users"></i> <?= cclang('siswa_smp_aktif') ?> <small><?= cclang('list_all'); ?></small></h1>
   <ol class="breadcrumb"><li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li><li class="active"><?= cclang('siswa_smp_aktif') ?></li></ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            ...
            <div class="box-header with-border">
               <h3 class="box-title">
                  <i class="fa fa-graduation-cap"></i> Data Siswa SMP Aktif
                  <span class="label bg-yellow" style="margin-left:10px"><?= $siswa_smp_aktif_counts; ?> Data</span>
               </h3>
               <div class="box-tools pull-right">
                  <?php is_allowed('siswa_smp_aktif_add', function () { ?>
                  <a class="btn btn-sm btn-action btn-action-add btn-top" title="<?= cclang('add_new_button', [cclang('siswa_smp_aktif')]); ?> (Ctrl+a)" href="<?= site_url('administrator/siswa_smp_aktif/add'); ?>">
                     <i class="fa fa-plus"></i> Tambah Siswa
                  </a>
                  <a class="btn btn-sm btn-action btn-action-add btn-top" title="Tambah Raport" href="<?= site_url('administrator/siswa_smp_aktif/add_raport'); ?>">
                  ...
                  <?php }) ?>
                  <?php is_allowed('siswa_smp_aktif_export', function(){?>
                  <a class="btn btn-sm btn-action btn-action-export btn-top" title="<?= cclang('export'); ?> XLS" href="<?= site_url('administrator/siswa_smp_aktif/export') . '?' . http_build_query($_GET); ?>">
                     <i class="fa fa-file-excel-o"></i> XLS
                  </a>
                  <a class="btn btn-sm btn-action btn-action-pdf btn-top" title="<?= cclang('export'); ?> PDF" href="<?= site_url('administrator/siswa_smp_aktif/export_pdf'); ?>">
            ...
            <div class="box-body">
               ...
               <form name="form_siswa_smp_aktif" id="form_siswa_smp_aktif" action="<?= base_url('administrator/siswa_smp_aktif/index'); ?>">
               <div class="filter-builder" style="margin-bottom:15px;padding:12px;background:#f8f9fa;border-radius:5px;border:1px solid #e0e0e0">
                  ...
                  <div id="filter_rows">
                     <!-- Filter rows will be inserted here by JS -->
                  </div>
                  ...
                  <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                     ...
                     <a class="btn btn-flat btn-default" href="<?= base_url('administrator/siswa_smp_aktif'); ?>"><i class="fa fa-times"></i> Reset Semua</a>
                     ...
                     <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
                        <select class="form-control input-sm" name="s" id="sort" style="width:130px">
                           ...
                           <option <?= $this->input->get('s') == 'id_siswa_smp_aktif' ? 'selected' : ''; ?> value="id_siswa_smp_aktif">ID</option>
                        </select>
               ...
               <div class="table-responsive"> 
                  <table class="table table-bordered table-striped table-hover dataTable">
                     ...
                     <tbody id="tbody_siswa_smp_aktif">
                     <?php if($siswa_smp_aktif_counts > 0): ?>
                     <?php foreach($siswa_smp_aktifs as $siswa_smp_aktif): ?>
                        <tr>
                           <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $siswa_smp_aktif->id_siswa_smp_aktif; ?>"></td>
                           <td>
                              <i class="fa fa-user"></i> <?= _ent($siswa_smp_aktif->nama_lengkap); ?>
                           </td>
                           <td style="text-align:center"><?= _ent($siswa_smp_aktif->nis); ?></td>
                           <td style="text-align:center">
                              <?php if ($siswa_smp_aktif->id_tahun_ajaran): ?>
                                 <a href="<?= site_url('administrator/tahun_ajaran/view/'.$siswa_smp_aktif->id_tahun_ajaran.'?popup=show'); ?>" class="popup-view" title="Lihat tahun ajaran">
                                    <?= $siswa_smp_aktif->tahun_ajaran_label; ?>
                                 </a>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php if ($siswa_smp_aktif->id_kelas): ?>
                                 <a href="<?= site_url('administrator/kelas_smp/view/'.$siswa_smp_aktif->id_kelas.'?popup=show'); ?>" class="popup-view" title="Lihat kelas">
                                    <?= $siswa_smp_aktif->kelas_smp_label; ?>
                                 </a>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php 
                              $spp_type = $siswa_smp_aktif->spp_type ?: 'FULL';
                              if ($spp_type == 'FULL'): ?>
                           ...
                           <td style="text-align:right">
                              <?php if (!empty($siswa_smp_aktif->spp_custom) && $siswa_smp_aktif->spp_custom > 0): ?>
                                 Rp <?= number_format($siswa_smp_aktif->spp_custom, 0, ',', '.'); ?>
                              <?php else: ?>
                                 <small class="text-muted">-</small>
                              <?php endif; ?>
                           </td>
                           <td style="text-align:center">
                              <?php if ($siswa_smp_aktif->acc_ujian == 1): ?>
                           ...
                           <td style="text-align:center">
                              <?php if ($siswa_smp_aktif->is_active == 1): ?>
                           ...
                           <td style="text-align:center;font-size:11px">
                              <?php if (!empty($siswa_smp_aktif->deleted_at_siswa)): ?>
                                 <span class="label label-nonaktif"><i class="fa fa-user-times"></i> Siswa Terhapus</span>
                                 <br><small class="text-danger"><i class="fa fa-calendar"></i> <?= date('d M Y', strtotime($siswa_smp_aktif->deleted_at_siswa)); ?></small>
                                 <?php is_allowed('siswa_smp_aktif_update', function() use ($siswa_smp_aktif){ ?>
                                 <br><a href="<?= site_url('administrator/siswa_smp_aktif/restore/'.$siswa_smp_aktif->id_siswa_smp_aktif); ?>" class="btn btn-xs btn-success restore-link" style="margin-top:3px" title="Pulihkan akun siswa"><i class="fa fa-undo"></i> Pulihkan</a>
                                 <?php }) ?>
                              <?php else: ?>
                                 <span class="label label-aktif"><i class="fa fa-user"></i> Siswa Aktif</span>
                              <?php endif; ?>
                              <hr style="margin:5px 0">
                              <?php if (!empty($siswa_smp_aktif->deleted_at_ortu)): ?>
                                 <span class="label label-nonaktif"><i class="fa fa-user-times"></i> Ortu Terhapus</span>
                                 <br><small class="text-danger"><i class="fa fa-calendar"></i> <?= date('d M Y', strtotime($siswa_smp_aktif->deleted_at_ortu)); ?></small>
                                 <?php is_allowed('siswa_smp_aktif_update', function() use ($siswa_smp_aktif){ ?>
                                 <br><a href="<?= site_url('administrator/siswa_smp_aktif/restore_ortu/'.$siswa_smp_aktif->id_siswa_smp_aktif); ?>" class="btn btn-xs btn-success restore-link" style="margin-top:3px" title="Pulihkan akun ortu"><i class="fa fa-undo"></i> Pulihkan</a>
                                 <?php }) ?>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <?php if (!empty($siswa_smp_aktif->file_raport)): ?>
                                 <?php if (is_image($siswa_smp_aktif->file_raport)): ?>
                                    <a class="fancybox" rel="group" href="<?= BASE_URL . 'uploads/siswa_smp_aktif/' . $siswa_smp_aktif->file_raport; ?>">
                                       <img src="<?= BASE_URL . 'uploads/siswa_smp_aktif/' . $siswa_smp_aktif->file_raport; ?>" alt="raport" width="40px" style="border-radius:3px">
                                    </a>
                                 <?php else: ?>
                                    <a href="<?= BASE_URL . 'uploads/siswa_smp_aktif/' . $siswa_smp_aktif->file_raport; ?>" title="Download raport">
                                       <img src="<?= get_icon_file($siswa_smp_aktif->file_raport); ?>" alt="raport" width="30px">
                                    </a>
                                 <?php endif; ?>
                              <?php else: ?>
                           ...
                           <td style="text-align:center">
                              <div style="margin-bottom:3px">
                                 <?php is_allowed('siswa_smp_aktif_view', function() use ($siswa_smp_aktif){?>
                                 <button type="button" class="btn btn-sm btn-info btn-aksi btn_detail_siswa" data-id="<?= $siswa_smp_aktif->id_siswa_smp_aktif; ?>" title="Lihat detail">
                                    <i class="fa fa-eye"></i> Detail
                                 </button>
                                 <?php }) ?>
                                 <?php is_allowed('siswa_smp_aktif_update', function() use ($siswa_smp_aktif){?>
                                 <a href="<?= site_url('administrator/siswa_smp_aktif/edit/'.$siswa_smp_aktif->id_siswa_smp_aktif); ?>" class="btn btn-sm btn-warning btn-aksi" title="Edit data">
                                    <i class="fa fa-edit"></i> Edit
                                 </a>
                                 <?php }) ?>
                              </div>
                              <div style="margin-bottom:3px">
                                 <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-aksi btn_ubah_password" title="Ubah Password Siswa" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $siswa_smp_aktif->id_siswa_smp_aktif; ?>" data-email="<?= $siswa_smp_aktif->email_ms_office; ?>">
                                    <i class="fa fa-key"></i> Ubah Pass Siswa
                                 </a>
                                 <a href="javascript:void(0)" class="btn btn-sm btn-success btn-aksi btn_ubah_password" title="Ubah Password Ortu" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $siswa_smp_aktif->id_siswa_smp_aktif; ?>" data-email="<?= $siswa_smp_aktif->email_ms_office_ortu; ?>">
                                    <i class="fa fa-key"></i> Ubah Pass Ortu
                                 </a>
                              </div>
                              <?php is_allowed('siswa_smp_aktif_delete', function() use ($siswa_smp_aktif){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/siswa_smp_aktif/delete/'.$siswa_smp_aktif->id_siswa_smp_aktif); ?>" class="btn btn-sm btn-danger btn-aksi-full remove-data" title="Hapus data">
                        ...
                        <tr>
                           <td colspan="12" class="text-center" style="padding:30px">
                              ...
                              <span style="color:#999">
                                 ...
                                 <?php else: ?>
                                    Data siswa SMP aktif belum tersedia
                                 <?php endif; ?>
                              </span>
                           </td>
                        </tr>
...
<div class="modal fade" id="modal_generate" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_smp_aktif/generate_nis/'); ?>" method="post">
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_smp_aktif/import_siswa/'); ?>" method="post" enctype="multipart/form-data">
         <div class="modal-body">
            <div class="form-group">
...
<div class="modal fade" id="modal_import_raport" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_smp_aktif/import_raport_siswa/'); ?>" method="post" enctype="multipart/form-data">
         <div class="modal-body">
            <div class="form-group">
...
<div class="modal fade" id="modal_update_password" tabindex="-1" role="dialog">
   <div class="modal-dialog"><div class="modal-content">
      ...
      <form action="<?= base_url('administrator/siswa_smp_aktif/update_password/'); ?>" method="post">
         <div class="modal-body">
            <div class="form-group">
...
function getSelectOptions(fieldVal) {
   switch(fieldVal) {
      case 'kelas': return kelasOptions;
      case 'tahun_ajaran': return taOptions;
      case 'spp_type': return sppTypeOptions;
      case 'acc_ujian': return accUjianOptions;
      case 'is_active': return statusOptions;
      default: return [];
   }
}
...
function getFieldType(fieldVal) {
   for (var i = 0; i < filterFields.length; i++) {
      if (filterFields[i].value === fieldVal) return filterFields[i].type;
   }
   return 'text';
}

function getFieldOperators(fieldVal) {
   for (var i = 0; i < filterFields.length; i++) {
      if (filterFields[i].value === fieldVal) return filterFields[i].operators;
   }
   return ['contains'];
}

function buildOperatorSelect(operators, selectedOp) {
   var html = '<select class="form-control input-sm filter-operator" style="width:130px">';
   for (var i = 0; i < operators.length; i++) {
      var op = operators[i];
      var sel = (op === selectedOp) ? ' selected' : '';
      html += '<option value="' + op + '"' + sel + '>' + operatorLabels[op] + '</option>';
   }
   html += '</select>';
   return html;
}

function buildValueInput(fieldVal, currentValue) {
   var fType = getFieldType(fieldVal);
   if (fType === 'text' || fType === 'number') {
      var inputType = fType === 'number' ? 'number' : 'text';
      return '<input type="' + inputType + '" class="form-control input-sm filter-value" style="width:200px" placeholder="Masukkan nilai..." value="' + (currentValue||'') + '">';
   }
   // Select type
   var opts = getSelectOptions(fieldVal);
   var html = '<select class="form-control input-sm filter-value" style="width:200px">';
   html += '<option value="">-- Pilih --</option>';
   for (var i = 0; i < opts.length; i++) {
      var sel = (opts[i].id === currentValue) ? ' selected' : '';
      html += '<option value="' + opts[i].id + '"' + sel + '>' + opts[i].text + '</option>';
   }
   html += '</select>';
   return html;
}

function addFilterRow(fieldVal, operator, value) {
   fieldVal = fieldVal || '';
   operator = operator || '';
   value = value || '';

   var fieldSelectHtml = '<select class="form-control input-sm filter-field" style="width:180px">';
   fieldSelectHtml += '<option value="">-- Pilih Field --</option>';
   for (var i = 0; i < filterFields.length; i++) {
      var sel = (filterFields[i].value === fieldVal) ? ' selected' : '';
      fieldSelectHtml += '<option value="' + filterFields[i].value + '"' + sel + '>' + filterFields[i].label + '</option>';
   }
   fieldSelectHtml += '</select>';

   var operators = fieldVal ? getFieldOperators(fieldVal) : ['contains'];
   if (!operator || operators.indexOf(operator) === -1) operator = operators[0];

   var html = '<div class="filter-row" style="display:flex;gap:8px;align-items:center;margin-bottom:8px" data-index="' + filterIndex + '">';
   html += '<input type="hidden" name="ff[]" class="filter-field-hidden" value="' + fieldVal + '">';
   html += '<input type="hidden" name="fo[]" class="filter-operator-hidden" value="' + operator + '">';
   html += '<input type="hidden" name="fv[]" class="filter-value-hidden" value="' + value + '">';
   html += '<span style="color:#3498db;font-weight:bold;font-size:12px;min-width:20px">#' + (filterIndex + 1) + '</span>';
   html += fieldSelectHtml;
   html += '<span class="filter-operator-container">' + buildOperatorSelect(operators, operator) + '</span>';
   html += '<span class="filter-value-container">' + buildValueInput(fieldVal, value) + '</span>';
   html += '<button type="button" class="btn btn-xs btn-danger btn-remove-filter" title="Hapus filter"><i class="fa fa-times"></i></button>';
   html += '</div>';

   $('#filter_rows').append(html);
   filterIndex++;
   updateFilterEmpty();
}
...
$(document).ready(function(){
   ...
   // Field change -> update operator & value
   $(document).on('change', '.filter-field', function(){
      var $row = $(this).closest('.filter-row');
      var fieldVal = $(this).val();
      $row.find('.filter-field-hidden').val(fieldVal);

      // Update operator
      var ops = fieldVal ? getFieldOperators(fieldVal) : ['contains'];
      $row.find('.filter-operator-container').html(buildOperatorSelect(ops, ops[0]));
      $row.find('.filter-operator-hidden').val(ops[0]);

      // Update value input
      $row.find('.filter-value-container').html(buildValueInput(fieldVal, ''));
      $row.find('.filter-value-hidden').val('');
   });
   ...
   <?php if(!empty($multi_filters)): ?>
   <?php foreach($multi_filters as $i => $mf): ?>
   addFilterRow('<?= htmlspecialchars($mf['field']); ?>', '<?= htmlspecialchars($mf['operator']); ?>', '<?= htmlspecialchars($mf['value']); ?>');
   <?php endforeach; ?>
   <?php endif; ?>
...
$(document).ready(function(){
   ...
   $('#apply').click(function(){
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_siswa_smp_aktif').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
            ...
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/siswa_smp_aktif/delete?' + serialize_bulk;      
            }
         });
         return false;
      } else if(bulk.val() == '') {
   ...
   $('.btn_ubah_password').click(function(){
      ...
      $.ajax({
         ...
         success: function(res) {
            ...
            $('#reset_password').attr('href', `<?php echo base_url('administrator/siswa_smp_aktif/reset_password'); ?>?user_microsoft_id=${res.id}`);
         },
         error: function(err) {
            console.error(err);
         }
   ...
   $('.btn_detail_siswa').click(function(){
      ...
      $.ajax({
         url: '<?= site_url('administrator/siswa_smp_aktif/get_detail/' . ''); ?>' + id,
         method: 'GET',
         dataType: 'json',
         success: function(data) {
            ...
            var fotoProfil = data.foto_profil ? '<img src="<?= BASE_URL; ?>uploads/siswa_smp/' + data.foto_profil + '" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #3498db">' : '<div style="width:80px;height:80px;border-radius:50%;background:#e8f4fc;display:flex;align-items:center;justify-content:center"><i class="fa fa-user" style="font-size:35px;color:#3498db"></i></div>';
            ...
            html += '<tr><th style="background:#e8f4fc;width:200px">Kelas</th><td>' + (data.kelas_smp_label || '-') + '</td></tr>';
