...
class Model_siswa_sd_aktif extends MY_Model {

    private $primary_key    = 'id_siswa_sd_aktif';
    private $table_name     = 'siswa_sd_aktif';
    private $field_search   = ['nama_lengkap', 'nis', 'id_kelas', 'id_siswa_sd', 'id_tahun_ajaran', 'kewarganegaraan', 'nomor_peserta_ujian', 'nik', 'golongan_darah', 'telp', 'pendidikan_ayah', 'pendidikan_ibu', 'penghasilan_ayah', 'penghasilan_ibu', 'tgl_lahir_ayah', 'tgl_lahir_ibu', 'spp_custom', 'spp_type', 'acc_ujian'];
    private $field_search_join   = ['kelas_sd.label','tahun_ajaran.label'];
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_sd_aktif.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_sd_aktif.".$field . " LIKE '%" . $q . "%' ";
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $sort_by = (empty($sort)) ? 'id_siswa_sd_aktif' : $sort;
            $sort_type = (empty($sort_type)) ? 'desc' : $sort_type;
            $this->db->order_by('siswa_sd_aktif.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_sd', 'siswa_sd.id_siswa_sd = siswa_sd_aktif.id_siswa_sd', 'LEFT');
        $this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun_ajaran = siswa_sd_aktif.id_tahun_ajaran', 'LEFT');

        $this->db->select('siswa_sd_aktif.*,
        kelas_sd.label as kelas_sd_label,
        siswa_sd.nama_lengkap as siswa_sd_nama_lengkap,
        tahun_ajaran.label as tahun_ajaran_label,
        email_ms_office,
        email_ms_office_ortu,
        foto_profil,
        siswa_sd.deleted_at as deleted_at_siswa,
        siswa_sd.deleted_at_ortu as deleted_at_ortu
        ');
    ...
    public function join_avaiable_export() {
        ...
        $this->db->select('siswa_sd_aktif.nama_lengkap,
        kelas_sd.label as kelas,
        tahun_ajaran.label as tahun_ajaran,
        siswa_sd.email,
        siswa_sd.email_ms_office,
        siswa_sd.email_ms_office_ortu,
        siswa_sd.notelp_ibu,
        siswa_sd.notelp_ayah,
        siswa_sd.nisn,
        siswa_sd_aktif.file_raport,
        siswa_sd_aktif.kewarganegaraan,
        siswa_sd_aktif.nik,
        siswa_sd_aktif.golongan_darah,
        siswa_sd_aktif.telp,
        siswa_sd_aktif.pendidikan_ayah,
        siswa_sd_aktif.pendidikan_ibu,
        siswa_sd_aktif.penghasilan_ayah,
        siswa_sd_aktif.penghasilan_ibu,
        siswa_sd_aktif.tgl_lahir_ayah,
        siswa_sd_aktif.tgl_lahir_ibu,
        siswa_sd_aktif.spp_custom
        ');
    ...
    private function _build_multi_where($filters)
    {
        ...
        $field_map = [
            ...
            'nis'            => 'siswa_sd_aktif.nis',
            'kelas'          => 'kelas_sd.label',
            'tahun_ajaran'   => 'tahun_ajaran.label',
            'spp_type'       => 'siswa_sd_aktif.spp_type',
            'acc_ujian'      => 'siswa_sd_aktif.acc_ujian',
            'is_active'      => 'siswa_sd_aktif.is_active',
            'spp_custom'     => 'siswa_sd_aktif.spp_custom',
            'nik'            => 'siswa_sd_aktif.nik',
            'kewarganegaraan'=> 'siswa_sd_aktif.kewarganegaraan',
            'nomor_peserta_ujian' => 'siswa_sd_aktif.nomor_peserta_ujian',
        ];
        ...
        foreach ($filters as $filter) {
            ...
            $db_field = isset($field_map[$field]) ? $field_map[$field] : 'siswa_sd_aktif.' . $field;
        ...
        public function export_siswa($table, $subject = 'file',$value=null,$col=null)
        {
            ...
            if (empty($field)) {
                foreach ($this->field_search as $field) {
                    if ($iterasi == 1) {
                        $where .= "siswa_sd_aktif." . $field . " LIKE '%" . $value . "%' ";
                    } else {
                        $where .= "OR " . "siswa_sd_aktif." . $field . " LIKE '%" . $value . "%' ";
                    }
                    $iterasi++;
                }
            ...
            foreach ($result->result() as $data) {
                ...
                foreach ($fields as $field) {
                    if ($field == 'nama_lengkap') {
                        ...
                        $data_field = ($data->$field)?base_url('uploads/siswa_sd_aktif/') .$data->$field:$data->$field;
                    } else {
	...
	/**
	 * Definisi kolom untuk Export Data Lengkap (siswa_sd_aktif + siswa_sd).
	...
	public function export_lengkap_columns()
	{
		return array(
			'siswa_aktif' => array(
				'title' => 'Kolom Siswa Aktif (siswa_sd_aktif)',
				'cols' => array(
			...
			'pendaftar' => array(
				'title' => 'Kolom Pendaftar (siswa_sd)',
				'cols' => array(
...
/* End of file Model_siswa_sd_aktif.php */
/* Location: ./application/models/Model_siswa_sd_aktif.php */