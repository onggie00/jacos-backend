...
<section class="content-header">
   <h1>
      <?= cclang('guru_ft') ?><small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('guru_ft') ?></li>
   </ol>
</section>
<!-- Main content -->
            ...
            <div class="box-body ">
               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('guru_ft_add', function(){?>
                        <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('guru_ft')]); ?>  (Ctrl+a)" href="<?=  site_url('administrator/guru_ft/add'); ?>"><i class="fa fa-plus-square-o" ></i> <?= cclang('add_new_button', [cclang('guru_ft')]); ?></a>
                        <a style="display:none" class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('guru_ft')]); ?>  (Ctrl+a)" href="<?=  site_url('administrator/guru_ft/add_slip_gaji'); ?>"><i class="fa fa-plus-square-o" ></i> Tambah Slip Gaji</a>
                        ...
                        <?php }) ?>
                        <?php is_allowed('guru_ft_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('guru_ft') ?>']); ?>" href="<?= site_url('administrator/guru_ft/export'); ?>"><i class="fa fa-file-excel-o" ></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('guru_ft_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('guru_ft') ?>']); ?>" href="<?= site_url('administrator/guru_ft/export_pdf'); ?>"><i class="fa fa-file-pdf-o" ></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                     </div>
                     ...
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username"><?= cclang('guru_ft') ?></h3>
                     <h5 class="widget-user-desc"><?= cclang('list_all', [cclang('guru_ft')]); ?>  <i class="label bg-yellow"><?= $guru_ft_counts; ?>  <?= cclang('items'); ?></i></h5>
                  </div>

                  <form name="form_guru_ft" id="form_guru_ft" action="<?= base_url('administrator/guru_ft/index'); ?>">
                  ...
                  <table class="table table-bordered table-striped dataTable">
                     ...
                     <tbody id="tbody_guru_ft">
                     <?php foreach($guru_fts as $guru_ft): ?>
                        <tr>
                                                       <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $guru_ft->id_guru; ?>">
                           </td>
                           <td width="200">
                           
                           <?php is_allowed('guru_ft_view', function() use ($guru_ft){?>
                              <a href="<?= site_url('administrator/guru_ft/view/' . $guru_ft->id_guru); ?>" class="btn btn-success"><i class="fa fa-eye"></i> </a>
                           <?php }) ?>
                           <?php is_allowed('guru_ft_update', function() use ($guru_ft){?>
                              <a href="<?= site_url('administrator/guru_ft/edit/' . $guru_ft->id_guru); ?>" class="btn btn-info"><i class="fa fa-edit "></i> </a>
                           <?php }) ?> 
                           <?php is_allowed('guru_ft_delete', function() use ($guru_ft){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/guru_ft/delete/' . $guru_ft->id_guru); ?>" class="btn btn-danger remove-data"><i class="fa fa-close"></i> </a>
                           <?php }) ?>
                           <a style="margin-top:5px;" class="btn btn-primary btn_ubah_password" data-toggle="modal" data-target="#modal_update_password" data-id="<?= $guru_ft->id_guru; ?>" data-email="<?= $guru_ft->email_ms_office; ?>"><i class="fa fa-key"></i >Ubah Password</a>

                           </td>
                           <td><?= _ent($guru_ft->nama_lengkap); ?></td> 
                           <td><?= _ent($guru_ft->nik); ?></td> 
                           <td><?= _ent($guru_ft->nuptk); ?></td> 
                           <td><?= _ent($guru_ft->npp); ?></td> 
                           <td><?= _ent($guru_ft->npwp); ?></td> 
                           <td><?= _ent($guru_ft->alamat); ?></td> 
                           <td><?= _ent($guru_ft->agama); ?></td> 
                           <td><?= _ent($guru_ft->jenis_kelamin); ?></td> 
                           <td><?= _ent($guru_ft->status_menikah); ?></td> 
                           <td><?= _ent($guru_ft->jumlah_anak); ?></td> 
                           <td><?= _ent($guru_ft->no_telp); ?></td> 
                           <td><?= _ent($guru_ft->email); ?></td> 
                           <td><?php if  ($guru_ft->id_posisi) {

                              echo anchor('administrator/posisi_guru/view/'.$guru_ft->id_posisi.'?popup=show', $guru_ft->posisi_guru_nama_posisi, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($guru_ft->satuan_pendidikan); ?></td> 
                           <td><?= _ent($guru_ft->unit); ?></td> 
                           <td><?php if  ($guru_ft->id_mapel) {

                              echo anchor('administrator/mata_pelajaran_ft/view/'.$guru_ft->id_mapel.'?popup=show', $guru_ft->mata_pelajaran_ft_nama_mapel, ['class' => 'popup-view']); }?> </td>
                             
                           <td><?= _ent($guru_ft->status_kepegawaian); ?></td> 
                           <td><?= _ent($guru_ft->informasi_kepala_pimpinan); ?></td> 
                           <td><?= _ent($guru_ft->emp_code); ?></td> 
                           <td><?= _ent($guru_ft->token); ?></td> 
                           <td><?= _ent($guru_ft->token_expired); ?></td> 
                           <td><?= _ent($guru_ft->email_ms_office); ?></td> 
                           <td>
                              <?php if (!empty($guru_ft->foto_profil)): ?>
                                <?php if (is_image($guru_ft->foto_profil)): ?>
                                <a class="fancybox" rel="group" href="<?= BASE_URL . 'uploads/guru_ft/' . $guru_ft->foto_profil; ?>">
                                  <img src="<?= BASE_URL . 'uploads/guru_ft/' . $guru_ft->foto_profil; ?>" class="image-responsive" alt="image guru_ft" title="foto_profil guru_ft" width="40px">
                                </a>
                                <?php else: ?>
                                  <a href="<?= BASE_URL . 'uploads/guru_ft/' . $guru_ft->foto_profil; ?>">
                                   <img src="<?= get_icon_file($guru_ft->foto_profil); ?>" class="image-responsive image-icon" alt="image guru_ft" title="foto_profil <?= $guru_ft->foto_profil; ?>" width="40px"> 
                                 </a>
                                <?php endif; ?>
                              <?php endif; ?>
                           </td>
                            
                           <td><?= _ent($guru_ft->no_kk); ?></td> 
                           <td><?= _ent($guru_ft->tempat_lahir); ?></td> 
                           <td><?= _ent($guru_ft->tgl_lahir); ?></td> 
                           
                           
                           </tr>
                      <?php endforeach; ?>
                      <?php if ($guru_ft_counts == 0) :?>
                         <tr>
                           <td colspan="100">
                           Guru Ft data is not available
                           </td>
                         </tr>
                      <?php endif; ?>
                     </tbody>
               ...
               <div class="row">
                  <div class="col-md-8">
                     ...
                     <div class="col-sm-1 padd-left-0 ">
                        <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= base_url('administrator/guru_ft');?>" title="<?= cclang('reset_filter'); ?>">
...
<div class="modal fade" id="modal_add_new" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_ft/import/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
               ...
               <div class="form-group">
                  ...
                  <div class="col-xs-8">
                     ...
                     <span>*Silahkan isikan excel sesuai dengan format yang tersedia, berikut contoh format excel
                        <a class="text-danger" href="<?= base_url('uploads/example/format_guru_ft.xlsx'); ?>">download disini</a> </span>
                  </div>
               </div>
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_ft/upload_slip_gaji/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
                  <label class="control-label col-xs-3">File</label>
                  <div class="col-xs-8">
...
<div class="modal fade" id="modal_update_password" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/guru_ft/update_password/'); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="modal-body">
               <div class="form-group">
...
<script>
  $(document).ready(function(){
    ...
    $('#apply').click(function(){
      ...
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_guru_ft').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               document.location.href = BASE_URL + '/administrator/guru_ft/delete?' + serialize_bulk;      
            }
          });
    ...
    $('.btn_ubah_password').click(function(){
      ...
      $.ajax({
         ...
         success: function(res) {
            ...
            $('#reset_password').attr('href', `<?php echo base_url('administrator/guru_ft/reset_password'); ?>?user_microsoft_id=${res.id}`);
