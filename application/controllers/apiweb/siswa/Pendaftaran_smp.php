...
class Pendaftaran_smp extends REST_Controller {
    ...
    public function cek_notelp($nohp){
          // ponytail: fix bug — tambahkan default return agar nohp valid non-62/0 tidak jadi null
          $hp = trim($nohp);
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        if (empty($cek_email)) {
          ...
          //cek email duplicate
          // ponytail: fix bug — tambahkan escape_like_str untuk mitigasi SQL injection pada LIKE
          $cek_email = $this->mymodel->withquery("select * from siswa_smp where email like '%".$this->db->escape_like_str($this->post("email"))."%'", "row");
          if(!empty($cek_email)){
