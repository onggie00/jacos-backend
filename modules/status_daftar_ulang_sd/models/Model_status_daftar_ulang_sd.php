...
class Model_status_daftar_ulang_sd extends MY_Model {

    private $primary_key    = 'id_daftar_ulang';
    private $table_name     = 'status_daftar_ulang_sd';
    private $field_search   = ['id_siswa_sd', 'status', 'slip_pembayaran', 'kwitansi', 'kartu_sementara', 'tanggal_lulus', 'tgl_daftar_ulang', 'tgl_aktivasi', 'tgl_bayar', 'va_bri'];
    private $field_search_join   = ['siswa_sd.nama_lengkap', 'siswa_sd.email', 'siswa_sd.no_peserta', 'siswa_sd.nisn', 'siswa_sd.tahun_ajaran', 'siswa_sd.gelombang'];

    /**
     * Whitelist alias -> DB column for multi-filter (anti SQL injection via GET)
     */
    private $field_map = array(
        'nama_lengkap'        => 'siswa_sd.nama_lengkap',
        'email'               => 'siswa_sd.email',
        'no_peserta'          => 'siswa_sd.no_peserta',
        'nisn'                => 'siswa_sd.nisn',
        'tahun_ajaran'        => 'siswa_sd.tahun_ajaran',
        'gelombang'           => 'siswa_sd.gelombang',
        'status'              => 'status_daftar_ulang_sd.status',
        'va_bri'              => 'status_daftar_ulang_sd.va_bri',
        'id_daftar_ulang'     => 'status_daftar_ulang_sd.id_daftar_ulang',
        'id_siswa_sd'         => 'status_daftar_ulang_sd.id_siswa_sd',
        'tanggal_lulus'       => 'DATE(status_daftar_ulang_sd.tanggal_lulus)',
        'tgl_daftar_ulang'    => 'status_daftar_ulang_sd.tgl_daftar_ulang',
        'tgl_aktivasi'        => 'DATE(status_daftar_ulang_sd.tgl_aktivasi)',
        'tgl_bayar'           => 'DATE(status_daftar_ulang_sd.tgl_bayar)',
        'expired_datetime'    => 'DATE(transaksi.expired_datetime)',
        'custom_payment'      => 'status_daftar_ulang_sd.custom_payment',
    );
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "status_daftar_ulang_sd.".$field . " LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_sd.nama_lengkap LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_sd.email LIKE '%" . $q . "%' ";
                    $where .= "OR " . "siswa_sd.no_peserta LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "status_daftar_ulang_sd.".$field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
            ...
            $where .= "OR (" . "siswa_sd.nama_lengkap LIKE '%" . $q . "%' )";
            $where .= "OR (" . "siswa_sd.email LIKE '%" . $q . "%' )";
            $where .= "OR (" . "siswa_sd.va_number_bri LIKE '%" . $q . "%' )";
        }
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $this->db->order_by('status_daftar_ulang_sd.' . $sort_by, $sort_type);
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->select('siswa_sd.nama_lengkap,
                            siswa_sd.no_peserta,
                            siswa_sd.nama_ayah,
                           ...
                           (CASE
                                WHEN status_daftar_ulang_sd.status = 0 THEN "Menunggu Aktivasi"
                                WHEN status_daftar_ulang_sd.status = 1 THEN "Menunggu Pembayaran"
                                WHEN status_daftar_ulang_sd.status = 2 THEN "Lunas"
                                ELSE "-"
                            END) as status
                          ');
...
/* End of file Model_status_daftar_ulang_sd.php */
/* Location: ./application/models/Model_status_daftar_ulang_sd.php */