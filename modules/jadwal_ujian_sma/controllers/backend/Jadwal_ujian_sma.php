...
/**
*| --------------------------------------------------------------------------
*| Jadwal Ujian Sma Controller
*| --------------------------------------------------------------------------
*| Jadwal Ujian Sma site
*|
*/
class Jadwal_ujian_sma extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_jadwal_ujian_sma');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Jadwal Ujian Smas
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('jadwal_ujian_sma_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['jadwal_ujian_smas'] = $this->model_jadwal_ujian_sma->get($filter, $field, $this->limit_page, $offset);
		$this->data['jadwal_ujian_sma_counts'] = $this->model_jadwal_ujian_sma->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/jadwal_ujian_sma/index/',
			'total_rows'   => $this->model_jadwal_ujian_sma->count_all($filter, $field),
		...
		$this->template->title('Jadwal Ujian Aktif SMA List');
		$this->render('backend/standart/administrator/jadwal_ujian_sma/jadwal_ujian_sma_list', $this->data);
	}
	
	/**
	* Add new jadwal_ujian_smas
	*
	*/
	public function add()
	{
		$this->is_allowed('jadwal_ujian_sma_add');

		$this->template->title('Jadwal Ujian Aktif SMA New');
		$this->render('backend/standart/administrator/jadwal_ujian_sma/jadwal_ujian_sma_add', $this->data);
	}

	/**
	* Add New Jadwal Ujian Smas
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('jadwal_ujian_sma_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_jadwal_ujian_sma = $this->model_jadwal_ujian_sma->store($save_data);
            

			if ($save_jadwal_ujian_sma) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_jadwal_ujian_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/jadwal_ujian_sma/edit/' . $save_jadwal_ujian_sma, 'Edit Jadwal Ujian Sma'),
						anchor('administrator/jadwal_ujian_sma', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/jadwal_ujian_sma');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Jadwal Ujian Smas
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('jadwal_ujian_sma_update');

		$this->data['jadwal_ujian_sma'] = $this->model_jadwal_ujian_sma->find($id);

		$this->template->title('Jadwal Ujian Aktif SMA Update');
		$this->render('backend/standart/administrator/jadwal_ujian_sma/jadwal_ujian_sma_update', $this->data);
	}

	/**
	* Update Jadwal Ujian Smas
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('jadwal_ujian_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$save_jadwal_ujian_sma = $this->model_jadwal_ujian_sma->change($id, $save_data);
	...
	/**
	* delete Jadwal Ujian Smas
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('jadwal_ujian_sma_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'jadwal_ujian_sma'), 'success');
        } else {
            set_message(cclang('error_delete', 'jadwal_ujian_sma'), 'error');
        }
		...
		/**
	* View view Jadwal Ujian Smas
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('jadwal_ujian_sma_view');

		$this->data['jadwal_ujian_sma'] = $this->model_jadwal_ujian_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Jadwal Ujian Aktif SMA Detail');
		$this->render('backend/standart/administrator/jadwal_ujian_sma/jadwal_ujian_sma_view', $this->data);
	}
	...
	*/
	...
	{
		$jadwal_ujian_sma = $this->model_jadwal_ujian_sma->find($id);

		
		
		return $this->model_jadwal_ujian_sma->remove($id);
	}
	...
	*/
	...
	{
		$this->is_allowed('jadwal_ujian_sma_export');
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select j.*, m.nama_mapel, ju.nama_ujian, t.label as tingkatan, ta.label as tahun_ajaran from jadwal_ujian_sma j join ujian_mapel_sma m on j.id_mapel = m.id_mapel join jenis_ujian ju on j.id_jenis_ujian = ju.id_jenis_ujian join tingkatan_sma t on j.id_tingkatan = t.id_tingkatan_sma join tahun_ajaran ta on j.id_tahun_ajaran = ta.id_tahun_ajaran ","result");
		}
			...
			$get_data = $this->mymodel->withquery("select j.*, m.nama_mapel, ju.nama_ujian, t.label as tingkatan, ta.label as tahun_ajaran from jadwal_ujian_sma j join ujian_mapel_sma m on j.id_mapel = m.id_mapel join jenis_ujian ju on j.id_jenis_ujian = ju.id_jenis_ujian join tingkatan_sma t on j.id_tingkatan = t.id_tingkatan_sma join tahun_ajaran ta on j.id_tahun_ajaran = ta.id_tahun_ajaran where ".$where,"result");
		}
		else{
		...
		header('Content-Disposition: attachment;filename="Data Jadwal Ujian Aktif SMA - '.date("Y-m-d Hi").'.xls"'); 
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_jadwal_ujian_sma->pdf('jadwal_ujian_sma', 'jadwal_ujian_sma');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_jadwal_ujian_sma->find($id);
	...
	public function import_jadwal()
	{
		...
		if (isset($_FILES["file_import"]["name"])) {
			...
			//delete semua ujian
			$this->mymodel->delete("jadwal_ujian_sma", "id_jadwal !=", 0);
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					$get_mapel = $this->mymodel->withquery("select id_mapel from ujian_mapel_sma where nama_mapel = '".$nama_mapel."'","row");
					if (empty($get_mapel)) {
					...
					$get_tingkatan = $this->mymodel->withquery("select id_tingkatan_sma from tingkatan_sma where label = '".$tingkatan."'","row");
					...
					$data_jadwal = array(
						...
						"id_tingkatan" => $get_tingkatan->id_tingkatan_sma,
					...
					$save_data = $this->mymodel->insertid("jadwal_ujian_sma", $data_jadwal);
					// dd($data_pendaftaran);
					if (empty($save_data)) {
						$this->db->trans_rollback();
						//$this->load->library("session");
...
/* End of file jadwal_ujian_sma.php */
/* Location: ./application/controllers/administrator/Jadwal Ujian Sma.php */