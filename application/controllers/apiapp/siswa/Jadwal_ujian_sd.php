...
//echo $this->db->last_query();
    if (!empty($data)) {
      $get_siswa = $this->mymodel->withquery("select d.*, k.kelas, k.id_tingkatan, k.id_ruang_kelas, s.nis from ujian_ruang_detail d 
      left join ujian_ruang_kelas k on d.id_ruang_kelas = k.id_ruang_kelas 
      left join siswa_sd_aktif s on d.nomor_peserta_ujian = s.nomor_peserta_ujian 
      where d.nomor_peserta_ujian = '".$data->nomor_peserta_ujian."' order by d.urutan_kursi ASC limit 0,1","result");
      if (!empty($get_siswa)) {
        $get_ujian = $this->mymodel->withquery("select j.id_jadwal, j.id_jenis_ujian, j.tanggal, j.hari, j.jam_mulai, j.jam_selesai, j.id_tahun_ajaran, m.nama_mapel, t.label as tahun_ajaran, j.daftar_kelas from jadwal_ujian_sd j join ujian_mapel_sd m on j.id_mapel = m.id_mapel join tahun_ajaran t on j.id_tahun_ajaran = t.id_tahun_ajaran where j.id_tingkatan = '".$get_siswa[0]->id_tingkatan."' order by tanggal ASC, jam_mulai ASC","result");
