...
class Model_program_anggaran_sd extends MY_Model {
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "program_anggaran_sd.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "program_anggaran_sd.".$field . " LIKE '%" . $q . "%' ";
        ...
        $this->db->where($where);
        $this->db->group_by('program_anggaran_sd.id');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = array())
    {
        ...
        $this->db->limit($limit, $offset);
                $this->db->order_by('program_anggaran_sd.'.$this->primary_key, "DESC");
                $query = $this->db->get($this->table_name);
    ...
    /**
     * Resolusi nama field ke nama kolom SQL (termasuk prefix tabel join untuk status)
     */
    private function _resolve_column($field)
    {
        if ($field === 'status_pengajuan') {
            return 'program_anggaran_sd.status_pengajuan';
        }
        if ($field === 'status_laporan') {
            return 'program_anggaran_sd.status_laporan';
        }
        return 'program_anggaran_sd.' . $field;
    }
        ...
        if (!empty($search)) {
            ...
            foreach ($this->field_search as $field) {
                if ($field === 'status_pengajuan') {
                    ...
                    $column = 'program_anggaran_sd.' . $field;
                }
    ...
    public function join_avaiable() {
        $this->db->join('program_anggaran', 'program_anggaran.nomor_program = program_anggaran_sd.nomor_program', 'LEFT');
        $this->db->join('program_anggaran_status_pengajuan', 'program_anggaran_status_pengajuan.id_program_anggaran_status_pengajuan = program_anggaran_sd.status_pengajuan', 'LEFT');
        $this->db->join('program_anggaran_status_laporan', 'program_anggaran_status_laporan.id_program_anggaran_status_laporan = program_anggaran_sd.status_laporan', 'LEFT');

        $this->db->select('program_anggaran_sd.*,program_anggaran.nomor_program as program_anggaran_nomor_program,program_anggaran_status_pengajuan.status as program_anggaran_status_pengajuan_status,program_anggaran_status_laporan.status as program_anggaran_status_laporan_status');
...
/* End of file Model_program_anggaran_sd.php */
/* Location: ./application/models/Model_program_anggaran_sd.php */