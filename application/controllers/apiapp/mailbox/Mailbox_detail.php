...
class Mailbox_detail extends REST_Controller {
    ...
    public function index_get()
    {
        ...
        $id_recipient = $this->input->get('id_detail');
        ...
        if (!empty($data)) {
            ...
            $update = $this->mymodel->update("mailbox_recipient", $data_update, "id", $id_recipient);
        //add url pdf
            if(!empty($data->mail_number)){
                $data->mailbox_pdf_file = base_url("apiapp/mailbox/export_pdf_mail?id_mail=".$data->id_mail."&npp=".$data->npp);
            }
