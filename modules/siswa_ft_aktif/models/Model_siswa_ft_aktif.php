...
class Model_siswa_ft_aktif extends MY_Model {

    private $primary_key    = 'id_siswa_ft_aktif';
    private $table_name     = 'siswa_ft_aktif';
    private $field_search   = ['nama_lengkap', 'nis', 'id_kelas', 'id_siswa_ft', 'id_tahun_ajaran', 'kewarganegaraan', 'nik', 'golongan_darah', 'telp', 'pendidikan_ayah', 'pendidikan_ibu', 'penghasilan_ayah', 'penghasilan_ibu', 'tgl_lahir_ayah', 'tgl_lahir_ibu', 'spp_custom', 'spp_type', 'acc_ujian'];
    private $field_search_join   = ['kelas_ft.label','tahun_ajaran.label'];
    ...
    public function count_all($q = null, $field = null, $filters = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_ft_aktif.".$field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_ft_aktif.".$field . " LIKE '%" . $q . "%' ";
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null, $filters = null)
    {
        ...
        if (is_array($filters) && count($filters) > 0) {
            ...
            $sort_by = (empty($sort)) ? 'id_siswa_ft_aktif' : $sort;
            $sort_type = (empty($sort_type)) ? 'desc' : $sort_type;
            $this->db->order_by('siswa_ft_aktif.' . $sort_by, $sort_type);
    ...
    public function join_avaiable() {
        ...
        $this->db->join('siswa_ft', 'siswa_ft.id_siswa_ft = siswa_ft_aktif.id_siswa_ft', 'LEFT');
        $this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun_ajaran = siswa_ft_aktif.id_tahun_ajaran', 'LEFT');

        $this->db->select('siswa_ft_aktif.*,
        kelas_ft.label as kelas_ft_label,
        siswa_ft.nama_lengkap as siswa_ft_nama_lengkap,
        tahun_ajaran.label as tahun_ajaran_label,
        email_ms_office,
        email_ms_office_ortu,
        foto_profil,
        siswa_ft.deleted_at as deleted_at_siswa,
        siswa_ft.deleted_at_ortu as deleted_at_ortu
        ');
    ...
    public function join_avaiable_export() {
        ...
        $this->db->select('siswa_ft_aktif.nama_lengkap,
        kelas_ft.label as kelas,
        tahun_ajaran.label as tahun_ajaran,
        siswa_ft.email,
        siswa_ft.email_ms_office,
        siswa_ft.email_ms_office_ortu,
        siswa_ft.notelp_ibu,
        siswa_ft.notelp_ayah,
        siswa_ft.nisn,
        siswa_ft_aktif.file_raport,
        siswa_ft_aktif.kewarganegaraan,
        siswa_ft_aktif.nik,
        siswa_ft_aktif.golongan_darah,
        siswa_ft_aktif.telp,
        siswa_ft_aktif.pendidikan_ayah,
        siswa_ft_aktif.pendidikan_ibu,
        siswa_ft_aktif.penghasilan_ayah,
        siswa_ft_aktif.penghasilan_ibu,
        siswa_ft_aktif.tgl_lahir_ayah,
        siswa_ft_aktif.tgl_lahir_ibu,
        siswa_ft_aktif.spp_custom
        ');
    ...
    private function _build_multi_where($filters)
    {
        ...
        $field_map = [
            'nama_lengkap'   => 'siswa_ft_aktif.nama_lengkap',
            'nis'            => 'siswa_ft_aktif.nis',
            'kelas'          => 'kelas_ft.label',
            'tahun_ajaran'   => 'tahun_ajaran.label',
            'spp_type'       => 'siswa_ft_aktif.spp_type',
            'acc_ujian'      => 'siswa_ft_aktif.acc_ujian',
            'is_active'      => 'siswa_ft_aktif.is_active',
            'spp_custom'     => 'siswa_ft_aktif.spp_custom',
            'nik'            => 'siswa_ft_aktif.nik',
            'kewarganegaraan'=> 'siswa_ft_aktif.kewarganegaraan',
            'nomor_peserta_ujian' => 'siswa_ft_aktif.nomor_peserta_ujian',
        ];
        ...
        foreach ($filters as $filter) {
            ...
            $db_field = isset($field_map[$field]) ? $field_map[$field] : 'siswa_ft_aktif.' . $field;
        ...
        public function export_siswa($table, $subject = 'file',$value=null,$col=null)
        {
            ...
            if (empty($field)) {
                foreach ($this->field_search as $field) {
                    if ($iterasi == 1) {
                        $where .= "siswa_ft_aktif." . $field . " LIKE '%" . $value . "%' ";
                    } else {
                        $where .= "OR " . "siswa_ft_aktif." . $field . " LIKE '%" . $value . "%' ";
                    }
                    $iterasi++;
                }
            ...
            foreach ($result->result() as $data) {
                ...
                foreach ($fields as $field) {
                    if ($field == 'nama_lengkap') {
                        ...
                        $data_field = ($data->$field)?base_url('uploads/siswa_ft_aktif/') .$data->$field:$data->$field;
                    } else {
	...
	/**
	 * Definisi kolom untuk Export Data Lengkap (siswa_ft_aktif + siswa_ft).
	...
	public function export_lengkap_columns()
	{
		return array(
			'siswa_aktif' => array(
				'title' => 'Kolom Siswa Aktif (siswa_ft_aktif)',
				'cols' => array(
					'nama_lengkap'        => array('NAMA LENGKAP', 'fa.nama_lengkap'),
					'nis'                 => array('NIS', 'fa.nis'),
					'kelas'               => array('KELAS', 'k.label'),
					'tahun_ajaran'        => array('TAHUN AJARAN', 't.label'),
					'nomor_peserta_ujian' => array('NOMOR PESERTA UJIAN', 'fa.nomor_peserta_ujian'),
					'acc_ujian'           => array('ACC UJIAN', 'fa.acc_ujian'),
					'kewarganegaraan'     => array('KEWARGANEGARAAN', 'fa.kewarganegaraan'),
					'nik'                 => array('NIK', 'fa.nik'),
					'golongan_darah'      => array('GOLONGAN DARAH', 'fa.golongan_darah'),
					'telp'                => array('TELP', 'fa.telp'),
					'pendidikan_ayah'     => array('PENDIDIKAN AYAH', 'fa.pendidikan_ayah'),
					'pendidikan_ibu'      => array('PENDIDIKAN IBU', 'fa.pendidikan_ibu'),
					'penghasilan_ayah'    => array('PENGHASILAN AYAH', 'fa.penghasilan_ayah'),
					'penghasilan_ibu'     => array('PENGHASILAN IBU', 'fa.penghasilan_ibu'),
					'tgl_lahir_ayah'      => array('TGL LAHIR AYAH', 'fa.tgl_lahir_ayah'),
					'tgl_lahir_ibu'       => array('TGL LAHIR IBU', 'fa.tgl_lahir_ibu'),
					'is_active'           => array('STATUS AKTIF', 'fa.is_active'),
					'spp_custom'          => array('SPP CUSTOM', 'fa.spp_custom'),
					'spp_type'            => array('SPP TYPE', 'fa.spp_type'),
				),
			),
			'pendaftar' => array(
				'title' => 'Kolom Pendaftar (siswa_ft)',
				'cols' => array(
...
/* End of file Model_siswa_ft_aktif.php */
/* Location: ./application/models/Model_siswa_ft_aktif.php */