-- ============================================================================
-- Rekonsiliasi anomali SPP "lunas palsu" lintas jenjang — TABEL: spp_sma
...
-- 1. BACKUP ------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS spp_sma_backup_anomali AS SELECT * FROM spp_sma;
...
WHERE t.status_transaksi = 2
  ...
  AND t.no_transaksi LIKE 'LISPP-SMA-%'
  AND b.id IS NULL
  AND r.id IS NULL
