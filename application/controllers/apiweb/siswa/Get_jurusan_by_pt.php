...
class Get_jurusan_by_pt extends REST_Controller {
    ...
    public function index_get()
    {
      ...
      foreach (getallheaders() as $name => $value) {
        ...
        if (empty($id_pt)) {
          $data = $this->mymodel->withquery("select j.id, j.id_perguruan_tinggi as id_pt, j.jurusan, j.passing_grade, pt.nama_pt, pt. inisial from pt_jurusan j join pt_perguruan_tinggi pt on j.id_perguruan_tinggi = pt.id ".$where." order by j.passing_grade DESC, pt.nama_pt ASC","result");
        }
        else{
          $data = $this->mymodel->withquery("select j.id, j.id_perguruan_tinggi as id_pt, j.jurusan, j.passing_grade, pt.nama_pt, pt. inisial from pt_jurusan j join pt_perguruan_tinggi pt on j.id_perguruan_tinggi = pt.id ".$where." order by j.passing_grade DESC, pt.nama_pt ASC","result");
        }
