...
/**
*| --------------------------------------------------------------------------
*| Laporan Kinerja Guru Sma Controller
*| --------------------------------------------------------------------------
*| Laporan Kinerja Guru Sma site
*|
*/
class Laporan_kinerja_guru_sma extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_laporan_kinerja_guru_sma');
		$this->lang->load('web_lang', $this->current_lang);
	}

	/**
	* show all Laporan Kinerja Guru Smas
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('laporan_kinerja_guru_sma_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['laporan_kinerja_guru_smas'] = $this->model_laporan_kinerja_guru_sma->get($filter, $field, $this->limit_page, $offset);
		$this->data['laporan_kinerja_guru_sma_counts'] = $this->model_laporan_kinerja_guru_sma->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/laporan_kinerja_guru_sma/index/',
			'total_rows'   => $this->model_laporan_kinerja_guru_sma->count_all($filter, $field),
		...
		$this->template->title('Laporan Kinerja Guru SMA List');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sma/laporan_kinerja_guru_sma_list', $this->data);
	}
	
	/**
	* Add new laporan_kinerja_guru_smas
	*
	*/
	public function add()
	{
		$this->is_allowed('laporan_kinerja_guru_sma_add');

		$this->template->title('Laporan Kinerja Guru SMA New');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sma/laporan_kinerja_guru_sma_add', $this->data);
	}

	/**
	* Add New Laporan Kinerja Guru Smas
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('laporan_kinerja_guru_sma_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_kinerja_guru_sma = $this->model_laporan_kinerja_guru_sma->store($save_data);
            

			if ($save_laporan_kinerja_guru_sma) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_laporan_kinerja_guru_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/laporan_kinerja_guru_sma/edit/' . $save_laporan_kinerja_guru_sma, 'Edit Laporan Kinerja Guru Sma'),
						anchor('administrator/laporan_kinerja_guru_sma', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/laporan_kinerja_guru_sma');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
		...
		/**
	* Update view Laporan Kinerja Guru Smas
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('laporan_kinerja_guru_sma_update');

		$this->data['laporan_kinerja_guru_sma'] = $this->model_laporan_kinerja_guru_sma->find($id);

		$this->template->title('Laporan Kinerja Guru SMA Update');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sma/laporan_kinerja_guru_sma_update', $this->data);
	}

	/**
	* Update Laporan Kinerja Guru Smas
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('laporan_kinerja_guru_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$save_laporan_kinerja_guru_sma = $this->model_laporan_kinerja_guru_sma->change($id, $save_data);
	...
	/**
	* delete Laporan Kinerja Guru Smas
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('laporan_kinerja_guru_sma_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'laporan_kinerja_guru_sma'), 'success');
        } else {
            set_message(cclang('error_delete', 'laporan_kinerja_guru_sma'), 'error');
        }
		...
		/**
	* View view Laporan Kinerja Guru Smas
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('laporan_kinerja_guru_sma_view');

		$this->data['laporan_kinerja_guru_sma'] = $this->model_laporan_kinerja_guru_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Laporan Kinerja Guru SMA Detail');
		$this->render('backend/standart/administrator/laporan_kinerja_guru_sma/laporan_kinerja_guru_sma_view', $this->data);
	}
	...
	*/
	...
	{
		$laporan_kinerja_guru_sma = $this->model_laporan_kinerja_guru_sma->find($id);

		
		
		return $this->model_laporan_kinerja_guru_sma->remove($id);
	}
	...
	*/
	...
	{
		$this->is_allowed('laporan_kinerja_guru_sma_export');

		//$this->model_laporan_kinerja_guru_sma->export('laporan_kinerja_guru_sma', 'laporan_kinerja_guru_sma');
		$f = $this->input->get('f');
		$q = $this->input->get('q');
		$where = "";
		...
		$list_kinerja = array();
		if (!empty($f) && !empty($q)) {
			$query = "select p.id_guru, p.nama_lengkap, p.npp, k.unit_kerja, k.mata_pelajaran, k.umur, k.masa_kerja, k.golongan, k.kompetensi_pedagogik, k.kompetensi_profesional, k.kompetensi_kepribadian, k.kompetensi_sosial, k.leadership, k.nilai_prestasi, k.nilai_presensi, k.total_skor from laporan_kinerja_guru_sma k 
			...
			$list_kinerja = $this->mymodel->withquery("select p.id_guru, p.nama_lengkap, p.npp, k.unit_kerja, k.mata_pelajaran, k.umur, k.masa_kerja, k.golongan, k.kompetensi_pedagogik, k.kompetensi_profesional, k.kompetensi_kepribadian, k.kompetensi_sosial, k.leadership, k.nilai_prestasi, k.nilai_presensi, k.total_skor from laporan_kinerja_guru_sma k 
			join guru_sma p on k.id_guru = p.id_guru order by k.tahun_ajaran DESC, p.nama_lengkap ASC ","result");
		}
		...
		header('Content-Disposition: attachment;filename="Laporan Kinerja Guru SMA - '.date("Y-m-d").'.xls"');
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_laporan_kinerja_guru_sma->pdf('laporan_kinerja_guru_sma', 'laporan_kinerja_guru_sma');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'laporan_kinerja_guru_sma';
        ...
        $data = $this->model_laporan_kinerja_guru_sma->find($id);
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 5; $row <= $highestRow; $row++) {
					...
					if(empty($check)){
						...
						$check2=$this->mymodel->withquery("select * from guru_sma where nama_lengkap like '%".$nama."%'", 'row');
						if(empty($check2)){
							$this->db->trans_rollback();
							$this->load->library("session");
							$this->session->set_flashdata('failed', "Input NPP guru_sma ".$check2->nama_lengkap." ".strtoupper($nama)." salah. (NPP : ".$check2->npp." ".$npp.")");
							redirect($_SERVER['HTTP_REFERER']);
						}
						else{
							$check=$check2;
					...
					$check_nilai = $this->mymodel->withquery("select * from laporan_kinerja_guru_sma where id_guru = '".$check->id_guru."' and tahun_ajaran = '".$this->input->post('tahun_ajaran')."'", 'row');

					if(!empty($check_nilai)){
						$data_laporan['created_at'] = date("Y-m-d H:i:s");
						$this->mymodel->update("laporan_kinerja_guru_sma", $data_laporan, 'id_guru', $check->id_guru);
						$insertId = $check_nilai->id_laporan;
					}else{
						$insertId = $this->mymodel->insertid("laporan_kinerja_guru_sma", $data_laporan);
					}
...
/* End of file laporan_kinerja_guru_sma.php */
/* Location: ./application/controllers/administrator/Laporan Kinerja Guru Sma.php */