...
class Model_siswa_smp extends MY_Model
{
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_smp." . $field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_smp." . $field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
        ...
        $this->db->where($where);
        $this->db->where('siswa_smp.is_show', 1);
        $this->db->group_by('siswa_smp.id_siswa_smp');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                ...
                // }
                // if($field=="jenis_ppsbb"){
                //     $q=jenis_ppsbb_id(strtoupper($q));
                // }
                if ($iterasi == 1) {
                    $where .= "siswa_smp." . $field . " LIKE '%" . $q . "%' ";
                } else {
        ...
        $sort_by = (empty($sort)) ? 'id_siswa_smp' : $sort;
        ...
        $this->db->order_by('siswa_smp.' . $sort_by, $sort_type);
        $this->db->group_by('siswa_smp.id_siswa_smp');
        $query = $this->db->get($this->table_name);
    ...
    {
        $wlike ="( transaksi.no_transaksi  LIKE '%LI-PSBSMP%' ESCAPE '!' 
        OR  transaksi.no_transaksi  LIKE '%LI-PPSBBSMP%' ESCAPE '!' )";
        $this->db->join('areas', 'areas.id = siswa_smp.kelurahan', 'LEFT');
        $this->db->join('districts', 'districts.id = siswa_smp.kecamatan', 'LEFT');
        $this->db->join('regencies', 'regencies.id = siswa_smp.kota', 'LEFT');
        $this->db->join('status_lulus', 'status_lulus.id_status_lulus = siswa_smp.status_lulus', 'LEFT');
        $this->db->join('provinces', 'provinces.id = siswa_smp.provinsi', 'LEFT');
        $this->db->join('status_daftar_ulang_smp daftar_ulang', 'daftar_ulang.id_siswa_smp = siswa_smp.id_siswa_smp', 'LEFT');
        $this->db->join('transaksi', 'transaksi.user_email = siswa_smp.email', 'LEFT');
        $this->db->where($wlike);
        
        // $this->db->or_like(array('transaksi.no_transaksi'=>'LI-PSBSMP'));
        // $this->db->or_like(array('transaksi.no_transaksi'=>'LI-PPSBBSMP'));
        $this->db->select(
            'siswa_smp.*,
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->where('siswa_smp.is_show', 1);
        $this->db->order_by('id_siswa_smp', 'ASC');
    ...
    public function export_siswa($table, $subject = 'file',$value=null,$col=null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_smp." . $field . " LIKE '%" . $value . "%' ";
                } else {
                    $where .= "OR " . "siswa_smp." . $field . " LIKE '%" . $value . "%' ";
                }
                $iterasi++;
            }
...
/* End of file Model_siswa_smp.php */
/* Location: ./application/models/Model_siswa_smp.php */
