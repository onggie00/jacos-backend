...
class Guru_sma extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_guru_sma');
	...
	*/
	public function index($offset = 0)
	{
		...
		$this->data['guru_smas'] = $this->model_guru_sma->get($filter, $field, $this->limit_page, $offset,[],$sort,$sort_type);
		$this->data['guru_sma_counts'] = $this->model_guru_sma->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/guru_sma/index/',
			'total_rows'   => $this->model_guru_sma->count_all($filter, $field),
		...
		$this->template->title('Guru Sma List');
		$this->render('backend/standart/administrator/guru_sma/guru_sma_list', $this->data);
	}
	
	public function add_slip_gaji()
	{
		$this->is_allowed('guru_sma_add');

		$this->template->title('Guru SMA Slip Gaji');
		$this->render('backend/standart/administrator/guru_sma/guru_sma_add_slip_gaji', $this->data);
	}

	public function add_slip_gaji_save()
	{
		if (!$this->is_allowed('guru_sma_add', false)) {
			echo json_encode([
				'success' => false,
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/guru_sma/')) {
				mkdir(FCPATH . '/uploads/guru_sma/');
			}
			...
			if (!empty($slip_gaji_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/guru_sma/' . $slip_gaji_name_copy
				);

				if (!is_file(FCPATH . '/uploads/guru_sma/' . $slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_guru_sma = $this->model_guru_sma->change($item, $save_data);
		}
		...
		if ($save_guru_sma) {
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->data['id'] 	   = $save_guru_sma;
				$this->data['message'] = cclang('success_save_data_stay', [

					anchor('administrator/guru_sma', ' Go back to list')
				]);
			} else {
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/guru_sma/edit/' . $save_guru_sma, 'Edit Siswa Ft')
					]),
					'success'
				);

				$this->data['success'] = true;
				$this->data['redirect'] = base_url('administrator/guru_sma');
			}
		} else {
			if ($this->input->post('save_type') == 'stay') {
	...
	*/
	public function add()
	{
		...
		$this->template->title('Guru Sma New');
		$this->render('backend/standart/administrator/guru_sma/guru_sma_add', $this->data);
	}
	...
	*/
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			$guru_sma_foto_profil_uuid = $this->input->post('guru_sma_foto_profil_uuid');
			$guru_sma_foto_profil_name = $this->input->post('guru_sma_foto_profil_name');
			$guru_sma_slip_gaji_uuid = $this->input->post('guru_sma_slip_gaji_uuid');
			$guru_sma_slip_gaji_name = $this->input->post('guru_sma_slip_gaji_name');

			$check=$this->mymodel->withquery("select * from guru_sma where emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
			...
			if (!empty($guru_sma_foto_profil_name)) {
				$guru_sma_foto_profil_name_copy = date('YmdHis') . '-' . $guru_sma_foto_profil_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_sma_foto_profil_uuid . '/' . $guru_sma_foto_profil_name, 
						FCPATH . 'uploads/guru_sma/' . $guru_sma_foto_profil_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_sma/' . $guru_sma_foto_profil_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['foto_profil'] = $guru_sma_foto_profil_name_copy;
			}
		
			if (!empty($guru_sma_slip_gaji_name)) {
				$guru_sma_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_sma_slip_gaji_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_sma_slip_gaji_uuid . '/' . $guru_sma_slip_gaji_name, 
						FCPATH . 'uploads/guru_sma/' . $guru_sma_slip_gaji_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_sma/' . $guru_sma_slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['slip_gaji'] = $guru_sma_slip_gaji_name_copy;
			}
		
			
			$save_guru_sma = $this->model_guru_sma->store($save_data);
			...
			if ($save_guru_sma) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_guru_sma;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/guru_sma/edit/' . $save_guru_sma, 'Edit Guru Sma'),
						anchor('administrator/guru_sma', ' Go back to list')
					]);
				} else {
					set_message(
		...
		/**
	* Update view Guru Smas
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('guru_sma_update');

		$this->data['guru_sma'] = $this->model_guru_sma->find($id);

		$this->template->title('Guru Sma Update');
		$this->render('backend/standart/administrator/guru_sma/guru_sma_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('guru_sma_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$check=$this->mymodel->withquery("select * from guru_sma where id_guru != $id and emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
				$this->db->trans_rollback();
				$this->load->library("session");
				$this->session->set_flashdata('error', "Emp Code sudah digunakan!");
			...
			if (!empty($guru_sma_foto_profil_uuid)) {
				$guru_sma_foto_profil_name_copy = date('YmdHis') . '-' . $guru_sma_foto_profil_name;
			...
			if (!empty($guru_sma_slip_gaji_uuid)) {
				$guru_sma_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_sma_slip_gaji_name;
			...
			$get_data = $this->mymodel->withquery("select * from guru_sma where id_guru = ?", "row", [$id]);
			$save_guru_sma = $this->model_guru_sma->change($id, $save_data);
	...
	*/
	...
	{
		$this->is_allowed('guru_sma_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'guru_sma'), 'success');
        } else {
            set_message(cclang('error_delete', 'guru_sma'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('guru_sma_view');

		$this->data['guru_sma'] = $this->model_guru_sma->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Guru Sma Detail');
		$this->render('backend/standart/administrator/guru_sma/guru_sma_view', $this->data);
	}
	...
	*/
	...
	{
		$guru_sma = $this->model_guru_sma->find($id);

		if (!empty($guru_sma->foto_profil)) {
			$path = FCPATH . '/uploads/guru_sma/' . $guru_sma->foto_profil;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($guru_sma->slip_gaji)) {
			$path = FCPATH . '/uploads/guru_sma/' . $guru_sma->slip_gaji;
		...
		return $this->model_guru_sma->remove($id);
	}
	
	/**
	* Upload Image Guru Sma	* 
	* @return JSON
	*/
	...
	/**
	* Delete Image Guru Sma	* 
	* @return JSON
	*/
	public function delete_foto_profil_file($uuid)
	{
		if (!$this->is_allowed('guru_sma_delete', false)) {
			echo json_encode([
				'success' => false,
	...
	*/
	public function get_foto_profil_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_sma/',
            'delete_endpoint'   => 'administrator/guru_sma/delete_foto_profil_file'
        ]);
	}
	...
	*/
	public function get_slip_gaji_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_sma/',
            'delete_endpoint'   => 'administrator/guru_sma/delete_slip_gaji_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('guru_sma_export');

		$this->model_guru_sma->export('guru_sma', 'guru_sma');
	}
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_guru_sma->pdf('guru_sma', 'guru_sma');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_guru_sma->find($id);
		...
		if (isset($_FILES["file_guru"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					// check ump code
					$check=$this->mymodel->withquery("select * from guru_sma where emp_code = ".(int)$worksheet->getCellByColumnAndRow(1, $row)->getValue(), 'row');
					/* if($check){
						$this->db->trans_rollback();
						$this->load->library("session");
					...
					if (!empty($worksheet->getCellByColumnAndRow(15, $row)->getValue())) {
						$data_pendaftaran["no_telp"] = $worksheet->getCellByColumnAndRow(15, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(16, $row)->getValue())) {
						$data_pendaftaran["keterangan_jabatan"] = $worksheet->getCellByColumnAndRow(16, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(17, $row)->getValue())) {
						$data_pendaftaran["jam_ajar"] = $worksheet->getCellByColumnAndRow(17, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(18, $row)->getValue())) {
						$data_pendaftaran["pendidikan_terakhir"] = $worksheet->getCellByColumnAndRow(18, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(19, $row)->getValue())) {
						$data_pendaftaran["universitas"] = $worksheet->getCellByColumnAndRow(19, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(20, $row)->getValue())) {
						$data_pendaftaran["jurusan"] = $worksheet->getCellByColumnAndRow(20, $row)->getValue();
					}
					if (!empty($worksheet->getCellByColumnAndRow(21, $row)->getValue())) {
						$data_pendaftaran["tahun_lulus"] = $worksheet->getCellByColumnAndRow(21, $row)->getValue();
					}
														//check null
					...
					$check=$this->mymodel->withquery("select * from guru_sma where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					if($check){
						$insertId = $this->mymodel->update2("guru_sma", $data_pendaftaran, 'email_ms_office', $worksheet->getCellByColumnAndRow(2, $row)->getValue());
						$insertId = $check->id_guru;
					}else{
						$insertId = $this->mymodel->insertid("guru_sma", $data_pendaftaran);
					}
	...
	public function upload_slip_gaji(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_slip_gaji']['name'][$i])) {
                  $trans_folder = './uploads/guru_sma/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_slip_gaji']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("guru_sma",$data, "npp", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/guru_sma/").$filename_no_extension.".pdf",
...
/* End of file guru_sma.php */
/* Location: ./application/controllers/administrator/Guru Sma.php */