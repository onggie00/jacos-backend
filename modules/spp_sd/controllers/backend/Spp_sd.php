...
class Spp_sd extends Admin
{
	...
	{
		$this->is_allowed('spp_sd_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['spp_sds'] = $this->model_spp_sd->get($filter, $field, $this->limit_page, $offset);
		$this->data['spp_sd_counts'] = $this->model_spp_sd->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/spp_sd/index/',
			'total_rows'   => $this->model_spp_sd->count_all($filter, $field),
		...
		$this->template->title('Spp Sd List');
		$this->render('backend/standart/administrator/spp_sd/spp_sd_list', $this->data);
	}
	...
	public function add_save()
	{
		...
		if (!$this->is_allowed('spp_sd_list', false)) {
			echo json_encode([
				'success' => false,
		...
		if (empty($id_siswa)) {
			...
			$list_siswa = $this->mymodel->getbywhere("siswa_sd_aktif", "id_kelas", $this->input->post("id_kelas"), "result");
			$id_siswa = array();
		...
		foreach ($id_siswa as $key_siswa => $value_siswa) {
			...
			$get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.* from siswa_sd_aktif sa join siswa_sd s on s.id_siswa_sd = sa.id_siswa_sd join kelas_sd k on k.id_kelas_sd = sa.id_kelas where sa.id_siswa_sd_aktif = " . (int)$value_siswa, 'row');
			if (!$get_siswa) {
			...
			if (!empty($custom_nominal_spp) && $custom_nominal_spp > 0) {
				...
				$this->mymodel->update('siswa_sd_aktif', ['spp_custom' => $biaya], 'id_siswa_sd_aktif', $value_siswa);
				$spp_custom = $biaya;
			...
			if (!$cek_detail) {
				$detail_spp = array(
					...
					"id_kelas" => $get_siswa->id_kelas_sd,
				...
				if (empty($cek_detail[0]->id_kelas) || $cek_detail[0]->id_kelas != $get_siswa->id_kelas_sd) $update_spp['id_kelas'] = $get_siswa->id_kelas_sd;
				if (!empty($update_spp)) {
					$this->mymodel->update("spp_sd", $update_spp, "id", $id_detail_spp);
					$is_update = true;
				}
			}

			// Update kolom bulan di spp_sd
			foreach ($arrMonth as $in) {
				...
				if ($bln && in_array((int)$bulan[1], $bln)) {
					$this->mymodel->update("spp_sd", array($kolom_bulan => null), "id", $id_detail_spp);
				} else if (!$bln) {
					// Generate semua bulan, set null
					$this->mymodel->update("spp_sd", array($kolom_bulan => null), "id", $id_detail_spp);
				} else {
					// Bulan tidak dipilih, set '-'
					$this->mymodel->update("spp_sd", array($kolom_bulan => '-'), "id", $id_detail_spp);
				}
			}
			...
			foreach ($arrMonth as $i) {
				...
				// Filter no_transaksi LIKE '%sd%' untuk hindari konflik dengan SMP/SMA
	...
	{
		$this->is_allowed('spp_sd_update');

		$this->data['spp_sd'] = $this->model_spp_sd->find($id);

		$this->template->title('Spp sd Update');
		$this->render('backend/standart/administrator/spp_sd/spp_sd_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('spp_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_spp_sd = $this->model_spp_sd->change($id, $save_data);
			...
			if ($save_spp_sd) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/spp_sd', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/spp_sd');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	public function delete($id = null)
	{
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'spp_sd'), 'success');
		} else {
			set_message(cclang('error_delete', 'spp_sd'), 'error');
		}
	...
	{
		$this->is_allowed('spp_sd_view');

		$this->data['spp_sd'] = $this->model_spp_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Spp Sd Detail');
		$this->render('backend/standart/administrator/spp_sd/spp_sd_view', $this->data);
	}
	...
	{
		$spp_sd = $this->model_spp_sd->find($id);
		...
		return $this->model_spp_sd->remove($id);
	}
	...
	{
		$this->is_allowed('spp_sd_export');

		//$this->model_spp_sd->export('spp_sd', 'spp_sd');
		$field = $this->input->get("f");
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_sd spp join siswa_sd_aktif s on spp.id_siswa_aktif = s.id_siswa_sd_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
			...
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_sd spp join siswa_sd_aktif s on spp.id_siswa_aktif = s.id_siswa_sd_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran where ".$where." order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
		else{
	...
	public function export_pdf()
	{
		...
		$this->model_spp_sd->pdf('spp_sd', 'spp_sd');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_spp_sd->find($id);
	...
	public function sync_spp_data(){
		$get_spp = $this->mymodel->withquery("select id as id_spp, nama, id_siswa_aktif, id_tahun_ajaran from spp_sd order by id_spp ASC","result");
		foreach ($get_spp as $key => $value) {
			...
			foreach ($get_transaksi as $key_transaksi => $value_transaksi) {
				if (!empty($value_transaksi->updated_at)) {
					...
					$this->mymodel->update("spp_sd",$data_update,"id",$value->id_spp);
		...
		redirect('administrator/spp_sd');
	}
}


/* End of file spp_sd.php */
/* Location: ./application/controllers/administrator/Spp Sd.php */