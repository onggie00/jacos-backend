...
class Pendaftaran_ft extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        $gelombang = $this->mymodel->withquery("select gelombang from web_periode_daftar where periode_pendaftaran_mulai <= '".date("Y-m-d H:i:s")."' and periode_pendaftaran_selesai >= '".date("Y-m-d H:i:s")."' and kelas = 'France Track' ","row")->gelombang;
          ...
          $data = array(
            ...
            "peminatan_ft" => $this->post("peminatan_ft")
          );
          //"is_ft" => 1,
          ...
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              if (!empty($data_transaksi)) {
                ...
                $get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_ft as id_siswa from siswa_ft where va_number like '".$prefix.$client_id.$kode_tahun."9%' and va_number != '' order by id_siswa_ft DESC","row");
                ...
                $va_number = $prefix.$client_id.$kode_tahun."9".$no_urut;
                ...
                if(empty($payment_response['virtual_account'])){
                  for ($i=2; $i < 100; $i++) { 
                    $no_transaksi = "LI-FT-".date("Ymd").$i."-".$id_siswa;
