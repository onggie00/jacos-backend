...
class Pendaftaran_ft extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        $sekolah_asal = "";
        //get tahun ajaran
        $tahun_ajaran_aktif = $this->mymodel->withquery("select * from tahun_ajaran_psb where id_tahun_ajaran_psb = '1'","row")->label;
        $gelombang = $this->mymodel->withquery("select gelombang from web_periode_daftar where periode_pendaftaran_mulai <= '".date("Y-m-d H:i:s")."' and periode_pendaftaran_selesai >= '".date("Y-m-d H:i:s")."'","row")->gelombang;
          ...
          //cek email duplicate
          $cek_email = $this->mymodel->withquery("select * from siswa_ft where email = '".$this->post("email")."'", "row");
          if(!empty($cek_email)){
