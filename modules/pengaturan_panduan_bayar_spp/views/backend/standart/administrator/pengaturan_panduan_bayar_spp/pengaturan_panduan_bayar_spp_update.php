...
<section class="content-header">
    <h1>
        Pengaturan Panduan Bayar Spp        <small>Edit Pengaturan Panduan Bayar Spp</small>
    </h1>
    <ol class="breadcrumb">
                ...
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <div class="widget-user-header ">
                            ...
                            <h5 class="widget-user-desc">Edit Pengaturan Panduan Bayar Spp</h5>
                            <hr>
                        </div>
                        <?= form_open(base_url('administrator/pengaturan_panduan_bayar_spp/edit_save/'.$this->uri->segment(4)), [
                            ...
                            <div class="col-sm-8">
                                ...
                                <input class="data_file data_file_uuid" name="pengaturan_panduan_bayar_spp_file_pdf_uuid" id="pengaturan_panduan_bayar_spp_file_pdf_uuid" type="hidden" value="<?= set_value('pengaturan_panduan_bayar_spp_file_pdf_uuid'); ?>">
                                <input class="data_file" name="pengaturan_panduan_bayar_spp_file_pdf_name" id="pengaturan_panduan_bayar_spp_file_pdf_name" type="hidden" value="<?= set_value('pengaturan_panduan_bayar_spp_file_pdf_name', $pengaturan_panduan_bayar_spp->file_pdf); ?>">
                            ...
                            <div class="col-sm-8">
                                <select  class="form-control chosen chosen-select" name="jenjang" id="jenjang" data-placeholder="Select Jenjang" >
                                    <option value=""></option>
                                    <option <?= $pengaturan_panduan_bayar_spp->jenjang == "sd" ? 'selected' :''; ?> value="sd">SD</option>
                                    <option <?= $pengaturan_panduan_bayar_spp->jenjang == "smp" ? 'selected' :''; ?> value="smp">SMP</option>
                                    <option <?= $pengaturan_panduan_bayar_spp->jenjang == "sma" ? 'selected' :''; ?> value="sma">SMA</option>
                                    <option <?= $pengaturan_panduan_bayar_spp->jenjang == "ft" ? 'selected' :''; ?> value="ft">FT</option>
                                    </select>
                                <small class="info help-block">
...
<script>
    $(document).ready(function(){
      ...
      $('.btn_save').click(function(){
        ...
        $.ajax({
          url: form_pengaturan_panduan_bayar_spp.attr('action'),
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
        .done(function(res) {
          ...
          if(res.success) {
            var id = $('#pengaturan_panduan_bayar_spp_image_galery').find('li').attr('qq-file-id');
            if (save_type == 'back') {
       ...
       $('#pengaturan_panduan_bayar_spp_file_pdf_galery').fineUploader({
          ...
          thumbnails: {
           ...
           session : {
             endpoint: BASE_URL + 'administrator/pengaturan_panduan_bayar_spp/get_file_pdf_file/<?= $pengaturan_panduan_bayar_spp->id; ?>',
