...
class Spp_smp extends Admin
{
	...
	{
		$this->is_allowed('spp_smp_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['spp_smps'] = $this->model_spp_smp->get($filter, $field, $this->limit_page, $offset);
		$this->data['spp_smp_counts'] = $this->model_spp_smp->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/spp_smp/index/',
			'total_rows'   => $this->model_spp_smp->count_all($filter, $field),
		...
		$this->template->title('Spp Smp List');
		$this->render('backend/standart/administrator/spp_smp/spp_smp_list', $this->data);
	}

	public function add_save()
	{
		$this->load->library("session");
		if (!$this->is_allowed('spp_smp_list', false)) {
			echo json_encode([
				'success' => false,
		...
		if (empty($id_siswa)) {
			...
			$list_siswa = $this->mymodel->getbywhere("siswa_smp_aktif", "id_kelas", $this->input->post("id_kelas"), "result");
			$id_siswa = array();
			foreach ($list_siswa as $key => $value) {
				array_push($id_siswa, $value->id_siswa_smp_aktif);
			}
		...
		foreach ($id_siswa as $key_siswa => $value_siswa) {
			...
			$get_siswa = $this->mymodel->withquery("select sa.*,s.*,k.* from siswa_smp_aktif sa join siswa_smp s on s.id_siswa_smp = sa.id_siswa_smp join kelas_smp k on k.id_kelas_smp = sa.id_kelas where sa.id_siswa_smp_aktif = " . (int)$value_siswa, 'row');
			if (!$get_siswa) {
			...
			if (!empty($custom_nominal_spp) && $custom_nominal_spp > 0) {
				...
				$this->mymodel->update('siswa_smp_aktif', ['spp_custom' => $biaya], 'id_siswa_smp_aktif', $value_siswa);
				$spp_custom = $biaya;
			...
			$cek_detail = $this->mymodel->getbywhere("spp_smp", array("id_siswa_aktif" => $value_siswa, "id_tahun_ajaran" => $get_tahun_ajaran_aktif->id_tahun_ajaran), "row");
			...
			if (!$cek_detail) {
				...
				if (!empty($update_spp)) {
					$this->mymodel->update("spp_smp", $update_spp, "id", $id_detail_spp);
					$is_update = true;
				}
			}

			// Update kolom bulan di spp_smp
			foreach ($arrMonth as $in) {
				...
				if ($bln && in_array((int)$bulan[1], $bln)) {
					$this->mymodel->update("spp_smp", array($kolom_bulan => null), "id", $id_detail_spp);
				} else if (!$bln) {
					$this->mymodel->update("spp_smp", array($kolom_bulan => null), "id", $id_detail_spp);
				} else {
					$this->mymodel->update("spp_smp", array($kolom_bulan => '-'), "id", $id_detail_spp);
				}
			}
			...
			foreach ($arrMonth as $i) {
				...
				// Cek transaksi dengan filter jenjang SMP
				$cek_transaksi_bln = $this->mymodel->withquery("
					...
					AND (no_transaksi LIKE '%SMP%' OR no_transaksi LIKE '%smp%')
				", "row");
	...
	/**
	 * Update view Spp Smps
	 *
	 * @var $id String
	 */
	public function edit($id)
	{
		$this->is_allowed('spp_smp_update');

		$this->data['spp_smp'] = $this->model_spp_smp->find($id);

		$this->template->title('Spp Smp Update');
		$this->render('backend/standart/administrator/spp_smp/spp_smp_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('spp_smp_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_spp_smp = $this->model_spp_smp->change($id, $save_data);
			...
			if ($save_spp_smp) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/spp_smp', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/spp_smp');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	{
		$this->is_allowed('spp_smp_delete');
		...
		if ($remove) {
			set_message(cclang('has_been_deleted', 'spp_smp'), 'success');
		} else {
			set_message(cclang('error_delete', 'spp_smp'), 'error');
		}
	...
	{
		$this->is_allowed('spp_smp_view');

		$this->data['spp_smp'] = $this->model_spp_smp->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Spp Smp Detail');
		$this->render('backend/standart/administrator/spp_smp/spp_smp_view', $this->data);
	}
	...
	{
		$spp_smp = $this->model_spp_smp->find($id);
		...
		return $this->model_spp_smp->remove($id);
	}
	...
	{
		$this->is_allowed('spp_smp_export');

		//$this->model_spp_smp->export('spp_smp', 'spp_smp');
		$field = $this->input->get("f");
		...
		if (empty($field) && empty($inputan)) {
			//export all data
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_smp spp join siswa_smp_aktif s on spp.id_siswa_aktif = s.id_siswa_smp_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
			...
			$get_data = $this->mymodel->withquery("select spp.*, s.nama_lengkap, s.nis from spp_smp spp join siswa_smp_aktif s on spp.id_siswa_aktif = s.id_siswa_smp_aktif join tahun_ajaran t on spp.id_tahun_ajaran = t.id_tahun_ajaran where ".$where." order by spp.kelas ASC, spp.nama ASC, spp.tahun_ajaran DESC","result");
		}
		else{
		...
		header('Content-Disposition: attachment;filename="Rekap SPP SMP_'.date("Y-m-d Hi").'.xls"'); 
	...
	public function export_pdf()
	{
		...
		$this->model_spp_smp->pdf('spp_smp', 'spp_smp');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$data = $this->model_spp_smp->find($id);
	...
	public function sync_spp_data(){
		...
		redirect('administrator/spp_smp');
	}
...
/* End of file spp_smp.php */
/* Location: ./application/controllers/administrator/Spp Smp.php */