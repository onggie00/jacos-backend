...
/**
*| --------------------------------------------------------------------------
*| Program Anggaran Smp Controller
*| --------------------------------------------------------------------------
*| Program Anggaran Smp site
*|
*/
class Program_anggaran_smp extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_program_anggaran_smp');
	...
	function get_program($id)
    {
        ...
        $get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_smp", "nomor_program = '".$data->nomor_program."' and jenis_kegiatan=", $data->jenis_kegiatan,"result");
        $okr_terpakai = 0;
	...
	/**
	* Get detail Program Anggaran SMP untuk modal
	*
	* @var $id String
	* @return JSON
	*/
	...
	{
		if (!$this->is_allowed('program_anggaran_smp_view', false)) {
		...
		$program_anggaran_smp = $this->model_program_anggaran_smp->join_avaiable()->filter_avaiable()->find($id);

		if (!$program_anggaran_smp) {
			echo json_encode(array(
				'success' => false,
		...
		// Hitung sisa OKR
		$okr = $program_anggaran_smp->nominal_okr;
		$get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_smp", "nomor_program = '".$program_anggaran_smp->nomor_program."' and jenis_kegiatan=", $program_anggaran_smp->jenis_kegiatan, "result");
		$okr_terpakai = 0;
		foreach ($get_program_by_jenis_kegiatan as $key => $value) {
			$okr_terpakai = $okr_terpakai + $value->nominal_pengajuan;
		}
		...
		// Format tanggal
		$tanggal_program       = !empty($program_anggaran_smp->tanggal_program) ? date("d-m-Y", strtotime($program_anggaran_smp->tanggal_program)) : '-';
		$tanggal_pencairan     = !empty($program_anggaran_smp->tanggal_pencairan) ? date("d-m-Y", strtotime($program_anggaran_smp->tanggal_pencairan)) : '-';
		$tanggal_pengajuan     = !empty($program_anggaran_smp->created_at) ? date("d-m-Y H:i", strtotime($program_anggaran_smp->created_at)) : '-';

		// Status color
		$sp = $program_anggaran_smp->status_pengajuan;
		$sp_color = '#95a5a6';
		...
		$sl = $program_anggaran_smp->status_laporan;
		$sl_color = '#95a5a6';
		...
		$data = array(
			'success' => true,
			'nomor_program' => $program_anggaran_smp->nomor_program,
			'nama_program' => $program_anggaran_smp->nama_program,
			'jenis_kegiatan' => $program_anggaran_smp->jenis_kegiatan,
			'sub_jenis_kegiatan' => $program_anggaran_smp->sub_jenis_kegiatan,
			'tahun_ajaran' => $program_anggaran_smp->tahun_ajaran,
			'tanggal_program' => $tanggal_program,
			'tanggal_pengajuan' => $tanggal_pengajuan,
			'tanggal_pencairan' => $tanggal_pencairan,
			'nominal_okr' => number_format($program_anggaran_smp->nominal_okr, 0, ',', '.'),
			'nominal_pengajuan' => number_format($program_anggaran_smp->nominal_pengajuan, 0, ',', '.'),
			'sisa_okr' => number_format($sisa_okr, 0, ',', '.'),
			'dasar_pelaksanaan_kegiatan' => $program_anggaran_smp->dasar_pelaksanaan_kegiatan,
			'tema_kegiatan' => $program_anggaran_smp->tema_kegiatan,
			'tujuan_kegiatan' => $program_anggaran_smp->tujuan_kegiatan,
			'gambaran_acara_kegiatan' => $program_anggaran_smp->gambaran_acara_kegiatan,
			'hasil_yang_diharapkan' => $program_anggaran_smp->hasil_yang_diharapkan,
			'tempat_dan_waktu_pelaksanaan' => $program_anggaran_smp->tempat_dan_waktu_pelaksanaan,
			'mitra_kegiatan' => $program_anggaran_smp->mitra_kegiatan,
			'kepanitiaan' => $program_anggaran_smp->kepanitiaan,
			'catatan_pengajuan' => $program_anggaran_smp->catatan_pengajuan,
			'catatan_laporan' => $program_anggaran_smp->catatan_laporan,
			'status_pengajuan_text' => $program_anggaran_smp->program_anggaran_status_pengajuan_status,
			'status_pengajuan_color' => $sp_color,
			'status_laporan_text' => $program_anggaran_smp->program_anggaran_status_laporan_status,
			'status_laporan_color' => $sl_color,
			'file_proposal_pengajuan' => $program_anggaran_smp->file_proposal_pengajuan,
			'file_proposal_keuangan' => $program_anggaran_smp->file_proposal_keuangan,
			'file_laporan_kegiatan' => $program_anggaran_smp->file_laporan_kegiatan,
			'file_laporan_keuangan' => $program_anggaran_smp->file_laporan_keuangan,
			'file_kwitansi' => $program_anggaran_smp->file_kwitansi,
		);
	...
	/**
	* show all Program Anggaran Smps
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		$this->is_allowed('program_anggaran_smp_list');
		...
		$this->data['program_anggaran_smps'] = $this->model_program_anggaran_smp->get_filtered($filters, $search, $this->limit_page, $offset);
		$this->data['program_anggaran_smp_counts'] = $this->model_program_anggaran_smp->count_filtered($filters, $search);
		//$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_smp where (status_pengajuan != '4' or status_pengajuan != '6') and status_laporan != '4'","row")->total_proposal;
		$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_smp where (status_pengajuan = '1' or status_pengajuan = '2' or status_pengajuan = '3' or status_pengajuan = '5') or (status_laporan = '1' or status_laporan = '2' or status_laporan = '3' or status_laporan = '5' or status_laporan IS NULL)","row")->total_proposal;
		//$this->data['total_proposal'] = $this->mymodel->withquery("select count(id) as total_proposal from program_anggaran_smp where (status_laporan != '4')","row")->total_proposal;

		$config = [
			'base_url'     => 'administrator/program_anggaran_smp/index/',
			'total_rows'   => $this->model_program_anggaran_smp->count_filtered($filters, $search),
		...
		$this->template->title('Program Anggaran Smp List');
		$this->render('backend/standart/administrator/program_anggaran_smp/program_anggaran_smp_list', $this->data);
	}
	
	/**
	* Add new program_anggaran_smps
	*
	*/
	public function add()
	{
		$this->is_allowed('program_anggaran_smp_add');

		$this->template->title('Program Anggaran Smp New');
		$this->render('backend/standart/administrator/program_anggaran_smp/program_anggaran_smp_add', $this->data);
	}

	/**
	* Add New Program Anggaran Smps
	*
	* @return JSON
	*/
	public function add_save()
	{
		if (!$this->is_allowed('program_anggaran_smp_add', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		$this->form_validation->set_rules('nomor_program', 'Nomor Program', 'trim|required|max_length[255]');
		...
		$this->form_validation->set_rules('sub_jenis_kegiatan', 'Sub Jenis Kegiatan', 'trim|max_length[255]');
		$this->form_validation->set_rules('nominal_okr', 'Nominal OKR', 'trim|required|max_length[20]');
		$this->form_validation->set_rules('nominal_pengajuan', 'Nominal Pengajuan', 'trim|max_length[20]');
		

		if ($this->form_validation->run()) {
			$program_anggaran_smp_file_proposal_pengajuan_uuid = $this->input->post('program_anggaran_smp_file_proposal_pengajuan_uuid');
			$program_anggaran_smp_file_proposal_pengajuan_name = $this->input->post('program_anggaran_smp_file_proposal_pengajuan_name');
			$program_anggaran_smp_file_proposal_keuangan_uuid = $this->input->post('program_anggaran_smp_file_proposal_keuangan_uuid');
			$program_anggaran_smp_file_proposal_keuangan_name = $this->input->post('program_anggaran_smp_file_proposal_keuangan_name');
        	...
        	$get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_smp", "nomor_program = '".$get_program->nomor_program."' and jenis_kegiatan=", $get_program->jenis_kegiatan,"result");
        	$okr_terpakai = 0;
        	foreach ($get_program_by_jenis_kegiatan as $key => $value) {
                        $okr_terpakai = $okr_terpakai + $value->nominal_pengajuan;
        	}
			...
			if (!is_dir(FCPATH . '/uploads/program_anggaran_smp/')) {
				mkdir(FCPATH . '/uploads/program_anggaran_smp/');
			}

			if (!empty($program_anggaran_smp_file_proposal_pengajuan_name)) {
				$program_anggaran_smp_file_proposal_pengajuan_name_copy = date('YmdHis') . '-' . $program_anggaran_smp_file_proposal_pengajuan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_smp_file_proposal_pengajuan_uuid . '/' . $program_anggaran_smp_file_proposal_pengajuan_name, 
						FCPATH . 'uploads/program_anggaran_smp/' . $program_anggaran_smp_file_proposal_pengajuan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp_file_proposal_pengajuan_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['file_proposal_pengajuan'] = $program_anggaran_smp_file_proposal_pengajuan_name_copy;
			}
		
			if (!empty($program_anggaran_smp_file_proposal_keuangan_name)) {
				$program_anggaran_smp_file_proposal_keuangan_name_copy = date('YmdHis') . '-' . $program_anggaran_smp_file_proposal_keuangan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_smp_file_proposal_keuangan_uuid . '/' . $program_anggaran_smp_file_proposal_keuangan_name, 
						FCPATH . 'uploads/program_anggaran_smp/' . $program_anggaran_smp_file_proposal_keuangan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp_file_proposal_keuangan_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['file_proposal_keuangan'] = $program_anggaran_smp_file_proposal_keuangan_name_copy;
			}

			if (!empty($save_data['file_proposal_pengajuan']) && !empty($save_data['file_proposal_keuangan'])) {
				$save_data['status_pengajuan'] = "2";
				$data_email["file_proposal_keuangan"] = $program_anggaran_smp_file_proposal_keuangan_name_copy;
				$data_email["file_proposal_pengajuan"] = $program_anggaran_smp_file_proposal_pengajuan_name_copy;
				$data["pengajuan_laporan"] = "pengajuan";
			}		
			
			$save_program_anggaran_smp = $this->model_program_anggaran_smp->store($save_data);
            

			if ($save_program_anggaran_smp) {
				if ($save_data['status_pengajuan'] == 2) {
					//kirim email ke Bagian Anggaran
					$save_data["nominal_okr"] = $get_program->nominal_okr;
					$save_data["id_program"] = $save_program_anggaran_smp;
					...
					$data_email["id_program"] = $save_program_anggaran_smp;
					$data_email["jenis_anggaran"] = "SMP";
					$data_email["data_anggaran"] = $save_data;
	                $this->send_email_file("",$data_email['email'],$data_email);	
				}
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_program_anggaran_smp;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/program_anggaran_smp/edit/' . $save_program_anggaran_smp, 'Edit Program Anggaran Smp'),
						anchor('administrator/program_anggaran_smp', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/program_anggaran_smp');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = false;
		...
		/**
	* Update view Program Anggaran Smps
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('program_anggaran_smp_update');

		$this->data['program_anggaran_smp'] = $this->model_program_anggaran_smp->find($id);

		$this->template->title('Program Anggaran Smp Update');
		$this->render('backend/standart/administrator/program_anggaran_smp/program_anggaran_smp_update', $this->data);
	}

	/**
	* Update Program Anggaran Smps
	*
	* @var $id String
	*/
	public function edit_save($id)
	{
		if (!$this->is_allowed('program_anggaran_smp_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		if ($this->form_validation->run()) {
			...
			$program_anggaran_smp_file_proposal_keuangan_uuid = $this->input->post('program_anggaran_smp_file_proposal_keuangan_uuid');
			$program_anggaran_smp_file_proposal_keuangan_name = $this->input->post('program_anggaran_smp_file_proposal_keuangan_name');
			$program_anggaran_smp_file_laporan_kegiatan_uuid = $this->input->post('program_anggaran_smp_file_laporan_kegiatan_uuid');
			$program_anggaran_smp_file_laporan_kegiatan_name = $this->input->post('program_anggaran_smp_file_laporan_kegiatan_name');
		
			//cek OKR
			//hitung max okr yang tersisa
			$get_program = $this->mymodel->getbywhere("program_anggaran_smp","id",$id,"row");
            $okr = $get_program->nominal_okr;
            $this->db->where("nomor_program", $this->input->post('nomor_program'));
            $this->db->where("jenis_kegiatan", $this->input->post('jenis_kegiatan'));
            $this->db->where("id !=", $id);
            $get_program_by_jenis_kegiatan = $this->db->get("program_anggaran_smp")->result();
            $okr_terpakai = 0;
            foreach ($get_program_by_jenis_kegiatan as $key => $value) {
                $okr_terpakai = $okr_terpakai + $value->nominal_pengajuan;
            }
			...
			if (!empty($program_anggaran_smp_file_proposal_pengajuan_uuid)) {
				$program_anggaran_smp_file_proposal_pengajuan_name_copy = date('YmdHis') . '-' . $program_anggaran_smp_file_proposal_pengajuan_name;
			...
			if (!empty($program_anggaran_smp_file_proposal_keuangan_uuid)) {
				$program_anggaran_smp_file_proposal_keuangan_name_copy = date('YmdHis') . '-' . $program_anggaran_smp_file_proposal_keuangan_name;
			...
			if (!empty($program_anggaran_smp_file_laporan_kegiatan_uuid)) {
				$program_anggaran_smp_file_laporan_kegiatan_name_copy = date('YmdHis') . '-' . $program_anggaran_smp_file_laporan_kegiatan_name;

				rename(FCPATH . 'uploads/tmp/' . $program_anggaran_smp_file_laporan_kegiatan_uuid . '/' . $program_anggaran_smp_file_laporan_kegiatan_name, 
						FCPATH . 'uploads/program_anggaran_smp/' . $program_anggaran_smp_file_laporan_kegiatan_name_copy);

				if (!is_file(FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp_file_laporan_kegiatan_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['file_laporan_kegiatan'] = $program_anggaran_smp_file_laporan_kegiatan_name_copy;
				$save_data['status_laporan'] = 2;
				$save_data['status'] = 2;
				$save_data['catatan_laporan'] = "";
				$data_email["file_laporan_kegiatan"] = $program_anggaran_smp_file_laporan_kegiatan_name_copy;
				$data["pengajuan_laporan"] = "laporan";
			}
		
			$listed_image = [];
			//if (count((array) $this->input->post('program_anggaran_smp_file_laporan_keuangan_name'))) {
			if (!empty($this->input->post('program_anggaran_smp_file_laporan_keuangan_name')[0])) {
				foreach ((array) $_POST['program_anggaran_smp_file_laporan_keuangan_name'] as $idx => $file_name) {
					if (isset($_POST['program_anggaran_smp_file_laporan_keuangan_uuid'][$idx]) AND !empty($_POST['program_anggaran_smp_file_laporan_keuangan_uuid'][$idx])) {
						$program_anggaran_smp_file_laporan_keuangan_name_copy = date('YmdHis') . '-' . $idx . '-' . $file_name;

						rename(FCPATH . 'uploads/tmp/' . $_POST['program_anggaran_smp_file_laporan_keuangan_uuid'][$idx] . '/' .  $file_name, 
								FCPATH . 'uploads/program_anggaran_smp/' . $program_anggaran_smp_file_laporan_keuangan_name_copy);

						$listed_image[] = $program_anggaran_smp_file_laporan_keuangan_name_copy;

						if (!is_file(FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp_file_laporan_keuangan_name_copy)) {
							echo json_encode([
								'success' => false,
								'message' => 'Error uploading file'
								]);
			...
			$save_program_anggaran_smp = $this->model_program_anggaran_smp->change($id, $save_data);

			if ($save_program_anggaran_smp) {
				if (!empty($this->input->post('tanggal_pencairan')) && $this->session->userdata('user_keuangan') == true) {
					$this->cetak_kwitansi_program($id, "SMP");
				}
	...
	/**
	* delete Program Anggaran Smps
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		$this->is_allowed('program_anggaran_smp_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'program_anggaran_smp'), 'success');
        } else {
            set_message(cclang('error_delete', 'program_anggaran_smp'), 'error');
        }
		...
		/**
	* View view Program Anggaran Smps
	*
	* @var $id String
	*/
	public function view($id)
	{
		$this->is_allowed('program_anggaran_smp_view');

		$this->data['program_anggaran_smp'] = $this->model_program_anggaran_smp->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Program Anggaran Smp Detail');
		$this->render('backend/standart/administrator/program_anggaran_smp/program_anggaran_smp_view', $this->data);
	}
	...
	*/
	...
	{
		$program_anggaran_smp = $this->model_program_anggaran_smp->find($id);

		if (!empty($program_anggaran_smp->file_proposal_pengajuan)) {
			$path = FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp->file_proposal_pengajuan;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($program_anggaran_smp->file_proposal_keuangan)) {
			$path = FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp->file_proposal_keuangan;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($program_anggaran_smp->file_laporan_kegiatan)) {
			$path = FCPATH . '/uploads/program_anggaran_smp/' . $program_anggaran_smp->file_laporan_kegiatan;
		...
		if (!empty($program_anggaran_smp->file_laporan_keuangan)) {
			foreach ((array) explode(',', $program_anggaran_smp->file_laporan_keuangan) as $filename) {
				$path = FCPATH . '/uploads/program_anggaran_smp/' . $filename;
		...
		return $this->model_program_anggaran_smp->remove($id);
	}
	
	/**
	* Upload Image Program Anggaran Smp	* 
	* @return JSON
	*/
	public function upload_file_proposal_pengajuan_file()
	{
		if (!$this->is_allowed('program_anggaran_smp_add', false) && !$this->is_allowed('program_anggaran_smp_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		echo $this->upload_file([
			'uuid' 		 	=> $uuid,
			'table_name' 	=> 'program_anggaran_smp',
		]);
	}

	/**
	* Delete Image Program Anggaran Smp	* 
	* @return JSON
	*/
	public function delete_file_proposal_pengajuan_file($uuid)
	{
		if (!$this->is_allowed('program_anggaran_smp_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'table_name'        => 'program_anggaran_smp',
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_smp/'
        ]);
	}

	/**
	* Get Image Program Anggaran Smp	* 
	* @return JSON
	*/
	public function get_file_proposal_pengajuan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_smp/',
            'delete_endpoint'   => 'administrator/program_anggaran_smp/delete_file_proposal_pengajuan_file'
        ]);
	}
	...
	*/
	public function get_file_proposal_keuangan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_smp/',
            'delete_endpoint'   => 'administrator/program_anggaran_smp/delete_file_proposal_keuangan_file'
        ]);
	}
	...
	*/
	public function get_file_laporan_kegiatan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_smp/',
            'delete_endpoint'   => 'administrator/program_anggaran_smp/delete_file_laporan_kegiatan_file'
        ]);
	}
	...
	*/
	public function get_file_laporan_keuangan_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/program_anggaran_smp/',
            'delete_endpoint'   => 'administrator/program_anggaran_smp/delete_file_laporan_keuangan_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('program_anggaran_smp_export');

		//$this->model_program_anggaran_smp->export('program_anggaran_smp', 'program_anggaran_smp');
		...
		{
			if (strpos($field_search[$i], "created_at")) {
		...
		foreach ($get_data as $key => $value) {
            ...
            $get_program_by_jenis_kegiatan = $this->mymodel->getbywhere("program_anggaran_smp", "nomor_program = '".$value->nomor_program."' and jenis_kegiatan=", $value->jenis_kegiatan,"result");
            $okr_terpakai = 0;
			...
			for ($j=0; $j < count($field_search); $j++) {
		        ...
		        if (strpos($kolom, "file") !== false && !empty($value_data)) {
		        	$value_data = base_url("uploads/program_anggaran_smp/").$value_data;
		        }
		...
		header('Content-Disposition: attachment;filename="Program Anggaran SMP_'.date("Y-m-d Hi").'.xls"'); 
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_program_anggaran_smp->pdf('program_anggaran_smp', 'program_anggaran_smp');
	}
	...
	public function single_pdf($id = null)
	{
		...
		$table = $title = 'program_anggaran_smp';
        ...
        $data = $this->model_program_anggaran_smp->find($id);
	...
	public function send_email_file($file="",$to='',$data)
    {
      ...
      $mail->addAddress($to);
      //$mail->addAddress("onggiedannys@gmail.com");
	...
	public function cetak_kwitansi_program($id_program, $jenjang_program){
        ...
        foreach ($array_program as $key => $value) {
		  ...
		  $this->mymodel->update("program_anggaran_smp", array("file_kwitansi" => $nama_file), "id", $id_program);
		  //var_dump($data);
		  $pdf->Output(FCPATH.'/uploads/program_anggaran_smp/'.$nama_file, 'F');
	  }

	  public function resend_email($id){
	  	$get_program = $this->mymodel->withquery("select * from program_anggaran_smp where id='".$id."'","row");
	  	$save_data = [
			'nomor_program' => $get_program->nomor_program,
			'jenis_kegiatan' => $get_program->jenis_kegiatan,
			'sub_jenis_kegiatan' => $get_program->sub_jenis_kegiatan,
		...
		$save_data["id_program"] = $id;
		$save_data["jenis_anggaran"] = "SMP";
		$save_data["data_anggaran"] = $save_data;
		$this->send_email_file("",$save_data['email'],$save_data);
		redirect('administrator/program_anggaran_smp','refresh');
	 }
	
}


/* End of file program_anggaran_smp.php */
/* Location: ./application/controllers/administrator/Program Anggaran Smp.php */