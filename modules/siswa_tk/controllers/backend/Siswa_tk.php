...
class Siswa_tk extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$limit = 20;
		// dd($offset);
		$this->data['siswa_tks'] = $this->model_siswa_tk->get($filter, $field, $limit, $offset, [], $sort, $sort_type);
		$this->data['siswa_tk_counts'] = $this->model_siswa_tk->count_all($filter, $field);
		
		// Info box data
		$this->data['total_daftar'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_tk', 'row')->cnt;
		$this->data['total_aktif'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_tk_aktif', 'row')->cnt;
		...
		if ($tahun_ajaran_aktif) {
			...
			$this->data['total_daftar_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_tk WHERE id_siswa_tk IN (SELECT id_siswa_tk FROM siswa_tk_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta . ')', 'row')->cnt;
			$this->data['total_aktif_ta'] = $this->mymodel->withquery('SELECT COUNT(*) AS cnt FROM siswa_tk_aktif WHERE id_tahun_ajaran = ' . (int)$id_ta, 'row')->cnt;
		} else {
	...
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			...
			// Validasi usia sesuai tingkatan TK
			...
			if ($save_siswa_tk) {
				...
				$no_transaksi = "LI-TK-" . date("Ymd") . "-" . $save_siswa_tk;
				...
				$data_transaksi = array(
					...
					"user_phone" => "",
					"description" => "Tagihan Pendaftaran TK a.n " . strtoupper($this->input->post('nama_lengkap')),
				...
				$this->mymodel->update("siswa_tk", array("no_transaksi" => $no_transaksi), "id_siswa_tk", $save_siswa_tk);
			}
	...
	public function edit($id)
	{
		...
		$this->db->join('status_daftar_ulang_tk daftar_ulang', 'daftar_ulang.id_siswa_tk = siswa_tk.id_siswa_tk', 'LEFT');
	...
	public function edit_save($id)
	{
		...
		//$this->form_validation->set_rules('siswa_tk_foto_peserta_name', 'Foto Peserta', 'trim');
		//$this->form_validation->set_rules('siswa_tk_akte_lahir_name', 'Akte Lahir', 'trim');
		///$this->form_validation->set_rules('siswa_tk_kartu_keluarga_name', 'Kartu Keluraga', 'trim');
		...
		if ($this->form_validation->run()) {
			...
			// Validasi usia sesuai tingkatan TK (bila tingkatan diubah)
			$get_tingkatan = $this->mymodel->getbywhere("tingkatan_tk", "id_tingkatan_tk", $this->input->post('id_tingkatan'), "row");
			if (!$this->_cek_usia($this->input->post('tgl_lahir'), $get_tingkatan)) {
				$this->data['success'] = false;
				$this->data['message'] = 'Usia calon siswa tidak sesuai dengan tingkatan ' . (!empty($get_tingkatan) ? $get_tingkatan->label : '');
			...
			if (count($update_transaksi)) {
				$this->mymodel->update3('transaksi', $update_transaksi, array('user_email' => $old_data->email), array('no_transaksi' => 'TK'));
			}
				...
				if ($get_transaksi->status_transaksi == 1) { //jika sudah membayar 
					//if ($this->input->post('no_peserta') != $old_data->no_peserta || $old_data->foto_peserta != $siswa_tk_foto_peserta_name_copy || $this->input->post('nama_lengkap') != $old_data->nama_lengkap) { // cek jika ada perubahan no peserta
						...
						if ($jenis_pembayaran == 'LI') { //cetak kartu peserta dan kwitansi ulang
							$this->cetak_kartu(array("tipe_siswa" => 'tk', "id_siswa" => $id));
							...
							$this->cetak_kartu_siswa_sementara(array("tipe_siswa" => 'tk', "id_siswa" => $id));
							$this->cetak_kwitansi(array(
								"tipe_siswa" => 'tk',
								"jenjang" => $jenjang,
								"nama_ortu" => $nama_ortu,
			...
			if (!empty($save_siswa_tk) && $this->input->post('status_lulus') == "2") {
		...
		if (isset($_FILES["file_siswa"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					$check=$this->mymodel->withquery("select * from siswa_tk where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					...
					$check2=$this->mymodel->withquery("select * from siswa_tk where email_ms_office_ortu = '".$worksheet->getCellByColumnAndRow(3, $row)->getValue()."'", 'row');
					if(!empty($check2)){
					...
					$check = $this->mymodel->withquery("select * from siswa_tk where nama_lengkap like '%".ucwords($worksheet->getCellByColumnAndRow(0, $row)->getValue())."%'", 'row');
					
					if($check){
						$insertId = $this->mymodel->update("siswa_tk", $data_pendaftaran, 'id_siswa_tk', $check->id_siswa_tk);
						$insertId = $check->id_siswa_tk;
					}else{
						$insertId = $this->mymodel->insertid("siswa_tk", $data_pendaftaran);
					}
					...
					if(!$check){
						//CREATE TRANSAKSI LUNAS
						$no_transaksi = "LI-TK-" . date("Ymd") . "-" . $insertId;
						//get biaya pendaftaran
						$get_biaya = $this->mymodel->getbywhere("biaya_pendaftaran", "jenjang", "TK", "row");
						$total_biaya = $get_biaya->nominal_pendaftaran;
						$data_transaksi = array(
							...
							"user_phone" => "",
							"description" => "Tagihan Pendaftaran TK a.n " . strtoupper($data_pendaftaran['nama_lengkap']),
							"id_biaya_pendaftaran" => $get_biaya->id_biaya_pendaftaran,
							"total_biaya" => $total_biaya,
							"status_transaksi" => "1",
							"created_at" => date("Y-m-d H:i:s"),
						...
						$id_transaksi = $this->mymodel->insertid("transaksi", $data_transaksi);
						$this->mymodel->update("siswa_tk", array("no_transaksi" => $no_transaksi), "id_siswa_tk", $insertId);
					}
				}
			}
	...
	public function update_status_daftar_ulang($id = null)
	{
		...
		$arr_id = $this->input->get('id');
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->withquery("select id_siswa_tk, no_peserta, nama_lengkap, va_number from siswa_tk where id_siswa_tk = '" . $id ."'" ,"row");
			$data_daftar_ulang = array(
				"id_siswa_tk" => $id,
				"tanggal_lulus" =>  date("Y-m-d H:i:s"),
			...
			$this->mymodel->update('siswa_tk', array('status_lulus' => $this->input->get('st')), 'id_siswa_tk', $id);
			$insert = $this->mymodel->insertid("status_daftar_ulang_tk", $data_daftar_ulang);
			// dd(!$insert);
			if (!$insert) {
				$this->db->trans_rollback();
	...
	public function update_status($id = null)
	{
		...
		foreach ($arr_id as $id) {
			...
			if ($get_siswa->status_lulus == 2) {
				$get_status_du = $this->mymodel->getbywhere("status_daftar_ulang_tk", "id_siswa_tk", $id, "row");
	...
	public function download_file($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $id, "row");
			$data[] = [
				"file_foto_path" => FCPATH . 'uploads/siswa_tk/' . $get_siswa->foto_peserta,
				"file_foto" => $get_siswa->foto_peserta,
				"file_kk_path" => FCPATH . 'uploads/siswa_tk/' . $get_siswa->kartu_keluarga,
				"file_kk" => $get_siswa->kartu_keluarga,
				"file_akte_path" => FCPATH . 'uploads/siswa_tk/' . $get_siswa->akte_lahir,
				"file_akte" => $get_siswa->akte_lahir,
				"nama_folder" => $get_siswa->id_siswa_tk . "_" . str_replace(" ", "_", $get_siswa->nama_lengkap)
			];
		}
		// dd($data);
	...
	public function download_kartu_peserta($id = null)
	{
		...
		foreach ($arr_id as $id) {
			$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $id, "row");
			$kartu_peserta = 'tk' . '-' . $get_siswa->no_peserta . '-' . $get_siswa->nama_lengkap . ".pdf";
	...
	public function export()
	{
		...
		// dd($field);
		$this->model_siswa_tk->export_siswa('siswa_tk', 'siswa_tk', $value, $col);
	}
	...
	/**
	 * Generate VA BNI utk siswa TK (kode VA 12).
	 * Pemakaian: administrator/siswa_tk/generate_va_bni?id_siswa=<id>
	 */
	public function generate_va_bni()
	{
		...
		$get_siswa = $this->mymodel->getbywhere("siswa_tk", "id_siswa_tk", $id_siswa, "row");
		if (empty($get_siswa)) {
		...
		$kode_va = "12"; // TK=12, KB=11
		$prefix_cari = $prefix . $client_id . date('y', strtotime('+1 years')) . $kode_va;
		$get_no_urut = $this->mymodel->withquery("select va_number, id_siswa_tk as id_siswa from siswa_tk where va_number like '" . $prefix_cari . "%' and va_number != '' order by id_siswa_tk DESC", "row");
		...
		$this->mymodel->update("siswa_tk", array("va_number" => $payment_response['virtual_account']), "id_siswa_tk", $id_siswa);
