...
class Model_siswa_kb extends MY_Model
{

    private $primary_key    = 'id_siswa_kb';
    private $table_name     = 'siswa_kb';
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_kb." . $field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_kb." . $field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
        ...
        $this->db->where($where);
        $this->db->where('siswa_kb.is_show', 1);
        $this->db->group_by('siswa_kb.id_siswa_kb');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null)
    {
        ...
        $sort_by = (empty($sort)) ? 'id_siswa_kb' : $sort;
        ...
        $this->db->order_by('siswa_kb.' . $sort_by, $sort_type);
        $this->db->group_by('siswa_kb.id_siswa_kb');
        $query = $this->db->get($this->table_name);
    ...
    {
        $this->db->join('provinces', 'provinces.id = siswa_kb.provinsi', 'LEFT');
        $this->db->join('areas', 'areas.id = siswa_kb.kelurahan', 'LEFT');
        $this->db->join('districts', 'districts.id = siswa_kb.kecamatan', 'LEFT');
        $this->db->join('regencies', 'regencies.id = siswa_kb.kota', 'LEFT');
        $this->db->join('status_lulus', 'status_lulus.id_status_lulus = siswa_kb.status_lulus', 'LEFT');
        $this->db->join('tingkatan_kb', 'tingkatan_kb.id_tingkatan_kb = siswa_kb.id_tingkatan', 'LEFT');
/*        $this->db->join('provinces provinces1', 'provinces1.id = siswa_kb.provinsi_sekolah', 'LEFT');
        $this->db->join('regencies regencies1', 'regencies1.id = siswa_kb.kota_sekolah', 'LEFT');
        $this->db->join('districts districts1', 'districts1.id = siswa_kb.kecamatan_sekolah', 'LEFT');
        $this->db->join('areas areas1', 'areas1.id = siswa_kb.kelurahan_sekolah', 'LEFT');*/
        $this->db->join('status_daftar_ulang_kb daftar_ulang', 'daftar_ulang.id_siswa_kb = siswa_kb.id_siswa_kb', 'LEFT');
        $this->db->join('transaksi', 'transaksi.user_email = siswa_kb.email', 'LEFT');
        $this->db->group_start();
        $this->db->where('transaksi.nama_bank', 'BNI');
        $this->db->or_where('transaksi.nama_bank', 'BRI');
        $this->db->group_end();
        $this->db->like(array('transaksi.no_transaksi'=>'LI-KB'));
        $this->db->select('siswa_kb.*,
                           ...
                           (CASE
                            ...
                            (CASE WHEN siswa_kb.is_mutasi = 1 THEN "YA" ELSE "TIDAK" END) as is_mutasi,
                           tingkatan_kb.label as tingkatan_label
                        ');
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->where('siswa_kb.is_show', 1);
        $this->db->order_by('id_siswa_kb', 'ASC');
        return $this;
    }
    ...
    public function export_siswa($table, $subject = 'file',$value=null,$col=null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_kb." . $field . " LIKE '%" . $value . "%' ";
                } else {
                    $where .= "OR " . "siswa_kb." . $field . " LIKE '%" . $value . "%' ";
                }
                $iterasi++;
            }
...
/* End of file Model_siswa_kb.php */
/* Location: ./application/models/Model_siswa_kb.php */
