...
class Cron_presensi_siswa extends MY_Controller {
    ...
    if(isset($headers['x-token']))
        ...
        $where_cron_setting = "";
        //cek cron setting
        $where_cs_sd = "and cs.nama_cron = 'cron_presensi_siswa' and cs.is_active = '1' and (cs.id_siswa_aktif = sa.id_siswa_sd_aktif)";
        ...
        /* $get_siswa_sd = $this->mymodel->withquery("select sa.id_siswa_sd_aktif as id_siswa_aktif, sa.nama_lengkap, sa.device_id_siswa, sa.device_id_ortu, s.notelp_ibu, s.notelp_ayah from siswa_sd_aktif sa 
        join siswa_sd s on sa.id_siswa_sd = s.id_siswa_sd and s.deleted_at is null 
        join kelas_sd k on sa.id_kelas = k.id_kelas_sd 
        left join cron_setting cs on sa.id_siswa_sd_aktif = cs.id_siswa_aktif and cs.id_kelas = k.id_kelas_sd and cs.jenjang = 'sd'
        where sa.id_tahun_ajaran = '".$tahun_ajaran_aktif."' $where_cs_sd 
        and (k.label not like '%alumni%' and k.label not like '%mutasi%') 
        and sa.id_siswa_sd_aktif not in (select id_siswa_aktif from presensi_sd where tanggal_absen = '".$today."') 
        and (cs.is_active = '1')
        order by nama_lengkap ASC", "result"); */
        $get_siswa_smp = $this->mymodel->withquery("select sa.id_siswa_smp_aktif as id_siswa_aktif, sa.nama_lengkap, sa.device_id_siswa, sa.device_id_ortu, s.notelp_ibu, s.notelp_ayah from siswa_smp_aktif sa 
        join siswa_smp s on sa.id_siswa_smp = s.id_siswa_smp and s.deleted_at is null 
        join kelas_smp k on sa.id_kelas = k.id_kelas_smp 
        left join cron_setting cs on sa.id_siswa_smp_aktif = cs.id_siswa_aktif and cs.id_kelas = k.id_kelas_smp and cs.jenjang = 'smp'
        where sa.id_tahun_ajaran = '".$tahun_ajaran_aktif."' $where_cs_smp 
        and (k.label not like '%alumni%' and k.label not like '%mutasi%') 
        and sa.id_siswa_smp_aktif not in (select id_siswa_aktif from presensi_smp where tanggal_absen = '".$today."') 
        and (cs.is_active = '1')
        order by nama_lengkap ASC", "result");
        $get_siswa_sma = $this->mymodel->withquery("select sa.id_siswa_sma_aktif as id_siswa_aktif, sa.nama_lengkap, sa.device_id_siswa, sa.device_id_ortu, s.notelp_ibu, s.notelp_ayah from siswa_sma_aktif sa 
        join siswa_sma s on sa.id_siswa_sma = s.id_siswa_sma and s.deleted_at is null 
        join kelas_sma k on sa.id_kelas = k.id_kelas_sma 
        left join cron_setting cs on sa.id_siswa_sma_aktif = cs.id_siswa_aktif and cs.id_kelas = k.id_kelas_sma and cs.jenjang = 'sma'
        where sa.id_tahun_ajaran = '".$tahun_ajaran_aktif."' $where_cs_sma 
        and (k.label not like '%alumni%' and k.label not like '%mutasi%' ) 
        and sa.id_siswa_sma_aktif not in (select id_siswa_aktif from presensi_sma where tanggal_absen = '".$today."') 
        and (cs.is_active = '1')
        order by nama_lengkap ASC", "result");
        $get_siswa_ft = $this->mymodel->withquery("select sa.id_siswa_ft_aktif as id_siswa_aktif, sa.nama_lengkap, sa.device_id_siswa, sa.device_id_ortu, s.notelp_ibu, s.notelp_ayah from siswa_ft_aktif sa 
        join siswa_ft s on sa.id_siswa_ft = s.id_siswa_ft and s.deleted_at is null 
        join kelas_ft k on sa.id_kelas = k.id_kelas_ft 
        left join cron_setting cs on sa.id_siswa_ft_aktif = cs.id_siswa_aktif and cs.id_kelas = k.id_kelas_ft and cs.jenjang = 'ft'
        where sa.id_tahun_ajaran = '".$tahun_ajaran_aktif."' $where_cs_ft 
        and (k.label not like '%alumni%' and k.label not like '%mutasi%' ) 
        and sa.id_siswa_ft_aktif not in (select id_siswa_aktif from presensi_ft where tanggal_absen = '".$today."') 
        and (cs.is_active = '1')
        order by nama_lengkap ASC", "result");
