...
class Guru_sd extends Admin	
{
	...
	*/
	public function index($offset = 0)
	{
		...
		$this->data['guru_sds'] = $this->model_guru_sd->get($filter, $field, $this->limit_page, $offset,[],$sort,$sort_type);
		$this->data['guru_sd_counts'] = $this->model_guru_sd->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/guru_sd/index/',
			'total_rows'   => $this->model_guru_sd->count_all($filter, $field),
		...
		$this->template->title('Guru Sd List');
		$this->render('backend/standart/administrator/guru_sd/guru_sd_list', $this->data);
	}
	
	public function add_slip_gaji()
	{
		$this->is_allowed('guru_sd_add');

		$this->template->title('Guru SD Slip Gaji');
		$this->render('backend/standart/administrator/guru_sd/guru_sd_add_slip_gaji', $this->data);
	}

	public function add_slip_gaji_save()
	{
		if (!$this->is_allowed('guru_sd_add', false)) {
			echo json_encode([
				'success' => false,
		...
		foreach ($ids as $key => $item) {
			...
			if (!is_dir(FCPATH . '/uploads/guru_sd/')) {
				mkdir(FCPATH . '/uploads/guru_sd/');
			}
			...
			if (!empty($slip_gaji_name)) {
				...
				rename(
					...
					FCPATH . 'uploads/guru_sd/' . $slip_gaji_name_copy
				);

				if (!is_file(FCPATH . '/uploads/guru_sd/' . $slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
			...
			$save_guru_sd = $this->model_guru_sd->change($item, $save_data);
		}
		...
		if ($save_guru_sd) {
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->data['id'] 	   = $save_guru_sd;
				$this->data['message'] = cclang('success_save_data_stay', [

					anchor('administrator/guru_sd', ' Go back to list')
				]);
			} else {
				set_message(
					cclang('success_save_data_redirect', [
						anchor('administrator/guru_sd/edit/' . $save_guru_sd, 'Edit Siswa Ft')
					]),
					'success'
				);

				$this->data['success'] = true;
				$this->data['redirect'] = base_url('administrator/guru_sd');
			}
		} else {
			if ($this->input->post('save_type') == 'stay') {
	...
	*/
	public function add()
	{
		...
		$this->template->title('Guru Sd New');
		$this->render('backend/standart/administrator/guru_sd/guru_sd_add', $this->data);
	}
	...
	*/
	public function add_save()
	{
		...
		$this->form_validation->set_rules('presensi_role', 'Emp Code', 'trim|required');
		...
		if ($this->form_validation->run()) {
			$guru_sd_foto_profil_uuid = $this->input->post('guru_sd_foto_profil_uuid');
			$guru_sd_foto_profil_name = $this->input->post('guru_sd_foto_profil_name');
			$guru_sd_slip_gaji_uuid = $this->input->post('guru_sd_slip_gaji_uuid');
			$guru_sd_slip_gaji_name = $this->input->post('guru_sd_slip_gaji_name');
		
			$check=$this->mymodel->withquery("select * from guru_sd where emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
			...
			if (!empty($guru_sd_foto_profil_name)) {
				$guru_sd_foto_profil_name_copy = date('YmdHis') . '-' . $guru_sd_foto_profil_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_sd_foto_profil_uuid . '/' . $guru_sd_foto_profil_name, 
						FCPATH . 'uploads/guru_sd/' . $guru_sd_foto_profil_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_sd/' . $guru_sd_foto_profil_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['foto_profil'] = $guru_sd_foto_profil_name_copy;
			}
		
			if (!empty($guru_sd_slip_gaji_name)) {
				$guru_sd_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_sd_slip_gaji_name;

				rename(FCPATH . 'uploads/tmp/' . $guru_sd_slip_gaji_uuid . '/' . $guru_sd_slip_gaji_name, 
						FCPATH . 'uploads/guru_sd/' . $guru_sd_slip_gaji_name_copy);

				if (!is_file(FCPATH . '/uploads/guru_sd/' . $guru_sd_slip_gaji_name_copy)) {
					echo json_encode([
						'success' => false,
						'message' => 'Error uploading file'
						]);
				...
				$save_data['slip_gaji'] = $guru_sd_slip_gaji_name_copy;
			}
		
			
			$save_guru_sd = $this->model_guru_sd->store($save_data);
			...
			if ($save_guru_sd) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_guru_sd;
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/guru_sd/edit/' . $save_guru_sd, 'Edit Guru Sd'),
						anchor('administrator/guru_sd', ' Go back to list')
					]);
				} else {
					set_message(
		...
		/**
	* Update view Guru Sds
	*
	* @var $id String
	*/
	public function edit($id)
	{
		$this->is_allowed('guru_sd_update');

		$this->data['guru_sd'] = $this->model_guru_sd->find($id);

		$this->template->title('Guru Sd Update');
		$this->render('backend/standart/administrator/guru_sd/guru_sd_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('guru_sd_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
		...
		$this->form_validation->set_rules('informasi_kepala_pimpinan', 'Informasi Kepala Pimpinan', 'trim|max_length[255]');
		$this->form_validation->set_rules('emp_code', 'emp code', 'trim|required');
		$this->form_validation->set_rules('presensi_role', 'Presensi Role', 'trim|required');
		$this->form_validation->set_rules('keterangan_jabatan', 'Keterangan Jabatan', 'trim|max_length[50]');
		$this->form_validation->set_rules('jam_ajar', 'Jam Ajar', 'trim|max_length[50]');
		$this->form_validation->set_rules('pendidikan_terakhir', 'Pendidikan Terakhir', 'trim|max_length[50]');
		$this->form_validation->set_rules('universitas', 'Universitas', 'trim|max_length[200]');
		...
		if ($this->form_validation->run()) {
			...
			$check=$this->mymodel->withquery("select * from guru_sd where id_guru != $id and emp_code = ".$this->input->post('emp_code'), 'row');
			if($check){
				$this->db->trans_rollback();
				$this->load->library("session");
				$this->session->set_flashdata('error', "Emp Code sudah digunakan!");
			...
			if (!empty($guru_sd_foto_profil_uuid)) {
				$guru_sd_foto_profil_name_copy = date('YmdHis') . '-' . $guru_sd_foto_profil_name;
			...
			if (!empty($guru_sd_slip_gaji_uuid)) {
				$guru_sd_slip_gaji_name_copy = date('YmdHis') . '-' . $guru_sd_slip_gaji_name;
			...
			$get_data = $this->mymodel->withquery("select * from guru_sd where id_guru = ?", "row", [$id]);
			$save_guru_sd = $this->model_guru_sd->change($id, $save_data);
	...
	*/
	...
	{
		$this->is_allowed('guru_sd_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'guru_sd'), 'success');
        } else {
            set_message(cclang('error_delete', 'guru_sd'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('guru_sd_view');

		$this->data['guru_sd'] = $this->model_guru_sd->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Guru Sd Detail');
		$this->render('backend/standart/administrator/guru_sd/guru_sd_view', $this->data);
	}
	...
	*/
	...
	{
		$guru_sd = $this->model_guru_sd->find($id);

		if (!empty($guru_sd->foto_profil)) {
			$path = FCPATH . '/uploads/guru_sd/' . $guru_sd->foto_profil;

			if (is_file($path)) {
				$delete_file = unlink($path);
			}
		}
		if (!empty($guru_sd->slip_gaji)) {
			$path = FCPATH . '/uploads/guru_sd/' . $guru_sd->slip_gaji;
		...
		return $this->model_guru_sd->remove($id);
	}
	
	/**
	* Upload Image Guru Sd	* 
	* @return JSON
	*/
	...
	/**
	* Delete Image Guru Sd	* 
	* @return JSON
	*/
	public function delete_foto_profil_file($uuid)
	{
		if (!$this->is_allowed('guru_sd_delete', false)) {
			echo json_encode([
				'success' => false,
	...
	*/
	public function get_foto_profil_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_sd/',
            'delete_endpoint'   => 'administrator/guru_sd/delete_foto_profil_file'
        ]);
	}
	...
	*/
	public function get_slip_gaji_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_guru',
            'upload_path'       => 'uploads/guru_sd/',
            'delete_endpoint'   => 'administrator/guru_sd/delete_slip_gaji_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('guru_sd_export');

		$this->model_guru_sd->export('guru_sd', 'guru_sd');
	}
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_guru_sd->pdf('guru_sd', 'guru_sd');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_guru_sd->find($id);
		...
		if (isset($_FILES["file_guru"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					// check emp code
					$check=$this->mymodel->withquery("select * from guru_sd where emp_code = ".(int)$worksheet->getCellByColumnAndRow(1, $row)->getValue(), 'row');
					/* if($check){
						$this->db->trans_rollback();
						$this->load->library("session");
					...
					$check=$this->mymodel->withquery("select * from guru_sd where email_ms_office = '".$worksheet->getCellByColumnAndRow(2, $row)->getValue()."'", 'row');
					if($check){
						$insertId = $this->mymodel->update("guru_sd", $data_pendaftaran, 'email_ms_office', $worksheet->getCellByColumnAndRow(2, $row)->getValue());
						$insertId = $check->id_guru;
					}else{
						$insertId = $this->mymodel->insertid("guru_sd", $data_pendaftaran);
					}
	...
	public function upload_slip_gaji(){
        ...
        for ($i=0; $i < $total_file; $i++) {
            if (!empty($_FILES['file_slip_gaji']['name'][$i])) {
                  $trans_folder = './uploads/guru_sd/'; //path htrans
                  ...
                  if (move_uploaded_file($_FILES['file_slip_gaji']['tmp_name'][$i], $uploadfile)) {
                    ...
                    $up = $this->mymodel->update("guru_sd",$data, "npp", $filename_original);
                    $msg = array('success'=>1,'message'=>'Upload File Berhasil');
                    $filenya = array(
                      "file_upload" => base_url("uploads/guru_sd/").$filename_no_extension.".pdf",
...
/* End of file guru_sd.php */
/* Location: ./application/controllers/administrator/Guru Sd.php */