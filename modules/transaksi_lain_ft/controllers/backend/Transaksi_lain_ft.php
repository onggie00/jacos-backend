...
*|
*/
class Transaksi_lain_ft extends Admin	
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_transaksi_lain_ft');
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_ft_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['transaksi_lain_fts'] = $this->model_transaksi_lain_ft->get($filter, $field, $this->limit_page, $offset);
		$this->data['transaksi_lain_ft_counts'] = $this->model_transaksi_lain_ft->count_all($filter, $field);

		$config = [
			'base_url'     => 'administrator/transaksi_lain_ft/index/',
			'total_rows'   => $this->model_transaksi_lain_ft->count_all($filter, $field),
		...
		// Infobox stats
		$this->data['stats'] = $this->model_transaksi_lain_ft->get_stats($filter, $field);

		// Chart data
		$this->data['chart_data'] = $this->model_transaksi_lain_ft->get_chart_data($filter, $field);

		// Pass current filter to view
		$this->data['current_filter_q'] = $filter;
		$this->data['current_filter_f'] = $field;

		$this->template->title('List Tagihan FT List');
		$this->render('backend/standart/administrator/transaksi_lain_ft/transaksi_lain_ft_list', $this->data);
	}
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_ft_update');

		$this->data['transaksi_lain_ft'] = $this->model_transaksi_lain_ft->find($id);

		$this->template->title('List Tagihan FT Update');
		$this->render('backend/standart/administrator/transaksi_lain_ft/transaksi_lain_ft_update', $this->data);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_ft_update', false)) {
			echo json_encode([
				'success' => false,
		...
		$this->form_validation->set_rules('transaksi_lain_ft_file_kwitansi_name', 'File Kwitansi', 'trim');
		...
		if ($this->form_validation->run()) {
			$transaksi_lain_ft_file_kwitansi_uuid = $this->input->post('transaksi_lain_ft_file_kwitansi_uuid');
			$transaksi_lain_ft_file_kwitansi_name = $this->input->post('transaksi_lain_ft_file_kwitansi_name');
			...
			if (!is_dir(FCPATH . '/uploads/transaksi_lain_ft/')) {
				mkdir(FCPATH . '/uploads/transaksi_lain_ft/');
			}

			if (!empty($transaksi_lain_ft_file_kwitansi_uuid)) {
				$transaksi_lain_ft_file_kwitansi_name_copy = date('YmdHis') . '-' . $transaksi_lain_ft_file_kwitansi_name;

				rename(FCPATH . 'uploads/tmp/' . $transaksi_lain_ft_file_kwitansi_uuid . '/' . $transaksi_lain_ft_file_kwitansi_name, 
						FCPATH . 'uploads/transaksi_lain_ft/' . $transaksi_lain_ft_file_kwitansi_name_copy);

				if (!is_file(FCPATH . '/uploads/transaksi_lain_ft/' . $transaksi_lain_ft_file_kwitansi_name_copy)) {
					echo json_encode([
						'success' => false,
				...
				$save_data['file_kwitansi'] = $transaksi_lain_ft_file_kwitansi_name_copy;
			}
		
			
			$save_transaksi_lain_ft = $this->model_transaksi_lain_ft->change($id, $save_data);

			if ($save_transaksi_lain_ft) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						anchor('administrator/transaksi_lain_ft', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/transaksi_lain_ft');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_ft_delete');
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', 'transaksi_lain_ft'), 'success');
        } else {
            set_message(cclang('error_delete', 'transaksi_lain_ft'), 'error');
        }
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_ft_view');

		$this->data['transaksi_lain_ft'] = $this->model_transaksi_lain_ft->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('List Tagihan FT Detail');
		$this->render('backend/standart/administrator/transaksi_lain_ft/transaksi_lain_ft_view', $this->data);
	}
	...
	*/
	...
	{
		$transaksi_lain_ft = $this->model_transaksi_lain_ft->find($id);

		if (!empty($transaksi_lain_ft->file_kwitansi)) {
			$path = FCPATH . '/uploads/transaksi_lain_ft/' . $transaksi_lain_ft->file_kwitansi;
		...
		return $this->model_transaksi_lain_ft->remove($id);
	}
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_ft_add', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
				]);
	...
	*/
	...
	{
		if (!$this->is_allowed('transaksi_lain_ft_delete', false)) {
			echo json_encode([
				'success' => false,
		...
		echo $this->delete_file([
            ...
            'table_name'        => 'transaksi_lain_ft',
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/transaksi_lain_ft/'
        ]);
	}
	...
	*/
	public function get_file_kwitansi_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id',
            'upload_path'       => 'uploads/transaksi_lain_ft/',
            'delete_endpoint'   => 'administrator/transaksi_lain_ft/delete_file_kwitansi_file'
        ]);
	}
	...
	*/
	...
	{
		$this->is_allowed('transaksi_lain_ft_export');

		//$this->model_transaksi_lain_ft->export('transaksi_lain_ft', 'transaksi_lain_ft');
		$f = $this->input->get('f');
		$q = $this->input->get('q');
		$where = "";
		...
		$get_data = $this->mymodel->withquery("select s.id_siswa_ft_aktif as id_siswa, s.nama_lengkap, s.nis, s.id_kelas, k.label as nama_kelas, m.nama_transaksi, t.va_number, t.kode_tagihan, t.tanggal_bayar, t.nominal_bayar, 
			...
			when t.status_transaksi = '3' then 'kadaluarsa' end as status_transaksi
		 	from transaksi_lain_ft t join transaksi_lain_ft_manajemen m on t.id_transaksi_lain = m.id join siswa_ft_aktif s on t.id_siswa_aktif = s.id_siswa_ft_aktif join kelas_ft k on s.id_kelas = k.id_kelas_ft ".$where." order by s.nama_lengkap ASC","result");
	...
	*/
	public function export_pdf()
	{
		...
		$this->model_transaksi_lain_ft->pdf('transaksi_lain_ft', 'transaksi_lain_ft');
	}
	...
	public function single_pdf($id = null)
	{
        ...
        $data = $this->model_transaksi_lain_ft->find($id);
...
/* End of file transaksi_lain_ft.php */
