...
class Presensi_siswa extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        for ($i=0; $i < count($list_siswa); $i++) {
            ...
            $get_siswa_aktif = $this->mymodel->withquery("select id_siswa_".$jenjang."_aktif as id_siswa_aktif, nis, nama_lengkap, device_id_siswa, device_id_ortu, id_siswa_".$jenjang." from siswa_".$jenjang."_aktif where id_siswa_".$jenjang."_aktif = ".$list_siswa[$i],"row");
            $get_siswa = null;
            ...
            $data = array(
                ...
                "nis" => $get_siswa_aktif->nis,
                "wifi_ssid" => "Presensi oleh Guru",//$cek_ssid->id_pengaturan,
                "wifi_ip" => "",
                "hari_absen" => $hari_absen,
                "waktu_absen" => (ucfirst($status_hadir) == "Hadir") ? "07:00:00" : $waktu_absen,
                "tanggal_absen" => $tanggal_absen,
                "status_absen" => ucfirst($status_hadir),
                "sumber_absen" => "guru",
            );
            if (!empty($data)) {
                //cek sudah absen: tanggal_absen + hari_absen + id_siswa_aktif
                $cek_presensi = $this->mymodel->withquery("select * from presensi_".$jenjang." where tanggal_absen = '".$tanggal_absen."' and hari_absen = '".$hari_absen."' and id_siswa_aktif = '".$list_siswa[$i]."' limit 1","row");
                if (empty($cek_presensi)) {
                    ...
                    if($insert){
                        ...
                        //$this->send_notif("Presensi Siswa ".strtoupper($nama_siswa), "Siswa ananda ".$nama_siswa." telah melakukan presensi kehadiran di sekolah pada ".formatTanggal(date("Y-m-d"))." Pukul ".date("H:i")." WIB", $get_siswa_aktif->device_id_siswa, array("id_siswa_aktif" => $list_siswa[$i], "id_presensi" => $insert, "jenjang" => $jenjang, "nis" => $get_siswa_aktif->nis) );
                        if (!empty($get_siswa_aktif->device_id_ortu)) {
                            ...
                            if (!empty($cek_setting) && $cek_setting->notifikasi_presensi == 1) {
                                $this->send_notif("Presensi Siswa ".strtoupper($nama_siswa), $pesan, $get_siswa_aktif->device_id_ortu, array("nis" => $get_siswa_aktif->nis,"id_siswa_aktif" => $list_siswa[$i], "id_presensi" => $insert, "jenjang" => $jenjang, "nis" => $get_siswa_aktif->nis) );
                            }
                            else if(empty($cek_setting)){
                                $this->send_notif("Presensi Siswa ".strtoupper($nama_siswa), $pesan, $get_siswa_aktif->device_id_ortu, array("nis" => $get_siswa_aktif->nis,"id_siswa_aktif" => $list_siswa[$i], "id_presensi" => $insert, "jenjang" => $jenjang, "nis" => $get_siswa_aktif->nis) );
                            }
                        }
                        if (!empty($cek_setting) && $cek_setting->notifikasi_wa_presensi == 1 && $tanggal_absen == date("Y-m-d") ) {
                            if ($data['status_absen'] == "Hadir") {
                                $pesan = "Yth. Bapak/Ibu ananda ".strtoupper($nama_siswa)." hari ini tanggal ".formatTanggal(date("Y-m-d"))." telah di lakukan presensi ".strtoupper($data['status_absen'])." oleh guru pada pukul ".date("H:i")." WIB. Terimakasih";
                            }
                            else if($data['status_absen'] == "Terlambat"){
                                $pesan = "Yth. Bapak/Ibu ananda ".strtoupper($nama_siswa)." hari ini tanggal ".formatTanggal(date("Y-m-d"))." telah di lakukan presensi ".strtoupper($data['status_absen'])." HADIR oleh guru pada pukul ".date("H:i")." WIB. Terimakasih";
                            }
                    ...
                    //update / edit data presensi siswa: hanya status_absen + updated_at
                    $data_update = array(
                        "status_absen" => ucfirst($status_hadir),
                        "waktu_absen" => (ucfirst($status_hadir) == "Hadir") ? "07:00:00" : $waktu_absen,
                        "sumber_absen" => "guru",
                    ...
                    $update=$this->mymodel->update("presensi_".$jenjang, $data_update, "id", $cek_presensi->id);
                    if($update){
                        $data['id'] = $cek_presensi->id;
                        ...
                        if (!empty($get_siswa_aktif->device_id_ortu)) {
                            ...
                            if (!empty($cek_setting) && $cek_setting->notifikasi_presensi == 1) {
                                $this->send_notif("Presensi Siswa ".strtoupper($nama_siswa), $pesan, $get_siswa_aktif->device_id_ortu, array("id_siswa_aktif" => $list_siswa[$i], "id_presensi" => $insert, "jenjang" => $jenjang, "nis" => $get_siswa_aktif->nis) );
                            }
                            else if(empty($cek_setting)){
                                $this->send_notif("Presensi Siswa ".strtoupper($nama_siswa), $pesan, $get_siswa_aktif->device_id_ortu, array("id_siswa_aktif" => $list_siswa[$i], "id_presensi" => $insert, "jenjang" => $jenjang, "nis" => $get_siswa_aktif->nis) );
                            }
