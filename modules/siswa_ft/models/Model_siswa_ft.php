...
class Model_siswa_ft extends MY_Model
{

    private $primary_key    = 'id_siswa_ft';
    private $table_name     = 'siswa_ft';
    private $field_search   = ['nama_lengkap', 'email', 'email_ms_office', 'nisn', 'tempat_lahir', 'tgl_lahir', 'jenis_kelamin', 'agama', 'email_ms_office_ortu', 'nama_ibu', 'pekerjaan_ibu', 'notelp_ibu', 'nama_ayah', 'pekerjaan_ayah', 'notelp_ayah', 'alamat', 'kelurahan', 'kecamatan', 'kota', 'provinsi', 'kode_pos', 'sekolah_asal', 'foto_peserta', 'akte_lahir', 'kartu_keluarga', 'sumber_informasi', 'alasan_tertarik', 'peminatan_ft', 'status_lulus', 'no_transaksi', 'no_peserta', 'va_number', 'gelombang', 'tahun_ajaran'];
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_ft." . $field . " LIKE '%" . $q . "%' ";
                } else {
                    $where .= "OR " . "siswa_ft." . $field . " LIKE '%" . $q . "%' ";
                }
                $iterasi++;
            }
        ...
        $this->db->where($where);
        $this->db->where('siswa_ft.is_show', 1);
        $this->db->group_by('siswa_ft.id_siswa_ft');
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [], $sort = null, $sort_type = null)
    {
        ...
        $sort_by = (empty($sort)) ? 'id_siswa_ft' : $sort;
        ...
        $this->db->order_by('siswa_ft.' . $sort_by, $sort_type);
        $this->db->group_by('siswa_ft.id_siswa_ft');
        $query = $this->db->get($this->table_name);
    ...
    {
        $this->db->join('areas', 'areas.id = siswa_ft.kelurahan', 'LEFT');
        $this->db->join('districts', 'districts.id = siswa_ft.kecamatan', 'LEFT');
        $this->db->join('regencies', 'regencies.id = siswa_ft.kota', 'LEFT');
        $this->db->join('provinces', 'provinces.id = siswa_ft.provinsi', 'LEFT');
        $this->db->join('peminatan_ft', 'peminatan_ft.id_peminatan_ft = siswa_ft.peminatan_ft', 'LEFT');
        $this->db->join('status_lulus', 'status_lulus.id_status_lulus = siswa_ft.status_lulus', 'LEFT');
        $this->db->join('status_daftar_ulang_ft daftar_ulang', 'daftar_ulang.id_siswa_ft = siswa_ft.id_siswa_ft', 'LEFT');
        $this->db->join('transaksi', 'transaksi.user_email = siswa_ft.email', 'LEFT');
        $this->db->like('transaksi.no_transaksi', 'LI-FT');
        $this->db->join('nilai_raport_ft', 'nilai_raport_ft.id_siswa = siswa_ft.id_siswa_ft', 'LEFT');

        $this->db->select('siswa_ft.*,
    ...
    public function join_avaiable_export()
    {
        ...
        $this->db->where('siswa_ft.is_show', 1);
        $this->db->order_by('id_siswa_ft', 'ASC');
        return $this;
    }
    ...
    public function export_siswa($table, $subject = 'file',$value=null,$col=null)
    {
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "siswa_ft." . $field . " LIKE '%" . $value . "%' ";
                } else {
                    $where .= "OR " . "siswa_ft." . $field . " LIKE '%" . $value . "%' ";
                }
                $iterasi++;
            }
        ...
        foreach ($result->result() as $data) {
            ...
            foreach ($fields as $field) {
                ...
                } elseif ($field == 'file_raport' || $field == 'file_raport2' || $field == 'file_raport3' || $field == 'file_raport4') {
                    $data_field = ($data->$field)?base_url('uploads/siswa_ft/') .$data->$field:$data->$field;
                } else {
...
/* End of file Model_siswa_ft.php */
/* Location: ./application/models/Model_siswa_ft.php */
