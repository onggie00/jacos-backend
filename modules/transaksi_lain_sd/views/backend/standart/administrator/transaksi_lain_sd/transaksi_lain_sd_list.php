...
<section class="content-header">
  <h1>
    List Tagihan SD <small>Daftar Transaksi Tagihan</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active">List Tagihan SD</li>
  </ol>
</section>
    ...
    <div class="col-md-12">
      <div class="box box-warning">
        ...
        <div class="box-body">
          ...
          <!-- Filter Section -->
          <form name="form_transaksi_lain_sd" id="form_transaksi_lain_sd" action="<?= base_url('administrator/transaksi_lain_sd/index'); ?>">
          <div class="filter-section">
            <div class="row">
              <div class="col-md-5 col-sm-12">
                <div class="input-group">
                  ...
                  <span class="input-group-btn">
                    ...
                    <a class="btn btn-flat btn-default" href="<?= base_url('administrator/transaksi_lain_sd'); ?>" title="Reset Filter"><i class="fa fa-undo"></i></a>
                  </span>
                </div>
              </div>
              ...
              <div class="col-md-4 col-sm-6 text-right">
                <?php is_allowed('transaksi_lain_sd_export', function(){ ?>
                <a class="btn btn-flat btn-success btn-sm" title="Export Excel" id="btnExportXls" href="<?= site_url('administrator/transaksi_lain_sd/export'); ?>">
                  <i class="fa fa-file-excel-o"></i> Export XLS
                </a>
                <a class="btn btn-flat btn-danger btn-sm" title="Export PDF" href="<?= site_url('administrator/transaksi_lain_sd/export_pdf'); ?>">
          ...
          <div class="filter-badge-bar">
            ...
            <a href="<?= base_url('administrator/transaksi_lain_sd'); ?>" class="filter-badge-clear" title="Hapus filter"><i class="fa fa-times"></i> Hapus</a>
          </div>
          <?php endif; ?>
          ...
          <div class="row" style="margin-bottom:10px;">
            ...
            <div class="col-md-4 text-right">
              <span class="item-count-badge"><?= $transaksi_lain_sd_counts; ?> item</span>
            </div>
          </div>
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-striped table-modern dataTable">
              ...
              <tbody id="tbody_transaksi_lain_sd">
              <?php foreach($transaksi_lain_sds as $row): ?>
                <tr>
                  <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $row->id; ?>"></td>
                  <td>
                    <?php if ($row->id_siswa_aktif): ?>
                      <a href="<?= site_url('administrator/siswa_sd_aktif/view/'.$row->id_siswa_aktif.'?popup=show'); ?>" class="popup-view">
                        <i class="fa fa-user"></i> <?= _ent($row->siswa_sd_aktif_nama_lengkap); ?>
                      </a>
                    <?php endif; ?>
                  </td>
                  ...
                  <td>
                    <?php if ($row->id_transaksi_lain): ?>
                      <a href="<?= site_url('administrator/transaksi_lain_sd_manajemen/view/'.$row->id_transaksi_lain.'?popup=show'); ?>" class="popup-view">
                        <?= _ent($row->transaksi_lain_sd_manajemen_nama_transaksi); ?>
                      </a>
                    <?php endif; ?>
                  </td>
                  ...
                  <td>
                    <?php if (!empty($row->file_kwitansi)): ?>
                      <a href="<?= BASE_URL . 'uploads/transaksi_lain_sd/' . $row->file_kwitansi; ?>" target="_blank" class="btn btn-xs btn-default" title="Lihat Kwitansi">
                  ...
                  <td>
                    <div class="action-btn-group">
                      <?php is_allowed('transaksi_lain_sd_view', function() use ($row){ ?>
                        <a href="<?= site_url('administrator/transaksi_lain_sd/view/' . $row->id); ?>" class="btn btn-xs btn-info" title="Detail"><i class="fa fa-eye"></i></a>
                      <?php }) ?>
                      <?php is_allowed('transaksi_lain_sd_update', function() use ($row){ ?>
                        <a href="<?= site_url('administrator/transaksi_lain_sd/edit/' . $row->id); ?>" class="btn btn-xs btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                      <?php }) ?>
                      <?php is_allowed('transaksi_lain_sd_delete', function() use ($row){ ?>
                        <a href="javascript:void(0);" data-href="<?= site_url('administrator/transaksi_lain_sd/delete/' . $row->id); ?>" class="btn btn-xs btn-danger remove-data" title="Hapus"><i class="fa fa-trash"></i></a>
                      <?php }) ?>
                    </div>
                  </td>
                </tr>
              <?php endforeach; ?>
              <?php if ($transaksi_lain_sd_counts == 0): ?>
                <tr>
          ...
          <div class="row" style="margin-top:15px;">
            <div class="col-md-8">
              <small class="text-muted">Menampilkan <?= $transaksi_lain_sd_counts > 0 ? ($this->uri->segment(4) + 1) : 0; ?> - <?= min($this->uri->segment(4) + $this->limit_page, $transaksi_lain_sd_counts); ?> dari <?= $transaksi_lain_sd_counts; ?> data</small>
            </div>
            <div class="col-md-4">
...
$(document).ready(function(){
  ...
  $('#apply').click(function(){
    var bulk = $('#bulk');
    var serialize_bulk = $('#form_transaksi_lain_sd').serialize();
    if (bulk.val() == 'delete') {
      swal({
        title: "<?= cclang('are_you_sure'); ?>",
        text: "<?= cclang('data_to_be_deleted_can_not_be_restored'); ?>",
        ...
        if (isConfirm) {
          document.location.href = BASE_URL + '/administrator/transaksi_lain_sd/delete?' + serialize_bulk;
        }
      });
      return false;
    } else if(bulk.val() == '') {
