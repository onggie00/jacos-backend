...
class Tahun_ajaran extends Admin	
{
	...
	*/
	public function add_save()
	{
		...
		$cek_aktif = $this->mymodel->withquery("select * from tahun_ajaran where tanggal_mulai <= '".$this->input->post('tanggal_mulai')."' and tanggal_selesai >= '".$this->input->post('tanggal_selesai')."'", "row");

		if($cek_aktif){
			echo json_encode([
				'success' => false,
				'message' => "Sudah ada tahun ajaran pada periode ini"
				]);
			exit;
		}
