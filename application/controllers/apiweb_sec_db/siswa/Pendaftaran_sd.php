...
class Pendaftaran_sd extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_sd from siswa_sd where email like '%".$this->post("email")."%' and is_show = 1","row");

        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->second_db->getbywhere("list_sekolah_tk", "id_list_sekolah_tk",$this->post("sekolah_asal"),"row")->nama_sekolah;
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select * from siswa_sd where email = '".$this->post("email")."'", "row");
          if(!empty($cek_email)){
            ...
            if (!empty($id_siswa)) {
              ...
              $data_transaksi = array(
                ...
                "nama_bank" => 'BRI',//$this->post('nama_bank'),
              ...
              if (!empty($data_transaksi)) {
                ...
                $get_no_urut = $this->second_db->withquery("select va_number_bri, id_siswa_sd as id_siswa from siswa_sd where va_number_bri like '".$brivaNo.date('y', strtotime('+1 years'))."01%' and va_number_bri != '' and is_mutasi = '2' order by id_siswa_sd DESC","row");
