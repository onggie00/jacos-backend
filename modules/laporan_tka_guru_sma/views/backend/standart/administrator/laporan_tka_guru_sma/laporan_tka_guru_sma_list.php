...
<section class="content-header">
   <h1><?= cclang('laporan_tka_guru_sma') ?> <small><?= cclang('list_all'); ?></small></h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('laporan_tka_guru_sma') ?></li>
   </ol>
</section>
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-header with-border">
               <h3 class="box-title">
                  <i class="fa fa-list"></i> Data Laporan TKA Guru SMA
                  <span class="label bg-yellow" style="margin-left:10px"><?= $laporan_tka_guru_sma_counts; ?> Data</span>
                  <?php if(!empty($_GET['tahun'])): ?>
                  <span class="filter-badge">
                     <i class="fa fa-calendar"></i> Tahun: <?= $_GET['tahun']; ?>
                     <a href="<?= base_url('administrator/laporan_tka_guru_sma'); ?>" title="Hapus Filter"><i class="fa fa-times"></i></a>
                  </span>
                  <?php endif; ?>
               </h3>
               <div class="box-tools pull-right">
                  ...
                  <?php is_allowed('laporan_tka_guru_sma_export', function(){?>
                  <a class="btn btn-sm btn-info btn-top btn-export <?= empty($_GET['tahun']) ? 'btn-export-disabled' : ''; ?>" id="btn_export" href="<?= site_url('administrator/laporan_tka_guru_sma/export?q='.$this->input->get('q').'&f='.$this->input->get('f').'&tahun='.$this->input->get('tahun')); ?>"><i class="fa fa-file-excel-o"></i> Export</a>
                  <?php }) ?>
               </div>
            </div>

            <div class="box-body">
               <form name="form_search" id="form_search" action="<?= base_url('administrator/laporan_tka_guru_sma/index'); ?>" method="get">
                  ...
                  <div class="col-md-5">
                     <div class="input-group">
                        ...
                        <span class="input-group-btn">
                           ...
                           <a class="btn btn-flat btn-default" href="<?= base_url('administrator/laporan_tka_guru_sma?tahun='.$this->input->get('tahun')); ?>"><i class="fa fa-undo"></i></a>
                           <?php endif; ?>
                        </span>
                     </div>
                  </div>
               ...
               <form name="form_laporan_tka_guru_sma" id="form_laporan_tka_guru_sma" action="">
               <input type="hidden" name="tahun" value="<?= $this->input->get('tahun'); ?>">
               ...
               <table class="table table-bordered table-striped table-hover">
                  ...
                  <tbody>
                  <?php if($laporan_tka_guru_sma_counts > 0): ?>
                     <?php foreach($laporan_tka_guru_smas as $row): ?>
                     <tr>
                        <td><input type="checkbox" class="flat-red check" name="id[]" value="<?= $row->id_laporan; ?>"></td>
                        <td><?= $row->guru_sma_nama_lengkap ?? '-'; ?></td>
                        <td><?= _ent($row->npp ?? '-'); ?></td>
                        ...
                        <td style="white-space:nowrap">
                           <a target="_blank" href="<?= site_url('apiapp/export_tka_guru?tipe_guru=guru_sma&id='.$row->id_laporan); ?>" class="btn btn-action btn-action-pdf btn-xs"><i class="fa fa-file-pdf-o"></i> PDF</a>
                           <?php is_allowed('laporan_tka_guru_sma_view', function() use ($row){?>
                           ...
                           <?php }) ?>
                           <?php is_allowed('laporan_tka_guru_sma_update', function() use ($row){?>
                           <a href="<?= site_url('administrator/laporan_tka_guru_sma/edit/'.$row->id_laporan); ?>" class="btn btn-action btn-action-edit btn-xs"><i class="fa fa-edit"></i> Edit</a>
                           <?php }) ?>
                           <?php is_allowed('laporan_tka_guru_sma_delete', function() use ($row){?>
                           <a href="javascript:void(0);" data-href="<?= site_url('administrator/laporan_tka_guru_sma/delete/'.$row->id_laporan); ?>" class="btn btn-action btn-action-delete btn-xs remove-data"><i class="fa fa-trash"></i></a>
                           <?php }) ?>
                        </td>
                     </tr>
                     <?php endforeach; ?>
...
<div class="modal fade" id="modal_import" tabindex="-1" role="dialog">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <form action="<?= base_url('administrator/laporan_tka_guru_sma/import'); ?>" method="post" enctype="multipart/form-data">
            <div class="modal-body">
               <div class="form-group">
...
$(document).ready(function(){
   ...
   $('#formImportDetail').on('submit', function(e){
      ...
      result.hide();
      $.ajax({
         url: BASE_URL + '/administrator/laporan_tka_guru_sma/import_detail',
   ...
   $(document).on('click', '.btn-detail-nilai', function(){
      ...
      $.ajax({
         url: BASE_URL + '/administrator/laporan_tka_guru_sma/detail_nilai/' + id,
   ...
   $('#btn_apply_filter').on('click', function() {
      ...
      var url = '<?= base_url('administrator/laporan_tka_guru_sma'); ?>';
   ...
   $('#btn_kosongkan').on('click', function() {
      swal({ title: "Hapus Semua Data " + currentTahun + "?", text: "Data yang dihapus tidak dapat dikembalikan!", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Ya, Hapus!", cancelButtonText: "Batal" }, function(isConfirm){
         if (isConfirm) { var form = $('<form>', { method: 'POST', action: '<?= base_url('administrator/laporan_tka_guru_sma/delete_by_tahun'); ?>' }); form.append($('<input>', {type: 'hidden', name: 'tahun', value: currentTahun})); form.appendTo('body').submit(); }
      });
   });
   ...
   $('#apply').click(function(){
      ...
      if (bulk == 'delete') {
         ...
         swal({ title: "<?= cclang('are_you_sure'); ?>", text: "<?= cclang('data_to_be_deleted_can_not_be_restored'); ?>", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "<?= cclang('yes_delete_it'); ?>" }, function(isConfirm){ if (isConfirm) $('#form_laporan_tka_guru_sma').attr('action', '<?= base_url('administrator/laporan_tka_guru_sma/delete'); ?>').submit(); });
      }
      return false;
   });
