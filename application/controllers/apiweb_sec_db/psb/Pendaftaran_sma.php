...
class Pendaftaran_sma extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['x-token']))
        ...
        //cek email
        $cek_email = $this->second_db->withquery("select id_siswa_sma from calon_siswa_sma where email like '%".$this->db->escape_like_str($this->post("email"))."%' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".date("Y-m-d", strtotime($this->post("tgl_lahir")))."' and is_show = 1","row");
        // dd($cek_email);
        if (empty($cek_email)) {
          $sekolah_asal = "";
          if ($this->post("sekolah_asal") > 0) {
            $sekolah_asal = $this->second_db->getbywhere("list_sekolah_smp", "id_list_sekolah_smp",$this->db->escape($this->post("sekolah_asal")),"row")->nama_sekolah;
            ...
            $sekolah_asal = $this->$this->post("sekolah_asal");
          }
          $pekerjaan_ibu = "";
          $pekerjaan_ayah = "";
          if ($this->post("pekerjaan_ayah") > 0) {
            $pekerjaan_ayah = $this->second_db->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->db->escape($this->post("pekerjaan_ayah"))."'","row")->nama_pekerjaan;
          ...
          if ($this->post("pekerjaan_ibu") > 0) {
            $pekerjaan_ibu = $this->second_db->withquery("select * from pekerjaan_ortu where id_pekerjaan_ortu = '".$this->db->escape($this->post("pekerjaan_ibu"))."'","row")->nama_pekerjaan;
          ...
          $data = array(
            ...
            "peminatan_sma" => $this->post("peminatan_sma"),
          ...
          $ppsbb = $this->db->escape($this->post('ppsbb'));
          // if($ppsbb==1){
              //foto Raport 1 - 4
              if (!empty($_FILES['file_raport']['name'])) {
                $uploaddir = './uploads/siswa_sma/raport/';
          ...
          //cek email duplicate
          $cek_email = $this->second_db->withquery("select * from calon_siswa_sma where email = '".$this->db->escape($this->post("email"))."' and nama_lengkap like '%".$this->db->escape_like_str($this->post("nama_lengkap"))."%' and tgl_lahir = '".$this->post("tgl_lahir")."'", "row");
          if(!empty($cek_email)){
            ...
            $id_siswa = $this->second_db->insertid("siswa_sma",$data);
            ...
            if (!empty($id_siswa)) {
              ...
              $id_raport = $this->second_db->insertid("nilai_raport_sma",$data_raport);
              //insert transaksi
              //get biaya pendaftaran
              
              $peminatan = $this->db->escape($this->post('peminatan_sma'));
              if ($ppsbb == "1") {
                if ($peminatan == "1") {
                  $va_class = "04";
                }
                ...
                $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","PPSBB SMA","row");
                $no_transaksi = "LI-PPSBBSMA-".date("Ymd")."-".$id_siswa;

                $get_setting = $this->second_db->getall("pengaturan_akun");
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_sma as id_siswa from calon_siswa_sma where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')).$va_class)."%' and va_number != '' order by id_siswa_sma DESC","row");
                ...
                $va_number = $prefix.$client_id.date("y", strtotime('+1 years')).$va_class.$no_urut;
              }
              else{
                //1=ipa 2=ips
                if ($peminatan == "1") {
                ...
                $get_biaya = $this->second_db->getbywhere("biaya_pendaftaran","jenjang","PSB SMA","row");
                $no_transaksi = "LI-PSBSMA-".date("Ymd")."-".$id_siswa;
                $get_setting = $this->second_db->getall("pengaturan_akun");
                foreach ($get_setting as $key => $value) {
                  if ($value->name_setting == "bni_client_id") {
                    $client_id = $value->value;
                ...
                $get_no_urut = $this->second_db->withquery("select va_number, id_siswa_sma as id_siswa from siswa_sma where va_number like '".$this->db->escape_like_str($prefix.$client_id.date('y', strtotime('+1 years')).$va_class)."%' and va_number != '' order by id_siswa_sma DESC","row");
                if (empty($get_no_urut)) {
                  $no_urut = "0001";
                }
                else{
              ...
              $data_transaksi = array(
                ...
                "user_phone" => "",
                "description" => "Tagihan Pendaftaran SMA a.n ".strtoupper($this->post('nama_lengkap')),
              ...
              if (!empty($data_transaksi)) {
                ...
                /* if(!$payment_response['virtual_account']){
                  ...
                  $status="200";
                  $this->second_db->insertid("error_log_bni",array("status"=>$payment_response['status'],"message"=>$payment_response['message'],"va_number"=>$va_number));
                  $this->response($msg,$status);
                } */

                $id_transaksi = $this->second_db->insertid("transaksi",$data_transaksi);
                $this->second_db->update("siswa_sma", array("no_transaksi" => $no_transaksi, "va_number" => $va_number), "id_siswa_sma", $id_siswa);
                //$this->cetak_slip(array("id_siswa" => $id_siswa, "tipe_siswa" => "sma"));
                $get_transaksi = $this->second_db->withquery("select id_transaksi, no_transaksi, va_number, nama_bank, total_biaya, expired_datetime from transaksi where id_transaksi = '".$id_transaksi."'","row");

                //kirim email slip pembayaran
                $data_email = array(
                  "email" => $this->post("email"),
                  "nama_lengkap" => $this->post("nama_lengkap"),
                  "tipe_pendaftaran" => "PSB SMA",
                  "jenjang" => "SMA",
                  "transaksi" => $get_transaksi,
                  "nama_panitia" => "Panitia PSB SMA Labschool Cibubur ".date("Y", strtotime("+1 years"))."-".date("Y", strtotime("+2 years")),
    ...
    function create_billing($production, $total, $no_transaksi, $data_user){
      ...
      $get_pengaturan_masa_aktif = $this->second_db->getall("pengaturan_masa_aktif_va");
